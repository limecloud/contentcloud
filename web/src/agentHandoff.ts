...
function validateClientHandoff(clientID: AgentClientID, integration: Record<string, unknown>, launch: Record<string, unknown>, prompt: string, fallbackURL: string, expectation: AgentHandoffExpectation): void {
  switch (clientID) {
    case 'codex':
      if (integration.kind !== 'plugin' || integration.id !== 'contentcloud-video-production' || integration.version !== '0.26.0' || launch.mode !== 'deep_link' || fallbackURL !== '/codex' || !parseCodexLaunchURL(launch.url, prompt) || !promptBindsTarget(prompt, integration.id, expectation)) {
