...
import type { ContentItem, ContentShot, ReviewProjection } from '../types';
...
function reviewSubject(projection:ReviewProjection):ReviewSubject|undefined {
  ...
  const value=projection.submission.objects.find(object=>object.type==='content_item'&&object.schema_version==='contentcloud.content-item/3.0'&&Array.isArray(object.shots)) as unknown as ContentItem|undefined;
  if(!value)return undefined;
  return {versionLabel:`Content Item 3.0 · ${Math.round(value.duration_ms/1000)}s`,title:value.title,objective:value.direction?.title||value.direction?.angle||value.experiment?.hypothesis||'',status:value.status,hash:projection.submission.subject_hash,shots:value.shots};
}
