...
export function CreateProjectModal({role,onClose,onCreated}:{role:string;onClose:()=>void;onCreated:(project:Project)=>void}) {
  ...
  const loadTemplates=()=>api<ProjectTemplate[]>('/api/bff/project-templates').then(setTemplates).catch(()=>setTemplates([]));
  ...
  return <Modal title="新建品牌项目" onClose={onClose}>
    <div className="project-template-picker"><Field label="项目模板"><select value={form.template_id} onChange={e=>selectTemplate(e.target.value)}><option value="">不使用模板</option>{templates.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>{role==='tenant_admin'&&<Button variant="secondary" onClick={()=>setTemplateOpen(value=>!value)}><Plus size={15}/>{templateOpen?'收起':'新建模板'}</Button>}</div>
