  ...
  return <Modal title="初始化本地工作区" onClose={onClose}>
    ...
    {!progress&&session.state==='waiting_for_computer'&&<>
      <ol className="agent-bootstrap-steps">
        ...
        <li><span>2</span><div><strong>粘贴操作指令</strong><section className="agent-prompt"><pre className="agent-prompt-instruction"><code>{promptInstruction}</code></pre><pre className="agent-prompt-values"><code>{promptValues}</code></pre></section><p>{BOOTSTRAP_PLAN_CONFIRMATION}</p></div></li>
      </ol>
      {slow&&<Banner kind="warning">Codex 暂未开始。请确认操作指令已完整发送，并允许执行只读环境检查。</Banner>}
      <div className="agent-waiting-footer"><div><span className={`agent-waiting-dot ${slow?'is-slow':''}`}/><p><strong>{state.title}</strong><small>初始化会话于 {formatExpiry(session.expires_at)} 失效</small></p></div><Button className="agent-copy-button" onClick={()=>copy(prompt,'prompt')}>{copied==='prompt'?<Check size={16}/>:<Clipboard size={16}/>} {copied==='prompt'?'已复制':'复制操作指令'}</Button></div>
    </>}
    ...
    {session.state==='connected'&&<div className="agent-complete"><CheckCircle2 size={22}/><div><strong>本地负责创作，云端负责治理</strong><span>初始化没有上传已有文件，也没有自动启动后台进程。</span></div></div>}
