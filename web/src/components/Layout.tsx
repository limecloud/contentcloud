...
const projectItems: {label:string;icon:typeof LayoutDashboard;path:(id:string)=>string}[] = [
  ...
  {label:'SOP',icon:Workflow,path:id=>`/projects/${encodeURIComponent(id)}/sop`}
];
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileOpen?'sidebar-open':''}`}>
      ...
      <nav>
        ...
        <div className="nav-label"><span>Project</span>{project&&<Status value={project.status}/>}</div>
        {project ? (needsConnection?projectItems.slice(0,1):projectItems).map(item=><NavItem key={item.path(project.id)} id={item.path(project.id)} label={item.label} icon={item.icon} active={active(item.path(project.id))} onClick={()=>go(item.path(project.id))}/>) : canManage ? <button className="nav-create" onClick={onCreateProject}><Plus size={15}/>创建 Project</button> : null}
    ...
    <main className="main"><header className="topbar"><div className="project-select-wrap"><span>当前 Project</span><select value={project?.id||''} onChange={(event)=>{const next=projects.find(p=>p.id===event.target.value);if(next)onProject(next)}}><option value="">{projects.length ? '选择 Project' : '还没有 Project'}</option>{projects.map(p=><option key={p.id} value={p.id}>{p.brand_name} · {p.product_name}</option>)}</select></div></header>{children}</main>
  </div>
}
