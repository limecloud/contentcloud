...
describe('用户界面中文标签', () => {
  it('覆盖内容生产主流程中的常用枚举', () => {
    ...
    expect(sourceTypeLabel('workspace_file')).toBe('本地工作区文件');
    ...
    expect(deliveryDestinationLabel('workspace')).toBe('本地工作区');
  ...
  it('把知识查询的阻断原因和下一步转换为中文', () => {
    expect(knowledgeBlockReasonLabel('EVIDENCE_REQUIRED')).toBe('缺少可核验的证据');
  ...
  it('未识别枚举提供中文上下文并保留原值', () => {
    expect(statusLabel('future_state')).toBe('未识别状态（future_state）');
    expect(roleLabel('future_role')).toBe('未识别角色（future_role）');
    expect(stageLabel('future_stage')).toBe('未识别流程阶段（future_stage）');
    expect(evidenceLocatorLabel('future_locator')).toBe('未识别证据位置（future_locator）');
  });
});
