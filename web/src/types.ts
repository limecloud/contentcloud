...
export interface StageDefinition { stage_id:string; name:string; order:number; owner_roles:string[]; input_refs:string[]; output_schema:string; required_capabilities:string[]; execution_modes:string[]; checks:string[]; gate_ids:string[]; retry_max_attempts:number }
...
export interface StageRun { id:string; tenant_id:string; task_id:string; stage_id:string; status:string; execution_mode:string; input_refs:string[]; output_refs:string[]; started_at?:string; completed_at?:string; updated_at:string }
...
export interface TaskDelivery { id:string; tenant_id:string; project_id:string; task_id:string; revision_id:string; destination:string; status:string; manifest:string[]; delivery_digest:string; delivered_by?:string; delivered_at?:string; error_code?:string; created_at:string; updated_at:string }
export interface WorkTaskView { task:WorkTask; project:Project; environment:Environment; sop:SOPVersion; stage_runs:StageRun[]; runs:TaskRun[]; gates:GateEvaluation[]; revisions:TaskRevision[]; deliveries:TaskDelivery[]; allowed_actions:string[]; generated_at:string }
