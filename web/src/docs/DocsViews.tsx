...
export function DocsHome(){
  ...
  return <div className="docs-home">
    ...
    <section className="docs-index-section"><header><div><span className="section-kicker">Content Types</span><h2>按内容形态查看</h2></div><p>只有具备真实实现的组合才提供可执行场景教程。</p></header><div className="docs-content-index">{catalog.content_types.map(content=><Link key={content.id} to={docsWebPath(content.page_slug)!}><span className="docs-index-icon">{content.id==='marketing-video'?<Video size={19}/>:<FileText size={19}/>}</span><span><strong>{content.title}</strong><small>{content.summary}</small></span><b className={`docs-status is-${content.status}`}>{docsStatusLabel(content.status)}</b><ChevronRight size={17}/></Link>)}</div></section>
    <section className="docs-index-section docs-guide-section"><header><div><span className="section-kicker">Available Guides</span><h2>当前可用场景</h2></div></header>{catalog.guides.map(guide=><Link key={guide.id} to={docsWebPath(guide.page_slug)!}><BookOpen size={18}/><span><strong>{guide.title}</strong><small>{guide.summary}</small></span><ChevronRight size={17}/></Link>)}</section>
  </div>;
}
