...
export function validateCodexHandoff(value: unknown, expectation: CodexHandoffExpectation): CodexHandoff {
  ...
  if (target.digest !== undefined && !isDigest(target.digest)) {
    throw new Error('Codex 恢复摘要无效');
      ...
      if (value.plugin_id !== 'contentcloud-video-production@contentcloud' || value.plugin_version !== '0.14.0' || value.requires_new_chat !== true || value.requires_workspace_selection !== true || value.fallback_url !== '/codex' || typeof value.prompt !== 'string' || !Array.isArray(value.steps) || value.steps.some(step => typeof step !== 'string' || step.length === 0)) {
