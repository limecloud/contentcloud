...
describe('admin routes',()=>{
  ...
  it('mounts the new operations workspaces as independent pages',()=>{
    const paths=['products','releases','customers','capabilities','skills','executors','jobs','alerts','tenants','audit','costs'];
  ...
  it('does not expose retired or contract-free admin routes',()=>{
    for(const path of ['environments','sops','gates','runtime','usage','connectors','providers','policies','effects','results','rights','duplicates','projections','users','support']){
      const matches=matchRoutes(appRoutes,`/admin/${path}`);
