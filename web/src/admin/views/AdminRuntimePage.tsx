...
export function AdminRuntimePage() {
  ...
  return <div className="admin-runtime-page">
    <div className="admin-heading"><div><span className="eyebrow">执行观察</span><h1>Runtime 运行</h1><p>查看任务执行状态、人工等待点和外部副作用。</p></div><div className="admin-runtime-actions"><label className="admin-runtime-filter"><span>状态</span><select value={state} onChange={event=>setState(event.target.value)}><option value="">全部</option><option value="running">运行中</option><option value="waiting_human">等待人工</option><option value="failed">失败</option><option value="completed">已完成</option></select></label><button className="icon-button" aria-label="刷新运行列表" title="刷新运行列表" disabled={loading} onClick={()=>void loadList()}><RefreshCw size={16} className={loading?'is-spinning':''}/></button></div></div>
