...
function SOPPreviewPanel({preview}: {preview:SOPVersionPreview}) {
  return <div className={`sop-preview-panel ${preview.publishable ? 'is-ready' : 'is-blocked'}`}>
    <div className="sop-preview-conclusion"><span>{preview.publishable ? <CircleCheck size={16}/> : <CircleAlert size={16}/>}</span><div><strong>{preview.publishable ? '当前版本可以进入发布确认' : '当前版本不能进入发布确认'}</strong><small>{preview.sop.digest ? `版本摘要：${preview.sop.digest}` : '版本摘要尚未由服务端生成'}</small></div><b>{preview.publishable ? '可发布' : '需处理'}</b></div>
    ...
    <div className="sop-preview-grid"><section><header><strong>能力覆盖</strong><small>{preview.required_capabilities.length ? `${preview.required_capabilities.length} 项流程要求` : '产品未声明能力'}</small></header>{preview.capabilities.length === 0 ? <p className="sop-preview-empty">没有需要绑定的能力。</p> : preview.capabilities.map(item => <CapabilityCoverageRow key={item.id} item={item}/>)}</section><section><header><strong>客户环境</strong><small>{preview.environments.length ? `${preview.environments.length} 个预览对象` : '没有匹配环境'}</small></header>{preview.environments.length === 0 ? <p className="sop-preview-empty">当前没有客户环境事实；不会填充演示绑定。</p> : preview.environments.map(item => <EnvironmentPreviewRow key={item.environment_id} item={item}/>)}</section></div>
  </div>;
}
function CapabilityCoverageRow({item}: {item:SOPCapabilityCoverage}) { return <div className="sop-preview-row"><span className="sop-preview-state"><CircleCheck size={14}/></span><span><strong>{item.id}</strong><small>使用于 {item.required_by_stages.join('、') || '未返回阶段'} · 执行端 {item.registered_executor_count} 个</small></span><code>{item.registered_versions.length ? item.registered_versions.join('、') : '未登记'}</code></div>; }
function EnvironmentPreviewRow({item}: {item:SOPEnvironmentBindingPreview}) { return <div className={`sop-preview-row ${item.ready ? 'is-ready' : 'is-missing'}`}><span className="sop-preview-state">{item.ready ? <CircleCheck size={14}/> : <CircleAlert size={14}/>}</span><span><strong>{item.name}</strong><small>{item.status === 'active' ? '已启用' : '已暂停'} · 候选执行端 {item.candidate_executor_count} 个</small></span><code>{item.ready ? '可绑定' : item.missing_capabilities.length ? `缺少 ${item.missing_capabilities.length} 项` : '未登记'}</code></div>; }
