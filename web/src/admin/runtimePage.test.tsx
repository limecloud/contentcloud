...
describe('runtime recovery work surface', () => {
  it('renders replay, checkpoint fork, lineage and unknown-effect reconciliation controls', () => {
    ...
    expect(technical).toContain('重建运行投影');
    expect(technical).toContain('运行投影已重建；完整性已验证，已校验 4 个持久化事件，外部调用 0 次');
    expect(technical).toContain('执行身份');
