...
const knowledgeStatusLabel: Record<string, string> = {candidate: '候选', needs_review: '待审核', pending: '待处理', verified: '已核验', approved: '已批准', valid: '有效', active: '生效中', blocked: '已阻断', conflicted: '有冲突', prohibited: '已禁止', expired: '已过期', rejected: '已拒绝', superseded: '已替代', revoked: '已撤销', open: '待解决', source_missing: '缺少来源', collecting: '补料中', resolved: '已解决', waived: '已豁免', accepted_risk: '已接受风险', published: '已发布', draft: '草稿', retired: '已退役'};
...
export function WorkOSHomePage() {
  ...
  return <div className="workos-page">
    ...
    <section className="workos-metrics" aria-label="工作区状态"><Metric icon={ListTodo} label="待处理任务" value={String(actionable.length)} detail="真实工作任务" tone="ink"/><Metric icon={PlayCircle} label="本地运行中" value={String(runningCount)} detail="服务端当前状态" tone="production"/><Metric icon={ShieldAlert} label="待决定 / 阻断" value={String(waitingCount)} detail="按配置产生的检查" tone="review"/><Metric icon={PackageCheck} label="已交付任务" value={String(deliveredCount)} detail="当前项目累计" tone="success"/></section>
...
function StatusText({value}: {value: string}) { const label = statusLabel[value as TaskStatus] || knowledgeStatusLabel[value] || inputStatusLabel[value] || value; return <span className={`status-text ${workOSStatusTone(value)}`}><i></i>{label}</span>; }
