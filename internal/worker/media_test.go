...
func TestProcessPendingMediaProcessesJobsInOrder(t *testing.T) {
	processor := &mediaProcessorFixture{jobs: []domain.MediaGenerationJob{
...
type mediaProcessorFixture struct {
	jobs      []domain.MediaGenerationJob
	failJobID string
	limit     int
	calls     []string
}

func (f *mediaProcessorFixture) PendingMediaGenerationJobs(_ context.Context, limit int) ([]domain.MediaGenerationJob, error) {
	f.limit = limit
	return append([]domain.MediaGenerationJob(nil), f.jobs...), nil
}
