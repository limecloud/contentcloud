...
func TestCreativeExecutionBundleIsDeterministicAndBindsSubjectEnvironmentAndTrust(t *testing.T) {
	...
	request := environment.ExecutionBundleRequest{ProjectID: "project-1", ContentTypes: []string{identitydomain.ContentTypeVideoScript}, Subject: subject, RequiredCapabilities: required, PackIDs: []string{"contentcloud-visual-storytelling"}}
	...
	capabilities := []catalogdomain.Capability{{ID: required[0].ID, Version: required[0].SchemaVersion, Kind: "business_capability", Digest: required[0].Digest, LocalOnly: true}}
...
func TestCreativeExecutionBundleFailsClosedForPackRegistryLockAndCapabilityDrift(t *testing.T) {
	...
	bundle, err := controlPlane.IssueExecutionBundle(environment.ExecutionBundleRequest{ProjectID: "project-1", ContentTypes: []string{identitydomain.ContentTypeVideoScript}, Subject: subject, RequiredCapabilities: []environment.CapabilityRequirement{required}, PackIDs: []string{"contentcloud-visual-storytelling"}}, now)
	must(t, err)
	manifest := signedManifest(t, issuer, now)
	lock := lockForManifest(manifest)
	matching := []catalogdomain.Capability{{ID: required.ID, Version: required.SchemaVersion, Kind: "business_capability", Digest: required.Digest, LocalOnly: true}}
	...
	wrongDigest := append([]catalogdomain.Capability(nil), matching...)
	...
	_, err = controlPlane.IssueExecutionBundle(environment.ExecutionBundleRequest{ProjectID: "project-1", ContentTypes: []string{identitydomain.ContentTypeVideoScript}, Subject: subject, RequiredCapabilities: []environment.CapabilityRequirement{required}, PackIDs: []string{"contentcloud-video-production"}}, now)
	assertCode(t, err, "EXECUTION_BUNDLE_PACK_DENIED")

	unsupported := required
	unsupported.ID = "contentcloud.video.compose"
	_, err = controlPlane.IssueExecutionBundle(environment.ExecutionBundleRequest{ProjectID: "project-1", ContentTypes: []string{identitydomain.ContentTypeVideoScript}, Subject: subject, RequiredCapabilities: []environment.CapabilityRequirement{unsupported}}, now)
	assertCode(t, err, "EXECUTION_BUNDLE_CAPABILITY_DENIED")
}

func resolveBundleError(resolver *environment.Resolver, bundle environment.CreativeExecutionBundle, manifest environment.Manifest, registry environment.VerifiedRegistry, lock environment.EnvironmentLock, capabilities []catalogdomain.Capability, options environment.BundleVerifyOptions) error {
