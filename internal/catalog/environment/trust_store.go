...
func decodeTrustedKeySet(body []byte) ([]TrustedKey, error) {
	...
	if err := decoder.Decode(&document); err != nil {
		return nil, fault.Invalid("TRUST_STORE_INVALID", "可信公钥文件不是严格的 JSON 文档")
	}
	if err := ensureJSONEOF(decoder); err != nil {
		return nil, err
	}
	if strings.TrimSpace(document.SchemaURL) == "" || document.SchemaVersion != "1.0" {
		return nil, fault.Invalid("TRUST_STORE_INVALID", "可信公钥文件缺少 $schema 或 schema_version 1.0")
	}
	...
	for _, key := range document.Keys {
		...
		if err != nil || len(publicKey) != ed25519.PublicKeySize || key.Algorithm != "ed25519" {
			return nil, fault.Invalid("TRUST_STORE_KEY_INVALID", "可信公钥文件包含无效 Ed25519 公钥")
		}
	...
	if active == 0 {
		return nil, fault.Invalid("TRUST_STORE_ACTIVE_KEY_REQUIRED", "可信公钥文件至少需要一个状态为可用（active）的 Ed25519 公钥")
	}
	return decoded, nil
}
...
func ensureJSONEOF(decoder *json.Decoder) error {
	...
	if err := decoder.Decode(&extra); err != io.EOF {
		return fault.Invalid("TRUST_STORE_INVALID", "可信公钥文件只能包含一个 JSON 文档")
	}
	return nil
}
