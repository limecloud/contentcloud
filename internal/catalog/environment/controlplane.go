...
func NewControlPlane(issuer *Issuer, profile Profile, registry VerifiedRegistry, ttl time.Duration) (*ControlPlane, error) {
	if issuer == nil || ttl < time.Minute || ttl > 30*24*time.Hour {
		return nil, fault.Invalid("ENVIRONMENT_CONTROL_PLANE_INVALID", "环境控制面需要签发器，以及 1 分钟至 30 天的有效期")
	}
	publicKey, ok := issuer.privateKey.Public().(ed25519.PublicKey)
	if !ok {
		return nil, fault.Invalid("ENVIRONMENT_SIGNING_KEY_INVALID", "环境签发器无法导出 Ed25519 公钥")
	}
...
func NewControlPlaneWithVerifier(issuer *Issuer, verifier *Verifier, profile Profile, registry VerifiedRegistry, ttl time.Duration) (*ControlPlane, error) {
	if issuer == nil || verifier == nil || ttl < time.Minute || ttl > 30*24*time.Hour {
		return nil, fault.Invalid("ENVIRONMENT_CONTROL_PLANE_INVALID", "环境控制面需要签发器、可信校验器，以及 1 分钟至 30 天的有效期")
	}
	...
	manifest, err := BuildManifest("control-plane-validation", []string{identitydomain.ContentTypeVideoScript}, profileCopy, registry, validationTime, validationTime.Add(ttl))
	if err != nil {
		return nil, err
	}
...
func (controlPlane *ControlPlane) Issue(projectID string, contentTypes []string, now time.Time) (Manifest, error) {
	if controlPlane == nil {
		return Manifest{}, fault.Conflict("ENVIRONMENT_CONTROL_PLANE_UNAVAILABLE", "尚未配置环境控制面")
	}
	now = now.UTC()
	if now.IsZero() {
		now = time.Now().UTC()
...
func (controlPlane *ControlPlane) IssueExecutionBundle(request ExecutionBundleRequest, now time.Time) (CreativeExecutionBundle, error) {
	if controlPlane == nil {
		return CreativeExecutionBundle{}, fault.Conflict("ENVIRONMENT_CONTROL_PLANE_UNAVAILABLE", "尚未配置环境控制面")
	}
	now = now.UTC()
	if now.IsZero() {
		now = time.Now().UTC()
...
func (controlPlane *ControlPlane) Registry() (Registry, error) {
	if controlPlane == nil {
		return Registry{}, fault.Conflict("ENVIRONMENT_CONTROL_PLANE_UNAVAILABLE", "尚未配置环境控制面")
	}
...
func (controlPlane *ControlPlane) ResolveExecutionBundle(bundle CreativeExecutionBundle, manifest Manifest, lock EnvironmentLock, capabilities []catalogdomain.Capability, options BundleVerifyOptions) (BundleResolution, error) {
	if controlPlane == nil || controlPlane.resolver == nil {
		return BundleResolution{}, fault.Conflict("ENVIRONMENT_CONTROL_PLANE_UNAVAILABLE", "尚未配置环境控制面")
	}
