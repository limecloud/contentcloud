...
func TestBuildManifestUsesOnlyExactPublishedCompatibleRegistryEntries(t *testing.T) {
	...
	must(t, err)
	if len(manifest.ContentTypes) != 1 || manifest.ContentTypes[0] != identitydomain.ContentTypeVideoScript {
...
func TestCreativeEnvironmentManifestRejectsInvalidContentTypes(t *testing.T) {
	...
	for _, contentTypes := range [][]string{nil, {identitydomain.ContentTypeWeChatArticle}, {identitydomain.ContentTypeVideoScript, identitydomain.ContentTypeVideoScript}, {identitydomain.ContentTypeVideoScript, "unknown"}} {
...
func fixtureProfileAndRegistry() (environment.Profile, environment.Registry) {
	...
	profile := environment.Profile{
		...
		Plugins: []environment.ProfilePlugin{
			{ID: "contentcloud-video-production", Kind: "scene_plugin", Version: "0.8.0", Required: true, Scope: "environment", Capabilities: []string{sourcedomain.KnowledgeExtractCapability}},
		...
		Capabilities:      []string{sourcedomain.KnowledgeExtractCapability, "contentcloud.asset.generate"},
...
func signedManifest(t *testing.T, issuer *environment.Issuer, now time.Time) environment.Manifest {
	...
	manifest, err := environment.BuildManifest("project-1", []string{identitydomain.ContentTypeVideoScript}, profile, verifiedRegistry, now, now.Add(24*time.Hour))
	must(t, err)
...
func buildManifestError(profile environment.Profile, registry environment.VerifiedRegistry, now time.Time) error {
	_, err := environment.BuildManifest("project-1", []string{identitydomain.ContentTypeVideoScript}, profile, registry, now, now.Add(24*time.Hour))
	return err
}
