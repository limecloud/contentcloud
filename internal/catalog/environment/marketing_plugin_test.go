...
func TestMarketingSkillPackComposesWithCoreEnvironmentExecution(t *testing.T) {
	...
	profile := environment.Profile{
		...
		Plugins: []environment.ProfilePlugin{
			{ID: "contentcloud-video-production", Kind: "scene_plugin", Version: "0.27.0", Required: true, Scope: "environment", Capabilities: []string{sourcedomain.KnowledgeExtractCapability}},
		...
		Capabilities:      []string{sourcedomain.KnowledgeExtractCapability, "contentcloud.marketing.knowledge-governance", "contentcloud.marketing.content-orchestration"},
	...
	unsigned, err := environment.BuildManifest("project-marketing", []string{identitydomain.ContentTypeVideoScript}, profile, verifiedRegistry, now, now.Add(24*time.Hour))
	if err != nil {
		t.Fatal(err)
	}
