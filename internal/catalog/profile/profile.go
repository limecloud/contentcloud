...
var builtins = map[string]Profile{
	"douyin-commerce-video": {
		...
		ContentTypes: []string{identitydomain.ContentTypeMarketingVideo}, Channels: []string{"douyin"}, RequiredGates: []string{"offer_valid", "rights_review", "final_media_review", "publication_preview"}, DeliverySchema: "contentcloud.douyin-commerce-delivery/1.0",
		Stages: []Stage{
	...
	"wechat-official-article": {
		...
		ContentTypes: []string{identitydomain.ContentTypeWeChatArticle}, Channels: []string{"wechat_official_account"}, RequiredGates: []string{"citation_review", "rights_review", "mobile_preview", "publication_preview"}, DeliverySchema: "contentcloud.wechat-delivery/1.0",
		Stages: []Stage{
	...
	"serialized-novel": {
		...
		ContentTypes: []string{identitydomain.ContentTypeSerializedNovel}, Channels: []string{"ebook", "web_novel", "content_store"}, RequiredGates: []string{"canon_consistency", "editor_review", "rights_review", "release_preview"}, DeliverySchema: "contentcloud.novel-release/1.0",
		Stages: []Stage{
...
func (v Profile) Validate() error {
	if strings.TrimSpace(v.ID) == "" || strings.TrimSpace(v.Version) == "" || strings.TrimSpace(v.Name) == "" || len(v.ContentTypes) == 0 || len(v.Stages) == 0 || strings.TrimSpace(v.DeliverySchema) == "" {
		return fault.Invalid("CONTENT_PROFILE_INVALID", "内容 Profile 缺少 ID、版本、内容类型、Stage 或交付 Schema")
	}
	gates := map[string]bool{}
	for _, gate := range v.RequiredGates {
		if strings.TrimSpace(gate) == "" || gates[gate] {
			return fault.Invalid("CONTENT_PROFILE_GATE_INVALID", "内容 Profile Gate 为空或重复")
		}
		gates[gate] = true
	}
	...
	for _, stage := range v.Stages {
		if stage.ID == "" || stage.Name == "" || stage.CapabilityID == "" || stage.OutputSchema == "" || len(stage.ExecutorKinds) == 0 || stages[stage.ID] {
			return fault.Invalid("CONTENT_PROFILE_STAGE_INVALID", "内容 Profile Stage 缺少唯一 ID、能力、执行者或输出 Schema")
		}
		stages[stage.ID] = true
		for _, gate := range stage.GateIDs {
			if !gates[gate] {
				return fault.Invalid("CONTENT_PROFILE_GATE_REFERENCE_INVALID", "内容 Profile Stage 引用了未知 Gate")
			}
		}
	}
	return nil
