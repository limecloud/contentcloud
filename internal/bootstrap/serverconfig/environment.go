...
func LoadEnvironment(config EnvironmentConfig) (EnvironmentRuntime, error) {
	...
	if configured != len(values) {
		return EnvironmentRuntime{}, fault.Invalid("ENVIRONMENT_CONFIG_INCOMPLETE", "环境控制平面配置必须同时提供环境配置、插件注册表、两类信任库、签名文件、key_id 和能力发布版本")
	}
	if !releaseVersionPattern.MatchString(config.CapabilityReleaseVersion) {
		return EnvironmentRuntime{}, fault.Invalid("CAPABILITY_RELEASE_VERSION_INVALID", "能力发布版本必须是不可变的语义化版本")
	}
...
func validateCurrentEnvironmentProfile(profile environment.Profile, releaseVersion string) error {
	if releaseVersion != pluginidentity.VideoProductionVersion {
		return fault.Invalid("CAPABILITY_RELEASE_VERSION_MISMATCH", "能力发布版本与当前内置场景插件版本不一致")
	}
	if profile.WorkspaceTemplate != localworkspace.CurrentTemplateRef() {
		return fault.Invalid("ENVIRONMENT_WORKSPACE_TEMPLATE_MISMATCH", "创作环境配置必须使用当前工作区模板身份和摘要")
	}
	for _, candidate := range profile.Plugins {
		...
		if candidate.Kind != "scene_plugin" || candidate.Version != releaseVersion || !candidate.Required || candidate.Scope != "environment" {
			return fault.Invalid("ENVIRONMENT_PLUGIN_RELEASE_MISMATCH", "创作环境配置中的必装场景插件与当前发布版本不一致")
		}
		return nil
	}
	return fault.Invalid("ENVIRONMENT_PLUGIN_RELEASE_MISMATCH", "创作环境配置缺少当前必装场景插件")
}
...
func automationPolicy(profile environment.Profile, releaseVersion string) ([]environment.CapabilityRequirement, map[string][]string, error) {
	...
	for _, capabilityID := range profile.Capabilities {
		...
		if !exists {
			return nil, nil, fault.Policy("AUTOMATION_CAPABILITY_UNSUPPORTED", "创作环境配置包含服务端能力目录尚未实现的能力", "使用当前发布版本支持的确定性能力")
		}
...
func loadPrivateKey(path, repositoryRoot string) (ed25519.PrivateKey, error) {
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 {
		return nil, fault.Invalid("ENVIRONMENT_SIGNING_KEY_FILE_INVALID", "环境签名私钥必须是普通文件，不能是符号链接")
	}
	if info.Mode().Perm()&0o077 != 0 {
		return nil, fault.Policy("ENVIRONMENT_SIGNING_KEY_PERMISSIONS", "环境签名私钥不能允许用户组或其他用户访问", "将私钥权限收紧为 0400 或 0600")
	}
	if repositoryRoot != "" {
		...
		if inside {
			return nil, fault.Policy("ENVIRONMENT_SIGNING_KEY_IN_REPOSITORY", "环境签名私钥必须位于代码仓库之外", "将私钥移动到访问受限的密钥目录")
		}
	}
	...
	if len(body) != ed25519.PrivateKeySize {
		return nil, fault.Invalid("ENVIRONMENT_SIGNING_KEY_FORMAT", "环境签名私钥必须是 64 字节的 Ed25519 私钥二进制文件")
	}
