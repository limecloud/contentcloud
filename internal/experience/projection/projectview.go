...
type Focus = ProjectNavigationFocus
type Target = ProjectNavigation
...
func Build(serverBase, projectID string, target Target) (Link, error) {
	...
	if !idPattern.MatchString(projectID) {
		return Link{}, fault.Invalid("PROJECT_ID_INVALID", "工作区绑定中的 project_id 无效")
	}
...
func BuildStudioConnect(serverBase, sessionID string) (string, error) {
	...
	if !idPattern.MatchString(sessionID) {
		return "", fault.Invalid("CONNECT_SESSION_ID_INVALID", "执行客户端连接会话标识无效")
	}
...
func Validate(target Target) error {
	...
	if !ok {
		return fault.Invalid("PROJECT_VIEW_INVALID", "不支持该 Content Work OS 项目视图")
	}
	if target.Focus != nil {
		focusContract, ok := allowedFocus(page, target.Focus.Kind)
		if !ok || !idPattern.MatchString(target.Focus.ID) {
			return fault.Invalid("PROJECT_FOCUS_INVALID", "项目页面焦点与视图不匹配或 ID 无效")
		}
		if target.Focus.Digest != "" && !digestPattern.MatchString(target.Focus.Digest) {
			return fault.Invalid("PROJECT_FOCUS_DIGEST_INVALID", "项目页面焦点摘要（digest）必须是完整的 sha256 摘要")
		}
		if focusContract.DigestRequired && target.Focus.Digest == "" {
			return fault.Invalid("PROJECT_FOCUS_DIGEST_REQUIRED", "该项目页面焦点需要不可变的版本摘要")
		}
	}
	return nil
}
...
func trustedServerBase(value string) (*url.URL, error) {
	...
	if err != nil || parsed.Host == "" || parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" || (parsed.Path != "" && parsed.Path != "/") {
		return nil, fault.Invalid("WEB_TARGET_UNTRUSTED", "工作区绑定中的 server_url 必须是可信的 Content Work OS 服务地址")
	}
	hostname := strings.ToLower(parsed.Hostname())
	secure := parsed.Scheme == "https"
	localHTTP := parsed.Scheme == "http" && (hostname == "localhost" || net.ParseIP(hostname) != nil && net.ParseIP(hostname).IsLoopback())
	if !secure && !localHTTP {
		return nil, fault.Invalid("WEB_TARGET_UNTRUSTED", "Content Work OS 工作台只允许使用 HTTPS；本机开发仅允许访问环回地址的 HTTP 服务")
	}
	parsed.Path = ""
	return parsed, nil
}
