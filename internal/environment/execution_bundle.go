...
func (issuer *Issuer) SignBundle(bundle CreativeExecutionBundle) (CreativeExecutionBundle, error) {
	if issuer == nil {
		return CreativeExecutionBundle{}, domain.Invalid("EXECUTION_BUNDLE_SIGNER_REQUIRED", "CreativeExecutionBundle 需要有效签发器")
	}
	bundle.BundleID = ""
	bundle.Digest = ""
...
func (verifier *Verifier) VerifyBundle(bundle CreativeExecutionBundle, manifest Manifest, registry VerifiedRegistry, options BundleVerifyOptions) error {
	if verifier == nil {
		return domain.Invalid("EXECUTION_BUNDLE_VERIFIER_REQUIRED", "CreativeExecutionBundle 需要可信 verifier")
	}
	...
	if bundle.IssuedAt.After(now.Add(5 * time.Minute)) {
		return domain.Conflict("EXECUTION_BUNDLE_NOT_YET_VALID", "CreativeExecutionBundle 签发时间晚于本机允许的时钟偏差")
	}
	if !now.Before(bundle.ExpiresAt) {
		return domain.Conflict("EXECUTION_BUNDLE_EXPIRED", "CreativeExecutionBundle 已过期")
	}
	identity, err := executionBundleIdentityBytes(bundle)
	if err != nil {
		return err
	...
	if expected := "ceb_" + hex.EncodeToString(identitySum[:]); bundle.BundleID != expected {
		return domain.Conflict("EXECUTION_BUNDLE_ID_MISMATCH", "CreativeExecutionBundle 内容与 bundle_id 不一致")
	}
	payload, err := executionBundlePayloadBytes(bundle)
	if err != nil {
		return err
	...
	if expected := "sha256:" + hex.EncodeToString(payloadSum[:]); bundle.Digest != expected {
		return domain.Conflict("EXECUTION_BUNDLE_DIGEST_MISMATCH", "CreativeExecutionBundle 内容与 digest 不一致")
	}
	key, exists := verifier.keys[bundle.Signature.KeyID]
	if !exists || key.Status != "active" {
		return domain.Policy("EXECUTION_BUNDLE_SIGNING_KEY_UNTRUSTED", "CreativeExecutionBundle 签名 key 不受信任或已撤销", "刷新 ContentCloud CLI/Plugin 的可信公钥后重试")
	}
	signature, err := base64.StdEncoding.Strict().DecodeString(bundle.Signature.Value)
	if err != nil || len(signature) != ed25519.SignatureSize {
		return domain.Invalid("EXECUTION_BUNDLE_SIGNATURE_INVALID", "CreativeExecutionBundle 签名格式无效")
	}
	signed, err := executionBundleSignedBytes(bundle.Digest)
	if err != nil {
		return err
	...
	if !ed25519.Verify(key.PublicKey, signed, signature) {
		return domain.Conflict("EXECUTION_BUNDLE_SIGNATURE_MISMATCH", "CreativeExecutionBundle 签名验证失败")
	}
	...
	if bundle.ProjectID != manifest.ProjectID || bundle.ProjectID != projectID {
		return domain.Conflict("EXECUTION_BUNDLE_PROJECT_MISMATCH", "CreativeExecutionBundle 不属于当前项目")
	}
	if bundle.EnvironmentVersion != manifest.EnvironmentVersion {
		return domain.Conflict("EXECUTION_BUNDLE_ENVIRONMENT_MISMATCH", "CreativeExecutionBundle 与当前 Environment 版本不匹配")
	}
	if !emptyExecutionSubject(options.ExpectedSubject) && bundle.Subject != options.ExpectedSubject {
		return domain.Conflict("EXECUTION_BUNDLE_SUBJECT_MISMATCH", "CreativeExecutionBundle 与冻结业务对象不匹配")
	}
	if err := validateBundleAgainstManifest(bundle, manifest); err != nil {
		return err
	}
...
func (resolver *Resolver) ResolveBundle(bundle CreativeExecutionBundle, manifest Manifest, registry VerifiedRegistry, lock EnvironmentLock, capabilities []domain.Capability, options BundleVerifyOptions) (BundleResolution, error) {
	if resolver == nil || resolver.verifier == nil {
		return BundleResolution{}, domain.Invalid("ENVIRONMENT_RESOLVER_REQUIRED", "CreativeExecutionBundle 需要 Environment Resolver")
	}
...
func selectBundlePacks(manifest Manifest, packIDs []string) ([]PackRequirement, error) {
	...
	for _, id := range packIDs {
		if _, exists := seen[id]; exists {
			return nil, domain.Conflict("EXECUTION_BUNDLE_PACK_DUPLICATED", "CreativeExecutionBundle 请求包含重复 Pack")
		}
		seen[id] = struct{}{}
		plugin, exists := available[id]
		if !exists || plugin.Scope != "task" || (plugin.Kind != "skill_pack" && plugin.Kind != "provider_mcp_pack") {
			return nil, domain.Policy("EXECUTION_BUNDLE_PACK_DENIED", "CreativeExecutionBundle 只能选择 Manifest allowlist 中的任务级 Pack", "通过 ContentCloud Environment Profile 配置精选 Pack")
		}
...
func validateBundle(bundle CreativeExecutionBundle, requireSignature bool) error {
	if bundle.SchemaVersion != ExecutionBundleSchemaVersion || strings.TrimSpace(bundle.ProjectID) == "" || !dottedIDPattern.MatchString(bundle.ProfileID) || !versionPattern.MatchString(bundle.EnvironmentVersion) {
		return domain.Invalid("EXECUTION_BUNDLE_INVALID", "CreativeExecutionBundle 缺少有效项目、Profile 或 Environment 版本")
	}
	if !dottedIDPattern.MatchString(bundle.Subject.Type) || strings.TrimSpace(bundle.Subject.ID) == "" || !digestPattern.MatchString(bundle.Subject.Digest) {
		return domain.Invalid("EXECUTION_BUNDLE_SUBJECT_INVALID", "CreativeExecutionBundle subject 缺少有效类型、ID 或 digest")
	}
	if bundle.IssuedAt.IsZero() || bundle.ExpiresAt.IsZero() || !bundle.ExpiresAt.After(bundle.IssuedAt) {
		return domain.Invalid("EXECUTION_BUNDLE_TIME_INVALID", "CreativeExecutionBundle 签发或过期时间无效")
	}
	if len(bundle.RequiredCapabilities) == 0 {
		return domain.Invalid("EXECUTION_BUNDLE_CAPABILITIES_REQUIRED", "CreativeExecutionBundle 至少需要一个 capability")
	}
	...
	for _, capability := range bundle.RequiredCapabilities {
		if !dottedIDPattern.MatchString(capability.ID) || !versionPattern.MatchString(capability.SchemaVersion) || !digestPattern.MatchString(capability.Digest) {
			return domain.Invalid("EXECUTION_BUNDLE_CAPABILITY_INVALID", "CreativeExecutionBundle capability 引用无效")
		}
		if _, exists := capabilities[capability.ID]; exists {
			return domain.Conflict("EXECUTION_BUNDLE_CAPABILITY_DUPLICATED", "CreativeExecutionBundle 包含重复 capability")
		}
	...
	for _, pack := range bundle.Packs {
		if !pluginIDPattern.MatchString(pack.ID) || (pack.Kind != "skill_pack" && pack.Kind != "provider_mcp_pack") || !versionPattern.MatchString(pack.PluginVersion) || !digestPattern.MatchString(pack.Digest) || pack.Scope != "task" || !pack.Required {
			return domain.Invalid("EXECUTION_BUNDLE_PACK_INVALID", "CreativeExecutionBundle Pack 引用无效")
		}
		if _, exists := packs[pack.ID]; exists {
			return domain.Conflict("EXECUTION_BUNDLE_PACK_DUPLICATED", "CreativeExecutionBundle 包含重复 Pack")
		}
	...
	if requireSignature {
		if len(bundle.BundleID) != 68 || !strings.HasPrefix(bundle.BundleID, "ceb_") || !digestPattern.MatchString(bundle.Digest) || bundle.Signature.Algorithm != "ed25519" || !dottedIDPattern.MatchString(bundle.Signature.KeyID) || strings.TrimSpace(bundle.Signature.Value) == "" {
			return domain.Invalid("EXECUTION_BUNDLE_SIGNATURE_INVALID", "CreativeExecutionBundle 缺少有效 ID、digest 或 Ed25519 签名")
		}
	}
	return nil
}
...
func validateBundleAgainstManifest(bundle CreativeExecutionBundle, manifest Manifest) error {
	if bundle.ProjectID != manifest.ProjectID || bundle.ProfileID != manifest.ProfileID || bundle.EnvironmentVersion != manifest.EnvironmentVersion {
		return domain.Conflict("EXECUTION_BUNDLE_ENVIRONMENT_MISMATCH", "CreativeExecutionBundle 与 Environment Manifest 不匹配")
	}
	...
	for _, pack := range bundle.Packs {
		...
		if matched == nil {
			return domain.Policy("EXECUTION_BUNDLE_PACK_DENIED", "CreativeExecutionBundle Pack 不在当前 Manifest allowlist 或 digest 不匹配", "刷新项目 Environment Manifest 和执行 Bundle")
		}
	...
	for _, capability := range bundle.RequiredCapabilities {
		if _, allowed := allowedCapabilities[capability.ID]; !allowed {
			return domain.Policy("EXECUTION_BUNDLE_CAPABILITY_DENIED", "CreativeExecutionBundle capability 超出 Environment Manifest allowlist", "选择当前项目 Profile 允许的 capability")
		}
		if _, available := provided[capability.ID]; !available {
			return domain.Conflict("EXECUTION_BUNDLE_CAPABILITY_UNRESOLVED", "CreativeExecutionBundle capability 没有对应的环境 Plugin 或任务 Pack")
		}
	}
	return nil
}
...
func validateBundleRegistry(bundle CreativeExecutionBundle, manifest Manifest, verifiedRegistry VerifiedRegistry) error {
	...
	for _, pack := range bundle.Packs {
		...
		if entry.Kind != pack.Kind || !contains(entry.CompatibleProfiles, manifest.ProfileID) {
			return domain.Policy("EXECUTION_BUNDLE_REGISTRY_MISMATCH", "CreativeExecutionBundle Pack 与 Marketplace Registry 元数据不匹配", "刷新精选 Marketplace Registry")
		}
	}
	return nil
}
