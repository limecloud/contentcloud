...
func TestManifestSignatureBindsPayloadProjectExpiryAndTrust(t *testing.T) {
	...
	tampered.Capabilities[0] = "contentcloud.script.tampered"
...
func TestBuildManifestUsesOnlyExactPublishedCompatibleRegistryEntries(t *testing.T) {
	...
	if manifest.Distribution.Plugins[0].SourceRef != "v0.5.0" || manifest.Distribution.Plugins[1].SourceRef != "v1.2.0" {
...
func fixtureProfileAndRegistry() (environment.Profile, environment.Registry) {
	...
	profile := environment.Profile{
		...
		Plugins: []environment.ProfilePlugin{
			{ID: "contentcloud-video-production", Kind: "scene_plugin", Version: "0.5.0", Required: true, Scope: "environment", Capabilities: []string{"contentcloud.script.generate"}},
		...
		Capabilities:      []string{"contentcloud.script.generate", "contentcloud.asset.generate"},
	...
	registry := environment.Registry{SchemaVersion: "1.0", Entries: []environment.RegistryEntry{
		registryEntry("contentcloud-video-production", "scene_plugin", "0.5.0", "https://github.com/limecloud/contentcloud", "v0.5.0", sceneDigest, []string{profile.ID}),
