...
func TestEvaluateJoinUsesFrozenMembershipAndExplicitZeroPolicy(t *testing.T) {
	...
	set := FanoutSet{ID: "set-1", TenantID: "tenant-1", JobRunID: "job-1", MapNodeKey: "map", JoinNodeKey: "join", Generation: 1, IdempotencyKey: "fanout-1", MembershipDigest: "sha256:" + strings.Repeat("a", 64), RequestDigest: "sha256:" + strings.Repeat("b", 64), MemberCount: 0, JoinPolicy: JoinPolicy{Strategy: JoinAll, ZeroMemberPolicy: ZeroMemberFail}, Status: FanoutClosed, Version: 1, ClosedAt: &now, CreatedAt: now, UpdatedAt: now}
	decision, err := EvaluateJoin(set, []FanoutMember{})
	if err != nil || !decision.Terminal || decision.Status != FanoutFailed {
		t.Fatalf("zero-member fail policy was not applied: %#v err=%v", decision, err)
	}
	set.JoinPolicy.ZeroMemberPolicy = ZeroMemberSucceedEmpty
	decision, err = EvaluateJoin(set, []FanoutMember{})
	if err != nil || decision.Status != FanoutSucceeded {
...
func TestCreateAndJoinFanoutSetIsAtomicAndDeterministic(t *testing.T) {
	...
	created, err := service.CreateFanoutSet(t.Context(), CreateFanoutSetInput{
		...
		IdempotencyKey: "fanout-set-1", Reason: "按冻结受众集合展开", JoinPolicy: JoinPolicy{Strategy: JoinQuorum, QuorumPercent: 50, ZeroMemberPolicy: ZeroMemberFail, QuorumStopPolicy: QuorumCancelPending},
		NodeTemplate: JobPlanNode{Kind: "stage", Name: "受众脚本", OutputSchema: "contentcloud.script/1.0", RetryMaxAttempts: 1},
		Items:        []FanoutItemInput{{ItemKey: "audience:b", ItemDigest: "sha256:b"}, {ItemKey: "audience:a", ItemDigest: "sha256:a"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	if created.Set.Status != FanoutClosed || created.Set.MemberCount != 2 || len(created.Members) != 2 || len(created.Nodes) != 2 {
	...
	if _, err := service.TransitionNode(t.Context(), "tenant-1", first.ID, NodeReady, "runtime", "scheduler", first.Version); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.TransitionNode(t.Context(), "tenant-1", first.ID, NodeLeased, "runtime", "worker-1", first.Version); err != nil {
		t.Fatal(err)
	}
	first, err = repo.NodeRun(t.Context(), "tenant-1", first.ID)
	if err != nil {
	...
	if _, err := service.TransitionNode(t.Context(), "tenant-1", first.ID, NodeRunning, "worker", "worker-1", first.Version); err != nil {
		t.Fatal(err)
	}
	first, err = repo.NodeRun(t.Context(), "tenant-1", first.ID)
	if err != nil {
	...
	for _, member := range members {
		if member.ItemKey == "audience:b" && member.State != FanoutMemberCancelled {
...
func TestRuntimeSupportsArticleRetrospectiveFanoutAtCapacityWithoutBusinessTables(t *testing.T) {
	...
	sop.ContentTypes = []string{identitydomain.ContentTypeWeChatArticle}
	sop.Gates = nil
