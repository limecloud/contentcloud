...
func (p *Projector) RunOnce(ctx context.Context, tenantID, worker string, leaseFor time.Duration, limit int) (ProjectionRunResult, error) {
	...
	if p == nil || p.repo == nil {
		return result, fault.Policy("RUNTIME_PROJECTOR_UNAVAILABLE", "Runtime 投影器未配置存储", "配置 Runtime Repository 后重试")
	}
	commands := p.repo
	if leaseFor <= 0 {
	...
	if leaseOwner == "" {
		return result, fault.Invalid("RUNTIME_PROJECTOR_WORKER_REQUIRED", "Runtime 投影器需要稳定的工作器身份")
	}
	messages, err := commands.ClaimRuntimeOutbox(ctx, tenantID, RuntimeOutboxSubscriberProjection, leaseOwner, now, leaseFor, limit)
	if err != nil {
		return result, err
	}
	...
	for _, message := range messages {
		...
		if projectErr == nil {
			...
			if projectErr == nil {
				...
				view := RuntimeExplorerView{TenantID: tenantID, JobRunID: jobID, Job: job, Nodes: nodes, LastEventSeq: lastSequence, SourceEventID: message.EventID, ProjectedAt: now}
		...
		if projectErr != nil {
			...
			if hasDomainCode(projectErr, "RUNTIME_PROJECTION_STALE") {
				if err := commands.AckRuntimeOutbox(ctx, tenantID, message.ID, RuntimeOutboxSubscriberProjection, leaseOwner, now); err != nil {
					return result, err
				}
				result.Projected++
				continue
			...
			if retryErr := commands.RetryRuntimeOutbox(ctx, tenantID, message.ID, RuntimeOutboxSubscriberProjection, leaseOwner, now, retryAt, projectErr.Error()); retryErr != nil {
...
func (s *Service) RuntimeExplorer(ctx context.Context, tenantID, jobID string) (RuntimeExplorerView, error) {
	return s.repo.RuntimeExplorer(ctx, tenantID, jobID)
}

func (s *Service) RuntimeProjectionStats(ctx context.Context, tenantID string) (RuntimeProjectionStats, error) {
	return s.repo.RuntimeProjectionStats(ctx, tenantID)
}

func (s *Service) RuntimeOutboxStats(ctx context.Context, tenantID, subscriber string) (RuntimeOutboxStats, error) {
	return s.repo.RuntimeOutboxStats(ctx, tenantID, subscriber)
}

func (s *Service) SaveRuntimeMaintenanceHeartbeat(ctx context.Context, heartbeat RuntimeMaintenanceHeartbeat) error {
	return s.repo.SaveRuntimeMaintenanceHeartbeat(ctx, heartbeat)
}

func (s *Service) RuntimeMaintenanceHeartbeat(ctx context.Context, tenantID, kind string) (RuntimeMaintenanceHeartbeat, error) {
