...
type DispatchHandle struct {
	...
	Attempt       RuntimeAttempt                   `json:"attempt"`
...
type RemoteExecutionSpec struct {
	...
	TaskContract         sourcedomain.TaskContract `json:"task_contract"`
...
func (s *Service) Attempt(ctx context.Context, tenantID, id string) (RuntimeAttempt, error) {
	return s.repo.RuntimeAttempt(ctx, tenantID, id)
}

func (s *Service) Attempts(ctx context.Context, tenantID, jobID string) ([]RuntimeAttempt, error) {
...
func (s *Service) LoadDispatchHandle(ctx context.Context, tenantID, attemptID string) (DispatchHandle, error) {
	if s == nil || s.repo == nil {
		return DispatchHandle{}, fault.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime")
	}
...
func (s *Service) PrepareDispatch(ctx context.Context, input DispatchInput) (DispatchHandle, error) {
	...
	if s.harnesses == nil {
		return DispatchHandle{}, fault.Policy("AGENT_HARNESS_REGISTRY_UNAVAILABLE", "Runtime 尚未注入智能体执行适配器注册表", "联系平台运营人员检查 Runtime 启动配置")
	}
...
func (s *Service) PrepareRemoteDispatch(ctx context.Context, input DispatchInput, capabilities agentadapter.HarnessCapabilities) (DispatchHandle, error) {
	...
	if capabilities.Kind == "" || capabilities.Kind != strings.ToLower(strings.TrimSpace(input.HarnessKind)) {
		return DispatchHandle{}, fault.Invalid("AGENT_HARNESS_KIND_MISMATCH", "远程 worker 的 Harness 能力声明与请求类型不一致")
	}
	if capabilities.MaxParallelSessions < 1 || capabilities.MaxParallelSessions > 1024 {
		return DispatchHandle{}, fault.Invalid("AGENT_HARNESS_CAPACITY_INVALID", "远程 worker 的并发会话能力无效")
	}
	return s.prepareDispatchWithRetry(ctx, input, capabilities)
}
...
func (s *Service) PrepareAdmittedRemoteDispatch(ctx context.Context, input RemoteAdmissionInput) (DispatchHandle, error) {
	...
	if input.TenantID == "" || input.Owner == "" || input.HarnessKind == "" {
		return DispatchHandle{}, fault.Invalid("DISPATCH_INPUT_INVALID", "远程调度请求缺少租户、执行者或适配器")
	}
...
func (s *Service) remoteAdmissionCandidate(ctx context.Context, input RemoteAdmissionInput) (NodeRun, JobRun, error) {
	var job JobRun
	var node NodeRun
	var err error
	if input.JobRunID == "" {
		...
		if err == nil && !dispatchProjectAllowed(job.ProjectID, input.AllowedProjectIDs) {
			return node, job, fault.Policy("DISPATCH_PROJECT_SCOPE_DENIED", "执行设备未获授权处理该项目", "为设备授予项目执行权限后重试")
		}
		if err == nil {
...
func (s *Service) remoteDispatchInput(ctx context.Context, admission RemoteAdmissionInput, job JobRun, node NodeRun) (DispatchInput, RemoteExecutionSpec, error) {
	plan, err := s.repo.Plan(ctx, admission.TenantID, job.PlanRevisionID)
	if err != nil {
		return DispatchInput{}, RemoteExecutionSpec{}, err
	}
	var nodeSpec JobPlanNode
	found := false
	...
	if !found {
		return DispatchInput{}, RemoteExecutionSpec{}, fault.Conflict("DISPATCH_NODE_SPEC_MISSING", "执行节点在冻结计划中没有对应 NodeSpec")
	}
	...
	if !remoteHarnessAllowed(binding, nodeSpec, admission.HarnessKind) {
		return DispatchInput{}, RemoteExecutionSpec{}, fault.Policy("DISPATCH_HARNESS_NOT_ALLOWED", "当前 Harness 不符合节点冻结的执行者约束", "使用节点允许的执行器重新领取")
	}
	if !remoteSandboxAllowed(binding, admission.Capabilities.SandboxProfile) {
		return DispatchInput{}, RemoteExecutionSpec{}, fault.Policy("DISPATCH_SANDBOX_NOT_ALLOWED", "当前 Harness 的隔离配置不符合冻结执行绑定", "使用满足执行绑定隔离要求的 Harness")
	}
...
func remoteHarnessAllowed(binding ExecutionBindingSnapshot, node JobPlanNode, harnessKind string) bool {
...
func remoteSandboxAllowed(binding ExecutionBindingSnapshot, observed string) bool {
...
func remoteNodeRole(node JobPlanNode) string {
	if node.Kind == "gate" {
...
func remoteNodePrompt(job JobRun, node JobPlanNode) string {
...
func (s *Service) prepareDispatch(ctx context.Context, input DispatchInput, capabilities agentadapter.HarnessCapabilities) (DispatchHandle, error) {
	...
	if input.TenantID == "" || input.Owner == "" || input.HarnessKind == "" || input.Role == "" || input.ExecutionProfileID == "" {
		return DispatchHandle{}, fault.Invalid("DISPATCH_INPUT_INVALID", "调度请求缺少租户、执行者、适配器、角色或执行配置")
	}
	if !capabilities.Events || !capabilities.StructuredOutput {
		return DispatchHandle{}, fault.Policy("AGENT_HARNESS_CAPABILITY_MISSING", "当前适配器不支持 Runtime 所需的结构化事件和结果", "选择支持结构化事件的 Harness")
	}
	if input.MaxTokens <= 0 || input.BudgetMinor < 0 || input.RemainingDescendants < 0 {
		return DispatchHandle{}, fault.Invalid("DISPATCH_POLICY_INVALID", "调度请求的上下文、预算或派生额度无效")
	}
	if input.LeaseFor <= 0 {
	...
	if !dispatchProjectAllowed(job.ProjectID, input.AllowedProjectIDs) {
		return DispatchHandle{}, fault.Policy("DISPATCH_PROJECT_SCOPE_DENIED", "执行设备未获授权处理该项目", "为设备授予项目执行权限后重试")
	}
	if job.State == JobRunPaused {
		return DispatchHandle{}, fault.Conflict("JOB_RUN_PAUSED", "执行实例已暂停，不能领取新的执行节点")
	}
	if job.State == JobRunCompleted || job.State == JobRunFailed || job.State == JobRunCancelled || job.State == JobRunRejected {
		return DispatchHandle{}, fault.Conflict("JOB_RUN_TERMINAL", "执行实例已经结束，不能领取新的执行节点")
	}
	if node.ID == "" {
		node, err = s.repo.NextReadyNode(ctx, input.TenantID, input.JobRunID, input.AllowedProjectIDs)
		if err != nil {
	...
	gatewayToken, gatewayTokenHash, tokenErr := idgen.NewOpaqueToken("rtg_", 32)
	if tokenErr != nil {
		return DispatchHandle{}, tokenErr
	}
	...
	createAgent := fault.IsNotFound(agentErr)
	...
	if createAgent {
		agent = AgentInstance{
			ID: idgen.New(), TenantID: input.TenantID, JobRunID: node.JobRunID, NodeRunID: node.ID,
			Role: input.Role, HarnessKind: input.HarnessKind, ExecutionProfileID: input.ExecutionProfileID,
			ContextViewID: view.ID, State: AgentRunnable, RemainingDescendants: input.RemainingDescendants,
			BudgetMinor: input.BudgetMinor, UsedCostMinor: 0, Version: 1, CreatedAt: now, UpdatedAt: now,
		}
	} else {
		if agent.State != AgentRunnable || agent.HarnessKind != input.HarnessKind || agent.ExecutionProfileID != input.ExecutionProfileID {
			return DispatchHandle{}, fault.Conflict("AGENT_INSTANCE_NOT_REUSABLE", "节点 AgentInstance 尚未让出执行权或执行配置发生变化")
		}
		...
		if strings.TrimSpace(agent.SessionRef) != "" && capabilities.Resume {
			...
			if err := json.Unmarshal([]byte(agent.SessionRef), &session); err != nil || session.SessionID == "" || session.HarnessKind != input.HarnessKind {
				return DispatchHandle{}, fault.Conflict("AGENT_SESSION_REF_INVALID", "可恢复 AgentInstance 的会话引用无效")
			}
			resumeSession = &session
		}
	...
	attempt := RuntimeAttempt{
		...
		HarnessKind: input.HarnessKind, Capabilities: capabilitySnapshot, State: RuntimeAttemptPrepared,
	...
	if err := node.Transition(NodeLeased); err != nil {
		return DispatchHandle{}, err
	}
	...
	node.UpdatedAt = now
	event := JobEvent{
		ID: idgen.New(), TenantID: input.TenantID, JobRunID: node.JobRunID, NodeKey: node.NodeKey,
	...
	reservations := make([]ResourceReservation, 0, len(input.ResourceRequests))
	for index, request := range input.ResourceRequests {
		if strings.TrimSpace(request.ResourceKey) == "" || request.Quantity <= 0 || strings.TrimSpace(request.Unit) == "" {
			return DispatchHandle{}, fault.Invalid("RESOURCE_REQUEST_INVALID", "资源请求缺少资源键、正数数量或单位")
		}
		reservations = append(reservations, ResourceReservation{
			ID: idgen.New(), TenantID: input.TenantID, JobRunID: node.JobRunID, NodeRunID: node.ID, AttemptID: attempt.ID,
			ResourceKey: request.ResourceKey, Quantity: request.Quantity, Unit: request.Unit, State: ReservationHeld,
...
func (s *Service) ActivateDispatch(ctx context.Context, handle DispatchHandle, session agentadapter.AgentSessionRef) (DispatchHandle, error) {
	if session.SessionID == "" || len(session.SessionID) > 1024 || strings.TrimSpace(session.HarnessKind) != handle.Attempt.HarnessKind {
		return handle, fault.Invalid("DISPATCH_SESSION_INVALID", "Harness 会话引用与 RuntimeAttempt 不一致")
	}
	...
	if err := node.Transition(NodeRunning); err != nil {
		return handle, err
	}
	if err := attempt.Transition(RuntimeAttemptRunning); err != nil {
		return handle, err
	}
	...
	node.Version++
	node.UpdatedAt = now
	attempt.State = RuntimeAttemptRunning
	...
	agent.Version++
	agent.UpdatedAt = now
	event := JobEvent{ID: idgen.New(), TenantID: node.TenantID, JobRunID: node.JobRunID, NodeKey: node.NodeKey, Type: "attempt.running", ActorType: "worker", ActorID: attempt.LeaseOwner, IdempotencyKey: attempt.ID + ":running", Payload: map[string]any{"attempt_id": attempt.ID, "harness_kind": attempt.HarnessKind}, OccurredAt: now}
...
func (s *Service) FinalizeDispatch(ctx context.Context, handle DispatchHandle, outcome DispatchOutcome) (DispatchResult, error) {
	...
	if outcome.State == "" {
		outcome.State = RuntimeAttemptSucceeded
	}
	if outcome.State == RuntimeAttemptRetryableFailed {
		maxAttempts, err := s.maxAttemptsForNode(ctx, node)
		if err != nil {
			return DispatchResult{Handle: handle}, err
		}
		if attempt.AttemptNo >= maxAttempts {
			outcome.State = RuntimeAttemptFailed
		}
	}
	if outcome.State != RuntimeAttemptSucceeded && outcome.State != RuntimeAttemptRetryableFailed && outcome.State != RuntimeAttemptFailed && outcome.State != RuntimeAttemptCancelled {
		return DispatchResult{Handle: handle}, fault.Invalid("DISPATCH_OUTCOME_INVALID", "RuntimeAttempt 终态结果无效")
	}
	if outcome.State == RuntimeAttemptSucceeded {
		outcome.ErrorCode = ""
	} else {
	...
	if outcome.UsedCostMinor < agent.UsedCostMinor || outcome.UsedCostMinor > agent.BudgetMinor {
		return DispatchResult{Handle: handle}, fault.Policy("AGENT_INSTANCE_USAGE_INVALID", "AgentInstance 用量不能回退或超过预算", "重新核对累计用量")
	}
	...
	if attempt.Terminal() {
		...
		if !storedAttempt.Terminal() || storedAttempt.State != outcome.State || storedAttempt.ResultDigest != outcome.ResultDigest {
			return DispatchResult{Handle: handle}, fault.Conflict("RUNTIME_ATTEMPT_RESULT_CONFLICT", "RuntimeAttempt 已收到不同的终态结果")
		}
	...
	expectedNodeVersion, expectedAttemptVersion, expectedAgentVersion := node.Version, attempt.Version, agent.Version
	nodeState, agentState := NodeSucceeded, AgentCompleted
	switch outcome.State {
	case RuntimeAttemptRetryableFailed:
		nodeState, agentState = NodeRetryableFailed, AgentRunnable
	case RuntimeAttemptFailed:
		nodeState, agentState = NodeFailed, AgentFailed
	case RuntimeAttemptCancelled:
		nodeState, agentState = NodeCancelled, AgentCancelled
	}
	...
	if node.State == NodeSucceeded && node.OutputDigest == "" {
...
func (s *Service) DispatchNext(ctx context.Context, input DispatchInput) (DispatchResult, error) {
	if s == nil || s.harnesses == nil {
		return DispatchResult{}, fault.Policy("AGENT_HARNESS_REGISTRY_UNAVAILABLE", "Runtime 尚未注入智能体执行适配器注册表", "联系平台运营人员检查 Runtime 启动配置")
	}
	...
	if err != nil {
		return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: errorCode(err, sessionFailureCode), UsedCostMinor: handle.Agent.UsedCostMinor})
	}
	if stream == nil {
		return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_STREAM_MISSING", UsedCostMinor: handle.Agent.UsedCostMinor})
	}
	defer stream.Close()
	handle, err = s.ActivateDispatch(ctx, handle, session)
	if err != nil {
		_ = harness.Interrupt(context.WithoutCancel(ctx), session)
		cleanup, cleanupErr := s.FinalizeDispatch(context.WithoutCancel(ctx), handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "DISPATCH_ACTIVATION_FAILED", UsedCostMinor: handle.Agent.UsedCostMinor})
		if cleanupErr != nil {
	...
	for {
		...
		case <-ctx.Done():
			...
			result, finalizeErr := s.FinalizeDispatch(cleanupContext, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "DISPATCH_CONTEXT_CANCELLED", UsedCostMinor: handle.Agent.UsedCostMinor})
			if finalizeErr != nil {
		...
		case event, ok := <-stream.Events():
			if !ok {
				return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_STREAM_CLOSED", UsedCostMinor: handle.Agent.UsedCostMinor})
			}
			if event.Session != session {
				return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptFailed, ErrorCode: "HARNESS_SESSION_MISMATCH", UsedCostMinor: handle.Agent.UsedCostMinor})
			}
			switch event.Type {
			case "result.completed":
				outcome, parseErr := outcomeFromHarnessResult(event.Data)
				if parseErr != nil {
					return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_RESULT_INVALID", UsedCostMinor: handle.Agent.UsedCostMinor})
				}
				return s.FinalizeDispatch(ctx, handle, outcome)
			case "session.failed":
				return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: safeErrorCode(event.ErrorCode, "HARNESS_SESSION_FAILED"), UsedCostMinor: handle.Agent.UsedCostMinor})
			case "session.interrupted":
				return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_SESSION_INTERRUPTED", UsedCostMinor: handle.Agent.UsedCostMinor})
			case "runtime.yield":
				var request struct {
				...
				if len(event.Data) == 0 || json.Unmarshal(event.Data, &request) != nil {
					return s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_YIELD_INVALID", UsedCostMinor: handle.Agent.UsedCostMinor})
				}
...
func outcomeFromHarnessResult(data json.RawMessage) (DispatchOutcome, error) {
	...
	if len(data) == 0 || json.Unmarshal(data, &result) != nil {
		return DispatchOutcome{}, fault.Invalid("HARNESS_RESULT_INVALID", "Harness 返回的结构化结果无效")
	}
	var canonical any
	if err := json.Unmarshal(data, &canonical); err != nil {
		return DispatchOutcome{}, err
	}
	digest, err := stablehash.Sum(canonical)
	if err != nil {
		return DispatchOutcome{}, err
	}
	return DispatchOutcome{State: RuntimeAttemptSucceeded, OutputRefs: result.OutputRefs, OutputDigest: result.OutputDigest, ResultDigest: "sha256:" + digest, SafeSummary: sanitizeSafeSummary(result.SafeSummary), UsedCostMinor: result.UsedCostMinor}, nil
}
...
func (s *Service) RecordHarnessEvent(ctx context.Context, handle DispatchHandle, event agentadapter.AgentEvent) error {
	...
	if handle.Attempt.State != RuntimeAttemptRunning || json.Unmarshal([]byte(handle.Attempt.SessionRef), &boundSession) != nil || boundSession != event.Session {
		return fault.Conflict("HARNESS_SESSION_MISMATCH", "Harness 事件会话与当前 RuntimeAttempt 不一致")
	}
	if len(event.Data) > 64<<10 {
		return fault.Policy("HARNESS_EVENT_TOO_LARGE", "Harness 结构化事件超过持久化上限", "只上报事件类型、状态和受控摘要")
	}
	dataDigest := ""
	if len(event.Data) > 0 {
		var value any
		if json.Unmarshal(event.Data, &value) == nil {
			if digest, err := stablehash.Sum(value); err == nil {
				dataDigest = "sha256:" + digest
			}
		}
	}
	commands := s.repo
	dedupDigest, err := stablehash.Sum(struct {
		AttemptID string
		Type      string
		Data      string
		ErrorCode string
	...
	now := s.now().UTC()
	_, err = commands.AppendFencedRuntimeEvent(ctx, handle.Node.TenantID, handle.Attempt.ID, handle.Attempt.LeaseOwner, handle.Attempt.FenceToken, now, JobEvent{ID: idgen.New(), TenantID: handle.Node.TenantID, JobRunID: handle.Node.JobRunID, NodeKey: handle.Node.NodeKey, Type: "attempt.event", ActorType: "harness", ActorID: handle.Attempt.HarnessKind, IdempotencyKey: handle.Attempt.ID + ":event:" + dedupDigest, Payload: map[string]any{"attempt_id": handle.Attempt.ID, "event_type": safeEventType(event.Type), "data_digest": dataDigest, "error_code": safeErrorCode(event.ErrorCode, "")}, OccurredAt: now})
	return err
}

func (s *Service) maxAttemptsForNode(ctx context.Context, node NodeRun) (int, error) {
...
func harnessCapabilitiesFromMap(kind string, values map[string]any) (agentadapter.HarnessCapabilities, error) {
	...
	if err := json.Unmarshal(body, &capabilities); err != nil {
		return capabilities, fault.Invalid("AGENT_HARNESS_CAPABILITIES_INVALID", "RuntimeAttempt 的 Harness 能力快照无效")
	}
	capabilities.Kind = strings.ToLower(strings.TrimSpace(capabilities.Kind))
	...
	if capabilities.Kind != strings.ToLower(strings.TrimSpace(kind)) || !capabilities.Events || !capabilities.StructuredOutput {
		return capabilities, fault.Conflict("AGENT_HARNESS_CAPABILITIES_INVALID", "RuntimeAttempt 的 Harness 能力快照不满足执行协议")
	}
