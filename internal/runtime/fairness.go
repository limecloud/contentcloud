...
func (s *Service) FairnessReport(ctx context.Context, tenantIDs []string, resourceKey string) (domain.RuntimeFairnessReport, error) {
	if s == nil || s.repo == nil {
		return domain.RuntimeFairnessReport{}, domain.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime")
	}
	ids := uniqueTenantIDs(tenantIDs)
	if len(ids) == 0 {
		return domain.RuntimeFairnessReport{}, domain.Invalid("RUNTIME_FAIRNESS_TENANTS_REQUIRED", "公平性观测至少需要一个租户")
	}
	resourceKey = strings.TrimSpace(resourceKey)
	now := s.now().UTC()
	rows := make([]domain.RuntimeTenantCapacity, 0, len(ids))
	unit := ""
	for _, tenantID := range ids {
		quotas, err := s.repo.ResourceQuotas(ctx, tenantID)
		if err != nil {
			return domain.RuntimeFairnessReport{}, err
		}
		reservations, err := s.repo.ResourceReservations(ctx, tenantID, "")
		if err != nil {
			return domain.RuntimeFairnessReport{}, err
		}
		byKey := make(map[string]domain.ResourceQuota, len(quotas))
		...
		if resourceKey != "" {
			if _, ok := byKey[resourceKey]; !ok {
				...
				byKey[resourceKey] = domain.ResourceQuota{TenantID: tenantID, ResourceKey: resourceKey}
			}
		}
		for key, quota := range byKey {
			row := capacityRow(tenantID, key, quota, reservations, now)
	...
	if resourceKey == "" {
		...
		if len(keys) != 1 {
			return domain.RuntimeFairnessReport{}, domain.Invalid("RUNTIME_FAIRNESS_RESOURCE_REQUIRED", "多个资源配额并存时必须指定 resource_key")
		}
		for key := range keys {
			resourceKey = key
		}
	...
	if len(rows) == 0 {
		return domain.RuntimeFairnessReport{ResourceKey: resourceKey, Tenants: []domain.RuntimeTenantCapacity{}, ObservedAt: now}, nil
	}
	report := domain.RuntimeFairnessReport{ResourceKey: resourceKey, Unit: unit, Tenants: rows, ObservedAt: now}
...
func capacityRow(tenantID, resourceKey string, quota domain.ResourceQuota, reservations []domain.ResourceReservation, now time.Time) domain.RuntimeTenantCapacity {
	row := domain.RuntimeTenantCapacity{TenantID: tenantID, ResourceKey: resourceKey, Unit: quota.Unit, Capacity: quota.Capacity, ObservedAt: now}
	for _, reservation := range reservations {
		if reservation.ResourceKey != resourceKey || reservation.State != domain.ReservationHeld {
			continue
		}
