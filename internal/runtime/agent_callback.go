...
type AgentCallbackResult struct {
	Message  ProviderInboxMessage `json:"message"`
	Attempt  RuntimeAttempt       `json:"attempt"`
	Node     NodeRun              `json:"node"`
	Job      JobRun               `json:"job,omitempty"`
...
func (s *Service) ReceiveAgentCallback(ctx context.Context, input AgentCallbackInput) (AgentCallbackResult, error) {
	if s == nil || s.repo == nil {
		return AgentCallbackResult{}, fault.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime")
	}
	...
	if input.TenantID == "" || input.HarnessKind == "" || input.MessageID == "" || input.AttemptID == "" || input.SessionID == "" || input.EventType == "unknown" {
		return AgentCallbackResult{}, fault.Invalid("AGENT_CALLBACK_INVALID", "Agent 回调缺少租户、Harness、消息、Attempt、Session 或事件类型")
	}
	if !supportedAgentCallbackEvent(input.EventType) {
		return AgentCallbackResult{}, fault.Invalid("AGENT_CALLBACK_EVENT_UNSUPPORTED", "Agent 回调事件类型不受支持")
	}
	if len(input.Data) > 64<<10 {
		return AgentCallbackResult{}, fault.Policy("HARNESS_EVENT_TOO_LARGE", "Harness 结构化事件超过持久化上限", "只上报事件类型、状态和受控摘要")
	}
	dataValue := any(nil)
	if len(input.Data) > 0 && json.Unmarshal(input.Data, &dataValue) != nil {
		return AgentCallbackResult{}, fault.Invalid("AGENT_CALLBACK_DATA_INVALID", "Agent 回调 data 必须是合法 JSON")
	}
	...
	stored, err := commands.ReceiveAgentInboxCommand(ctx, message, JobEvent{
		ID: idgen.New(), TenantID: message.TenantID, JobRunID: message.JobRunID, NodeKey: handle.Node.NodeKey,
	...
	if stored.State == ProviderInboxApplied {
		return result, nil
	}
	if stored.State == ProviderInboxFailed {
		return result, fault.Conflict("AGENT_CALLBACK_PREVIOUSLY_FAILED", "相同 Agent 回调已经处理失败")
	}
	...
	case "session.failed":
		finalized, finalizeErr := s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: safeErrorCode(input.ErrorCode, "HARNESS_SESSION_FAILED"), UsedCostMinor: handle.Agent.UsedCostMinor})
		if finalizeErr != nil {
			return result, finalizeErr
		}
		handle, result.Job = finalized.Handle, finalized.Job
	case "session.interrupted":
		finalized, finalizeErr := s.FinalizeDispatch(ctx, handle, DispatchOutcome{State: RuntimeAttemptRetryableFailed, ErrorCode: "HARNESS_SESSION_INTERRUPTED", UsedCostMinor: handle.Agent.UsedCostMinor})
		if finalizeErr != nil {
			return result, finalizeErr
		}
		handle, result.Job = finalized.Handle, finalized.Job
	case "runtime.yield":
		...
		if len(input.Data) == 0 || json.Unmarshal(input.Data, &request) != nil {
			return result, fault.Invalid("HARNESS_YIELD_INVALID", "Harness 让出执行权事件无效")
		}
	...
	completed := stored
	completed.State = ProviderInboxApplied
	completed.ErrorCode = ""
	completed.ProcessedAt = &processedAt
	completed.Version++
	completed.UpdatedAt = processedAt
	completed, err = commands.CompleteAgentInboxCommand(ctx, completed, stored.Version, JobEvent{
		ID: idgen.New(), TenantID: completed.TenantID, JobRunID: completed.JobRunID, NodeKey: handle.Node.NodeKey,
...
func (s *Service) newAgentInboxMessage(input AgentCallbackInput, handle DispatchHandle, dataValue any) (ProviderInboxMessage, error) {
	dataDigest := ""
	if dataValue != nil {
		digest, err := stablehash.Sum(dataValue)
		if err != nil {
			return ProviderInboxMessage{}, err
		}
	...
	responseDigest, err := stablehash.Sum(struct {
		EventType  string
		DataDigest string
		ErrorCode  string
	...
	return ProviderInboxMessage{
		...
		State: ProviderInboxReceived, ErrorCode: input.ErrorCode, ReceivedAt: input.ReceivedAt,
		Version: 1, CreatedAt: input.ReceivedAt, UpdatedAt: input.ReceivedAt,
	}, nil
}

func boundAgentCallbackSession(attempt RuntimeAttempt, input AgentCallbackInput) (agentadapter.AgentSessionRef, error) {
	if attempt.HarnessKind != input.HarnessKind {
		return agentadapter.AgentSessionRef{}, fault.Conflict("AGENT_CALLBACK_HARNESS_MISMATCH", "Agent 回调 Harness 与 RuntimeAttempt 不一致")
	}
	...
	if attempt.SessionRef == "" || json.Unmarshal([]byte(attempt.SessionRef), &session) != nil || session.HarnessKind != input.HarnessKind || session.SessionID != input.SessionID || (session.TenantID != "" && session.TenantID != input.TenantID) {
		return agentadapter.AgentSessionRef{}, fault.Conflict("HARNESS_SESSION_MISMATCH", "Agent 回调 Session 与 RuntimeAttempt 不一致")
	}
	return session, nil
}
