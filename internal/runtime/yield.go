...
type YieldDispatchResult struct {
	Yield  RuntimeYield   `json:"yield"`
	Handle DispatchHandle `json:"handle"`
	Job    JobRun         `json:"job"`
}

func (s *Service) RuntimeYield(ctx context.Context, tenantID, id string) (RuntimeYield, error) {
	return s.repo.RuntimeYield(ctx, tenantID, id)
}

func (s *Service) RuntimeYields(ctx context.Context, tenantID, jobID string) ([]RuntimeYield, error) {
...
func (s *Service) YieldDispatch(ctx context.Context, handle DispatchHandle, input YieldDispatchInput) (YieldDispatchResult, error) {
	...
	if node.State != NodeRunning || attempt.State != RuntimeAttemptRunning || agent.State != AgentActive {
		return YieldDispatchResult{Handle: handle}, fault.Conflict("DISPATCH_NOT_YIELDABLE", "只有当前运行中的调度尝试可以让出执行权")
	}
	if attempt.LeaseExpiresAt == nil || node.LeaseExpiresAt == nil || !attempt.LeaseExpiresAt.After(now) || !node.LeaseExpiresAt.After(now) || attempt.FenceToken == "" || attempt.FenceToken != node.FenceToken {
		return YieldDispatchResult{Handle: handle}, fault.Conflict("DISPATCH_LEASE_STALE", "让出请求的调度租约已经过期")
	}
	input.Reason = strings.TrimSpace(input.Reason)
	input.WaitRefs = sortedRefs(input.WaitRefs)
	if len(input.WaitRefs) == 0 {
		return YieldDispatchResult{Handle: handle}, fault.Invalid("RUNTIME_YIELD_REFS_REQUIRED", "让出执行权必须冻结至少一个等待引用")
	}
	nodeState, agentState := "", ""
	switch input.Reason {
	case YieldWaitChildren:
		nodeState, agentState = NodeWaitingChildren, AgentWaitingChildren
	case YieldWaitHuman:
		nodeState, agentState = NodeWaitingHuman, AgentWaitingGate
	case YieldWaitEffect:
		nodeState, agentState = NodeWaitingExternal, AgentWaitingEffect
	default:
		return YieldDispatchResult{Handle: handle}, fault.Invalid("RUNTIME_YIELD_REASON_INVALID", "让出执行权的等待原因无效")
	}
	...
	if input.UsedCostMinor < agent.UsedCostMinor || input.UsedCostMinor > agent.BudgetMinor {
		return YieldDispatchResult{Handle: handle}, fault.Policy("AGENT_INSTANCE_USAGE_INVALID", "AgentInstance 用量不能回退或超过预算", "重新核对累计用量")
	}
	...
	if err := attempt.Transition(RuntimeAttemptYielded); err != nil {
		return YieldDispatchResult{Handle: handle}, err
	}
	...
	node.Version++
	node.UpdatedAt = now
	attempt.State = RuntimeAttemptYielded
	...
	agent.Version++
	agent.UpdatedAt = now
	yielded := RuntimeYield{ID: idgen.New(), TenantID: node.TenantID, JobRunID: node.JobRunID, NodeRunID: node.ID, AttemptID: attempt.ID, AgentInstanceID: agent.ID, Reason: input.Reason, WaitRefs: input.WaitRefs, State: RuntimeYieldOpen, Version: 1, YieldedAt: now, CreatedAt: now, UpdatedAt: now}
...
func (s *Service) ResumeYield(ctx context.Context, tenantID, yieldID, resumeKey, actorID string) (RuntimeYield, error) {
	resumeKey = strings.TrimSpace(resumeKey)
	if resumeKey == "" {
		return RuntimeYield{}, fault.Invalid("RUNTIME_YIELD_RESUME_KEY_REQUIRED", "恢复 RuntimeYield 必须提供幂等键")
	}
	yielded, err := s.repo.RuntimeYield(ctx, tenantID, yieldID)
	if err != nil {
		return yielded, err
	}
	if yielded.State == RuntimeYieldResolved {
		if yielded.ResumeKey == resumeKey {
			return yielded, nil
		}
		return yielded, fault.Conflict("RUNTIME_YIELD_ALREADY_RESOLVED", "RuntimeYield 已由其他恢复请求处理")
	}
	...
	if err := agent.Transition(AgentRunnable); err != nil {
		return yielded, err
	}
	now := s.now().UTC()
	yielded.State, yielded.ResumeKey, yielded.ResolvedAt = RuntimeYieldResolved, resumeKey, &now
	yielded.Version++
	yielded.UpdatedAt = now
	node.State = NodeReady
	node.Version++
	node.UpdatedAt = now
	agent.State = AgentRunnable
	agent.Version++
	agent.UpdatedAt = now
	yielded, _, _, err = s.repo.ResolveRuntimeYield(ctx, yielded, expectedYieldVersion, node, expectedNodeVersion, agent, expectedAgentVersion, JobEvent{ID: idgen.New(), TenantID: tenantID, JobRunID: yielded.JobRunID, NodeKey: node.NodeKey, Type: "attempt.resume_ready", ActorType: "runtime", ActorID: strings.TrimSpace(actorID), IdempotencyKey: "yield-resume:" + resumeKey, Payload: map[string]any{"yield_id": yielded.ID, "attempt_id": yielded.AttemptID, "reason": yielded.Reason}, OccurredAt: now})
	return yielded, err
}

func (s *Service) validateYieldRefs(ctx context.Context, node NodeRun, reason string, refs []string, requireReady bool) error {
	for _, ref := range refs {
		...
		case YieldWaitChildren:
			...
			if child.JobRunID != node.JobRunID || child.ID == node.ID {
				return fault.Invalid("RUNTIME_YIELD_CHILD_SCOPE_INVALID", "等待的子节点不属于当前执行实例")
			}
			if requireReady && child.State != NodeSucceeded && child.State != NodeSkipped {
				if child.State == NodeFailed || child.State == NodeBlocked || child.State == NodeCancelled {
					return fault.Conflict("RUNTIME_YIELD_CHILD_FAILED", "等待的子节点已经失败，不能恢复父节点")
				}
				return fault.Conflict("RUNTIME_YIELD_NOT_READY", "等待的子节点尚未完成")
			}
		case YieldWaitEffect:
			...
			if effect.JobRunID != node.JobRunID || effect.NodeRunID != node.ID {
				return fault.Invalid("RUNTIME_YIELD_EFFECT_SCOPE_INVALID", "等待的外部操作不属于当前执行节点")
			}
			if requireReady && effect.State != EffectSucceeded {
				if effect.State == EffectFailed || effect.State == EffectManual {
					return fault.Conflict("RUNTIME_YIELD_EFFECT_FAILED", "等待的外部操作未成功，不能自动恢复节点")
				}
				return fault.Conflict("RUNTIME_YIELD_NOT_READY", "等待的外部操作尚未收敛")
			}
		case YieldWaitHuman:
			if strings.TrimSpace(ref) == "" {
				return fault.Invalid("RUNTIME_YIELD_HUMAN_REF_INVALID", "人工等待引用不能为空")
			}
		}
	}
	return nil
