...
func (g *RuntimeMCPGateway) Call(ctx context.Context, request GatewayRequest) (GatewayResponse, error) {
	...
	if request.TenantID == "" || request.AttemptID == "" || request.FenceToken == "" || request.ToolName == "" {
		return GatewayResponse{}, domain.Invalid("MCP_GATEWAY_INPUT_INVALID", "MCP 调用缺少租户、Attempt、fence 或工具名")
	}
	if request.Arguments == nil {
		request.Arguments = map[string]any{}
	}
	if body, err := json.Marshal(request.Arguments); err != nil || len(body) > maxGatewayArgs {
		return GatewayResponse{}, domain.Policy("MCP_GATEWAY_ARGUMENTS_TOO_LARGE", "MCP 工具参数超过大小限制", "使用 ArtifactRef 或缩小参数")
	}
	...
	if lookupErr != nil && !domain.IsNotFound(lookupErr) {
		return GatewayResponse{}, lookupErr
	}
	if hasPrevious {
		if previous.RequestDigest != requestDigest {
			return GatewayResponse{}, domain.Conflict("MCP_GATEWAY_IDEMPOTENCY_MISMATCH", "MCP 幂等键已用于不同的工具参数")
		}
		if previous.State == domain.ToolCallSucceeded || previous.State == domain.ToolCallFailed || previous.State == domain.ToolCallUnknown {
	...
	if !hasPrevious {
		call = domain.ToolCall{
			ID: domain.NewID(), TenantID: request.TenantID, JobRunID: state.attempt.JobRunID,
			...
			SafeRequest: gatewaySafeRequest(request.ToolName, request.Arguments, idempotencyKey), State: domain.ToolCallProposed, Version: 1, CreatedAt: now, UpdatedAt: now,
		}
		if err := g.service.CreateFencedToolCall(ctx, call, request.FenceToken); err != nil {
			...
			call = concurrent
			if call.State == domain.ToolCallSucceeded || call.State == domain.ToolCallFailed || call.State == domain.ToolCallUnknown {
	...
	if execErr != nil {
		failed := call
		failed.State = domain.ToolCallFailed
	...
	if body, marshalErr := json.Marshal(result); marshalErr != nil || len(body) > maxGatewayResult {
		...
		return GatewayResponse{ToolCall: call}, domain.Policy("MCP_GATEWAY_RESULT_TOO_LARGE", "MCP 工具结果超过大小限制", "使用分页或 ArtifactRef")
	}
	resultDigest, err := domain.CanonicalHash(result)
	if err != nil {
		return GatewayResponse{}, err
	}
	succeeded := call
	succeeded.State = domain.ToolCallSucceeded
...
func (g *RuntimeMCPGateway) authorize(ctx context.Context, request GatewayRequest) (gatewayContext, error) {
	...
	now := g.service.now().UTC()
	if attempt.State != domain.RuntimeAttemptRunning || attempt.FenceToken != request.FenceToken || attempt.LeaseExpiresAt == nil || !attempt.LeaseExpiresAt.After(now) {
		return gatewayContext{}, domain.Conflict("MCP_GATEWAY_FENCE_STALE", "MCP 调用的 Attempt fence 或租约已失效")
	}
	if attempt.TenantID != request.TenantID || node.TenantID != request.TenantID || node.JobRunID != attempt.JobRunID || agent.TenantID != request.TenantID || agent.JobRunID != attempt.JobRunID || agent.NodeRunID != node.ID || agent.ContextViewID != view.ID || view.TenantID != request.TenantID || view.JobRunID != attempt.JobRunID || view.NodeRunID != node.ID || view.AttemptID != attempt.ID {
		return gatewayContext{}, domain.Invalid("MCP_GATEWAY_SCOPE_INVALID", "MCP 调用没有绑定同一租户、Job、Node、Attempt、Agent 和 ContextView")
	}
	if agent.State != domain.AgentActive {
		return gatewayContext{}, domain.Conflict("MCP_GATEWAY_AGENT_NOT_ACTIVE", "只有活动中的 AgentInstance 可以调用 MCP 工具")
	}
	if !view.ExpiresAt.After(now) {
		return gatewayContext{}, domain.Policy("MCP_GATEWAY_CONTEXT_EXPIRED", "MCP 调用的 ContextView 已过期", "重新创建执行尝试和 ContextView")
	}
	if !view.AllowsTool(request.ToolName) {
		return gatewayContext{}, domain.Policy("MCP_GATEWAY_TOOL_NOT_ALLOWED", "当前 Attempt 未授权该 MCP 工具", "仅调用 ContextView AllowedTools 中的工具")
	}
	if gatewayToolSchema(request.ToolName) == "" {
		return gatewayContext{}, domain.Invalid("MCP_GATEWAY_TOOL_UNSUPPORTED", "Runtime MCP Gateway 不支持该工具")
	}
...
func (g *RuntimeMCPGateway) execute(ctx context.Context, state gatewayContext, toolName string, arguments map[string]any, idempotencyKey string) (map[string]any, error) {
	...
	default:
		return nil, domain.Invalid("MCP_GATEWAY_TOOL_UNSUPPORTED", "Runtime MCP Gateway 不支持该工具")
	}
}
...
func (g *RuntimeMCPGateway) queryState(ctx context.Context, state gatewayContext, arguments map[string]any) (map[string]any, error) {
	...
	if !stateRefAllowed(state.view, collection) {
		return nil, domain.Policy("MCP_GATEWAY_STATE_NOT_ALLOWED", "ContextView 未授权读取该状态集合", "仅读取 ContextView StateRefs 中的集合")
	}
	...
	if limit < 1 || limit > 100 {
		return nil, domain.Invalid("MCP_GATEWAY_LIMIT_INVALID", "state.query 的 limit 必须在 1 到 100 之间")
	}
	result := make([]domain.StateRecord, 0, limit)
...
func (g *RuntimeMCPGateway) mutateState(ctx context.Context, state gatewayContext, arguments map[string]any, idempotencyKey string) (map[string]any, error) {
	...
	if !stateRefAllowed(state.view, collection) {
		return nil, domain.Policy("MCP_GATEWAY_STATE_NOT_ALLOWED", "ContextView 未授权写入该状态集合", "仅写入 ContextView StateRefs 中的集合")
	}
	key := stringArgument(arguments, "key")
	if key == "" {
		return nil, domain.Invalid("MCP_GATEWAY_STATE_KEY_REQUIRED", "state.mutate 需要记录键")
	}
	value, _ := arguments["value"].(map[string]any)
	artifactRef := stringArgument(arguments, "artifact_ref")
	if value == nil && artifactRef == "" {
		return nil, domain.Invalid("MCP_GATEWAY_STATE_VALUE_REQUIRED", "state.mutate 需要对象值或 ArtifactRef")
	}
	...
	recordID, createdBy := domain.NewID(), actor
	records, err := g.service.repo.StateRecords(ctx, state.attempt.TenantID, collection.ID)
	if err != nil {
		return nil, err
	}
	...
	record := domain.StateRecord{ID: recordID, TenantID: state.attempt.TenantID, CollectionID: collection.ID, Key: key, Value: value, ArtifactRef: artifactRef, SchemaRevision: collection.SchemaRevision, Digest: desiredDigest, CreatedBy: createdBy, UpdatedBy: actor, Version: expectedVersion + 1, CreatedAt: createdAt, UpdatedAt: g.service.now().UTC()}
...
func (g *RuntimeMCPGateway) prepareEffect(ctx context.Context, state gatewayContext, arguments map[string]any, idempotencyKey string) (map[string]any, error) {
	...
	if kind == "" || requestDigest == "" {
		return nil, domain.Invalid("MCP_GATEWAY_EFFECT_INPUT_INVALID", "effect.prepare 需要 kind 和 request_digest")
	}
	costMinor := int64Argument(arguments, "cost_minor", 0)
	if costMinor < 0 {
		return nil, domain.Invalid("MCP_GATEWAY_EFFECT_COST_INVALID", "effect.prepare 的费用不能为负数")
	}
	...
	effect, err := g.service.RegisterEffectForAttempt(ctx, domain.ExternalEffect{TenantID: state.attempt.TenantID, JobRunID: state.attempt.JobRunID, NodeRunID: state.attempt.NodeRunID, AttemptID: state.attempt.ID, Kind: kind, IdempotencyKey: idempotencyKey, RequestDigest: requestDigest, CostMinor: costMinor, Currency: currency, SafeSummary: summary}, state.attempt.FenceToken)
	if err != nil {
		return nil, err
	}
...
func (g *RuntimeMCPGateway) listChildren(ctx context.Context, state gatewayContext) (map[string]any, error) {
	...
	children := make([]domain.NodeRun, 0)
...
func (g *RuntimeMCPGateway) effectStatus(ctx context.Context, state gatewayContext, arguments map[string]any) (map[string]any, error) {
	...
	if effectID == "" {
		return nil, domain.Invalid("MCP_GATEWAY_EFFECT_ID_REQUIRED", "effect.status 需要 effect_id")
	}
	...
	if effect.AttemptID != state.attempt.ID || effect.JobRunID != state.attempt.JobRunID || effect.NodeRunID != state.node.ID {
		return nil, domain.Policy("MCP_GATEWAY_EFFECT_SCOPE_INVALID", "外部操作不属于当前 Attempt", "只能查询当前 Attempt 声明的外部操作")
	}
...
func (g *RuntimeMCPGateway) resolveCollection(ctx context.Context, state gatewayContext, reference string) (domain.StateCollection, error) {
	reference = strings.TrimSpace(reference)
	if reference == "" {
		return domain.StateCollection{}, domain.Invalid("MCP_GATEWAY_COLLECTION_REQUIRED", "状态工具需要 collection")
	}
	collections, err := g.service.repo.StateCollections(ctx, state.attempt.TenantID, state.attempt.JobRunID)
	if err != nil {
		return domain.StateCollection{}, err
	}
...
func stateRefAllowed(view domain.ContextView, collection domain.StateCollection) bool {
...
func (g *RuntimeMCPGateway) findExisting(ctx context.Context, tenantID, attemptID, toolName, idempotencyKey string) (domain.ToolCall, error) {
	return g.service.repo.ToolCallByIdempotencyKey(ctx, tenantID, attemptID, toolName, idempotencyKey)
}

func gatewayTerminalReplay(call domain.ToolCall) (GatewayResponse, error) {
	response := GatewayResponse{ToolCall: call, Replayed: true, Result: cloneGatewayResult(call.SafeResult)}
	switch call.State {
	case domain.ToolCallSucceeded:
		return response, nil
	case domain.ToolCallFailed:
		...
		return response, domain.Conflict(code, "MCP 工具调用此前已失败；相同幂等请求不会重复执行")
	case domain.ToolCallUnknown:
		code := strings.TrimSpace(call.ErrorCode)
		if code == "" {
			code = "MCP_GATEWAY_RESULT_UNKNOWN"
		}
		return response, domain.Conflict(code, "MCP 工具调用结果尚未确认；相同幂等请求不会重复执行")
	default:
		return GatewayResponse{}, domain.Conflict("MCP_GATEWAY_TOOL_CALL_STATE_INVALID", "MCP ToolCall 尚未进入可重放终态")
	}
}
...
func gatewayIdempotencyKey(request GatewayRequest) (string, error) {
	...
	if key == "" {
		return "", domain.Invalid("MCP_GATEWAY_IDEMPOTENCY_REQUIRED", "MCP 调用需要 request_id 或 idempotency_key")
	}
	if len(key) > 200 {
		return "", domain.Invalid("MCP_GATEWAY_IDEMPOTENCY_INVALID", "MCP 幂等键过长")
	}
	return key, nil
}
