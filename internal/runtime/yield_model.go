...
func (value RuntimeYield) Validate() error {
	if value.ID == "" || value.TenantID == "" || value.JobRunID == "" || value.NodeRunID == "" || value.AttemptID == "" || value.AgentInstanceID == "" || value.Version < 1 || value.YieldedAt.IsZero() || value.CreatedAt.IsZero() || value.UpdatedAt.IsZero() {
		return fault.Invalid("RUNTIME_YIELD_INVALID", "RuntimeYield 缺少执行范围、等待原因或版本")
	}
	switch value.Reason {
	case YieldWaitChildren, YieldWaitHuman, YieldWaitEffect:
	default:
		return fault.Invalid("RUNTIME_YIELD_REASON_INVALID", "RuntimeYield 等待原因无效")
	}
	if len(value.WaitRefs) == 0 {
		return fault.Invalid("RUNTIME_YIELD_REFS_REQUIRED", "RuntimeYield 必须冻结至少一个等待引用")
	}
	switch value.State {
	case RuntimeYieldOpen:
		if value.ResolvedAt != nil || value.ResumeKey != "" {
			return fault.Invalid("RUNTIME_YIELD_OPEN_INVALID", "开放的 RuntimeYield 不能包含恢复结果")
		}
	case RuntimeYieldResolved:
		if value.ResolvedAt == nil || value.ResumeKey == "" {
			return fault.Invalid("RUNTIME_YIELD_RESOLVED_INVALID", "已恢复的 RuntimeYield 缺少恢复身份或时间")
		}
	default:
		return fault.Invalid("RUNTIME_YIELD_STATE_INVALID", "RuntimeYield 状态无效")
	}
	return nil
}
