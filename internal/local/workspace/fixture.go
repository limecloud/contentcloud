...
func MaterializeFixture(fixture fixturev3.Fixture, options MaterializeFixtureOptions) (FixtureMaterialization, error) {
	if err := fixture.Validate(); err != nil {
		return FixtureMaterialization{}, fault.Invalid("FIXTURE_V3_INVALID", err.Error())
	}
	if fixture.Scenario == nil {
		return FixtureMaterialization{}, fault.Invalid("FIXTURE_SCENARIO_REQUIRED", "本地工作区测试数据必须包含场景定义（scenario）")
	}
	if strings.TrimSpace(options.Root) == "" || strings.TrimSpace(options.ProjectID) == "" || strings.TrimSpace(options.WorkspaceID) == "" {
		return FixtureMaterialization{}, fault.Invalid("FIXTURE_WORKSPACE_CONTEXT_REQUIRED", "fixture apply 需要 directory、project ID 和 workspace ID")
	}
	...
	if plan.State != "missing" && plan.State != "empty" {
		return FixtureMaterialization{}, fault.Conflict("FIXTURE_DIRECTORY_NOT_EMPTY", "测试数据只能写入空目录，拒绝修改已有工作区或文件")
	}
	...
	if !knowledgeLint.Valid {
		err := fault.Invalid("FIXTURE_KNOWLEDGE_LINT_FAILED", "Fixture 生成的知识未通过确定性校验")
	...
	if !sourceVerification.Valid || !runs.Valid || run.Status != "completed" || batch.Batch.Status != "blocked" || batch.Report.Blocked != 10 {
		return FixtureMaterialization{}, fault.Conflict("FIXTURE_MATERIALIZATION_INCOMPLETE", "Fixture 未收敛为完整的 V3 验收状态")
	}
...
func materializeFixtureSources(root string, sources []fixturev3.SourceSpec, now time.Time) (map[string]LocalEvidenceBundle, error) {
	...
	for index, source := range sources {
		...
		if bundle.Status != "ready" || len(bundle.Evidence) == 0 {
			return nil, fault.Invalid("FIXTURE_SOURCE_INGEST_FAILED", "Fixture 来源未生成可接受的证据："+source.ID)
		}
...
func materializeFixtureKnowledge(root string, fixture fixturev3.Fixture, evidence map[string]LocalEvidenceBundle, now time.Time) ([]LocalKnowledgeItem, error) {
	...
	for index, dimension := range fixture.Scenario.Methodology.Dimensions {
		...
		item := LocalKnowledgeItem{
			...
			Subject: fixture.Project.ProductName, Predicate: dimension.Label, Value: sourcedomain.TypedValue{Type: "text", Text: dimension.Statement},
			Scope:  sourcedomain.KnowledgeScope{Regions: []string{"CN"}, Channels: []string{fixture.Project.Channel}, Audiences: []string{}, ProductVariants: []string{}},
			Status: dimension.Status, RiskLevel: "low", AllowedChannels: []string{fixture.Project.Channel},
			Evidence: []sourcedomain.EvidenceRef{{SourceRevisionID: dimension.SourceID, LocatorKind: span.LocatorKind, Locator: string(locator), Quote: span.Quote}}, EvidenceIDs: []string{span.ID},
	...
	for index, spec := range fixture.Scenario.Governance {
		item := LocalKnowledgeItem{
			...
			Predicate: spec.Kind, Value: sourcedomain.TypedValue{Type: "text", Text: spec.Statement}, Scope: sourcedomain.KnowledgeScope{Regions: []string{}, Channels: []string{}, Audiences: []string{}, ProductVariants: []string{}},
			Status: spec.Status, RiskLevel: "low", AllowedChannels: []string{}, Evidence: []sourcedomain.EvidenceRef{}, EvidenceIDs: []string{}, ForbiddenExtensions: []string{},
...
func materializeFixtureContent(root string, fixture fixturev3.Fixture, knowledge []LocalKnowledgeItem, projectID, workspaceID string, now time.Time) (FinalizeContentBatchResult, []string, error) {
	...
	if err != nil || !briefLint.Valid {
		...
		lintErr := fault.Invalid("FIXTURE_BRIEF_LINT_FAILED", "Fixture 简报未通过确定性校验")
...
func storeFixtureApprovedObject[T any](root, projectID, workspaceID, snapshotID, submissionType string, objects []T, eligibleIDs []string, now time.Time) error {
	canonical, err := json.Marshal(map[string]any{
		"schema_version": reviewdomain.SubmissionSchemaVersion(submissionType), "submission_type": submissionType, "objects": objects,
	})
	if err != nil {
		return err
	}
	hash, err := stablehash.Sum(json.RawMessage(canonical))
	if err != nil {
		return err
	}
	_, err = StoreApprovedSnapshot(root, reviewdomain.ApprovedSnapshot{
		ID: snapshotID, ProjectID: projectID, WorkspaceID: workspaceID, SubmissionType: submissionType, SchemaVersion: reviewdomain.SubmissionSchemaVersion(submissionType),
		ContentHash: "sha256:" + hash, SubjectHash: "sha256:" + hash, CanonicalContent: canonical, EligibleIDs: append([]string(nil), eligibleIDs...),
		Artifacts: []reviewdomain.SubmissionArtifact{}, CreatedBy: "fixture-v3", CreatedAt: now,
	}, now)
	return err
}
