...
func TestKnowledgeCandidateFlowToApprovedQueryAndPack(t *testing.T) {
	...
	pkg := sourcedomain.KnowledgeExtractionPackage{SchemaVersion: "1.0", Candidates: []sourcedomain.KnowledgeCandidate{{
		Kind: "fact", Title: "产品价格", Statement: bundle.Evidence[0].Quote, Subject: "金陵古都香", Predicate: "建议零售价", Value: sourcedomain.TypedValue{Type: "number", Number: floatPointer(168), Unit: "CNY"},
		...
		Evidence: []sourcedomain.EvidenceRef{{SourceRevisionID: "source:product", LocatorKind: bundle.Evidence[0].LocatorKind, Locator: string(locator), Quote: bundle.Evidence[0].Quote}}, ForbiddenExtensions: []string{}, DependsOnFactIDs: []string{},
	}}, Warnings: []string{}}
	...
	snapshot := reviewdomain.ApprovedSnapshot{ID: "snapshot-1", SubmissionType: "knowledge", CanonicalContent: canonical, EligibleIDs: []string{imported.Imported[0].ID}, CreatedAt: now}
...
func TestKnowledgeImportRejectsInventedEvidence(t *testing.T) {
	...
	pkg := sourcedomain.KnowledgeExtractionPackage{SchemaVersion: "1.0", Candidates: []sourcedomain.KnowledgeCandidate{{
		Kind: "fact", Title: "伪造", Statement: "并不存在", Subject: "产品", Predicate: "属性", Value: sourcedomain.TypedValue{Type: "text", Text: "并不存在"},
		Scope: sourcedomain.KnowledgeScope{Regions: []string{}, Channels: []string{}, Audiences: []string{}, ProductVariants: []string{}}, RiskLevel: "low", AllowedChannels: []string{},
		Evidence: []sourcedomain.EvidenceRef{{SourceRevisionID: "source:product", LocatorKind: "paragraph", Locator: `{"paragraph":1}`, Quote: "伪造原文"}}, ForbiddenExtensions: []string{}, DependsOnFactIDs: []string{},
	}}, Warnings: []string{}}
