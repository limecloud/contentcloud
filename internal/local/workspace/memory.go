...
func resolveMemoryScope(root string) (string, MemoryScope, error) {
	...
	if scope.WorkspaceID == "" || scope.ProjectID == "" {
		return "", MemoryScope{}, fault.Invalid("MEMORY_SCOPE_INVALID", "记忆范围必须从已绑定工作区和项目推导")
	}
...
func normalizeMemoryKinds(values []string) ([]string, error) {
	...
	for _, value := range values {
		...
		if !validMemoryKinds[kind] {
			return nil, fault.Invalid("MEMORY_KIND_INVALID", "记忆类型必须为 working、execution、knowledge 或 interaction")
		}
		seen[kind] = true
...
func normalizeMemoryBudget(limit, maxChars int) (int, int, error) {
	if limit < 0 || limit > maximumMemoryLimit {
		return 0, 0, fault.Invalid("MEMORY_LIMIT_INVALID", fmt.Sprintf("记忆召回条数必须在 1 到 %d 之间", maximumMemoryLimit))
	}
	if maxChars < 0 || maxChars > maximumMemoryMaxChars {
		return 0, 0, fault.Invalid("MEMORY_BUDGET_INVALID", fmt.Sprintf("记忆召回字符预算必须在 1 到 %d 之间", maximumMemoryMaxChars))
	}
	if limit == 0 {
