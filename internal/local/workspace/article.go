...
func ValidateArticleItemForSubmission(raw json.RawMessage, projectID string) (ArticleItem, error) {
	...
	if err := strictUnmarshal(raw, &item); err != nil {
		return item, fault.Invalid("ARTICLE_ITEM_JSON_INVALID", err.Error())
	}
	if item.ProjectID != projectID {
		return item, fault.Conflict("ARTICLE_ITEM_PROJECT_MISMATCH", "文章对象不属于当前项目")
	}
	batch := ContentBatch{ID: item.ContentBatchID, ContentKind: identitydomain.ContentTypeWeChatArticle, ContentSchemaRef: ArticleSchema, ProjectID: item.ProjectID, BriefRef: item.BriefRef, ContextSnapshotID: item.ContextSnapshotID}
	...
	if !report.Valid {
		err := fault.Invalid("ARTICLE_ITEM_SUBMISSION_INVALID", "文章对象未通过服务端结构复验")
		err.Details = report
		return item, err
	}
	return item, nil
...
func ValidateArticleBriefForSubmission(raw json.RawMessage) (ArticleBrief, error) {
	...
	if err := strictUnmarshal(raw, &brief); err != nil {
		return brief, fault.Invalid("ARTICLE_BRIEF_JSON_INVALID", err.Error())
	}
	query := KnowledgeQueryResult{Eligible: []KnowledgeQueryEntry{}}
	...
	if !report.Valid {
		err := fault.Invalid("ARTICLE_BRIEF_SUBMISSION_INVALID", "文章简报未通过服务端结构复验")
		err.Details = report
		return brief, err
	}
	return brief, nil
...
func LintArticleBrief(root, file string) (ArticleBriefLintReport, ArticleBrief, error) {
	...
	if err := readStrictJSON(path, &brief); err != nil {
		return ArticleBriefLintReport{}, brief, fault.Invalid("ARTICLE_BRIEF_JSON_INVALID", err.Error())
	}
...
func CreateArticleBatch(options CreateArticleBatchOptions) (CreateContentBatchResult, error) {
	...
	if err := strictUnmarshal(briefRaw, &brief); err != nil {
		return CreateContentBatchResult{}, fault.Invalid("APPROVED_ARTICLE_BRIEF_INVALID", "批准快照中的 ArticleBrief 无效："+err.Error())
	}
	...
	if report := lintArticleBrief(brief, query); !report.Valid || brief.Deliverability != "review_ready" {
		validationErr := fault.Policy("APPROVED_ARTICLE_BRIEF_BLOCKED", "批准的 ArticleBrief 未通过公众号门禁", "修订并重新批准 ArticleBrief")
		validationErr.Details = report
		return CreateContentBatchResult{}, validationErr
	}
	if query.ApprovedSnapshotID == "" {
		return CreateContentBatchResult{}, fault.Policy("KNOWLEDGE_SNAPSHOT_REQUIRED", "创建公众号文章批次需要已拉取的知识批准快照", "先执行 contentcloud pull approved --type knowledge")
	}
	...
	if count < 1 || count > 10 {
		return CreateContentBatchResult{}, fault.Invalid("ARTICLE_BATCH_COUNT_INVALID", "requested_count 必须为 1 到 10")
	}
	...
	batch := ContentBatch{
		SchemaVersion: ContentBatchSchema, ID: batchID, IntentID: brief.IntentID, ContentKind: identitydomain.ContentTypeWeChatArticle, ContentSchemaRef: ArticleSchema, DeliveryProfiles: deliveryProfiles,
...
func LintArticleItem(root, file, batchFile string) (ArticleItemLintReport, ArticleItem, error) {
	...
	if err := readStrictJSON(path, &item); err != nil {
		return ArticleItemLintReport{}, item, fault.Invalid("ARTICLE_ITEM_JSON_INVALID", err.Error())
	}
	...
	if err := strictUnmarshal(batch.BriefRaw, &brief); err != nil {
		return ArticleItemLintReport{}, item, fault.Invalid("ARTICLE_BATCH_BRIEF_INVALID", "冻结 ArticleBrief 无效："+err.Error())
	}
	report = lintArticleItemAgainstBrief(report, brief)
	report.File = relativeWorkspacePath(resolved, path)
	if hash, hashErr := stablehash.Sum(item); hashErr == nil {
...
func lintArticleItem(item ArticleItem, batch ContentBatch, query KnowledgeQueryResult, refs map[string]LocalKnowledgeItem) ArticleItemLintReport {
	...
	if batch.ContentKind != identitydomain.ContentTypeWeChatArticle || batch.ContentSchemaRef != ArticleSchema {
...
func LintArticleBatch(root, batchFile string, contentFiles []string) (ArticleBatchLintReport, error) {
	...
	if batch.ContentKind != identitydomain.ContentTypeWeChatArticle || len(contentFiles) != batch.RequestedCount {
		report.Valid = false
	}
	seen := map[string]bool{}
...
func FinalizeArticleBatch(root, batchFile string, contentFiles []string, now time.Time) (ContentBatch, ArticleBatchLintReport, error) {
	...
	if !report.Valid {
		validationErr := fault.Invalid("ARTICLE_BATCH_LINT_FAILED", "公众号文章批次校验失败")
		validationErr.Details = report
...
func DiffArticleItems(root, baselineFile, candidateFile string, allowedPaths []string) (ArticleItemDiff, error) {
	...
	if err := readStrictJSON(baselinePath, &baseline); err != nil {
		return ArticleItemDiff{}, fault.Invalid("ARTICLE_ITEM_BASELINE_INVALID", err.Error())
	}
	if err := readStrictJSON(candidatePath, &candidate); err != nil {
		return ArticleItemDiff{}, fault.Invalid("ARTICLE_ITEM_CANDIDATE_INVALID", err.Error())
	}
	if candidate.BasedOnVersionID != baseline.ID || strings.TrimSpace(candidate.ChangeSummary) == "" {
		return ArticleItemDiff{}, fault.Invalid("ARTICLE_ITEM_REVISION_METADATA_INVALID", "修订稿必须引用基线 ID 并填写 change_summary")
	}
...
func ExportWeChatPackage(root, contentItemID, outputDirectory string, now time.Time) (ExportWeChatPackageResult, error) {
	...
	if err := strictUnmarshal(raw, &item); err != nil || item.SchemaVersion != ArticleSchema || item.Deliverability != "review_ready" {
		return ExportWeChatPackageResult{}, fault.Policy("APPROVED_ARTICLE_ITEM_INVALID", "只有已批准且处于 review_ready 状态的文章对象才能导出公众号交付包", "修订并重新批准文章对象")
	}
	contentHash, err := stablehash.Sum(item)
	if err != nil {
		return ExportWeChatPackageResult{}, err
	}
	...
	if err != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
		return ExportWeChatPackageResult{}, fault.Policy("DELIVERY_PATH_OUTSIDE_WORKSPACE", "交付目录必须位于当前工作区", "使用 60-delivery/packages 下的目录")
	}
...
func LintWeChatPackage(root, packageFile string) (WeChatPackageLintReport, error) {
	...
	if err := readStrictJSON(path, &pkg); err != nil {
		return WeChatPackageLintReport{}, fault.Invalid("WECHAT_PACKAGE_JSON_INVALID", err.Error())
	}
...
func InspectWeChatPlatformDOM(root, packageFile, observedFile string, now time.Time) (WeChatPlatformDOMDiff, error) {
	...
	if err := readStrictJSON(packagePath, &pkg); err != nil {
		return WeChatPlatformDOMDiff{}, fault.Invalid("WECHAT_PACKAGE_JSON_INVALID", err.Error())
	}
	var expected string
	...
	if expected == "" {
		return WeChatPlatformDOMDiff{}, fault.Invalid("WECHAT_HTML_MISSING", "交付包缺少公众号 HTML")
	}
