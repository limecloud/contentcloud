...
type ApprovedSnapshotCacheRecord struct {
	...
	Snapshot reviewdomain.ApprovedSnapshot `json:"snapshot"`
}

func StoreApprovedSnapshot(root string, snapshot reviewdomain.ApprovedSnapshot, now time.Time) (ApprovedSnapshotCacheRecord, error) {
...
func StoreApprovedSnapshots(root string, snapshots []reviewdomain.ApprovedSnapshot, now time.Time) ([]ApprovedSnapshotCacheRecord, error) {
	resolved, err := FindRoot(root)
	if err != nil {
		return nil, err
	}
	...
	ordered := append([]reviewdomain.ApprovedSnapshot(nil), snapshots...)
...
func storeApprovedSnapshot(root string, snapshot reviewdomain.ApprovedSnapshot, now time.Time) (ApprovedSnapshotCacheRecord, error) {
	...
	hash, err := stablehash.Sum(snapshot)
	if err != nil {
		return ApprovedSnapshotCacheRecord{}, err
	}
...
func ApprovedSnapshotInbox(root, submissionType string) ([]ApprovedSnapshotCacheSummary, error) {
	...
	if submissionType != "" && !approvedSubmissionType(submissionType) {
		return nil, fault.Invalid("APPROVED_SNAPSHOT_TYPE_INVALID", "批准快照的 submission_type 无效")
	}
...
func ShowApprovedSnapshot(root, snapshotID string) (ApprovedSnapshotCacheRecord, error) {
	...
	if !safePulledBundleID(snapshotID) {
		return ApprovedSnapshotCacheRecord{}, fault.Invalid("APPROVED_SNAPSHOT_ID_INVALID", "批准快照 ID 无效")
	}
	return loadApprovedSnapshot(resolved, snapshotID)
}

func ApprovedSnapshotForObject(root, submissionType, objectID string) (reviewdomain.ApprovedSnapshot, error) {
...
func loadApprovedSnapshot(root, snapshotID string) (ApprovedSnapshotCacheRecord, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		return ApprovedSnapshotCacheRecord{}, fault.NotFound("本地 ApprovedSnapshot")
	}
	if err != nil {
		return ApprovedSnapshotCacheRecord{}, err
	}
	...
	if err := json.Unmarshal(body, &snapshot); err != nil {
		return ApprovedSnapshotCacheRecord{}, fault.Invalid("APPROVED_SNAPSHOT_INVALID", "本地 ApprovedSnapshot 不是有效 JSON")
	}
	if snapshot.ID != snapshotID {
		return ApprovedSnapshotCacheRecord{}, fault.Conflict("APPROVED_SNAPSHOT_ID_MISMATCH", "本地 ApprovedSnapshot 目录与内容 ID 不一致")
	}
	...
	if errors.Is(err, os.ErrNotExist) {
		missing := fault.Conflict("APPROVED_SNAPSHOT_DIGEST_MISSING", "本地 ApprovedSnapshot 缺少可信 digest")
	...
	if strings.TrimSpace(string(expected)) != actualDigest {
		return ApprovedSnapshotCacheRecord{}, fault.Conflict("APPROVED_SNAPSHOT_DIGEST_MISMATCH", "本地 ApprovedSnapshot 内容与 pull 时记录的 digest 不一致")
	}
...
func validateApprovedSnapshot(root string, snapshot reviewdomain.ApprovedSnapshot) error {
	if !safePulledBundleID(snapshot.ID) || !approvedSubmissionType(snapshot.SubmissionType) || snapshot.CreatedAt.IsZero() || len(snapshot.CanonicalContent) == 0 || !json.Valid(snapshot.CanonicalContent) {
		return fault.Invalid("APPROVED_SNAPSHOT_INVALID", "批准快照缺少有效 ID、类型、规范内容或创建时间")
	}
	...
	if snapshot.ProjectID != "" && snapshot.ProjectID != status.Binding.ProjectID {
		return fault.Conflict("APPROVED_SNAPSHOT_PROJECT_MISMATCH", "批准快照不属于当前项目")
	}
	if snapshot.WorkspaceID != "" && snapshot.WorkspaceID != status.Binding.WorkspaceID {
		return fault.Conflict("APPROVED_SNAPSHOT_WORKSPACE_MISMATCH", "批准快照不属于当前工作区")
	}
	...
	if err := json.Unmarshal(snapshot.CanonicalContent, &canonical); err != nil || canonical.SubmissionType != snapshot.SubmissionType || canonical.Objects == nil {
		return fault.Invalid("APPROVED_SNAPSHOT_CANONICAL_INVALID", "批准快照的规范内容与提交类型不一致，或缺少 objects 数组")
	}
	if snapshot.SchemaVersion != "" && canonical.SchemaVersion != snapshot.SchemaVersion {
		return fault.Conflict("APPROVED_SNAPSHOT_SCHEMA_MISMATCH", "批准快照的 schema_version 与规范内容不一致")
	}
	...
	for _, raw := range canonical.Objects {
		...
		if json.Unmarshal(raw, &identity) != nil || strings.TrimSpace(identity.ID) == "" {
			return fault.Invalid("APPROVED_SNAPSHOT_OBJECT_INVALID", "批准快照的规范内容包含无 ID 对象")
		}
	...
	for _, id := range snapshot.EligibleIDs {
		if strings.TrimSpace(id) == "" || eligible[id] || !objectIDs[id] {
			return fault.Invalid("APPROVED_SNAPSHOT_ELIGIBLE_INVALID", "批准快照的 eligible_ids 包含空值、重复值或不在规范内容中的对象")
		}
		eligible[id] = true
	}
	if snapshot.ContentHash != "" && snapshot.SubjectHash != "" && normalizeApprovedHash(snapshot.ContentHash) != normalizeApprovedHash(snapshot.SubjectHash) {
		return fault.Conflict("APPROVED_SNAPSHOT_SUBJECT_MISMATCH", "批准快照的 content_hash 与 subject_hash 不一致")
	}
	return nil
}
...
func storeImmutableDigest(path, value string) error {
	if existing, err := os.ReadFile(path); err == nil {
		if string(existing) != value {
			return fault.Conflict("APPROVED_SNAPSHOT_DIGEST_CONFLICT", "本地 ApprovedSnapshot 已有不同 digest")
		}
...
func requireImmutableCacheFile(path string) error {
	...
	if !info.Mode().IsRegular() || info.Mode().Perm()&0o222 != 0 {
		return fault.Conflict("APPROVED_SNAPSHOT_CACHE_MUTABLE", "批准快照缓存文件不是只读普通文件")
	}
	return nil
}
...
func approvedSubmissionType(value string) bool {
	return reviewdomain.ValidSubmissionType(value)
}
