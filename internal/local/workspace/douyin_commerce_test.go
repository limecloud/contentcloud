...
func TestDouyinCommerceValidationFreezesOfferAndCreativeLineage(t *testing.T) {
	...
	strategy := work.AudienceStrategyVersion{ID: "strategy-1", Type: "audience_strategy_version", SchemaVersion: work.AudienceStrategySchema, ProjectID: "project-1", TaxonomySnapshotID: "taxonomy-1", AudienceCode: "gen_z", AudienceLabel: "Z世代", SegmentDefinition: "需求状态", Objective: "conversion", DemandMoment: "通勤", InsightStatement: "证据洞察", HookHypotheses: []string{"场景钩子"}, Scenario: "通勤场景", ProofOrder: []string{"规格"}, Objections: []string{"价格"}, CTAStrategy: "查看详情", EvidenceRefs: []string{"evidence-1"}, Confidence: "medium", TestType: "audience_expression_fit_test", PrimaryVariable: "audience", ControlledVariables: []string{"cta"}, TargetMetrics: []string{"clicks"}, Status: "review_ready"}
	offer := work.CommerceOfferSnapshot{ID: "offer-1", Type: "commerce_offer_snapshot", SchemaVersion: work.CommerceOfferSchema, ProjectID: "project-1", SKUID: "sku-1", ProductVersionID: "product-v1", ApprovedClaimRefs: []string{"claim-1"}, DisplayPrice: "168", Currency: "CNY", Benefits: []string{"包邮"}, Conditions: []string{"现货"}, EvidenceRefs: []string{"evidence-offer"}, CapturedAt: now, ValidFrom: now.Add(-time.Hour), ValidUntil: now.Add(24 * time.Hour), Status: "verified"}
	...
	asset := work.StoryboardAsset{ID: "frame-1", Role: "first_frame", ShotID: "shot-1", Path: "50-production/media/frame-1.png", MediaType: "image/png", SHA256: strings.Repeat("a", 64), ByteSize: 10, RightsRefs: []string{"rights-1"}}
	review := work.StoryboardAsset{ID: "review-1", Role: "review_sheet", Path: "50-production/media/review.png", MediaType: "image/png", SHA256: strings.Repeat("b", 64), ByteSize: 10, RightsRefs: []string{"rights-1"}}
	storyboard := work.StoryboardPackage{ID: "storyboard-1", Type: "storyboard_package", SchemaVersion: work.StoryboardPackageSchema, ProjectID: "project-1", ApprovedSnapshotID: "content-snapshot", ContentItemID: content.ID, GeneratorCapability: work.CapabilityRef{ID: "image.test", Version: "1.0.0", Digest: "sha256:" + strings.Repeat("c", 64)}, Status: "review_ready", Shots: []work.StoryboardShot{{ShotID: "shot-1", StartMS: 0, EndMS: 1000, Role: "hook", FirstFrameArtifactID: asset.ID, ImagePromptZH: "展示商品", NegativeConstraints: []string{"无错误文字"}, AcceptanceCriteria: []string{"主体清晰"}, PlanB: "实拍"}}, Assets: []work.StoryboardAsset{asset, review}, ReviewSheetArtifactID: review.ID, RightsRefs: []string{"rights-1"}, SourceDigest: "sha256:" + strings.Repeat("d", 64)}
	var err error
...
func storeCommerceSnapshot(t *testing.T, root, snapshotID, submissionType, objectID string, object any, createdAt time.Time) {
	...
	canonical, err := json.Marshal(map[string]any{"schema_version": reviewdomain.SubmissionSchemaVersion(submissionType), "submission_type": submissionType, "objects": json.RawMessage(body)})
	if err != nil {
		t.Fatal(err)
	}
	snapshot := reviewdomain.ApprovedSnapshot{ID: snapshotID, ProjectID: "project-1", SubmissionType: submissionType, CanonicalContent: canonical, EligibleIDs: []string{objectID}, CreatedAt: createdAt}
