...
func RegisterLocalSource(options RegisterLocalSourceOptions) (LocalSource, error) {
	...
	if info.IsDir() || info.Size() <= 0 || info.Size() > localSourceSizeLimit {
		return LocalSource{}, fault.Invalid("LOCAL_SOURCE_SIZE_INVALID", "本地来源必须是 1B 到 100MB 的文件")
	}
	...
	if mode != "copy" && mode != "reference" {
		return LocalSource{}, fault.Invalid("LOCAL_SOURCE_STORAGE_MODE_INVALID", "storage_mode 只允许 copy 或 reference")
	}
	...
	if !localSourceIDPattern.MatchString(id) {
		return LocalSource{}, fault.Invalid("LOCAL_SOURCE_ID_INVALID", "来源 ID 只能包含字母、数字、冒号、点、下划线和连字符")
	}
	...
	for _, existing := range registry.Sources {
		if existing.ID == id {
			...
			return LocalSource{}, fault.Conflict("LOCAL_SOURCE_ID_CONFLICT", "相同来源 ID 已绑定不同内容；请使用新的稳定 ID")
		}
		if existing.SHA256 == hash {
			return LocalSource{}, fault.Conflict("LOCAL_SOURCE_DUPLICATE", "相同内容已登记为 "+existing.ID)
		}
	}
		...
		if filepath.Clean(destination) != filepath.Clean(absolute) {
			if existing, readErr := os.ReadFile(destination); readErr == nil {
				if digest(existing) != hash {
					return LocalSource{}, fault.Conflict("LOCAL_SOURCE_COPY_CONFLICT", "20-sources/originals 中目标文件内容不同")
				}
...
func LocalSourceByID(root, id string) (LocalSource, error) {
	...
	return LocalSource{}, fault.NotFound("本地来源")
}
...
func IngestLocalSource(root, id string, now time.Time) (LocalEvidenceBundle, error) {
	...
	if index < 0 {
		return LocalEvidenceBundle{}, fault.NotFound("本地来源")
	}
	...
	if len(body) == 0 || len(body) > localSourceSizeLimit {
		return LocalEvidenceBundle{}, fault.Invalid("LOCAL_SOURCE_SIZE_INVALID", "本地来源必须是 1B 到 100MB 的文件")
	}
	if digest(body) != source.SHA256 {
		return LocalEvidenceBundle{}, fault.Conflict("LOCAL_SOURCE_HASH_MISMATCH", "来源文件已变化；必须登记为新的不可变来源")
	}
	detected := ingest.DetectMIME(body)
	if detected != source.MIMEType {
		return LocalEvidenceBundle{}, fault.Conflict("LOCAL_SOURCE_MIME_MISMATCH", "来源文件 MIME 与登记值不一致")
	}
...
func loadSourceRegistry(root string) (SourceRegistry, error) {
	...
	if err := readYAML(path, &registry); err != nil {
		return registry, fault.Invalid("LOCAL_SOURCE_REGISTRY_INVALID", "20-sources/registry.yaml 必须符合 V3 来源目录规范")
	}
	if registry.SchemaVersion != SourceRegistrySchemaVersion {
		return registry, fault.Conflict("LOCAL_SOURCE_REGISTRY_VERSION_UNSUPPORTED", "不支持该来源目录 Schema 版本")
	}
...
func resolveLocalSourcePath(root string, location SourceLocation) (string, error) {
	...
	case "workspace_file":
		if location.Path == "" || filepath.IsAbs(location.Path) {
			return "", fault.Invalid("LOCAL_SOURCE_LOCATION_INVALID", "workspace_file 来源必须使用 20-sources 下的相对路径")
		}
		...
		if !strings.HasPrefix(path+string(os.PathSeparator), base) {
			return "", fault.Policy("LOCAL_SOURCE_PATH_OUTSIDE_WORKSPACE", "来源路径越过 20-sources 边界", "重新登记来源")
		}
		return path, nil
	case "external_readonly_ref":
		if !filepath.IsAbs(location.Ref) {
			return "", fault.Invalid("LOCAL_SOURCE_LOCATION_INVALID", "external_readonly_ref 必须是只读的绝对路径引用")
		}
		return filepath.Clean(location.Ref), nil
	default:
		return "", fault.Policy("LOCAL_SOURCE_LOCATION_UNREADABLE", "本地导入程序无法读取当前来源位置", "先把来源下载或复制到当前工作区")
	}
}
