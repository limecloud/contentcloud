...
type ReviewFeedbackInboxItem struct {
	...
	Feedback reviewdomain.ReviewFeedbackBundle `json:"feedback"`
}

func StoreReviewFeedback(root string, feedback reviewdomain.ReviewFeedbackBundle, now time.Time) (ReviewFeedbackInboxItem, error) {
...
func ReviewFeedbackInbox(root string) ([]ReviewFeedbackInboxItem, error) {
	...
	for _, path := range paths {
		...
		if err := json.Unmarshal(body, &feedback); err != nil {
			return nil, fault.Invalid("REVIEW_FEEDBACK_INVALID", "本地审核反馈不是有效的 JSON 数据包")
		}
		id, hash, err := reviewFeedbackIdentity(feedback)
		if err != nil {
			return nil, err
		...
		if strings.TrimSuffix(filepath.Base(path), filepath.Ext(path)) != id {
			return nil, fault.Conflict("REVIEW_FEEDBACK_DIGEST_MISMATCH", "本地审核反馈文件名与内容 digest 不一致")
		}
...
func reviewFeedbackIdentity(feedback reviewdomain.ReviewFeedbackBundle) (string, string, error) {
	if feedback.BundleVersion != "1.0" || strings.TrimSpace(feedback.SubmissionID) == "" || strings.TrimSpace(feedback.SubmissionRevisionID) == "" || strings.TrimSpace(feedback.SubjectHash) == "" || feedback.CreatedAt.IsZero() {
		return "", "", fault.Invalid("REVIEW_FEEDBACK_INVALID", "审核反馈数据包缺少固定版本、提交记录、审核对象摘要或创建时间")
	}
	for _, comment := range feedback.Comments {
		if strings.TrimSpace(comment.ID) == "" || strings.TrimSpace(comment.Body) == "" || comment.CreatedAt.IsZero() {
			return "", "", fault.Invalid("REVIEW_FEEDBACK_INVALID", "审核反馈包含无 ID、正文或创建时间的 comment")
		}
	}
	hash, err := stablehash.Sum(feedback)
	if err != nil {
		return "", "", err
	}
