...
func CreateDouyinAudienceTaxonomy(options CreateDouyinTaxonomyOptions) (string, work.AudienceTaxonomySnapshot, error) {
	root, err := FindRoot(options.Root)
	if err != nil {
		return "", work.AudienceTaxonomySnapshot{}, err
	}
	if options.ID == "" {
		options.ID = idgen.New()
	}
	if localSafeName(options.ID) != options.ID {
		return "", work.AudienceTaxonomySnapshot{}, fault.Invalid("AUDIENCE_TAXONOMY_ID_INVALID", "人群目录 ID 只能包含字母、数字、点、下划线和连字符")
	}
	...
	taxonomy := work.AudienceTaxonomySnapshot{
		ID: options.ID, Type: "audience_taxonomy_snapshot", SchemaVersion: work.AudienceTaxonomySchema,
		...
		Segments: work.DefaultDouyinAudienceSegments(), SourceURL: strings.TrimSpace(options.SourceURL), CapturedAt: options.CapturedAt.UTC(),
...
func ScaffoldAudienceStrategies(options ScaffoldAudienceStrategiesOptions) ([]string, []work.AudienceStrategyVersion, error) {
	root, err := FindRoot(options.Root)
	if err != nil {
		return nil, nil, err
	}
	...
	if err != nil {
		if fault.IsNotFound(err) {
			return nil, nil, fault.Policy("AUDIENCE_TAXONOMY_PULL_REQUIRED", "本机没有指定的已批准人群目录", "先执行 contentcloud pull approved --type strategy")
		}
		return nil, nil, err
	}
	...
	if err := json.Unmarshal(raw, &taxonomy); err != nil || taxonomy.Type != "audience_taxonomy_snapshot" {
		return nil, nil, fault.Invalid("AUDIENCE_TAXONOMY_INVALID", "批准快照对象不是有效人群目录")
	}
	...
	strategies := make([]work.AudienceStrategyVersion, 0, len(selected))
	for _, segment := range selected {
		strategy := work.AudienceStrategyVersion{
			ID: idgen.New(), Type: "audience_strategy_version", SchemaVersion: work.AudienceStrategySchema, ProjectID: status.Binding.ProjectID,
...
func LintAudienceTaxonomy(root, file string, now time.Time) (V5LintReport, work.AudienceTaxonomySnapshot, error) {
	resolved, path, err := resolveV5JSON(root, file)
	if err != nil {
		return V5LintReport{}, work.AudienceTaxonomySnapshot{}, err
	}
	var value work.AudienceTaxonomySnapshot
	if err := readStrictJSON(path, &value); err != nil {
		return V5LintReport{}, value, fault.Invalid("AUDIENCE_TAXONOMY_JSON_INVALID", err.Error())
	}
...
func LintAudienceStrategy(root, file string, now time.Time) (V5LintReport, work.AudienceStrategyVersion, error) {
	resolved, path, err := resolveV5JSON(root, file)
	if err != nil {
		return V5LintReport{}, work.AudienceStrategyVersion{}, err
	}
	var value work.AudienceStrategyVersion
	if err := readStrictJSON(path, &value); err != nil {
		return V5LintReport{}, value, fault.Invalid("AUDIENCE_STRATEGY_JSON_INVALID", err.Error())
	}
	report := v5Report(resolved, path, value.ID, value.Type)
...
func LintCommerceOffer(root, file string, at time.Time) (V5LintReport, work.CommerceOfferSnapshot, error) {
	resolved, path, err := resolveV5JSON(root, file)
	if err != nil {
		return V5LintReport{}, work.CommerceOfferSnapshot{}, err
	}
	var value work.CommerceOfferSnapshot
	if err := readStrictJSON(path, &value); err != nil {
		return V5LintReport{}, value, fault.Invalid("COMMERCE_OFFER_JSON_INVALID", err.Error())
	}
	report := v5Report(resolved, path, value.ID, value.Type)
...
func selectAudienceSegments(all []work.AudienceSegment, mode string, codes []string) ([]work.AudienceSegment, error) {
	mode = strings.TrimSpace(mode)
	if mode != "single" && mode != "compare" && mode != "explore" {
		return nil, fault.Invalid("AUDIENCE_MODE_INVALID", "mode 只允许 single、compare 或 explore")
	}
	if mode == "explore" {
		return append([]work.AudienceSegment(nil), all...), nil
	}
	if (mode == "single" && len(codes) != 1) || (mode == "compare" && (len(codes) < 2 || len(codes) > 3)) {
		return nil, fault.Invalid("AUDIENCE_SELECTION_INVALID", "single 必须选择 1 类，compare 必须选择 2 至 3 类")
	}
	index := map[string]work.AudienceSegment{}
	for _, segment := range all {
		index[segment.Code] = segment
	}
	selected := []work.AudienceSegment{}
	seen := map[string]bool{}
	for _, code := range codes {
		if seen[code] {
			return nil, fault.Invalid("AUDIENCE_SELECTION_DUPLICATE", "人群代码不能重复")
		}
		segment, ok := index[code]
		if !ok {
			return nil, fault.Invalid("AUDIENCE_CODE_UNKNOWN", "人群代码不在已批准目录中："+code)
		}
		seen[code] = true
