...
import (
	...
	"github.com/limecloud/contentcloud/internal/local/ingest"
	"gopkg.in/yaml.v3"
)
...
func BuildWorkspaceView(options WorkspaceViewOptions) (WorkspaceView, error) {
	...
	if !validWorkspaceViewKind(kind) {
		return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_KIND_INVALID", "view 必须是 workspace_summary、file、run、handoff、content_item、render、diff 或 delivery")
	}
	...
	if kind == "run" && strings.TrimSpace(options.Ref) == "" {
		...
		if runID == "" {
			return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_RUN_REQUIRED", "run 视图需要 run_id 或 ref")
		}
	...
	if kind == "handoff" && strings.TrimSpace(options.Ref) == "" {
		return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_REF_REQUIRED", "handoff 视图需要 40-work/handoffs 下的 ref")
	}
	...
	if strings.Contains(file.MIMEType, "json") {
		if err := json.Unmarshal(file.Body, &data); err != nil {
			return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_DOCUMENT_INVALID", "JSON 文件无法解析为类型化视图")
		}
	} else if strings.HasSuffix(file.Ref, ".yaml") || strings.HasSuffix(file.Ref, ".yml") {
		var value any
		if err := yaml.Unmarshal(file.Body, &value); err != nil {
			return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_DOCUMENT_INVALID", "YAML 文件无法解析为类型化视图")
		}
		body, marshalErr := json.Marshal(value)
		if marshalErr != nil || json.Unmarshal(body, &data) != nil {
			return WorkspaceView{}, fault.Invalid("WORKSPACE_VIEW_DOCUMENT_INVALID", "YAML 文件无法转换为安全结构化内容")
		}
...
func ReadWorkspaceResource(root, uri string) (WorkspaceResource, error) {
	...
	if stream.ByteSize > workspaceViewMaxBytes {
		policy := fault.Policy("MCP_RESOURCE_TOO_LARGE", "资源超过 MCP 内联读取大小上限", "通过本地 Workbench 使用 Range 流式读取该媒体资源")
...
func OpenWorkspaceResource(root, uri string) (WorkspaceResourceStream, error) {
	...
	if err != nil || !info.Mode().IsRegular() || info.Size() != metadata.Size {
		reader.Close()
		return WorkspaceResourceStream{}, fault.Conflict("WORKSPACE_VIEW_STALE", "资源在打开过程中发生变化")
	}
...
func parseWorkspaceFileResourceURI(uri string) (string, string, error) {
	...
	if err != nil || parsed.Scheme != "contentcloud" || parsed.Host != "workspace" {
		return "", "", fault.Invalid("MCP_RESOURCE_URI_INVALID", "本地工作台资源 URI 无效")
	}
	parts := strings.Split(strings.TrimPrefix(parsed.EscapedPath(), "/"), "/")
	if len(parts) < 2 || parts[0] != "files" || parsed.Query().Get("digest") == "" {
		return "", "", fault.Invalid("MCP_RESOURCE_URI_INVALID", "本地文件资源 URI 必须包含 files 路径和 digest")
	}
	...
	for _, part := range parts[1:] {
		...
		if decodeErr != nil || value == "" || value == "." || value == ".." || strings.ContainsAny(value, "/\\") {
			return "", "", fault.Invalid("MCP_RESOURCE_URI_INVALID", "本地文件资源路径无效")
		}
...
func readWorkspaceViewFile(root, raw string) (workspaceViewFile, error) {
	...
	if !info.Mode().IsRegular() {
		return workspaceViewFile{}, fault.Policy("WORKSPACE_VIEW_FILE_TYPE_DENIED", "本地展示只允许普通文件", "选择普通 Workspace 文件")
	}
	...
	if mediaType == "application/octet-stream" {
		return workspaceViewFile{}, fault.Policy("WORKSPACE_VIEW_MIME_UNSUPPORTED", "文件类型不能安全地通过本地工作台展示", "使用受支持的文本、JSON、YAML、图片、音频、视频或 PDF 文件")
	}
...
func readWorkspaceViewDirectory(root, raw string) ([]WorkspaceDirectoryEntry, error) {
	...
	if err != nil || !info.IsDir() {
		return nil, fault.Invalid("WORKSPACE_VIEW_NOT_DIRECTORY", "ref 不是可浏览目录")
	}
	...
	if len(entries) > 200 {
		return nil, fault.Policy("WORKSPACE_VIEW_DIRECTORY_TOO_LARGE", "目录直接子项超过展示上限", "选择更具体的子目录")
	}
...
func normalizeWorkspaceViewRef(raw string) (string, error) {
	...
	if strings.Contains(raw, "\\") || strings.ContainsRune(raw, '\x00') || (len(raw) >= 2 && raw[1] == ':') {
		return "", fault.Invalid("WORKSPACE_VIEW_PATH_INVALID", "ref 必须使用正斜杠的 Workspace-relative 路径")
	}
	...
	if ref == "." || filepath.IsAbs(ref) || ref == ".." || strings.HasPrefix(ref, "../") || strings.Contains(raw, "//") {
		return "", fault.Invalid("WORKSPACE_VIEW_PATH_INVALID", "ref 必须是允许目录下的 Workspace-relative 路径")
	}
	parts := strings.Split(ref, "/")
	if !workspaceViewRoots[parts[0]] {
		return "", fault.Policy("WORKSPACE_VIEW_PATH_DENIED", "ref 不在本地展示 allowlist 中", "只读取 context、sources、knowledge、work、production、delivery、results 或 archive")
	}
	for _, part := range parts[1:] {
		if deniedWorkspaceViewSegment(part) {
			return "", fault.Policy("WORKSPACE_VIEW_PATH_DENIED", "ref 包含默认拒绝展示的敏感路径段", "不要通过本地工作台读取凭据、Token、日志、transcript 或隐藏文件")
		}
	}
	return ref, nil
}
...
func workspaceFileTooLarge(size, limit int64) error {
	policy := fault.Policy("WORKSPACE_VIEW_FILE_TOO_LARGE", "文件超过本地展示大小上限", "选择受支持且大小受控的 Workspace 文件")
...
func verifyWorkspaceViewDigest(actual, expected string) error {
	...
	conflict := fault.Conflict("WORKSPACE_VIEW_STALE", "本地文件摘要已变化，旧视图不能继续使用")
...
func verifyWorkspaceViewRevision(actual, expected uint64) error {
	...
	conflict := fault.Conflict("WORKSPACE_VIEW_STALE", "LocalRun context revision 已变化，旧视图不能继续使用")
