...
func ResolveWorkspaceRoot(directory, cwd string) (WorkspaceResolution, error) {
	...
	if err != nil {
		if source == "process_cwd" {
			notFound := fault.NotFound("唯一的 Content Work OS 工作区")
...
func StoreBootstrapHandoff(root, pluginID, pluginVersion, pluginDigest string, now time.Time) (BootstrapHandoff, string, error) {
	if strings.TrimSpace(pluginID) == "" || strings.TrimSpace(pluginVersion) == "" || strings.TrimSpace(pluginDigest) == "" {
		return BootstrapHandoff{}, "", fault.Invalid("BOOTSTRAP_HANDOFF_SPEC_INVALID", "初始化交接需要固定的插件 ID、版本和标准包摘要")
	}
...
func loadBootstrapHandoff(root string) (*BootstrapHandoff, error) {
	...
	if handoff.SchemaVersion != "1.0" || handoff.Kind != "bootstrap_handoff" || handoff.Status != "ready" || handoff.WorkspaceID == "" || handoff.ProjectID == "" || handoff.PluginID == "" || handoff.PluginVersion == "" || handoff.PluginDigest == "" || handoff.EnvironmentDigest == "" || handoff.NextCapabilityID == "" || handoff.NextAction == "" || handoff.CreatedAt.IsZero() {
		return nil, fault.Invalid("BOOTSTRAP_HANDOFF_INVALID", "初始化交接文件无效")
	}
	return &handoff, nil
}
...
func activeLocalRuns(root string, now time.Time) ([]WorkspaceRunSummary, error) {
	...
	for _, path := range paths {
		...
		if problems := validateLocalRun(run); len(problems) > 0 {
			invalid := fault.Invalid("LOCAL_RUN_CONTEXT_INVALID", "本地运行上下文（LocalRunContext）校验失败")
