...
func EnsureSessionBinding(root, sessionID string, now time.Time) (SessionBinding, error) {
	...
	if sessionID == "" || len(sessionID) > 128 {
		return SessionBinding{}, fault.Invalid("WORKSPACE_SESSION_ID_INVALID", "工作区会话 ID 必须非空且不超过 128 个字符")
	}
	...
	if stored.SchemaVersion != workspaceSessionSchema || stored.SessionID != sessionID || stored.Generation == "" {
		return SessionBinding{}, fault.Invalid("WORKSPACE_SESSION_BINDING_INVALID", "工作区会话绑定文件无效")
	}
	if stored.Generation != current.Generation {
		err := fault.Conflict("WORKSPACE_SESSION_COMPONENTS_CHANGED", "当前对话绑定的 Plugin、Skill、MCP 或环境版本已经变化")
...
func ObserveWorkspace(root string, now time.Time) (workspacedomain.DaemonWorkspaceObservation, error) {
	...
	observation := workspacedomain.DaemonWorkspaceObservation{
...
func sessionBindingPath(root, sessionID string) string {
	return filepath.Join(root, ".contentcloud", "sessions", idgen.TokenHash(sessionID)+".json")
}
