...
func Initialize(options InitOptions) (Status, error) {
	...
	if strings.TrimSpace(options.ProjectID) == "" {
		return Status{}, fault.Invalid("PROJECT_ID_REQUIRED", "初始化工作区需要项目 ID")
	}
	...
	if environmentDigest == "" {
		hash, hashErr := stablehash.Sum(lock)
		if hashErr != nil {
...
func FindRoot(start string) (string, error) {
	...
	return "", fault.NotFound("Content Work OS 工作区")
}
...
func LoadStatus(root string) (Status, error) {
	...
	if binding.SchemaVersion != WorkspaceSchemaVersion || binding.LayoutVersion != LayoutVersion || binding.WorkspaceID == "" || binding.ProjectID == "" {
		return Status{}, fault.Conflict("WORKSPACE_LAYOUT_UNSUPPORTED", "工作区不是受支持的 Content Work OS V3 布局")
	}
...
func StorePulledBundle(root, kind, id string, value any, now time.Time) (string, error) {
	if id == "" || filepath.Base(id) != id || strings.ContainsAny(id, `/\\`) {
		return "", fault.Invalid("PULL_BUNDLE_ID_INVALID", "拉取包 ID 无效")
	}
	resolved, state, err := loadSyncState(root)
	if err != nil {
		return "", err
	...
	default:
		return "", fault.Invalid("PULL_KIND_INVALID", "pull 类型无效")
	}
	...
	if existing, err := os.ReadFile(destination); err == nil {
		if !bytes.Equal(existing, body) {
			return "", fault.Conflict("PULL_IMMUTABLE_CONFLICT", "本地已有相同 ID 但内容不同的拉取包")
		}
...
func conflictError(paths []string) error {
	...
	err := fault.Conflict("WORKSPACE_DIRECTORY_NOT_EMPTY", "目标目录非空，且不是 Content Work OS 工作区")
...
func writeNewFile(path string, body []byte) error {
	if _, err := os.Stat(path); err == nil {
		return fault.Conflict("WORKSPACE_FILE_EXISTS", "初始化拒绝覆盖已有文件："+path)
	} else if !errors.Is(err, os.ErrNotExist) {
		return err
	}
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
