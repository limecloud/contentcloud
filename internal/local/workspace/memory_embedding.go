...
func (a *MemoryRemoteAdapter) Embed(ctx context.Context, inputs []string) ([][]float32, error) {
	if a == nil || a.baseURL == nil || a.client == nil {
		return nil, fault.Invalid("MEMORY_EMBEDDING_ADAPTER_INVALID", "Embedding 适配器未配置")
	}
	clean := make([]string, 0, len(inputs))
	for _, input := range inputs {
		value := strings.TrimSpace(input)
		if value == "" {
			return nil, fault.Invalid("MEMORY_EMBEDDING_INPUT_INVALID", "Embedding 输入不能包含空文本")
		}
		clean = append(clean, value)
	}
	if len(clean) == 0 || len(clean) > 100 {
		return nil, fault.Invalid("MEMORY_EMBEDDING_INPUT_INVALID", "Embedding 一次只能处理 1 到 100 条文本")
	}
	...
	if err := json.Unmarshal(responseBody, &direct); err != nil || len(direct.Data) == 0 {
		...
		if json.Unmarshal(responseBody, &wrapped) != nil || len(wrapped.Data.Data) == 0 {
			return nil, fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 服务响应缺少 data")
		}
		direct = wrapped.Data
	}
	...
	for _, item := range direct.Data {
		if item.Index < 0 || item.Index >= len(result) || seen[item.Index] || len(item.Embedding) == 0 || len(item.Embedding) > 4096 {
			return nil, fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 响应的 index 或向量维度无效")
		}
		for _, value := range item.Embedding {
			if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) {
				return nil, fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 向量包含非有限数值")
			}
		}
	...
	for _, vector := range result {
		if len(vector) == 0 {
			return nil, fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 响应缺少输入对应向量")
		}
	}
	return result, nil
}
...
func QueryMemoryWithEmbedding(ctx context.Context, root string, adapter MemoryEmbeddingAdapter, options MemoryQueryOptions) (MemoryQueryResult, error) {
	if adapter == nil {
		return MemoryQueryResult{}, fault.Invalid("MEMORY_EMBEDDING_ADAPTER_REQUIRED", "混合记忆查询必须提供 Embedding 适配器")
	}
	...
	for index, candidate := range base.Candidates {
		if len(vectors[0]) != len(vectors[index+1]) {
			return MemoryQueryResult{}, fault.Invalid("MEMORY_EMBEDDING_DIMENSION_MISMATCH", "查询和候选 Embedding 维度不一致")
		}
...
func validateMemoryEmbeddingVectors(vectors [][]float32, expected int) error {
	if expected <= 0 || len(vectors) != expected {
		return fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 向量数量与输入不一致")
	}
	dimension := 0
	for _, vector := range vectors {
		if len(vector) == 0 || len(vector) > 4096 {
			return fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 响应的向量维度无效")
		}
		if dimension == 0 {
			dimension = len(vector)
		} else if len(vector) != dimension {
			return fault.Invalid("MEMORY_EMBEDDING_DIMENSION_MISMATCH", "Embedding 向量维度不一致")
		}
		for _, value := range vector {
			if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) {
				return fault.Invalid("MEMORY_EMBEDDING_RESPONSE_INVALID", "Embedding 向量包含非有限数值")
			}
		}
	}
	return nil
