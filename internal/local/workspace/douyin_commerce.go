...
type DouyinCommerceValidationResult struct {
	...
	Receipt work.DouyinCommerceValidationReceipt `json:"receipt"`
}
...
func douyinOutputPath(root, output string) (string, error) {
	...
	if err != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
		return "", fault.Policy("LOCAL_FILE_OUTSIDE_WORKSPACE", "校验回执必须位于当前工作区", "使用 60-delivery/validations 下的输出路径")
	}
	return filepath.Clean(absolute), nil
}

func LintDouyinCommerceReceipt(root, receiptFile, renderedCreativeFile, landingPageTextFile string) (V5LintReport, work.DouyinCommerceValidationReceipt, error) {
	resolved, path, err := resolveV5JSON(root, receiptFile)
	if err != nil {
		return V5LintReport{}, work.DouyinCommerceValidationReceipt{}, err
	}
	var receipt work.DouyinCommerceValidationReceipt
	if err := readStrictJSON(path, &receipt); err != nil {
		return V5LintReport{}, receipt, fault.Invalid("DOUYIN_COMMERCE_RECEIPT_JSON_INVALID", err.Error())
	}
...
func buildDouyinCommerceReceipt(root string, options ValidateDouyinCommerceOptions) (work.DouyinCommerceValidationReceipt, error) {
	scheduledAt, validatedAt := options.ScheduledAt.UTC(), localNow(options.ValidatedAt)
	if scheduledAt.IsZero() {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_SCHEDULE_REQUIRED", "抖音电商校验必须固定计划发布时间")
	}
	...
	if err != nil {
		return work.DouyinCommerceValidationReceipt{}, err
	}
	...
	if json.Unmarshal(strategyRaw, &strategy) != nil || strategy.Type != "audience_strategy_version" {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_AUDIENCE_INVALID", "人群策略批准快照不包含有效 AudienceStrategyVersion")
	}
	...
	var offer work.CommerceOfferSnapshot
	if json.Unmarshal(offerRaw, &offer) != nil || offer.Type != "commerce_offer_snapshot" {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_OFFER_INVALID", "商品方案批准快照不包含有效 CommerceOfferSnapshot")
	}
	...
	if strings.ToLower(strings.TrimSpace(content.Channel)) != "douyin" {
		return work.DouyinCommerceValidationReceipt{}, fault.Policy("DOUYIN_COMMERCE_CONTENT_CHANNEL_INVALID", "抖音电商校验只接受 channel=douyin 的已批准内容", "选择抖音内容项或重新提交审核")
	}
	...
	if json.Unmarshal(storyboardRaw, &storyboard) != nil || storyboard.Type != "storyboard_package" {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_STORYBOARD_INVALID", "分镜批准快照不包含有效 StoryboardPackage")
	}
	...
	if storyboard.LockedDigest != lockedDigest || storyboard.ApprovedSnapshotID != contentSnapshot.ID || storyboard.ContentItemID != content.ID {
		return work.DouyinCommerceValidationReceipt{}, fault.Conflict("DOUYIN_COMMERCE_STORYBOARD_LINEAGE_INVALID", "分镜摘要或 ContentItem 批准血缘不一致")
	}
	...
	for _, value := range []struct {
		...
		if value.got != strategy.ProjectID {
			return work.DouyinCommerceValidationReceipt{}, fault.Conflict("DOUYIN_COMMERCE_PROJECT_MISMATCH", value.name+" 不属于同一项目")
		}
	}
	...
	if landingText == "" {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_LANDING_TEXT_REQUIRED", "落地页文本不能为空")
	}
	...
	if err != nil || !info.Mode().IsRegular() || len(artifactBody) == 0 {
		return work.DouyinCommerceValidationReceipt{}, fault.Invalid("DOUYIN_COMMERCE_ARTIFACT_INVALID", "最终成片必须是工作区内非空普通文件")
	}
	artifactSum := sha256.Sum256(artifactBody)
	voiceDigest, _ := work.DouyinTextDigest(voiceover)
	onScreenDigest, _ := work.DouyinTextDigest(onScreen)
	landingDigest, _ := work.DouyinTextDigest(landingText)

	receipt := work.DouyinCommerceValidationReceipt{
		SchemaVersion: work.DouyinCommerceValidationSchema, ContentProfileID: work.DouyinCommerceProfileID, ProjectID: strategy.ProjectID,
		...
		Offer:            work.DouyinCommerceOfferFacts{SKUID: offer.SKUID, ProductVersionID: offer.ProductVersionID, DisplayPrice: strings.TrimSpace(offer.DisplayPrice), Currency: strings.ToUpper(strings.TrimSpace(offer.Currency)), Benefits: sortedUniqueStrings(offer.Benefits), Conditions: sortedUniqueStrings(offer.Conditions)},
...
func exactApprovedObject(root, submissionType, snapshotID, objectID string) (json.RawMessage, reviewdomain.ApprovedSnapshot, error) {
	...
	if snapshot.SubmissionType != submissionType || !containsLocalString(snapshot.EligibleIDs, objectID) {
		return nil, snapshot, fault.Conflict("DOUYIN_COMMERCE_APPROVED_REF_INVALID", "批准快照类型或 eligible_ids 与指定对象不一致")
	}
	var canonical struct {
		Objects []json.RawMessage `json:"objects"`
	}
	if json.Unmarshal(snapshot.CanonicalContent, &canonical) != nil {
		return nil, snapshot, fault.Invalid("DOUYIN_COMMERCE_APPROVED_SNAPSHOT_INVALID", "批准快照缺少规范对象列表")
	}
	...
	return nil, snapshot, fault.NotFound("抖音电商批准对象")
}
...
func validateDouyinShotClaims(content ContentItem, offer work.CommerceOfferSnapshot) error {
	...
	for _, shot := range content.Shots {
		...
		if !matched {
			return fault.Policy("DOUYIN_COMMERCE_DYNAMIC_CLAIM_REQUIRED", "镜头 "+shot.ShotID+" 的动态商品文案未引用 Offer 批准 Claim", "为该镜头补充 Offer approved_claim_refs 中的引用并重新审核")
		}
	}
	return nil
}

func validateDouyinOfferText(text string, offer work.CommerceOfferSnapshot, benefits, conditions []string) error {
	offerPrice, err := normalizedPrice(offer.DisplayPrice)
	if err != nil {
		return fault.Invalid("DOUYIN_COMMERCE_OFFER_PRICE_INVALID", "Offer display_price 无法规范化")
	}
	for _, match := range commerceCNYPricePattern.FindAllStringSubmatch(text, -1) {
		if strings.ToUpper(offer.Currency) != "CNY" || normalizedPriceValue(firstNonEmpty(match[1], match[2])) != offerPrice {
			return fault.Conflict("DOUYIN_COMMERCE_PRICE_MISMATCH", "口播、字幕或落地页中的人民币价格与批准 Offer 不一致")
		}
	}
	for _, match := range commerceCodePricePattern.FindAllStringSubmatch(text, -1) {
		if match[1] != strings.ToUpper(offer.Currency) || normalizedPriceValue(match[2]) != offerPrice {
			return fault.Conflict("DOUYIN_COMMERCE_PRICE_MISMATCH", "口播、字幕或落地页中的币种或价格与批准 Offer 不一致")
		}
	}
	if commercePromotionPattern.MatchString(text) && len(benefits)+len(conditions) == 0 {
		return fault.Policy("DOUYIN_COMMERCE_PROMOTION_FACT_REQUIRED", "内容包含优惠、赠品、库存或条件文案，但没有声明结构化观察事实", "使用 --benefit/--condition 声明正文中的 Offer 允许事实")
	}
...
func validateObservedOfferFacts(text string, observed, allowed []string, kind string) error {
	...
	for _, value := range observed {
		if !allowedSet[value] {
			return fault.Policy("DOUYIN_COMMERCE_"+kind+"_NOT_ALLOWED", "内容声明了 Offer 未允许的权益或条件："+value, "更新 Offer 并重新批准，或移除该文案")
		}
		if !strings.Contains(text, value) {
			return fault.Conflict("DOUYIN_COMMERCE_"+kind+"_NOT_IN_TEXT", "结构化观察事实未出现在口播、字幕或落地页："+value)
		}
	}
	return nil
}
