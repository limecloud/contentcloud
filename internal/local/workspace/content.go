...
func LintBrief(root, file string) (KnowledgeLintReport, LocalBrief, error) {
	...
	if err := readStrictJSON(path, &brief); err != nil {
		return KnowledgeLintReport{}, brief, fault.Invalid("BRIEF_JSON_INVALID", err.Error())
	}
...
func CreateContentBatch(options CreateContentBatchOptions) (CreateContentBatchResult, error) {
	...
	if err := strictUnmarshal(briefRaw, &brief); err != nil {
		return CreateContentBatchResult{}, fault.Invalid("APPROVED_BRIEF_INVALID", "批准快照中的 Brief V3 无效："+err.Error())
	}
	if brief.Deliverability != "review_ready" {
		return CreateContentBatchResult{}, fault.Policy("APPROVED_BRIEF_BLOCKED", "批准的 Brief 仍为 blocked，不能创建正式批次", "补齐输入并发布新的 Brief revision")
	}
	...
	if err := readStrictJSON(directionsPath, &directions); err != nil {
		return CreateContentBatchResult{}, fault.Invalid("CREATIVE_DIRECTIONS_INVALID", err.Error())
	}
	if len(directions) == 0 || len(directions) > 20 {
		return CreateContentBatchResult{}, fault.Invalid("CREATIVE_DIRECTIONS_COUNT_INVALID", "创意方向数量必须为 1 到 20")
	}
	...
	for _, direction := range directions {
		if direction.ID == "" || direction.Title == "" || direction.Angle == "" || direction.HookType == "" || direction.VisualMotif == "" || direction.Tone == "" || direction.TargetEmotion == "" || len(direction.Narrative) == 0 || direction.RiskRefs == nil || !validDirectionStatus(direction.Status) || !allUnique(direction.RiskRefs) {
			return CreateContentBatchResult{}, fault.Invalid("CREATIVE_DIRECTION_INVALID", "创意方向缺少必填字段或数组，或 status 无效")
		}
		if seen[direction.ID] {
			return CreateContentBatchResult{}, fault.Invalid("CREATIVE_DIRECTION_DUPLICATE", "创意方向 ID 重复："+direction.ID)
		}
	...
	if len(selected) == 0 {
		return CreateContentBatchResult{}, fault.Invalid("CREATIVE_DIRECTION_SELECTION_REQUIRED", "至少选择一个创意方向")
	}
	...
	if count < 1 || count > 10 {
		return CreateContentBatchResult{}, fault.Invalid("CONTENT_BATCH_COUNT_INVALID", "requested_count 必须为 1 到 10")
	}
	variant := strings.TrimSpace(options.VariantDimension)
	if !validVariantDimension(variant) {
		return CreateContentBatchResult{}, fault.Invalid("CONTENT_BATCH_VARIANT_INVALID", "variant_dimension 只允许 hook、audience、scenario、visualization、cta 或 duration")
	}
	...
	if query.ApprovedSnapshotID == "" {
		return CreateContentBatchResult{}, fault.Policy("KNOWLEDGE_SNAPSHOT_REQUIRED", "创建正式剧本批次需要已拉取的知识批准快照", "先执行 contentcloud pull approved --type knowledge")
	}
	...
	if containsString(controlled, variant) {
		return CreateContentBatchResult{}, fault.Invalid("CONTENT_BATCH_EXPERIMENT_INVALID", "variant_dimension 不能同时被 controlled_dimensions 锁定")
	}
	deliveryProfiles := []string{"json", "markdown", "xlsx"}
	hashInput := map[string]any{"project_id": status.Binding.ProjectID, "content_kind": identitydomain.ContentTypeVideoScript, "content_schema_ref": ContentItemSchema, "delivery_profiles": deliveryProfiles, "brief_id": brief.ID, "brief_snapshot_id": briefSnapshot.ID, "knowledge_snapshot_id": query.ApprovedSnapshotID, "direction_ids": directionIDs, "requested_count": count, "variant_dimension": variant, "controlled_dimensions": controlled}
	hash, err := stablehash.Sum(hashInput)
	if err != nil {
		return CreateContentBatchResult{}, err
	}
	...
	if !localSourceIDPattern.MatchString(batchID) {
		return CreateContentBatchResult{}, fault.Invalid("CONTENT_BATCH_ID_INVALID", "batch ID 无效")
	}
	contextHash, _ := stablehash.Sum(map[string]any{"brief_snapshot_id": briefSnapshot.ID, "knowledge_snapshot_id": query.ApprovedSnapshotID, "eligible_ids": knowledgeEntryIDs(query.Eligible)})
	...
	batch := ContentBatch{
		SchemaVersion: ContentBatchSchema, ID: batchID, IntentID: intentID, ContentKind: identitydomain.ContentTypeVideoScript, ContentSchemaRef: ContentItemSchema, DeliveryProfiles: deliveryProfiles, BriefRef: brief.ID, KnowledgeSnapshotRefs: []string{query.ApprovedSnapshotID}, Status: "candidate", Publishable: false,
	...
	if _, readErr := os.Stat(batchPath); readErr == nil {
		...
		return CreateContentBatchResult{}, fault.Conflict("CONTENT_BATCH_IMMUTABLE_CONFLICT", "相同 batch ID 已存在不同内容")
...
func LintContentItem(root, file, batchFile string) (ContentItemLintReport, ContentItem, error) {
	...
	if err := readStrictJSON(path, &pkg); err != nil {
		return ContentItemLintReport{}, pkg, fault.Invalid("CONTENT_ITEM_JSON_INVALID", err.Error())
	}
	...
	hash, hashErr := stablehash.Sum(pkg)
	if hashErr == nil {
...
func FinalizeContentBatch(root, batchFile string, contentFiles []string, now time.Time) (FinalizeContentBatchResult, error) {
	...
	if !report.Valid {
		err := fault.Invalid("CONTENT_BATCH_LINT_FAILED", "内容批次校验失败")
		err.Details = report
		return FinalizeContentBatchResult{}, err
	}
...
func DiffContentItems(root, baselineFile, candidateFile string, allowedPaths []string) (ContentItemDiff, error) {
	...
	if err := readStrictJSON(baselinePath, &baseline); err != nil {
		return ContentItemDiff{}, fault.Invalid("CONTENT_ITEM_BASELINE_INVALID", err.Error())
	}
	if err := readStrictJSON(candidatePath, &candidate); err != nil {
		return ContentItemDiff{}, fault.Invalid("CONTENT_ITEM_CANDIDATE_INVALID", err.Error())
	}
	if candidate.BasedOnVersionID != baseline.ID || strings.TrimSpace(candidate.ChangeSummary) == "" {
		return ContentItemDiff{}, fault.Invalid("CONTENT_ITEM_REVISION_METADATA_INVALID", "修订稿必须用 based_on_version_id 引用基线 ID 并填写 change_summary")
	}
...
func ExportApprovedContentItem(root, contentItemID, outputDirectory string, now time.Time) (ContentDeliveryManifest, error) {
	...
	if strings.TrimSpace(outputRoot) == "" {
		...
		if absErr != nil || relErr != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
			return ContentDeliveryManifest{}, fault.Policy("DELIVERY_PATH_OUTSIDE_WORKSPACE", "交付目录必须位于当前工作区", "使用 outputs/delivery 下的目录")
		}
		outputRoot = absolute
	}
...
func RenderContentItem(raw json.RawMessage) (RenderedContentDelivery, error) {
	...
	if pkg.SchemaVersion != ContentItemSchema || pkg.Type != "content_item" || pkg.Deliverability != "review_ready" {
		return RenderedContentDelivery{}, fault.Policy("APPROVED_CONTENT_ITEM_BLOCKED", "只有处于 review_ready 状态的内容对象才能生成正式交付包", "修订并批准内容对象")
	}
...
func lintContentItem(pkg ContentItem, batch ContentBatch, query KnowledgeQueryResult, refs map[string]LocalKnowledgeItem) ContentItemLintReport {
	...
	if batch.ContentKind != identitydomain.ContentTypeVideoScript || batch.ContentSchemaRef != ContentItemSchema {
...
func loadContentBatch(root, file, expectedID string) (ContentBatch, error) {
	...
	if strings.TrimSpace(path) == "" {
		if expectedID == "" {
			return ContentBatch{}, fault.Invalid("CONTENT_BATCH_FILE_REQUIRED", "必须指定内容批次 manifest")
		}
	...
	if err := readYAML(resolved, &batch); err != nil {
		return ContentBatch{}, fault.Invalid("CONTENT_BATCH_INVALID", err.Error())
	}
	if batch.SchemaVersion != ContentBatchSchema || batch.ID == "" || batch.IntentID == "" || !identitydomain.ValidTenantContentType(batch.ContentKind) || batch.ContentSchemaRef == "" || len(batch.DeliveryProfiles) == 0 || !allUnique(batch.DeliveryProfiles) || batch.BriefRef == "" || len(batch.KnowledgeSnapshotRefs) == 0 || batch.ContentItemRefs == nil || batch.BlockedReasons == nil || batch.Checks == nil || (expectedID != "" && batch.ID != expectedID) {
		return ContentBatch{}, fault.Invalid("CONTENT_BATCH_INVALID", "内容批次 manifest 标识或必填数组无效")
	}
	if batch.Publishable && (batch.Status != "review_ready" && batch.Status != "approved" && batch.Status != "delivered" || len(batch.BlockedReasons) > 0) {
		return ContentBatch{}, fault.Invalid("CONTENT_BATCH_INVALID", "可发布内容批次的状态或 blocked_reasons 无效")
	}
	if !batch.Publishable && len(batch.BlockedReasons) == 0 {
		return ContentBatch{}, fault.Invalid("CONTENT_BATCH_INVALID", "不可发布的内容批次必须说明 blocked_reasons")
	}
	...
	if err := readStrictJSON(contextPath, &frozen); err != nil {
		return ContentBatch{}, fault.Invalid("CONTENT_CONTEXT_INVALID", err.Error())
	}
	if frozen.SchemaVersion != ContentContextSchema || frozen.Batch.ID != batch.ID || frozen.ProjectID == "" || frozen.BriefSnapshotID == "" || frozen.ContextSnapshotID == "" || frozen.RequestedCount < 1 || frozen.ContentKind != batch.ContentKind || frozen.ContentSchemaRef != batch.ContentSchemaRef || !slices.Equal(frozen.DeliveryProfiles, batch.DeliveryProfiles) {
		return ContentBatch{}, fault.Invalid("CONTENT_CONTEXT_INVALID", "内容批次冻结上下文无效")
	}
...
func latestApprovedObject(root, submissionType, objectID string) (json.RawMessage, reviewdomain.ApprovedSnapshot, error) {
	summaries, err := ApprovedSnapshotInbox(root, submissionType)
	if err != nil {
		return nil, reviewdomain.ApprovedSnapshot{}, err
	}
	...
	return nil, reviewdomain.ApprovedSnapshot{}, fault.NotFound("已拉取的 " + submissionType + " ApprovedSnapshot 对象")
}
