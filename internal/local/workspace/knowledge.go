...
type LocalKnowledgeItem struct {
	...
	Value               sourcedomain.TypedValue     `json:"value"`
	Scope               sourcedomain.KnowledgeScope `json:"scope"`
	Status              string                      `json:"status"`
	RiskLevel           string                      `json:"risk_level"`
	AllowedChannels     []string                    `json:"allowed_channels"`
	Evidence            []sourcedomain.EvidenceRef  `json:"evidence"`
...
func ImportKnowledgeCandidates(options ImportKnowledgeOptions) (KnowledgeImportReport, error) {
	...
	if err := strictUnmarshal(body, &pkg); err != nil {
		return KnowledgeImportReport{}, fault.Invalid("KNOWLEDGE_CANDIDATES_JSON_INVALID", "候选包必须是 knowledge-candidates/1.0 JSON 对象")
	}
	...
	for _, candidate := range pkg.Candidates {
		...
		if existingBody, readErr := os.ReadFile(destination); readErr == nil {
			...
			return KnowledgeImportReport{}, fault.Conflict("KNOWLEDGE_ITEM_IMMUTABLE_CONFLICT", "相同知识 ID 已存在不同内容："+item.ID)
...
func PackKnowledge(options PackKnowledgeOptions) (KnowledgePackResult, error) {
	...
	if !lint.Valid {
		err := fault.Invalid("KNOWLEDGE_LINT_FAILED", "知识库存在阻断问题，不能打包")
		err.Details = lint
		return KnowledgePackResult{}, err
	}
	items, _, err := loadLocalKnowledgeItems(root)
	...
	if len(items) == 0 {
		return KnowledgePackResult{}, fault.Invalid("KNOWLEDGE_EMPTY", "没有可打包的知识对象")
	}
	...
	contentHash, err := stablehash.Sum(items)
	if err != nil {
		return KnowledgePackResult{}, err
	}
	...
	if !localSourceIDPattern.MatchString(packID) {
		return KnowledgePackResult{}, fault.Invalid("KNOWLEDGE_PACK_ID_INVALID", "知识包 ID 无效")
	}
	now := localNow(options.Now)
...
func validateKnowledgeCandidates(pkg sourcedomain.KnowledgeExtractionPackage) error {
	if pkg.SchemaVersion != "1.0" || len(pkg.Candidates) == 0 || len(pkg.Candidates) > 100 || pkg.Warnings == nil {
		return fault.Invalid("KNOWLEDGE_CANDIDATES_SCHEMA_INVALID", "schema_version 必须为 1.0，candidates 数量必须为 1 到 100")
	}
	if len(pkg.Warnings) > 100 {
		return fault.Invalid("KNOWLEDGE_CANDIDATES_WARNINGS_INVALID", "warnings 不能超过 100 条")
	}
	for index, candidate := range pkg.Candidates {
		if !validKnowledgeKind(candidate.Kind) || strings.TrimSpace(candidate.Title) == "" || strings.TrimSpace(candidate.Statement) == "" || strings.TrimSpace(candidate.Subject) == "" || strings.TrimSpace(candidate.Predicate) == "" {
			return fault.Invalid("KNOWLEDGE_CANDIDATE_INVALID", fmt.Sprintf("第 %d 个知识候选的类型、标题、陈述、主体或谓词无效", index+1))
		}
		if candidate.RiskLevel != "low" && candidate.RiskLevel != "medium" && candidate.RiskLevel != "high" {
			return fault.Invalid("KNOWLEDGE_RISK_INVALID", fmt.Sprintf("第 %d 个知识候选的风险级别（risk_level）无效", index+1))
		}
		if candidate.Value.Type != "text" && candidate.Value.Type != "number" && candidate.Value.Type != "boolean" && candidate.Value.Type != "date" && candidate.Value.Type != "enum" {
			return fault.Invalid("KNOWLEDGE_VALUE_INVALID", fmt.Sprintf("第 %d 个知识候选的 value.type 无效", index+1))
		}
		if len(candidate.Evidence) == 0 {
			return fault.Invalid("KNOWLEDGE_EVIDENCE_REQUIRED", fmt.Sprintf("第 %d 个知识候选必须包含证据（evidence）", index+1))
		}
		if !allUnique(candidate.AllowedChannels) || !allUnique(candidate.ForbiddenExtensions) || !allUnique(candidate.DependsOnFactIDs) {
			return fault.Invalid("KNOWLEDGE_ARRAY_DUPLICATE", fmt.Sprintf("第 %d 个知识候选的数组字段不能包含重复项", index+1))
		}
		if candidate.AllowedChannels == nil || candidate.ForbiddenExtensions == nil || candidate.DependsOnFactIDs == nil || candidate.Scope.Regions == nil || candidate.Scope.Channels == nil || candidate.Scope.Audiences == nil || candidate.Scope.ProductVariants == nil {
			return fault.Invalid("KNOWLEDGE_ARRAY_REQUIRED", fmt.Sprintf("第 %d 个知识候选必须明确返回所有数组字段", index+1))
		}
		if (candidate.Value.Type == "number" && candidate.Value.Number == nil) ||
			...
			return fault.Invalid("KNOWLEDGE_VALUE_REQUIRED", fmt.Sprintf("第 %d 个知识候选的值与类型不匹配", index+1))
		}
		...
		for _, ref := range candidate.Evidence {
			...
			if evidenceKeys[key] {
				return fault.Invalid("KNOWLEDGE_EVIDENCE_DUPLICATE", fmt.Sprintf("第 %d 个知识候选的证据不能重复", index+1))
			}
...
func loadEvidenceIndex(root string) (map[string][]LocalEvidence, error) {
	...
	for _, source := range sources {
		...
		if bundle.SourceID != source.ID || bundle.SourceSHA256 != source.SHA256 {
			return nil, fault.Conflict("LOCAL_EVIDENCE_SOURCE_MISMATCH", "证据包（EvidenceBundle）与来源目录（SourceRegistry）不一致："+source.ID)
		}
...
func validateKnowledgeEvidence(item LocalKnowledgeItem, index map[string][]LocalEvidence) error {
	...
	if len(item.EvidenceIDs) == 0 {
		return fault.Invalid("KNOWLEDGE_EVIDENCE_REQUIRED", "事实陈述和主张必须引用至少一条证据")
	}
	...
	for _, id := range item.EvidenceIDs {
		span, ok := known[id]
		if !ok {
			return fault.Invalid("KNOWLEDGE_EVIDENCE_REF_MISSING", "证据引用（evidence_refs）不存在："+id)
		}
		if span.ReviewStatus != "accepted" {
			return fault.Policy("KNOWLEDGE_EVIDENCE_REVIEW_REQUIRED", "证据尚未通过本地人工复核："+id, "先复核 OCR/视觉证据，再生成候选")
		}
	}
	...
	if strings.Join(uniqueStrings(matched), "\x00") != strings.Join(uniqueStrings(item.EvidenceIDs), "\x00") {
		return fault.Invalid("KNOWLEDGE_EVIDENCE_ID_MISMATCH", "证据内容与证据引用（evidence_refs）不是同一组证据")
	}
	return nil
}

func matchCandidateEvidence(refs []sourcedomain.EvidenceRef, index map[string][]LocalEvidence) ([]string, error) {
	matched := make([]string, 0, len(refs))
	for _, ref := range refs {
		spans := index[ref.SourceRevisionID]
		if len(spans) == 0 {
			return nil, fault.Invalid("KNOWLEDGE_SOURCE_EVIDENCE_MISSING", "来源尚未导入，或 source_revision_id 不是本地不可变来源 ID："+ref.SourceRevisionID)
		}
		locator, err := canonicalLocatorString(ref.Locator)
		if err != nil {
			return nil, fault.Invalid("KNOWLEDGE_LOCATOR_INVALID", "证据定位信息（evidence.locator）必须是 JSON 对象字符串")
		}
		found := false
		for _, span := range spans {
			...
			if span.LocatorKind == ref.LocatorKind && canonicalSpan == locator && span.Quote == ref.Quote {
				if span.ReviewStatus != "accepted" {
					return nil, fault.Policy("KNOWLEDGE_EVIDENCE_REVIEW_REQUIRED", "证据尚未通过本地人工复核："+span.ID, "先复核 OCR/视觉证据，再生成候选")
				}
		...
		if !found {
			return nil, fault.Invalid("KNOWLEDGE_EVIDENCE_NOT_EXACT", "候选证据未与证据包中的定位信息和引文精确匹配")
		}
	}
	return matched, nil
}
...
func knowledgeItemFromCandidate(candidate sourcedomain.KnowledgeCandidate, evidenceIDs []string, runID string, now time.Time) LocalKnowledgeItem {
	hashInput := struct {
		Kind      string                      `json:"kind"`
		Title     string                      `json:"title"`
		Statement string                      `json:"statement"`
...
func latestKnowledgeSnapshot(root string) (reviewdomain.ApprovedSnapshot, []LocalKnowledgeItem, bool, error) {
	files, err := filepath.Glob(filepath.Join(root, ".contentcloud", "cache", "approved", "*", "snapshot.json"))
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, nil, false, err
	}
	var latest reviewdomain.ApprovedSnapshot
	found := false
	for _, path := range files {
	...
	if err := json.Unmarshal(latest.CanonicalContent, &canonical); err != nil {
		return latest, nil, false, fault.Invalid("APPROVED_SNAPSHOT_CONTENT_INVALID", "批准快照中的规范内容（canonical_content）无效")
	}
	var raws []json.RawMessage
	if err := json.Unmarshal(canonical.Objects, &raws); err != nil {
		return latest, nil, false, fault.Invalid("APPROVED_SNAPSHOT_OBJECTS_INVALID", "批准快照中的对象列表无效")
	}
...
func knowledgeSourceDisclosures(root string, items []LocalKnowledgeItem) ([]reviewdomain.SourceDisclosure, error) {
	sources, err := LocalSources(root)
	if err != nil {
		return nil, err
	}
	...
	result := []reviewdomain.SourceDisclosure{}
	for _, source := range sources {
		if !needed[source.ID] {
			continue
		}
		...
		result = append(result, reviewdomain.SourceDisclosure{SourceRef: source.ID, Level: "evidence_pack", SHA256: source.SHA256, ByteSize: source.ByteSize, EvidencePack: evidencePack})
	}
...
func normalizeKnowledgeScope(scope sourcedomain.KnowledgeScope) sourcedomain.KnowledgeScope {
...
func ResolveWorkspaceFile(root, value string) (string, error) {
	if strings.TrimSpace(value) == "" {
		return "", fault.Invalid("LOCAL_FILE_REQUIRED", "必须指定工作区内文件")
	}
	...
	if err != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
		return "", fault.Policy("LOCAL_FILE_OUTSIDE_WORKSPACE", "文件必须位于当前工作区", "将智能体输出写入工作区后再导入")
	}
