...
func TestWeChatArticleGoldenJourney(t *testing.T) {
	...
	if created.Batch.ContentKind != identitydomain.ContentTypeWeChatArticle || created.Batch.ContentSchemaRef != ArticleSchema || !containsString(created.Batch.DeliveryProfiles, "wechat_html") {
...
func TestArticleAssertionAndRevisionGates(t *testing.T) {
	batch := ContentBatch{ID: "article-batch", ContentKind: identitydomain.ContentTypeWeChatArticle, ContentSchemaRef: ArticleSchema, ProjectID: "project-1", BriefRef: "article-brief", ContextSnapshotID: "context-1"}
...
func articleKnowledge(id, kind, statement string) LocalKnowledgeItem {
	return LocalKnowledgeItem{
		ID: id, Kind: kind, Status: "candidate", Title: statement, Statement: statement, Subject: "照片", Predicate: "能力", Value: sourcedomain.TypedValue{Type: "text", Text: statement},
		Scope: sourcedomain.KnowledgeScope{Regions: []string{}, Channels: []string{WeChatChannel}, Audiences: []string{}, ProductVariants: []string{}}, RiskLevel: "low", AllowedChannels: []string{WeChatChannel},
		Evidence: []sourcedomain.EvidenceRef{}, EvidenceIDs: []string{}, ForbiddenExtensions: []string{}, DependsOnFactIDs: []string{}, Dimensions: []string{"category"}, Layers: []string{"product"},
	}
}
...
func storeApprovedObjects(t *testing.T, root, snapshotID, submissionType string, objects []any, eligibleIDs []string, createdAt time.Time) {
	...
	canonical, err := json.Marshal(map[string]any{"schema_version": reviewdomain.SubmissionSchemaVersion(submissionType), "submission_type": submissionType, "objects": json.RawMessage(objectBody)})
	if err != nil {
		t.Fatal(err)
	}
	snapshot := reviewdomain.ApprovedSnapshot{ID: snapshotID, SubmissionType: submissionType, CanonicalContent: canonical, EligibleIDs: eligibleIDs, CreatedAt: createdAt}
