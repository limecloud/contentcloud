...
func TestConversationContextReadsPersistedOfflineState(t *testing.T) {
	...
	snapshot := reviewdomain.ApprovedSnapshot{ID: "snapshot-1", SubmissionType: "knowledge", SchemaVersion: "contentcloud.knowledge/2.0", CanonicalContent: canonical, EligibleIDs: []string{"fact-1"}, CreatedAt: now.Add(2 * time.Minute)}
...
func TestConversationContextExposesOneBusinessNextStep(t *testing.T) {
	...
	if _, err := Initialize(InitOptions{Root: root, ProjectID: "project-1", Target: "codex-plugin", CLIVersion: "0.25.0", Now: now}); err != nil {
		t.Fatal(err)
	}
