...
func (m *Manager) Status(root string) (Status, error) {
	...
	m.mu.Unlock()
	if session == nil {
		return Status{}, fault.NotFound("本地 Workbench 会话")
	}
...
func (s *Session) IssueHandoff(now time.Time) (string, error) {
	...
	if !now.Before(s.expiresAt) {
		return "", fault.Conflict("WORKBENCH_SESSION_EXPIRED", "本地 Workbench 会话已到期")
	}
...
func (s *Session) exchange(response http.ResponseWriter, request *http.Request) {
	if !s.validWriteOrigin(request) {
		writeHTTPError(response, fault.Policy("WORKBENCH_ORIGIN_INVALID", "Workbench 请求来源无效", "重新从 MCP 打开本地 Workbench"), http.StatusForbidden)
		return
	}
	if !isJSONRequest(request) {
		writeHTTPError(response, fault.Invalid("WORKBENCH_CONTENT_TYPE_INVALID", "Workbench 写入请求必须使用 application/json"), http.StatusUnsupportedMediaType)
		return
	}
	var input struct {
	...
	if err := decodeJSONBody(request, &input, 4096); err != nil || strings.TrimSpace(input.Token) == "" {
		writeHTTPError(response, fault.Invalid("WORKBENCH_HANDOFF_INVALID", "Workbench handoff token 无效"), http.StatusBadRequest)
		return
	}
	now := s.now()
	s.mu.Lock()
	...
	if !ok || !now.Before(handoff.ExpiresAt) {
		writeHTTPError(response, fault.Conflict("WORKBENCH_HANDOFF_EXPIRED", "Workbench handoff 已失效或已使用"), http.StatusGone)
		return
	}
...
func (s *Session) bootstrap(response http.ResponseWriter, request *http.Request) {
	...
	if !ok {
		writeHTTPError(response, fault.Policy("WORKBENCH_CAPABILITY_INVALID", "Workbench capability 无效或已过期", "从 MCP 重新打开本地 Workbench"), http.StatusUnauthorized)
		return
	}
	options := client.View
...
func (s *Session) viewHTTP(response http.ResponseWriter, request *http.Request) {
	...
	for key := range query {
		...
		default:
			writeHTTPError(response, fault.Invalid("WORKBENCH_VIEW_PARAMS_INVALID", "Workbench view 参数包含未知字段"), http.StatusBadRequest)
			return
		}
	}
	revision, err := strconv.ParseUint(defaultString(query.Get("expected_context_revision"), "0"), 10, 64)
	if err != nil {
		writeHTTPError(response, fault.Invalid("WORKBENCH_VIEW_PARAMS_INVALID", "expected_context_revision 必须是整数"), http.StatusBadRequest)
		return
	}
...
func (s *Session) resourceHTTP(response http.ResponseWriter, request *http.Request) {
	...
	s.mu.Unlock()
	if uri == "" {
		writeHTTPError(response, fault.NotFound("Workbench 资源"), http.StatusNotFound)
		return
	}
...
func (s *Session) closeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSONBody(request, &input, 1024); err != nil {
		writeHTTPError(response, fault.Invalid("WORKBENCH_REQUEST_INVALID", "关闭 Workbench 的请求体无效"), http.StatusBadRequest)
		return
	}
	http.SetCookie(response, &http.Cookie{
...
func (s *Session) claimOwnershipHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSONBody(request, &input, 16*1024); err != nil {
		writeHTTPError(response, fault.Invalid("WORKBENCH_OWNERSHIP_PARAMS_INVALID", "Workbench ownership claim 参数无效或包含未知字段"), http.StatusBadRequest)
		return
	}
...
func (s *Session) takeoverOwnershipHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSONBody(request, &input, 16*1024); err != nil {
		writeHTTPError(response, fault.Invalid("WORKBENCH_OWNERSHIP_PARAMS_INVALID", "Workbench ownership takeover 参数无效或包含未知字段"), http.StatusBadRequest)
		return
	}
...
func (s *Session) prepareProposalHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSONBody(request, &input, workspaceRequestBodyLimit); err != nil {
		writeHTTPError(response, fault.Invalid("WORKBENCH_PROPOSAL_PARAMS_INVALID", "Workbench Proposal 参数无效、过大或包含未知字段"), http.StatusBadRequest)
		return
	}
...
func (s *Session) applyProposalHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err := decodeJSONBody(request, &input, 16*1024); err != nil || !input.Confirm {
		writeHTTPError(response, fault.Invalid("WORKBENCH_PROPOSAL_APPLY_INVALID", "Apply 必须准确确认 Proposal 且参数不能包含未知字段"), http.StatusBadRequest)
		return
	}
...
func (s *Session) prepareIdempotentRequest(request *http.Request, operation string, input any) (string, string, any, error) {
	...
	if record.Operation != operation || record.Fingerprint != fingerprint {
		return "", "", nil, fault.Conflict("WORKBENCH_IDEMPOTENCY_CONFLICT", "Idempotency-Key 已用于不同的 Workbench 操作或参数")
	}
...
func (s *Session) withCapability(write bool, next http.HandlerFunc) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		if !originValid {
			writeHTTPError(response, fault.Policy("WORKBENCH_ORIGIN_INVALID", "Workbench 请求来源无效", "重新打开本地 Workbench"), http.StatusForbidden)
			return
		}
		value := strings.TrimSpace(strings.TrimPrefix(request.Header.Get("Authorization"), "Bearer "))
		client, ok := s.lookupClientCapability(value)
		...
		if write && subtle.ConstantTimeCompare([]byte(client.CSRF), []byte(request.Header.Get("X-Workbench-CSRF"))) != 1 {
			writeHTTPError(response, fault.Policy("WORKBENCH_CSRF_INVALID", "Workbench 写入请求缺少有效 CSRF nonce", "刷新工作台后重试"), http.StatusForbidden)
			return
		}
...
func (s *Session) withResourceCapability(next http.HandlerFunc) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		if !bearerOK && !resourceOK {
			writeHTTPError(response, fault.Policy("WORKBENCH_CAPABILITY_INVALID", "Workbench 资源 capability 无效或已过期", "从 MCP 重新打开本地 Workbench"), http.StatusUnauthorized)
			return
		}
		next(response, request)
	}
...
func writeHTTPError(response http.ResponseWriter, err error, status int) {
	...
	if !errors.As(err, &domainError) {
		domainError = fault.E("internal", "workbench", "WORKBENCH_INTERNAL", err.Error(), 1)
	}
...
func requestIdempotencyKey(request *http.Request) (string, error) {
	...
	if len(key) < 8 || len(key) > 128 {
		return "", fault.Invalid("WORKBENCH_IDEMPOTENCY_KEY_INVALID", "Workbench 写入请求需要 8 到 128 个字符的 Idempotency-Key")
	}
	return key, nil
}
