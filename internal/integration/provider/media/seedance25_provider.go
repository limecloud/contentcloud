...
type Seedance25InputResolver interface {
	Resolve(context.Context, Request, deliverydomain.ProviderProfile) (Seedance25Input, error)
}

type Seedance25InputResolverFunc func(context.Context, Request, deliverydomain.ProviderProfile) (Seedance25Input, error)

func (f Seedance25InputResolverFunc) Resolve(ctx context.Context, request Request, profile deliverydomain.ProviderProfile) (Seedance25Input, error) {
	if f == nil {
		return Seedance25Input{}, fault.Policy("SEEDANCE_INPUT_RESOLVER_REQUIRED", "Seedance 2.5 缺少受控输入解析器", "配置由 ContentCloud 控制面的 Artifact 解析器")
	}
...
func NewSeedance25Provider(config Seedance25ProviderConfig) (*Seedance25Provider, error) {
	...
	if config.Resolver == nil {
		return nil, fault.Policy("SEEDANCE_INPUT_RESOLVER_REQUIRED", "Seedance 2.5 缺少受控输入解析器", "配置由 ContentCloud 控制面的 Artifact 解析器")
	}
...
func (p *Seedance25Provider) Validate(request Request, profile deliverydomain.ProviderProfile) error {
	if p == nil || p.http == nil || p.resolver == nil {
		return fault.Policy("PROVIDER_ADAPTER_UNAVAILABLE", "Seedance 2.5 Provider 尚未配置完成", "配置 HTTPS 地址、密钥和 Artifact 输入解析器")
	}
	if strings.TrimSpace(request.JobID) == "" || strings.TrimSpace(request.IdempotencyKey) == "" || strings.TrimSpace(request.StoryboardSnapshotID) == "" || strings.TrimSpace(request.PromptPackageArtifactID) == "" || request.DurationSeconds < 1 {
		return fault.Invalid("PROVIDER_REQUEST_INVALID", "Seedance 2.5 请求缺少任务、幂等键、快照、提示包或时长")
	}
	if request.DurationSeconds > 30 {
		return fault.Invalid("PROVIDER_DURATION_LIMIT_EXCEEDED", "Seedance 2.5 第一阶段最大时长为 30 秒")
	}
	if request.Mode != "text_to_video" && request.Mode != "image_to_video" {
		return fault.Invalid("PROVIDER_MODE_UNSUPPORTED", "Seedance 2.5 第一阶段只支持 text_to_video 和 image_to_video")
	}
	if !contains(profile.Modes, request.Mode) {
		return fault.Invalid("PROVIDER_MODE_UNSUPPORTED", "Provider Profile 不支持当前 Seedance 2.5 模式")
	}
	if maxDuration := integerLimit(profile.Limits, "max_duration_seconds"); maxDuration > 0 && request.DurationSeconds > maxDuration {
		return fault.Invalid("PROVIDER_DURATION_LIMIT_EXCEEDED", "请求时长超过 Seedance 2.5 Profile 上限")
	}
	return nil
}

func (p *Seedance25Provider) Estimate(request Request, profile deliverydomain.ProviderProfile) (Estimate, error) {
	...
	if perJob < 0 || perSecond < 0 || minimum < 0 || (perJob == 0 && perSecond == 0 && minimum == 0) {
		return Estimate{}, fault.Policy("PROVIDER_PRICING_UNAVAILABLE", "Seedance 2.5 Profile 没有经核验的费用估算", "补充按任务或按秒计费的 Profile 价格")
	}
	...
	if maximum := pricingInt64(profile.Pricing, "max_estimated_minor"); maximum > 0 && cost > maximum {
		return Estimate{}, fault.Policy("PROVIDER_PRICING_INVALID", "Seedance 2.5 估算费用超过 Profile 声明的保守上限", "重新核验服务商价格或缩短视频时长")
	}
	return Estimate{CostMinor: cost, Currency: providerCurrency(profile.Pricing)}, nil
}

func (p *Seedance25Provider) Submit(ctx context.Context, request Request, profile deliverydomain.ProviderProfile) (Submission, error) {
	if err := p.Validate(request, profile); err != nil {
	...
	if len(content) == 0 {
		return Submission{}, fault.Invalid("SEEDANCE_INPUT_EMPTY", "Seedance 2.5 请求没有可提交的提示词或图片")
	}
	model := firstNonEmpty(profile.Model, p.model)
	if model == "" {
		return Submission{}, fault.Invalid("PROVIDER_MODEL_REQUIRED", "Seedance 2.5 请求缺少模型 ID")
	}
	body := map[string]any{
		"model":    model,
		"content":  content,
	...
	if externalID == "" {
		return Submission{}, fault.Invalid("PROVIDER_RESPONSE_INVALID", "ModelArk Seedance 响应缺少任务 ID")
	}
	return Submission{ExternalJobID: externalID, ProviderRequestID: providerRequestID, HTTPStatus: statusCode}, nil
}

func (p *Seedance25Provider) Status(ctx context.Context, externalJobID string, _ deliverydomain.ProviderProfile) (Status, error) {
	...
	if state == "" {
		return Status{}, fault.Invalid("PROVIDER_RESPONSE_INVALID", "ModelArk Seedance 状态响应缺少任务状态")
	}
	switch state {
	...
	case "expired":
		state = "failed"
	default:
		return Status{}, &fault.Error{Type: "provider", Subtype: "status", Code: "PROVIDER_STATUS_UNKNOWN", Message: "ModelArk Seedance 返回了未知任务状态", Retryable: true}
	}
	...
	if (state == "succeeded" || state == "completed") && outputRef == "" {
		return Status{}, &fault.Error{Type: "provider", Subtype: "status", Code: "PROVIDER_OUTPUT_PENDING", Message: "ModelArk Seedance 已成功但输出 URL 尚不可用", Retryable: true}
	}
	progress := 0
...
func (p *Seedance25Provider) Cancel(ctx context.Context, externalJobID string, _ deliverydomain.ProviderProfile) error {
	if err := validateExternalJobID(externalJobID); err != nil {
		return err
	}
...
func (p *Seedance25Provider) Download(ctx context.Context, outputRef string, profile deliverydomain.ProviderProfile) (Download, error) {
	return p.http.Download(ctx, outputRef, profile)
}

func (p *Seedance25Provider) OpenDownload(ctx context.Context, outputRef string, profile deliverydomain.ProviderProfile) (StreamedDownload, error) {
	return p.http.OpenDownload(ctx, outputRef, profile)
}

func validateSeedanceInput(request Request, input Seedance25Input, profile deliverydomain.ProviderProfile) error {
	if utf8.RuneCountInString(input.Prompt) > 32000 {
		return fault.Invalid("SEEDANCE_PROMPT_TOO_LARGE", "Seedance 单镜头提示词超过 32000 字符限制")
	}
	if request.Mode == "text_to_video" && strings.TrimSpace(input.Prompt) == "" {
		return fault.Invalid("SEEDANCE_PROMPT_REQUIRED", "text_to_video 必须提供非空提示词")
	}
	if request.Mode == "image_to_video" && len(input.Images) == 0 {
		return fault.Invalid("SEEDANCE_IMAGE_REQUIRED", "image_to_video 必须提供至少一张图片")
	}
	if max := integerLimit(profile.Limits, "max_reference_images"); max > 0 && len(input.Images) > max {
		return fault.Invalid("PROVIDER_REFERENCE_LIMIT_EXCEEDED", "图片引用数量超过 Seedance 2.5 Profile 上限")
	}
	if len(input.Images) > 30 {
		return fault.Invalid("PROVIDER_REFERENCE_LIMIT_EXCEEDED", "Seedance 2.5 第一阶段最多接受 30 张图片")
	}
	if len(input.Videos) > 0 || len(input.Audios) > 0 {
		return fault.Invalid("PROVIDER_MODE_UNSUPPORTED", "Seedance 2.5 第一阶段暂不接受视频或音频引用")
	}
	for _, media := range input.Images {
		if media.MediaType != "image/jpeg" && media.MediaType != "image/png" && media.MediaType != "image/webp" {
			return fault.Invalid("SEEDANCE_IMAGE_MIME_INVALID", "Seedance 图片引用必须是 JPEG、PNG 或 WebP")
		}
		...
		if strings.HasPrefix(strings.TrimSpace(media.URL), "data:") {
			...
			if mediaType != strings.ToLower(strings.TrimSpace(media.MediaType)) {
				return fault.Invalid("SEEDANCE_IMAGE_MIME_INVALID", "Seedance 图片 data URL 媒体类型与 Artifact 不一致")
			}
		}
	}
	return nil
...
func validateSeedanceURL(raw string, image bool) error {
	...
	if err != nil || raw == "" || strings.ContainsAny(raw, "\r\n") {
		return fault.Invalid("SEEDANCE_INPUT_URL_INVALID", "Seedance 输入引用不是有效 URL")
	}
	if parsed.Scheme == "data" {
		if !image || !strings.Contains(raw, ";base64,") {
			return fault.Policy("SEEDANCE_INPUT_URL_BLOCKED", "Seedance 只允许图片 data URL，且必须使用 base64", "使用受控 HTTPS 图片 URL")
		}
		...
		if len(parts) == 0 || (parts[0] != "image/jpeg" && parts[0] != "image/png" && parts[0] != "image/webp") || encoded == "" {
			return fault.Invalid("SEEDANCE_INPUT_URL_INVALID", "Seedance 图片 data URL 的媒体类型或内容无效")
		}
		decoded, err := base64.StdEncoding.DecodeString(encoded)
		if err != nil {
			return fault.Invalid("SEEDANCE_INPUT_URL_INVALID", "Seedance 图片 data URL 不是有效 Base64")
		}
		if len(decoded) == 0 || len(decoded) > seedance25MaxDataImageBytes {
			return fault.Invalid("SEEDANCE_INPUT_SIZE_INVALID", "Seedance 图片 data URL 为空或超过 8 MB 限制")
		}
		return nil
	}
	if parsed.Scheme != "https" || parsed.Host == "" || parsed.User != nil {
		return fault.Policy("SEEDANCE_INPUT_URL_BLOCKED", "Seedance 输入只允许无凭据 HTTPS URL", "使用受控 HTTPS 图片 URL")
	}
	return nil
}
...
func (p *Seedance25Provider) validateSeedanceInputHosts(input Seedance25Input) error {
	if p == nil || p.http == nil {
		return fault.Policy("PROVIDER_ADAPTER_UNAVAILABLE", "Seedance 2.5 Provider 尚未配置完成", "配置服务商 HTTP 适配器")
	}
	for _, media := range input.Images {
		...
		if len(p.http.allowedHosts) == 0 {
			return fault.Policy("SEEDANCE_INPUT_HOST_NOT_ALLOWED", "Seedance 图片 HTTPS 地址未配置域名白名单", "使用 data:image Base64 或配置受控图片域名")
		}
		if _, ok := p.http.allowedHosts[strings.ToLower(parsed.Hostname())]; !ok {
			return fault.Policy("SEEDANCE_INPUT_HOST_NOT_ALLOWED", "Seedance 图片 HTTPS 地址不在域名白名单中", "使用已核验的图片下载域名")
		}
	}
	return nil
}
...
func validateExternalJobID(value string) error {
	...
	if value == "" || strings.ContainsAny(value, "/?#") {
		return fault.Invalid("PROVIDER_EXTERNAL_ID_INVALID", "服务商外部任务标识无效")
	}
	return nil
}
