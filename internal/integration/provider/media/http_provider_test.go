...
func TestHTTPProviderSubmitStatusCancelAndStreamingDownload(t *testing.T) {
	...
	profile := deliverydomain.ProviderProfile{Modes: []string{"image_to_video"}, Limits: map[string]any{"max_duration_seconds": 60}, Pricing: map[string]any{"currency": "CNY", "per_job_minor": 10}}
...
func TestHTTPProviderRejectsSSRFAndOversizedDownload(t *testing.T) {
	...
	if _, err := provider.OpenDownload(t.Context(), "/take.mp4", deliverydomain.ProviderProfile{}); !containsDomainCode(err, "MEDIA_OUTPUT_SIZE_INVALID") {
...
func TestHTTPProviderRetryableHTTPErrorIsSafe(t *testing.T) {
	...
	_, err = provider.Submit(t.Context(), Request{JobID: "j", IdempotencyKey: "i", StoryboardSnapshotID: "s", DurationSeconds: 1}, deliverydomain.ProviderProfile{Modes: []string{""}})
	if err == nil {
