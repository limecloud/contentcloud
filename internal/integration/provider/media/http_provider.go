...
func NewHTTPProvider(config HTTPProviderConfig) (*HTTPProvider, error) {
	...
	if len(allowed) > 0 {
		if _, ok := allowed[strings.ToLower(base.Hostname())]; !ok {
			return nil, fault.Policy("PROVIDER_HOST_NOT_ALLOWED", "服务商基础地址不在出站允许列表中", "把服务商主机加入显式允许列表")
		}
	}
	...
	if !config.AllowPrivateNetworks {
		...
		if ok {
			...
			clone.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
				...
				return nil, fault.Policy("PROVIDER_SSRF_BLOCKED", "服务商主机名在连接时解析到了私有或本机网络", "使用公开 HTTPS 服务商地址并配置 DNS 防护")
			}
	...
	client.CheckRedirect = func(req *http.Request, _ []*http.Request) error {
		...
		if redirectHost != strings.ToLower(base.Hostname()) {
			if _, allowedRedirect := allowed[redirectHost]; !allowedRedirect {
				return fault.Policy("PROVIDER_REDIRECT_BLOCKED", "服务商响应重定向到未授权主机", "配置同一服务商的显式下载域名")
			}
		}
		if req.URL.Hostname() == "" {
			return fault.Policy("PROVIDER_REDIRECT_BLOCKED", "服务商响应重定向到未授权主机", "配置同一服务商的显式下载域名")
...
func (p *HTTPProvider) Validate(request Request, profile deliverydomain.ProviderProfile) error {
	if p == nil || p.baseURL == nil {
		return fault.Policy("PROVIDER_ADAPTER_UNAVAILABLE", "服务商 HTTP 适配器尚未配置", "配置服务商的 HTTPS 基础地址")
	}
	if strings.TrimSpace(request.JobID) == "" || strings.TrimSpace(request.IdempotencyKey) == "" || strings.TrimSpace(request.StoryboardSnapshotID) == "" || request.DurationSeconds < 1 {
		return fault.Invalid("PROVIDER_REQUEST_INVALID", "服务商请求缺少任务、幂等键、分镜或时长")
	}
	if !contains(profile.Modes, request.Mode) {
		return fault.Invalid("PROVIDER_MODE_UNSUPPORTED", "服务商配置不支持当前生成模式")
	}
	if maxDuration := integerLimit(profile.Limits, "max_duration_seconds"); maxDuration > 0 && request.DurationSeconds > maxDuration {
		return fault.Invalid("PROVIDER_DURATION_LIMIT_EXCEEDED", "请求时长超过服务商配置上限")
	}
	return nil
}

func (p *HTTPProvider) Estimate(_ Request, profile deliverydomain.ProviderProfile) (Estimate, error) {
	return Estimate{CostMinor: int64(integerLimit(profile.Pricing, "per_job_minor")), Currency: providerCurrency(profile.Pricing)}, nil
}

func (p *HTTPProvider) Submit(ctx context.Context, request Request, _ deliverydomain.ProviderProfile) (Submission, error) {
	var result struct {
	...
	if externalID == "" {
		return Submission{}, fault.Invalid("PROVIDER_RESPONSE_INVALID", "服务商提交响应缺少外部任务标识")
	}
	return Submission{ExternalJobID: externalID, ProviderRequestID: requestID, HTTPStatus: statusCode}, nil
}

func (p *HTTPProvider) Status(ctx context.Context, externalJobID string, _ deliverydomain.ProviderProfile) (Status, error) {
	externalJobID = strings.TrimSpace(externalJobID)
	if externalJobID == "" || strings.ContainsAny(externalJobID, "/?#") {
		return Status{}, fault.Invalid("PROVIDER_EXTERNAL_ID_INVALID", "服务商外部任务标识无效")
	}
	var result struct {
		State            string `json:"state"`
	...
	if state == "" {
		return Status{}, fault.Invalid("PROVIDER_RESPONSE_INVALID", "服务商状态响应缺少任务状态")
	}
	if result.Progress < 0 || result.Progress > 100 {
		return Status{}, fault.Invalid("PROVIDER_RESPONSE_INVALID", "服务商状态响应的进度无效")
	}
...
func (p *HTTPProvider) Cancel(ctx context.Context, externalJobID string, _ deliverydomain.ProviderProfile) error {
	externalJobID = strings.TrimSpace(externalJobID)
	if externalJobID == "" || strings.ContainsAny(externalJobID, "/?#") {
...
type StreamingDownloader interface {
	OpenDownload(context.Context, string, deliverydomain.ProviderProfile) (StreamedDownload, error)
}

func (p *HTTPProvider) OpenDownload(ctx context.Context, outputRef string, _ deliverydomain.ProviderProfile) (StreamedDownload, error) {
	...
	if resp.ContentLength > p.maxDownloadBytes {
		_ = resp.Body.Close()
		return StreamedDownload{}, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出超过配置的媒体大小上限")
	}
	mediaType := strings.ToLower(strings.TrimSpace(strings.Split(resp.Header.Get("Content-Type"), ";")[0]))
	if mediaType != "video/mp4" {
		_ = resp.Body.Close()
		return StreamedDownload{}, fault.Invalid("MEDIA_OUTPUT_MIME_INVALID", "服务商输出 MIME 不是受支持的 MP4")
	}
...
func (p *HTTPProvider) Download(ctx context.Context, outputRef string, profile deliverydomain.ProviderProfile) (Download, error) {
	...
	if int64(len(body)) > p.maxDownloadBytes {
		return Download{}, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出超过配置的媒体大小上限")
	}
...
func (p *HTTPProvider) doJSONWithMetadata(ctx context.Context, method, endpoint, idempotency string, payload any, target any) (int, string, error) {
	...
	if int64(len(responseBody)) > p.maxResponseBytes {
		return resp.StatusCode, requestID, fault.Invalid("PROVIDER_RESPONSE_TOO_LARGE", "服务商 JSON 响应超过大小上限")
	}
	if err := json.Unmarshal(responseBody, target); err != nil {
		return resp.StatusCode, requestID, fault.Invalid("PROVIDER_RESPONSE_INVALID", "服务商响应不是有效 JSON")
	}
	return resp.StatusCode, requestID, nil
}
...
func (p *HTTPProvider) do(ctx context.Context, req *http.Request, body string) (*http.Response, error) {
	if p == nil || p.client == nil {
		return nil, fault.Policy("PROVIDER_ADAPTER_UNAVAILABLE", "服务商 HTTP 适配器尚未配置", "检查 Provider 运行时配置")
	}
	...
	if err != nil {
		if callCtx.Err() != nil {
			return nil, &fault.Error{Type: "provider", Subtype: "timeout", Code: "PROVIDER_TIMEOUT", Message: "服务商请求超时", Retryable: true, Details: map[string]any{"endpoint": req.URL.Path}}
		}
		return nil, &fault.Error{Type: "provider", Subtype: "network", Code: "PROVIDER_NETWORK_ERROR", Message: "服务商网络请求失败", Retryable: true}
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, &fault.Error{Type: "provider", Subtype: "http", Code: "PROVIDER_HTTP_ERROR", Message: "服务商返回非成功状态", Retryable: retryable, Details: map[string]any{"status": resp.StatusCode, "retry_after_seconds": parseRetryAfter(resp.Header.Get("Retry-After")), "response_digest": "sha256:" + hex.EncodeToString(errorDigest[:])}}
	}
	return resp, nil
}
...
func (p *HTTPProvider) resolveEndpoint(endpoint string) (*url.URL, error) {
	...
	if err != nil || joined.IsAbs() || joined.Host != "" {
		return nil, fault.Invalid("PROVIDER_ENDPOINT_INVALID", "服务商接口路径无效")
	}
	base := *p.baseURL
...
func (p *HTTPProvider) resolveOutputURL(outputRef string) (*url.URL, error) {
	...
	if err != nil || strings.TrimSpace(outputRef) == "" {
		return nil, fault.Invalid("PROVIDER_OUTPUT_REF_INVALID", "服务商输出引用无效")
	}
	if !parsed.IsAbs() {
		return p.resolveEndpoint(parsed.RequestURI())
	}
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return nil, fault.Policy("PROVIDER_OUTPUT_SCHEME_BLOCKED", "服务商输出引用使用了不允许的协议", "仅允许 HTTPS 或测试环境显式允许的 HTTP")
	}
...
func parseProviderURL(raw string) (*url.URL, error) {
	...
	if err != nil || parsed.Scheme == "" || parsed.Host == "" || parsed.User != nil || (parsed.Scheme != "https" && parsed.Scheme != "http") {
		return nil, fault.Invalid("PROVIDER_BASE_URL_INVALID", "服务商基础地址必须是无用户信息的 HTTP(S) URL")
	}
	return parsed, nil
}
...
func validateProviderURL(target *url.URL, allowed map[string]struct{}, allowPrivate bool) error {
	if target == nil || target.Hostname() == "" || target.User != nil || (target.Scheme != "http" && target.Scheme != "https") {
		return fault.Invalid("PROVIDER_URL_INVALID", "服务商请求地址无效")
	}
	host := strings.ToLower(target.Hostname())
	if len(allowed) > 0 {
		if _, ok := allowed[host]; !ok {
			return fault.Policy("PROVIDER_HOST_NOT_ALLOWED", "服务商请求目标不在出站允许列表中", "只允许访问已核验的服务商或下载域名")
		}
	}
	if allowPrivate {
		return nil
	...
	if ip := net.ParseIP(host); ip != nil {
		if isPrivateProviderIP(ip) {
			return fault.Policy("PROVIDER_SSRF_BLOCKED", "服务商请求目标属于私有或本机网络", "使用公开 HTTPS 服务商地址")
		}
		return nil
	}
	ips, err := net.LookupIP(host)
	if err != nil {
		return fault.Invalid("PROVIDER_DNS_INVALID", "服务商主机名无法解析")
	}
	for _, ip := range ips {
		if isPrivateProviderIP(ip) {
			return fault.Policy("PROVIDER_SSRF_BLOCKED", "服务商主机名解析到了私有或本机网络", "使用公开 HTTPS 服务商地址并配置 DNS 防护")
		}
	}
	return nil
}
...
func (r *limitedReadCloser) Read(p []byte) (int, error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出超过配置的媒体大小上限")
	}
	return n, err
}
