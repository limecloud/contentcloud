...
type Adapter interface {
	Validate(Request, deliverydomain.ProviderProfile) error
	Estimate(Request, deliverydomain.ProviderProfile) (Estimate, error)
	Submit(context.Context, Request, deliverydomain.ProviderProfile) (Submission, error)
	Status(context.Context, string, deliverydomain.ProviderProfile) (Status, error)
	Cancel(context.Context, string, deliverydomain.ProviderProfile) error
	Download(context.Context, string, deliverydomain.ProviderProfile) (Download, error)
}
...
func (FakeProvider) Validate(request Request, profile deliverydomain.ProviderProfile) error {
	if request.JobID == "" || request.IdempotencyKey == "" || request.StoryboardSnapshotID == "" || request.DurationSeconds < 1 {
		return fault.Invalid("FAKE_PROVIDER_REQUEST_INVALID", "模拟服务商请求缺少任务、分镜或时长")
	}
...
func (FakeProvider) Estimate(Request, deliverydomain.ProviderProfile) (Estimate, error) {
	return Estimate{CostMinor: 0, Currency: "CNY"}, nil
}

func (FakeProvider) Submit(_ context.Context, request Request, _ deliverydomain.ProviderProfile) (Submission, error) {
...
func (FakeProvider) Status(_ context.Context, externalJobID string, _ deliverydomain.ProviderProfile) (Status, error) {
	if !strings.HasPrefix(externalJobID, "fake-job-") {
		return Status{}, fault.Invalid("FAKE_PROVIDER_JOB_INVALID", "模拟服务商返回的外部任务 ID 无效")
	}
	return Status{State: "succeeded", Progress: 100, OutputRef: "fake-output:" + strings.TrimPrefix(externalJobID, "fake-job-")}, nil
}

func (FakeProvider) Cancel(context.Context, string, deliverydomain.ProviderProfile) error { return nil }

func (FakeProvider) Download(_ context.Context, outputRef string, _ deliverydomain.ProviderProfile) (Download, error) {
	if !strings.HasPrefix(outputRef, "fake-output:") {
		return Download{}, fault.Invalid("FAKE_PROVIDER_OUTPUT_INVALID", "模拟服务商返回的输出引用无效")
	}
...
func ValidateDownload(value Download, maxBytes int64) (map[string]any, error) {
	if len(value.Body) == 0 || int64(len(value.Body)) > maxBytes {
		return nil, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出为空或超过大小限制")
	}
	if value.MediaType != "video/mp4" || len(value.Body) < 32 || string(value.Body[4:8]) != "ftyp" {
		return nil, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "服务商输出不是受支持的 MP4 容器")
	}
	...
	for offset := 0; offset+8 <= len(value.Body); {
		...
		case 1:
			if offset+16 > len(value.Body) {
				return nil, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 扩展数据块头不完整")
			}
			headerSize = 16
		...
		if size < headerSize || size > uint64(len(value.Body)-offset) {
			return nil, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 数据块长度无效")
		}
		boxes[string(value.Body[offset+4:offset+8])] = true
		offset += int(size)
	}
	if !boxes["ftyp"] || !boxes["moov"] || !boxes["mdat"] {
		return nil, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 缺少必要的容器数据块")
	}
...
func ValidateDownloadReader(reader io.Reader, mediaType string, maxBytes int64) (StreamValidation, error) {
	if reader == nil || maxBytes <= 0 || strings.ToLower(strings.TrimSpace(mediaType)) != "video/mp4" {
		return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_INVALID", "服务商输出流为空、媒体类型不受支持或缺少大小上限")
	}
	...
	for {
		...
		if err != nil {
			return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 数据块头不完整")
		}
		total = hasher.count
		if total > maxBytes {
			return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出为空或超过大小限制")
		}
		size32 := binary.BigEndian.Uint32(header[:4])
		boxType := string(header[4:8])
		if len(boxes) == 0 && boxType != "ftyp" {
			return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 首个数据块不是 ftyp")
		}
		boxes[boxType] = true
		headerSize := int64(8)
		var boxSize int64
		...
		case 0:
			if _, err := io.Copy(io.Discard, hasher); err != nil {
				return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 数据块读取失败")
			}
			total = hasher.count
			break
		case 1:
			...
			if _, err := io.ReadFull(hasher, extended[:]); err != nil {
				return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 扩展数据块头不完整")
			}
			total = hasher.count
			headerSize = 16
			boxSize64 := binary.BigEndian.Uint64(extended[:])
			if boxSize64 > uint64(^uint64(0)>>1) {
				return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 数据块长度无效")
			}
		...
		if size32 != 0 {
			...
			if copyErr != nil || copied != payload {
				return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 数据块长度与响应不一致")
			}
			total = hasher.count
		}
		if size32 == 0 {
	...
	if total == 0 || total > maxBytes || !boxes["ftyp"] || !boxes["moov"] || !boxes["mdat"] {
		return StreamValidation{}, fault.Invalid("MEDIA_OUTPUT_CONTAINER_INVALID", "MP4 缺少必要的容器数据块")
	}
