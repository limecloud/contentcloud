...
func TestSeedance25ProviderSubmitStatusAndCancel(t *testing.T) {
	...
	provider, err := NewSeedance25Provider(Seedance25ProviderConfig{
		...
		Resolver: Seedance25InputResolverFunc(func(_ context.Context, _ Request, _ deliverydomain.ProviderProfile) (Seedance25Input, error) {
	...
	profile := deliverydomain.ProviderProfile{Model: "dreamina-seedance-2-5-260628", Modes: []string{"image_to_video"}, Limits: map[string]any{"max_duration_seconds": 30, "max_reference_images": 30}, Pricing: map[string]any{"currency": "CNY", "per_second_minor": 2}}
...
func TestSeedance25ProviderRejectsUnsupportedInputAndUnknownSubmit(t *testing.T) {
	...
	profile := deliverydomain.ProviderProfile{Model: "model", Modes: []string{"image_to_video"}, Pricing: map[string]any{"currency": "CNY", "per_job_minor": 1}}
...
func TestSeedance25ProviderEnforcesPhaseOneLimits(t *testing.T) {
	...
	profile := deliverydomain.ProviderProfile{Modes: []string{"text_to_video", "image_to_video"}}
...
func TestSeedance25ProviderBlocksUncontrolledHTTPSAndInvalidDataURLs(t *testing.T) {
	request := Request{JobID: "job", IdempotencyKey: "key", StoryboardSnapshotID: "snapshot", PromptPackageArtifactID: "prompt", Mode: "image_to_video", DurationSeconds: 1}
	profile := deliverydomain.ProviderProfile{Modes: []string{"image_to_video"}}
	...
	provider, err := NewSeedance25Provider(Seedance25ProviderConfig{
		...
		Resolver: Seedance25InputResolverFunc(func(context.Context, Request, deliverydomain.ProviderProfile) (Seedance25Input, error) {
...
func TestSeedance25ProviderReadsWrappedStatusAndNestedOutput(t *testing.T) {
	...
	status, err := provider.Status(t.Context(), "wrapped-task", deliverydomain.ProviderProfile{})
