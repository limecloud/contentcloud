...
func (r *Registry) Resolve(id string) (Adapter, error) {
	if r == nil {
		return nil, fault.Policy("CHANNEL_ADAPTER_UNAVAILABLE", "渠道适配器注册表未配置", "配置渠道适配器后重试")
	}
	r.mu.RLock()
	defer r.mu.RUnlock()
	adapter, ok := r.adapters[strings.ToLower(strings.TrimSpace(id))]
	if !ok {
		return nil, fault.Policy("CHANNEL_ADAPTER_UNAVAILABLE", "渠道适配器不可用", "检查渠道绑定和服务端适配器配置")
	}
	return adapter, nil
}
