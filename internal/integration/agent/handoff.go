...
func SelectHandoff(clientID, version string) (HandoffAdapter, error) {
	...
	if !ok {
		return nil, fault.Policy("AGENT_HANDOFF_NOT_IMPLEMENTED", client.DisplayName+" 的恢复入口尚未实现", "选择已实现恢复入口的客户端")
	}
...
func codexAgentHandoffContent(pluginID, pluginVersion string, request HandoffRequest) (string, []string, error) {
	if pluginID == "" || pluginVersion == "" {
		return "", nil, fault.Invalid("AGENT_HANDOFF_PLUGIN_INVALID", "Codex 交接缺少标准插件身份")
	}
	switch request.Kind {
	case "project":
	...
	default:
		return "", nil, fault.Invalid("AGENT_HANDOFF_KIND_INVALID", "不支持该智能体恢复入口类型")
	}
}
