	...
	harness.detect = func(ctx context.Context) (string, error) {
		...
		for _, required := range []string{"--config", "--json", "--output-schema", "--output-last-message", "resume"} {
			if !strings.Contains(help, required) {
				return "", fault.Policy("CODEX_CAPABILITY_UNAVAILABLE", "Codex CLI 缺少 Runtime 所需的结构化输出、会话恢复或 MCP 配置能力", "升级 Codex CLI 或切换到支持 Runtime 协议的 Harness")
			}
		}
		...
		if authErr != nil || !strings.HasPrefix(strings.ToLower(strings.TrimSpace(string(authOutput))), "logged in") {
			return "", fault.Policy("CODEX_AUTH_REQUIRED", "Codex CLI 尚未完成认证", "在本机完成 Codex 登录后等待 Daemon 重探测或重启 Daemon")
		}
...
func (h *codexExecHarness) Detect(ctx context.Context) (HarnessCapabilities, error) {
	if h == nil || h.detect == nil {
		return HarnessCapabilities{}, fault.Policy("CODEX_HARNESS_UNAVAILABLE", "Codex Harness 尚未配置", "检查 Runtime 的 Codex 执行适配器配置")
	}
...
func (h *codexExecHarness) Resume(ctx context.Context, request ResumeAgentRequest) (EventStream, error) {
	if strings.TrimSpace(request.Session.SessionID) == "" || normalizeHarnessKind(request.Session.HarnessKind) != "codex" {
		return nil, fault.Invalid("AGENT_SESSION_INVALID", "Codex Resume 缺少有效会话引用")
	}
	if request.Session.TenantID != "" && request.TenantID != "" && request.Session.TenantID != request.TenantID {
		return nil, fault.Policy("AGENT_SESSION_TENANT_MISMATCH", "Codex 会话不属于当前租户", "使用当前 RuntimeAttempt 绑定的会话引用")
	}
	dir, contract, _, skill, err := loadWorkspace(request.Workspace)
	if err != nil {
		return nil, err
	...
	if ref.SessionID != request.Session.SessionID {
		_ = stream.Close()
		return nil, fault.Conflict("CODEX_SESSION_MISMATCH", "Codex Resume 返回了不同的会话标识")
	}
	return stream, nil
}
...
func (h *codexExecHarness) Interrupt(_ context.Context, ref AgentSessionRef) error {
	...
	if !ok || (ref.TenantID != "" && session.status.Session.TenantID != ref.TenantID) {
		h.mu.Unlock()
		return fault.NotFound("Codex 会话")
	}
...
func (h *codexExecHarness) Inspect(_ context.Context, ref AgentSessionRef) (AgentSessionStatus, error) {
	...
	session, ok := h.sessions[ref.SessionID]
	if !ok || (ref.TenantID != "" && session.status.Session.TenantID != ref.TenantID) {
		return AgentSessionStatus{}, fault.NotFound("Codex 会话")
	}
...
func (h *codexExecHarness) launch(ctx context.Context, tenantID, dir, prompt, resumeSessionID string, gateway RuntimeGatewayConfig) (AgentSessionRef, EventStream, error) {
	...
	case <-time.After(resolvedHarnessHandshakeTimeout(h.handshakeTimeout)):
		...
		_ = cmd.Wait()
		<-handshake
		return AgentSessionRef{}, nil, fault.Conflict("CODEX_HANDSHAKE_TIMEOUT", "Codex CLI 启动后未在期限内返回首个结构化事件")
	}
	if err != nil || first.Type != "thread.started" || !validCodexSessionID(first.ThreadID) {
		...
		if err == nil {
			err = fault.Invalid("CODEX_EVENT_PROTOCOL_INVALID", "Codex JSONL 未以有效 thread.started 事件开始")
		}
		return AgentSessionRef{}, nil, err
	}
	if resumeSessionID != "" && first.ThreadID != resumeSessionID {
		cancel()
		_ = cmd.Wait()
		return AgentSessionRef{}, nil, fault.Conflict("CODEX_SESSION_MISMATCH", "Codex Resume 返回了不同的会话标识")
	}
...
func (h *codexExecHarness) registerSession(session *codexExecSession) error {
	...
	if existing := h.sessions[session.status.Session.SessionID]; existing != nil {
		if existing.status.Session.TenantID != session.status.Session.TenantID {
			return fault.Policy("AGENT_SESSION_TENANT_MISMATCH", "Codex 会话已经绑定其他租户", "使用当前 RuntimeAttempt 绑定的会话引用")
		}
		if existing.status.State == "active" {
			return fault.Conflict("CODEX_SESSION_ACTIVE", "Codex 会话已有活动执行")
		}
	}
...
func decodeCodexEvent(line []byte) (codexJSONEvent, error) {
	...
	if len(line) == 0 || json.Unmarshal(line, &event) != nil || strings.TrimSpace(event.Type) == "" {
		return codexJSONEvent{}, fault.Invalid("CODEX_EVENT_PROTOCOL_INVALID", "Codex 返回了无效 JSONL 事件")
	}
	return event, nil
}
