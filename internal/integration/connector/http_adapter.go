...
func NewHTTPAdapter(config HTTPConfig) (*HTTPAdapter, error) {
	...
	if err != nil || endpoint.Host == "" || (endpoint.Scheme != "https" && !(config.AllowHTTP && endpoint.Scheme == "http")) {
		return nil, fault.Invalid("CONNECTOR_ENDPOINT_INVALID", "Connector Endpoint 必须是 HTTPS URL")
	}
...
func (a *HTTPAdapter) Pull(ctx context.Context, request PullRequest) (PullResult, error) {
	...
	if len(data) > maxHTTPResponseBytes {
		return PullResult{}, fault.Invalid("CONNECTOR_RESPONSE_TOO_LARGE", "Connector 响应超过 32 MB")
	}
	...
	if err := json.Unmarshal(data, &wire); err != nil {
		return PullResult{}, fault.Invalid("CONNECTOR_RESPONSE_INVALID", "Connector 响应不是有效 JSON")
	}
	...
	for _, value := range wire.Records {
		...
		if value.BodyBase64 != "" {
			...
			if err != nil {
				return PullResult{}, fault.Invalid("CONNECTOR_BODY_INVALID", "Connector 正文不是有效 Base64")
			}
		}
