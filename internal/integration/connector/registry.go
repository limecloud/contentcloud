...
func (r *Registry) Resolve(id string) (Adapter, error) {
	if r == nil {
		return nil, fault.Policy("CONNECTOR_ADAPTER_UNAVAILABLE", "Connector Adapter 注册表未配置", "配置 Connector 后重试")
	}
	r.mu.RLock()
	defer r.mu.RUnlock()
	adapter, ok := r.adapters[strings.ToLower(strings.TrimSpace(id))]
	if !ok {
		return nil, fault.Policy("CONNECTOR_ADAPTER_UNAVAILABLE", "Connector Adapter 不可用", "检查绑定的 Connector ID 和服务端配置")
	}
	return adapter, nil
}
