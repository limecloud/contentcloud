...
func TestOrchestrationBFFVerticalSlice(t *testing.T) {
	...
	if len(admin.Environments) != 1 || len(admin.SOPs) != 4 {
		t.Fatalf("unexpected admin data: %#v", admin)
	}
	defaultSOPID := ""
	for _, summary := range admin.SOPs {
		if summary.Definition.TemplateKey == "short_video_production" {
			defaultSOPID = summary.Definition.ID
			break
		}
	}
	if defaultSOPID == "" || admin.Environments[0].DefaultSOPID != defaultSOPID {
		t.Fatalf("short video built-in SOP was not selected as the default: %#v", admin)
	}
	...
	project := callBFF[domain.Project](t, client, http.MethodPost, server.URL+"/api/bff/projects", app.CreateProjectInput{BrandName: "后台品牌", ProductName: "后台产品"})
...
func TestTaskGovernanceBFFActions(t *testing.T) {
	...
	project := callBFF[domain.Project](t, jarClient, http.MethodPost, server.URL+"/api/bff/projects", app.CreateProjectInput{BrandName: "治理品牌", ProductName: "治理产品"})
...
func TestSOPGovernanceBFFActions(t *testing.T) {
	...
	project := callBFF[domain.Project](t, client, http.MethodPost, server.URL+"/api/bff/projects", app.CreateProjectInput{BrandName: "升级验证品牌", ProductName: "升级验证产品"})
