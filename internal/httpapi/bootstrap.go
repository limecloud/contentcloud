...
func (s *Server) bootstrapActions(w http.ResponseWriter, r *http.Request) {
	catalog, err := s.service.BootstrapActionCatalog()
	if err != nil {
...
func (s *Server) bootstrapAuthorizationView(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.BootstrapAuthorizationView(r.Context(), actor, chi.URLParam(r, "projectID"), chi.URLParam(r, "attemptID"))
...
func (s *Server) approveBootstrapAuthorization(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.ApproveBootstrapAuthorization(r.Context(), actor, chi.URLParam(r, "id"), chi.URLParam(r, "attemptID"), middleware.GetReqID(r.Context()))
...
func (s *Server) denyBootstrapAuthorization(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.DenyBootstrapAuthorization(r.Context(), actor, chi.URLParam(r, "id"), chi.URLParam(r, "attemptID"), middleware.GetReqID(r.Context()))
