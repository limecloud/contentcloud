...
func TestDaemonPollEnforcesMinimumVersionWithoutLeasing(t *testing.T) {
	service, actor, connected, project := connectedDaemonTestRuntime(t, app.WithDaemonVersionPolicy("0.10.0", "0.13.0", "https://content.example.com/downloads"))
	...
	if poll.Leased || !poll.Runtime.UpdateRequired || !poll.Runtime.UpdateAvailable || poll.Runtime.MinimumVersion != "0.10.0" || poll.Runtime.LatestVersion != "0.13.0" || poll.Runtime.UpdateURL == "" {
...
func TestDaemonPollHonorsShortLongPollDeadline(t *testing.T) {
	...
	started := time.Now()
	poll := dispatchDevice[app.DaemonPollResponse](t, server.URL, connected.DeviceToken, "daemon.poll", map[string]any{
		"capabilities": []domain.Capability{}, "environments": []app.AutomationEnvironmentClaim{}, "daemon_version": "0.13.0", "wait_ms": 60,
	})
