...
func TestRuntimeExplorerBFFShowsOperationsProjection(t *testing.T) {
	...
	detail := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+job.ID, nil)
...
func TestRuntimePauseBFFKeepsBusinessProjectionAndRequiresExplicitResume(t *testing.T) {
	...
	initial := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+jobID, nil)
	...
	paused := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/jobs/"+jobID+"/pause", nil)
	if paused.Summary.State != domain.JobRunPaused || !containsStringValue(paused.Summary.AllowedActions, "resume") || containsStringValue(paused.Summary.AllowedActions, "pause") {
	...
	refreshed := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/jobs/"+jobID+"/refresh", nil)
	if refreshed.Summary.State != domain.JobRunPaused || !containsStringValue(refreshed.Summary.AllowedActions, "resume") {
		t.Fatalf("refresh must preserve paused state: state=%s actions=%#v", refreshed.Summary.State, refreshed.Summary.AllowedActions)
	}

	resumed := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/jobs/"+jobID+"/resume", nil)
	if resumed.Summary.State != domain.JobRunRunning || !containsStringValue(resumed.Summary.AllowedActions, "pause") || containsStringValue(resumed.Summary.AllowedActions, "resume") {
...
func TestRuntimeExplorerProjectsAgentContextWithoutSessionReference(t *testing.T) {
	...
	client := &http.Client{Jar: jar}
	bootstrap := mustStudioBootstrap(t, client, server.URL)
	task := callBFF[app.StudioTaskView](t, client, http.MethodPost, server.URL+"/api/studio/tasks", app.StudioCreateTaskInput{ExperienceID: bootstrap.Experiences[0].ID, ProjectID: bootstrap.Projects[0].ID, Title: "Runtime Agent 观察", Goal: "验证智能体投影脱敏"})
	list := callBFF[app.RuntimeJobList](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs", nil)
	job := runtimeJobForTask(t, list, task.Task.ID)
	initial := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+job.ID, nil)
	actor, _, err := service.SessionActor(t.Context(), sessionIDFromJar(t, jar, server.URL))
	if err != nil {
		t.Fatal(err)
	}
	created := time.Now().UTC()
	view, err := service.Runtime().CreateContextView(t.Context(), contentruntime.ContextViewInput{TenantID: actor.TenantID, JobRunID: job.ID, NodeRunID: initial.Nodes[0].ID, AttemptID: "attempt-http-agent", AllowedTools: []string{"state.get"}, MaxTokens: 2048, BudgetMinor: 50, CreatedAt: created, ExpiresAt: created.Add(time.Hour)})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.Runtime().CreateAgentInstance(t.Context(), contentruntime.AgentInstanceInput{TenantID: actor.TenantID, JobRunID: job.ID, NodeRunID: initial.Nodes[0].ID, Role: "supervisor", HarnessKind: "fake", SessionRef: "opaque-session-token", ExecutionProfileID: "profile-http", ContextViewID: view.ID, RemainingDescendants: 1, BudgetMinor: 50}); err != nil {
		t.Fatal(err)
	}
	detail := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+job.ID, nil)
...
func TestRuntimeRecoveryBFFForksFromCheckpointAndReplaysDurableEvents(t *testing.T) {
	...
	client := &http.Client{Jar: jar}
	bootstrap := mustStudioBootstrap(t, client, server.URL)
	task := callBFF[app.StudioTaskView](t, client, http.MethodPost, server.URL+"/api/studio/tasks", app.StudioCreateTaskInput{ExperienceID: bootstrap.Experiences[0].ID, ProjectID: bootstrap.Projects[0].ID, Title: "Runtime 恢复", Goal: "验证检查点 Fork 和只读 Replay"})
	list := callBFF[app.RuntimeJobList](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs", nil)
	job := runtimeJobForTask(t, list, task.Task.ID)
	actor, _, err := service.SessionActor(t.Context(), sessionIDFromJar(t, jar, server.URL))
	if err != nil {
	...
	source := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+job.ID, nil)
	checkpoint, err := service.Runtime().Checkpoint(t.Context(), actor.TenantID, source.Summary.ID, source.Nodes[0].NodeKey, []string{"state:brief"}, []string{"output:brief"})
	if err != nil {
		t.Fatal(err)
	}
	cancelled := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/jobs/"+source.Summary.ID+"/cancel", nil)
	...
	forked := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/checkpoints/"+checkpoint.ID+"/fork", map[string]string{"idempotency_key": "fork-" + checkpoint.ID})
	...
	replayed := callBFF[app.RuntimeReplayResult](t, client, http.MethodPost, server.URL+"/api/bff/runtime/jobs/"+source.Summary.ID+"/replay", nil)
...
func TestRuntimeRecoveryBFFStartsUnknownEffectReconciliation(t *testing.T) {
	...
	client := &http.Client{Jar: jar}
	bootstrap := mustStudioBootstrap(t, client, server.URL)
	task := callBFF[app.StudioTaskView](t, client, http.MethodPost, server.URL+"/api/studio/tasks", app.StudioCreateTaskInput{ExperienceID: bootstrap.Experiences[0].ID, ProjectID: bootstrap.Projects[0].ID, Title: "未知副作用对账", Goal: "验证不会盲目重试"})
	list := callBFF[app.RuntimeJobList](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs", nil)
	job := runtimeJobForTask(t, list, task.Task.ID)
	detail := callBFF[app.RuntimeJobDetail](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs/"+job.ID, nil)
	actor, _, err := service.SessionActor(t.Context(), sessionIDFromJar(t, jar, server.URL))
	...
	effect, err := service.Runtime().RegisterEffect(t.Context(), domain.ExternalEffect{TenantID: actor.TenantID, JobRunID: detail.Summary.ID, NodeRunID: detail.Nodes[0].ID, Kind: "media.generate", IdempotencyKey: "unknown-effect", RequestDigest: "sha256:request", Currency: "CNY", SafeSummary: map[string]any{}})
	if err != nil {
		t.Fatal(err)
	}
	effect, err = service.Runtime().ReconcileEffect(t.Context(), actor.TenantID, effect.ID, domain.EffectUnknown, "", "", "TIMEOUT", effect.Version)
	if err != nil {
		t.Fatal(err)
	}
	reconciled := callBFF[app.RuntimeJobDetail](t, client, http.MethodPost, server.URL+"/api/bff/runtime/effects/"+effect.ID+"/reconcile", map[string]int{"expected_version": effect.Version})
	if len(reconciled.Effects) != 1 || reconciled.Effects[0].State != domain.EffectReconciling {
...
func TestRuntimeEventsStreamResumesFromLastEventID(t *testing.T) {
	...
	client := &http.Client{Jar: jar}
	bootstrap := mustStudioBootstrap(t, client, server.URL)
	task := callBFF[app.StudioTaskView](t, client, http.MethodPost, server.URL+"/api/studio/tasks", app.StudioCreateTaskInput{ExperienceID: bootstrap.Experiences[0].ID, ProjectID: bootstrap.Projects[0].ID, Title: "Runtime SSE", Goal: "验证事件增量"})
	list := callBFF[app.RuntimeJobList](t, client, http.MethodGet, server.URL+"/api/bff/runtime/jobs", nil)
	job := runtimeJobForTask(t, list, task.Task.ID)
...
func runtimeJobForTask(t *testing.T, list app.RuntimeJobList, workTaskID string) app.RuntimeJobSummary {
	t.Helper()
