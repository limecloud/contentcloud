...
type runtimeControlFixture struct {
	...
	connected  app.ConnectDeviceResult
	controlURL string
}

func (failingDeviceLookupStore) DeviceByTokenHash(context.Context, string) (domain.Device, error) {
	return domain.Device{}, errors.New("database unavailable")
}
...
func TestRuntimeWorkerControlAuthenticatesDeviceAndResyncsOnConnect(t *testing.T) {
	...
	instances, err := fixture.service.DaemonInstances(t.Context(), fixture.actor, fixture.connected.Device.ID)
	...
	if _, err := fixture.service.Runtime().Start(t.Context(), contentruntime.StartInput{TenantID: fixture.actor.TenantID, ProjectID: fixture.project.ID, WorkTaskID: "control-task", SOP: sop, BindingDigest: "sha256:" + strings.Repeat("a", 64), InputDigest: "sha256:" + strings.Repeat("b", 64), RuntimePolicyID: "runtime-policy/control", ContractMajor: 1, CreatedBy: fixture.actor.UserID, IdempotencyKey: "control-job"}); err != nil {
		t.Fatal(err)
	}
	readCtx, cancel := context.WithTimeout(t.Context(), time.Second)
	defer cancel()
...
func TestRuntimeWorkerControlTreatsDeviceLookupFailureAsTransient(t *testing.T) {
	service := app.New(failingDeviceLookupStore{Store: memory.New()}, slog.Default())
...
func newRuntimeControlFixture(t *testing.T) runtimeControlFixture {
	...
	session, err := service.Register(t.Context(), "control@example.com", "long-enough-password", "Control", "Control Tenant")
	if err != nil {
		t.Fatal(err)
	}
	...
	project, err := service.CreateProject(t.Context(), actor, app.CreateProjectInput{BrandName: "Brand", ProductName: "Product", Channel: "douyin"}, "control-project")
	if err != nil {
		t.Fatal(err)
	}
	connect, err := service.CreateConnectSession(t.Context(), actor, project.ID, "control-connect")
	if err != nil {
		t.Fatal(err)
	}
	connected, err := testsupport.ConnectBootstrap(t.Context(), service, actor, connect, app.ConnectDeviceInput{Hostname: "control-mac", Platform: "darwin", Arch: "arm64", Version: "test"})
	if err != nil {
		t.Fatal(err)
	}
	server := httptest.NewServer(httpapi.New(service, slog.Default(), false, "").Handler())
