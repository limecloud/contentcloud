...
func TestAgentCallbackIngressAuthenticatesAndCompletesAgentSaaSAttempt(t *testing.T) {
	...
	started, err := service.Runtime().Start(t.Context(), contentruntime.StartInput{TenantID: "tenant-1", ProjectID: "project-1", WorkTaskID: "agent-ingress-task", BusinessType: "content.produce", SOP: ingressSOP(), BindingDigest: "sha256:" + repeatHex("b"), InputDigest: "sha256:" + repeatHex("c"), RuntimePolicyID: "runtime.test/1", ContractMajor: 1, CreatedBy: "test", IdempotencyKey: "agent-ingress-job"})
	if err != nil {
		t.Fatal(err)
	}
	handle, err := service.Runtime().PrepareRemoteDispatch(t.Context(), contentruntime.DispatchInput{TenantID: "tenant-1", JobRunID: started.Job.ID, Owner: "worker-1", HarnessKind: "agent-saas", Role: "writer", ExecutionProfileID: "profile-agent-saas", MaxTokens: 4096, BudgetMinor: 100, RemainingDescendants: 1, LeaseFor: time.Minute}, agentadapter.HarnessCapabilities{Kind: "agent-saas", Events: true, Resume: true, MCPHTTP: true, StructuredOutput: true, SandboxProfile: "remote", MaxParallelSessions: 8})
	if err != nil {
		t.Fatal(err)
	}
	handle, err = service.Runtime().ActivateDispatch(t.Context(), handle, agentadapter.AgentSessionRef{TenantID: "tenant-1", HarnessKind: "agent-saas", SessionID: "remote-session-1"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := json.NewDecoder(response.Body).Decode(&envelope); err != nil || !envelope.OK || !envelope.Data.Applied || envelope.Data.Replayed || envelope.Data.Attempt.State != domain.RuntimeAttemptSucceeded {
