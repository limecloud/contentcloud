...
func (s *Server) agentCallback(w http.ResponseWriter, r *http.Request) {
	...
	if !decodeProviderIngressBody(body, &input) {
		s.fail(w, r, "agent.callback", domain.Invalid("AGENT_CALLBACK_INVALID", "Agent 回调 JSON 无效"))
		return
	}
	...
	if strings.TrimSpace(input.OccurredAt) != "" {
		...
		if err != nil {
			s.fail(w, r, "agent.callback", domain.Invalid("AGENT_CALLBACK_TIME_INVALID", "Agent 回调 occurred_at 必须是 RFC3339 时间"))
			return
		}
		occurredAt = parsed.UTC()
	}
	if s.service.Runtime() == nil {
		s.fail(w, r, "agent.callback", domain.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime"))
		return
	}
	result, err := s.service.Runtime().ReceiveAgentCallback(r.Context(), contentruntime.AgentCallbackInput{
...
func (s *Server) authenticateAgentIngress(w http.ResponseWriter, r *http.Request) (string, string, []byte, string, bool) {
	...
	if harnessKind == "" || tenantID == "" {
		s.fail(w, r, "agent.ingress.auth", domain.Invalid("AGENT_INGRESS_SCOPE_INVALID", "Agent ingress 缺少 Harness 或租户作用域"))
		return "", "", nil, "", false
	}
	secret := s.agentCallbackSecrets[tenantID+":"+harnessKind]
	if len(secret) == 0 {
		s.fail(w, r, "agent.ingress.auth", domain.Policy("AGENT_INGRESS_SECRET_UNAVAILABLE", "Agent ingress 未配置回调密钥", "在服务端密钥配置中登记租户和 Harness 的 HMAC 密钥"))
		return "", "", nil, "", false
	}
