...
func (s *Server) channelCallback(w http.ResponseWriter, r *http.Request) {
	...
	var input app.ReceiveChannelCallbackInput
	if !decodeProviderIngressBody(body, &input) {
		s.fail(w, r, "channel.callback", domain.Invalid("CHANNEL_CALLBACK_INVALID", "渠道 Callback JSON 无效"))
		return
	}
	input.PayloadDigest = digest
	value, err := s.service.ReceiveChannelCallback(r.Context(), tenantID, adapterID, input, middleware.GetReqID(r.Context()))
...
func (s *Server) authenticateChannelIngress(w http.ResponseWriter, r *http.Request) (string, string, []byte, string, bool) {
	...
	if adapterID == "" || tenantID == "" {
		s.fail(w, r, "channel.ingress.auth", domain.Invalid("CHANNEL_INGRESS_SCOPE_INVALID", "渠道 ingress 缺少 Adapter 或租户作用域"))
		return "", "", nil, "", false
	}
	secret := s.channelCallbackSecrets[tenantID+":"+adapterID]
	if len(secret) == 0 {
		s.fail(w, r, "channel.ingress.auth", domain.Policy("CHANNEL_INGRESS_SECRET_UNAVAILABLE", "渠道 ingress 未配置回调密钥", "在服务端密钥配置中登记租户和 Adapter 的 HMAC 密钥"))
		return "", "", nil, "", false
	}
