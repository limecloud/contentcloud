...
func (s *Server) contentProfiles(w http.ResponseWriter, r *http.Request) {
	s.ok(w, r, "content_profile.list", s.service.ContentProfiles())
}

func (s *Server) installContentProfile(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.InstallContentProfile(r.Context(), actor, chi.URLParam(r, "profileID"), middleware.GetReqID(r.Context()))
