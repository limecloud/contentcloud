...
func TestChannelCallbackIngressAuthenticatesAndDeduplicates(t *testing.T) {
	...
	session, err := service.Register(ctx, "channel-ingress@example.com", "long-enough-password", "Ingress", "Ingress Team")
	if err != nil {
		t.Fatal(err)
	}
	...
	delivery.Status, delivery.DeliveredAt, delivery.DeliveredBy = domain.TaskDeliveryReady, nil, ""
	if err := store.SaveTaskDelivery(ctx, delivery); err != nil {
		t.Fatal(err)
	}
	binding, err := service.CreateChannelBinding(ctx, actor, app.CreateChannelBindingInput{ProjectID: fixture.Project.ID, Channel: "douyin", AdapterID: "remote-ingress", AccountRef: "main", AuthorizationSecretRef: "secret://douyin/main"}, "")
	if err != nil {
		t.Fatal(err)
	}
	prepared, err := service.PrepareChannelPublication(ctx, actor, app.PrepareChannelPublicationInput{TaskDeliveryID: delivery.ID, ChannelBindingID: binding.ID, IdempotencyKey: "ingress-1"}, "")
	if err != nil {
		t.Fatal(err)
	}
