...
func TestBootstrapDocumentIsPublicAndAgentReady(t *testing.T) {
	...
	for _, required := range []string{"session-id", "浏览器设备授权", "@limecloud/contentcloud@0.20.0", "bootstrap preflight", "bootstrap plan", "bootstrap apply", "bootstrap resume", "plan_id", "--plan-id <plan_id-from-plan-json>", "新的 Codex 对话", "现有业务文件不会上传或替换"} {
