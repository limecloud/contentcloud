...
func TestBootstrapDocumentIsPublicAndAgentReady(t *testing.T) {
	...
	for _, required := range []string{"session-id", "browser device authorization", "@limecloud/contentcloud@0.14.0", "bootstrap preflight", "bootstrap plan", "bootstrap apply", "bootstrap resume", "plan_id", "--plan-id <plan_id-from-plan-json>", "new Codex chat", "must not upload existing files"} {
