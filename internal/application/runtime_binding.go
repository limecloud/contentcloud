...
func (s *CatalogService) buildRuntimeExecutionBinding(ctx context.Context, input runtimeExecutionBindingInput) (contentruntime.ExecutionBindingSnapshot, error) {
	policyID := strings.TrimSpace(input.RuntimePolicyID)
	binding := contentruntime.ExecutionBindingSnapshot{
		TenantID: input.TenantID, SchemaVersion: contentruntime.ExecutionBindingSnapshotSchema,
		...
		MaxCostMinor: 0, MaxDynamicDescendants: contentruntime.DefaultRuntimeLimits().MaxDynamicDescendants,
	...
	if environmentID := strings.TrimSpace(input.EnvironmentID); environmentID != "" {
		value, err := s.catalog.Environment(ctx, input.TenantID, environmentID)
		if err != nil {
			return contentruntime.ExecutionBindingSnapshot{}, err
		}
...
func (s *CatalogService) projectRuntimeEnvironmentID(ctx context.Context, tenantID, projectID string) (string, error) {
	binding, err := s.catalog.ProjectSOPBinding(ctx, tenantID, projectID)
