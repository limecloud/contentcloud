...
func (s *OperationsService) OperationsExecutors(ctx context.Context, actor Actor) (OperationsExecutorDirectory, error) {
	...
	if s.deviceControl == nil {
		return OperationsExecutorDirectory{}, fault.Policy("DAEMON_INSTANCE_STORE_UNAVAILABLE", "DaemonInstance 持久层未配置", "检查服务端设备控制存储配置")
	}
	instances, err := s.deviceControl.DaemonInstances(ctx, actor.TenantID, "")
	if err != nil {
		return OperationsExecutorDirectory{}, err
	}
	projectByID := make(map[string]workspacedomain.Project, len(projects))
...
func (s *OperationsService) OperationsExecutor(ctx context.Context, actor Actor, id string) (OperationsExecutor, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager"); err != nil && !actor.PlatformAdmin {
	...
	if s.deviceControl == nil {
		return OperationsExecutor{}, fault.Policy("DAEMON_INSTANCE_STORE_UNAVAILABLE", "DaemonInstance 持久层未配置", "检查服务端设备控制存储配置")
	}
...
func projectOperationsExecutor(device workspacedomain.Device, projectByID map[string]workspacedomain.Project, instance workspacedomain.DaemonInstance, now time.Time) OperationsExecutor {
	capabilities := append([]catalogdomain.Capability{}, device.Capabilities...)
...
func currentDaemonInstances(instances []workspacedomain.DaemonInstance) map[string]workspacedomain.DaemonInstance {
	current := make(map[string]workspacedomain.DaemonInstance)
...
func operationsExecutorHealth(device workspacedomain.Device, instance workspacedomain.DaemonInstance, now time.Time) (string, string, string, string, string, string) {
