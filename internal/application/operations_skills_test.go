...
func TestOperationsSkillsProjectsOnlyVerifiedSkillPacks(t *testing.T) {
	...
	service := application.New(application.DependenciesFrom(memory.New()), slog.Default(), application.WithEnvironmentControlPlane(controlPlane), application.WithPlatformAdminEmails("skills@example.com"))
	session, err := service.Identity.Register(t.Context(), "skills@example.com", "long-enough-password", "技能包运营", "技能包租户")
	must(t, err)
	actor, _, err := service.Identity.SessionActor(t.Context(), session.ID)
	must(t, err)

	directory, err := service.Operations.OperationsSkills(t.Context(), actor)
	must(t, err)
	...
	detail, err := service.Operations.OperationsSkill(t.Context(), actor, skill.ID, skill.Version)
	must(t, err)
...
func TestOperationsSkillsReturnsHonestUnconfiguredDirectory(t *testing.T) {
	service := application.New(application.DependenciesFrom(memory.New()), slog.Default(), application.WithPlatformAdminEmails("skills-empty@example.com"))
	session, err := service.Identity.Register(t.Context(), "skills-empty@example.com", "long-enough-password", "技能包运营", "技能包租户")
	must(t, err)
	actor, _, err := service.Identity.SessionActor(t.Context(), session.ID)
	must(t, err)
...
func TestOperationsSkillsRequirePlatformAdmin(t *testing.T) {
	...
	session, err := service.Identity.Register(t.Context(), "skills-member@example.com", "long-enough-password", "租户管理员", "技能包租户")
	must(t, err)
	actor, _, err := service.Identity.SessionActor(t.Context(), session.ID)
	must(t, err)
	if _, err := service.Operations.OperationsSkills(t.Context(), actor); err == nil {
