...
func TestImportFixtureV3IsIdempotentAndBuildsProjection(t *testing.T) {
	...
	session, err := service.Identity.Register(t.Context(), "fixture@example.com", "long-enough-password", "Fixture", "Fixture Tenant")
	if err != nil {
		t.Fatal(err)
	}
	...
	first, err := service.Review.ImportFixtureV3(t.Context(), actor, fixture, "fixture-first")
	if err != nil {
		t.Fatal(err)
	}
	second, err := service.Review.ImportFixtureV3(t.Context(), actor, fixture, "fixture-second")
	if err != nil {
		t.Fatal(err)
	}
	...
	projection, err := service.Operations.ProjectProjection(t.Context(), actor, first.Project.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	current, err := service.Review.ProjectSubmissionRevision(t.Context(), actor, first.Project.ID, projection.NextActions[0].Navigation.Focus.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportCompleteJinlingFixtureUsesExternalPackage(t *testing.T) {
	...
	service := application.New(application.DependenciesFrom(memory.New()), slog.Default())
	session, err := service.Identity.Register(t.Context(), "fixture-complete@example.com", "long-enough-password", "Fixture", "Fixture Tenant")
	if err != nil {
		t.Fatal(err)
	}
	actor, _, err := service.Identity.SessionActor(t.Context(), session.ID)
	...
	first, err := service.Review.ImportFixtureV3(t.Context(), actor, fixture, "fixture-complete-first")
	if err != nil {
		t.Fatal(err)
	}
	second, err := service.Review.ImportFixtureV3(t.Context(), actor, fixture, "fixture-complete-second")
	if err != nil {
		t.Fatal(err)
	}
