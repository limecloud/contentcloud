...
func (s *DeliveryService) CreateMediaGenerationJob(ctx context.Context, actor Actor, taskID string, input CreateMediaGenerationJobInput, requestID string) (deliverydomain.MediaGenerationJob, error) {
	...
	if task.ContentType != identitydomain.ContentTypeMarketingVideo {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_CONTENT_TYPE_REQUIRED", "只有营销视频任务可以创建视频生成任务", "使用营销视频全流程规范创建任务")
	}
	...
	if input.StageRunID != "" && input.StageRunID != run.ID {
		return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_STAGE_NOT_CURRENT", "视频生成任务必须绑定当前流程阶段执行记录")
	}
	if run.Status != work.StageRunStatusRunning || run.StageID != "generation" {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_STAGE_INVALID", "只有运行中的视频生成阶段可以创建生成任务", "先完成并批准分镜阶段")
	}
	storyboard, err := s.review.ApprovedSnapshot(ctx, actor.TenantID, strings.TrimSpace(input.StoryboardSnapshotID))
	if err != nil {
		return deliverydomain.MediaGenerationJob{}, err
	}
	if storyboard.ProjectID != task.ProjectID {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_STORYBOARD_SCOPE_INVALID", "分镜快照不属于当前项目", "选择当前任务项目内已批准的分镜")
	}
	...
	if mode != "text_to_video" {
		...
		if len(input.InputArtifactRefs) > 0 && !sameStringSet(input.InputArtifactRefs, verifiedInputRefs) {
			return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_INPUT_ARTIFACTS_MISMATCH", "视频生成任务的输入与服务端核验的锁定分镜素材不一致")
		}
	}
	if mode == "text_to_video" {
	...
	if providerID == Seedance25ProviderID {
		...
		if promptPackageID == "" {
			return deliverydomain.MediaGenerationJob{}, fault.Invalid("SEEDANCE_PROMPT_PACKAGE_REQUIRED", "Seedance 2.5 任务必须绑定已校验的 PromptPackage Artifact")
		}
		promptArtifact, artifactErr := s.artifacts.Artifact(ctx, actor.TenantID, promptPackageID)
		if artifactErr != nil {
			return deliverydomain.MediaGenerationJob{}, artifactErr
		}
		if promptArtifact.ProjectID != task.ProjectID || promptArtifact.ApprovedSnapshotID != storyboard.ID || promptArtifact.Kind != "prompt_package" || promptArtifact.MediaType != "application/json" {
			return deliverydomain.MediaGenerationJob{}, fault.Conflict("SEEDANCE_PROMPT_PACKAGE_SCOPE_INVALID", "Seedance PromptPackage Artifact 与当前项目或批准快照不一致")
		}
		if version := metadataString(promptArtifact.Metadata, "provider_profile_version"); version != "" && version != profile.Version {
			return deliverydomain.MediaGenerationJob{}, fault.Conflict("SEEDANCE_PROMPT_PACKAGE_STALE", "Seedance PromptPackage Artifact 与当前 Provider Profile 版本不一致")
		}
	}
	now := s.now().UTC()
	if profile.Status != "published" || !profile.ExpiresAt.After(now) {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_PROFILE_UNAVAILABLE", "服务商配置版本未发布或已过期", "选择有效的服务商配置版本")
	}
	if !containsString(profile.Modes, mode) {
		return deliverydomain.MediaGenerationJob{}, fault.Invalid("PROVIDER_MODE_UNSUPPORTED", "服务商配置版本不支持当前生成模式")
	}
	adapter, adapterErr := s.mediaAdapter(providerID)
	if adapterErr != nil {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_ADAPTER_UNAVAILABLE", "服务商适配器未配置", "配置已发布 Provider Adapter 后重试")
	}
	...
	maxJobCost := int64(0)
	if providerID != "fake" {
		binding, bindingErr := s.delivery.ProviderBinding(ctx, actor.TenantID, providerID)
		if bindingErr != nil {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_NOT_CONFIGURED", "当前租户尚未配置该服务商", "由租户管理员配置服务商绑定")
		}
		if binding.State != "active" || binding.ProfileVersion != profile.Version || !validProviderCredentialRef(binding.CredentialRef) {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_BINDING_UNAVAILABLE", "服务商绑定未启用、配置版本不一致或缺少受控凭据引用", "检查服务商配置")
		}
	...
	if estimatedCost < 0 {
		return deliverydomain.MediaGenerationJob{}, fault.Invalid("PROVIDER_ESTIMATE_INVALID", "服务商返回了负数费用估算")
	}
	...
	if maxJobCost > 0 && estimatedCost > maxJobCost {
		return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_COST_LIMIT_EXCEEDED", "视频生成任务的估算费用超过单次任务上限", "降低生成规格或调整预算")
	}
	idempotencyKey := strings.TrimSpace(input.IdempotencyKey)
	if idempotencyKey == "" {
		idempotencyKey = fmt.Sprintf("media:%s:%s:%s:%s", task.ID, storyboard.ID, providerID, mode)
	}
	state := deliverydomain.MediaJobQueued
	if estimatedCost > 0 {
		state = deliverydomain.MediaJobAwaitingCostApproval
	}
	...
	if job.RuntimeJobRunID != "" || job.RuntimeNodeRunID != "" || job.RuntimeAttemptID != "" || job.RuntimeEffectID != "" {
		if s.runtimeService == nil {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_RUNTIME_UNAVAILABLE", "媒体 Job 指定了 Runtime 关联但当前未启用 Runtime", "移除 Runtime 关联或启用 Runtime")
		}
		runtimeJob, runtimeErr := s.runtimeService.Job(ctx, actor.TenantID, job.RuntimeJobRunID)
		if runtimeErr != nil {
			return deliverydomain.MediaGenerationJob{}, runtimeErr
		}
		if runtimeJob.ProjectID != job.ProjectID || runtimeJob.WorkTaskID != job.TaskID {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("MEDIA_JOB_RUNTIME_SCOPE_INVALID", "媒体 Job 的 Runtime 作用域与业务任务不一致", "绑定当前项目和任务的 Runtime 执行实例")
		}
		nodes, nodesErr := s.runtimeService.Nodes(ctx, actor.TenantID, runtimeJob.ID)
		if nodesErr != nil {
			return deliverydomain.MediaGenerationJob{}, nodesErr
		}
		foundNode := false
		...
		if !foundNode {
			return deliverydomain.MediaGenerationJob{}, fault.NotFound("媒体 Job Runtime 节点")
		}
	}
...
func (s *DeliveryService) ApproveMediaGenerationJob(ctx context.Context, actor Actor, id string, input MediaJobDecisionInput, requestID string) (deliverydomain.MediaGenerationJob, error) {
	...
	job, err := s.delivery.MediaGenerationJob(ctx, actor.TenantID, id)
	if err != nil {
		return deliverydomain.MediaGenerationJob{}, err
	}
	if input.ExpectedVersion != job.RowVersion {
		return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_STALE", "视频生成任务已被其他操作更新")
	}
	if job.State != deliverydomain.MediaJobAwaitingCostApproval && job.State != deliverydomain.MediaJobBudgetBlocked {
		return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_COST_DECISION_INVALID", "当前视频生成任务不在费用确认状态")
	}
	job.State = deliverydomain.MediaJobQueued
	job.ErrorCode = ""
	job.ErrorDetailSafe = ""
	job.UpdatedAt = s.now().UTC()
	if err := s.delivery.SaveMediaGenerationJob(ctx, job, input.ExpectedVersion); err != nil {
		return deliverydomain.MediaGenerationJob{}, err
	}
...
func (s *DeliveryService) CancelMediaGenerationJob(ctx context.Context, actor Actor, id string, input MediaJobDecisionInput, requestID string) (deliverydomain.MediaGenerationJob, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "editor"); err != nil {
		return deliverydomain.MediaGenerationJob{}, err
	}
	job, err := s.delivery.MediaGenerationJob(ctx, actor.TenantID, id)
	...
	if !deliverydomain.CanTransitionMediaJob(job.State, deliverydomain.MediaJobCancelled) {
		return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_NOT_CANCELLABLE", "当前视频生成任务不能取消")
	}
	var attempt deliverydomain.ProviderAttempt
	var hasExternalJob bool
	if attempts, attemptsErr := s.delivery.ProviderAttempts(ctx, actor.TenantID, job.ID); attemptsErr != nil {
		return deliverydomain.MediaGenerationJob{}, attemptsErr
	...
	if hasExternalJob {
		...
		if profileErr != nil {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_CANCEL_UNKNOWN", "取消前无法读取原服务商配置，外部任务状态不明", "恢复服务商配置后进行对账")
		}
		adapter, adapterErr := s.mediaAdapter(job.ProviderID)
		if adapterErr != nil {
			return deliverydomain.MediaGenerationJob{}, fault.Policy("PROVIDER_CANCEL_UNKNOWN", "取消前无法读取服务商适配器，外部任务状态不明", "恢复服务商适配器后进行对账")
		}
		if cancelErr := adapter.Cancel(ctx, attempt.ExternalJobID, profile); cancelErr != nil {
			...
			job.UpdatedAt = now
			job.State = deliverydomain.MediaJobAwaitingExternal
			job.LeaseOwner = ""
			job.LeaseExpiresAt = nil
			if saveErr := s.delivery.SaveMediaGenerationJob(ctx, job, input.ExpectedVersion); saveErr != nil {
				return deliverydomain.MediaGenerationJob{}, saveErr
			}
			job.RowVersion = input.ExpectedVersion + 1
			...
			if job.RuntimeEffectID != "" {
				if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectUnknown, attempt.ExternalJobID, "", "PROVIDER_CANCEL_UNKNOWN"); effectErr != nil {
					return deliverydomain.MediaGenerationJob{}, effectErr
				}
			}
			...
			return job, fault.Policy("PROVIDER_CANCEL_UNKNOWN", "服务商取消结果不明，已进入外部任务对账状态", "查询服务商状态并完成外部操作对账")
		}
		...
		if job.RuntimeEffectID != "" {
			if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectFailed, attempt.ExternalJobID, "", "PROVIDER_JOB_CANCELLED"); effectErr != nil {
				return deliverydomain.MediaGenerationJob{}, effectErr
			}
		}
	}
	now := s.now().UTC()
	job.State = deliverydomain.MediaJobCancelled
	job.CancelRequestedAt = &now
	job.UpdatedAt = now
	if err := s.delivery.SaveMediaGenerationJob(ctx, job, input.ExpectedVersion); err != nil {
		return deliverydomain.MediaGenerationJob{}, err
...
func (s *DeliveryService) PendingMediaGenerationJobs(ctx context.Context, limit int) ([]deliverydomain.MediaGenerationJob, error) {
	return s.delivery.PendingMediaGenerationJobs(ctx, limit)
}
...
func (s *DeliveryService) ReconcileMediaGenerationSubmit(ctx context.Context, actor Actor, id string, input MediaJobSubmitReconciliationInput, requestID string) (deliverydomain.MediaGenerationJob, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "editor"); err != nil {
		return deliverydomain.MediaGenerationJob{}, err
	}
	job, err := s.delivery.MediaGenerationJob(ctx, actor.TenantID, id)
	...
	if job.State != deliverydomain.MediaJobAwaitingExternal {
		return deliverydomain.MediaGenerationJob{}, fault.Conflict("MEDIA_JOB_RECONCILIATION_INVALID", "只有等待外部对账的媒体任务可以补录外部任务标识")
	}
	externalID := strings.TrimSpace(input.ExternalJobID)
	if externalID == "" || strings.ContainsAny(externalID, "/?#") || len(externalID) > 256 {
		return deliverydomain.MediaGenerationJob{}, fault.Invalid("PROVIDER_EXTERNAL_ID_INVALID", "服务商外部任务标识无效")
	}
	attempts, err := s.delivery.ProviderAttempts(ctx, actor.TenantID, job.ID)
	if err != nil || len(attempts) == 0 {
		if err != nil {
			return deliverydomain.MediaGenerationJob{}, err
		}
		return deliverydomain.MediaGenerationJob{}, fault.NotFound("服务商调用尝试")
	}
	attempt := attempts[len(attempts)-1]
	if strings.TrimSpace(attempt.ExternalJobID) != "" {
		if attempt.ExternalJobID != externalID {
			return deliverydomain.MediaGenerationJob{}, fault.Conflict("PROVIDER_EXTERNAL_ID_CONFLICT", "服务商外部任务标识已经绑定且不能覆盖")
		}
		return job, nil
	}
	now := s.now().UTC()
	...
	job.RowVersion = input.ExpectedVersion + 1
	if job.RuntimeEffectID != "" {
		if err := s.transitionMediaEffect(ctx, job, contentruntime.EffectReconciling, externalID, "", ""); err != nil {
			return deliverydomain.MediaGenerationJob{}, err
		}
	}
...
func (s *DeliveryService) ProcessMediaGenerationJob(ctx context.Context, tenantID, id string) error {
	job, err := s.delivery.MediaGenerationJob(ctx, tenantID, id)
	if err != nil {
		return err
	}
	if job.State == deliverydomain.MediaJobRetryWait {
		job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobQueued, nil)
		if err != nil {
			return err
		}
	}
	resumingExternal := job.State == deliverydomain.MediaJobAwaitingExternal
	if job.State != deliverydomain.MediaJobQueued && !resumingExternal {
		return nil
	}
	...
	now := s.now().UTC()
	if resumingExternal {
		attempts, attemptsErr := s.delivery.ProviderAttempts(ctx, tenantID, job.ID)
		...
		job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobGenerating, func(value *deliverydomain.MediaGenerationJob) {
		...
		job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobSubmitting, func(value *deliverydomain.MediaGenerationJob) {
			value.AttemptCount++
			value.LeaseOwner = "media-worker"
			expires := s.now().UTC().Add(2 * time.Minute)
			value.LeaseExpiresAt = &expires
		...
		if submitErr != nil {
			...
			if job.RuntimeEffectID != "" {
				if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectUnknown, "", "", attempt.ErrorCode); effectErr != nil {
					return effectErr
				}
			}
		...
		if job.RuntimeEffectID != "" {
			if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectSubmitted, submission.ExternalJobID, "", ""); effectErr != nil {
				return effectErr
			}
		}
		externalJobID = submission.ExternalJobID
		job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobSubmitted, nil)
		if err != nil {
			return err
		}
		job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobGenerating, nil)
		if err != nil {
			return err
		}
	}
	...
	if statusErr != nil {
		...
		if job.RuntimeEffectID != "" {
			if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectUnknown, externalJobID, "", attempt.ErrorCode); effectErr != nil {
				return effectErr
			}
		}
		_, transitionErr := s.transitionMediaJob(ctx, job, deliverydomain.MediaJobAwaitingExternal, func(value *deliverydomain.MediaGenerationJob) {
	...
	if providerState == "failed" || providerState == "cancelled" || providerState == "canceled" {
		...
		if job.RuntimeEffectID != "" {
			if effectErr := s.transitionMediaEffect(ctx, job, contentruntime.EffectFailed, externalJobID, "", attempt.ErrorCode); effectErr != nil {
				return effectErr
			}
		}
		return s.failMediaJob(ctx, job, "PROVIDER_JOB_FAILED", "服务商任务未生成成功")
	}
	job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobDownloading, nil)
	if err != nil {
		return err
	}
	job, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobValidating, nil)
	if err != nil {
		return err
	}
	...
	technicalReview := deliverydomain.MediaReview{ID: idgen.New(), TenantID: job.TenantID, ProjectID: job.ProjectID, TaskID: job.TaskID, GenerationJobID: job.ID, SubjectArtifactID: artifact.ID, SubjectDigest: normalizedSHA256(artifact.SHA256), ReviewKind: deliverydomain.MediaReviewTechnical, Status: deliverydomain.MediaReviewApproved, Checks: output.Technical, RowVersion: 1, CreatedBy: "media-worker", DecidedBy: "media-worker", DecidedAt: &now, CreatedAt: now, UpdatedAt: now}
	if err := s.delivery.CreateMediaReview(ctx, technicalReview); err != nil {
		return s.failMediaJob(ctx, job, "MEDIA_TECHNICAL_REVIEW_FAILED", "无法创建媒体技术审核")
	}
	contentReview := deliverydomain.MediaReview{ID: idgen.New(), TenantID: job.TenantID, ProjectID: job.ProjectID, TaskID: job.TaskID, GenerationJobID: job.ID, SubjectArtifactID: artifact.ID, SubjectDigest: normalizedSHA256(artifact.SHA256), ReviewKind: deliverydomain.MediaReviewContent, Status: deliverydomain.MediaReviewPending, Checks: map[string]any{}, RowVersion: 1, CreatedBy: "media-worker", CreatedAt: now, UpdatedAt: now}
	if err := s.delivery.CreateMediaReview(ctx, contentReview); err != nil {
	...
	if job.RuntimeEffectID != "" {
		if err := s.transitionMediaEffect(ctx, job, contentruntime.EffectSucceeded, externalJobID, "sha256:"+artifact.SHA256, ""); err != nil {
			return err
		}
	}
	_, err = s.transitionMediaJob(ctx, job, deliverydomain.MediaJobSucceeded, func(value *deliverydomain.MediaGenerationJob) {
...
func (s *DeliveryService) ensureMediaRuntimeEffect(ctx context.Context, job deliverydomain.MediaGenerationJob, attempt deliverydomain.ProviderAttempt, requestDigest string) (contentruntime.ExternalEffect, error) {
	if s.runtimeService == nil || job.RuntimeJobRunID == "" || job.RuntimeNodeRunID == "" {
		return contentruntime.ExternalEffect{}, fault.Policy("RUNTIME_EFFECT_SCOPE_REQUIRED", "Runtime 管理的媒体提交缺少外部操作作用域", "绑定 JobRun 和 NodeRun 后重试")
	}
	if job.RuntimeEffectID != "" {
		effect, err := s.runtimeService.Effect(ctx, job.TenantID, job.RuntimeEffectID)
		if err != nil {
			return contentruntime.ExternalEffect{}, err
		}
		if effect.JobRunID != job.RuntimeJobRunID || effect.NodeRunID != job.RuntimeNodeRunID || effect.RequestDigest != requestDigest {
			return contentruntime.ExternalEffect{}, fault.Conflict("RUNTIME_EFFECT_SCOPE_CONFLICT", "媒体 Job 绑定的外部操作与请求摘要不一致")
		}
		return effect, nil
	}
	return s.runtimeService.RegisterEffect(ctx, contentruntime.ExternalEffect{
...
func (s *DeliveryService) transitionMediaEffect(ctx context.Context, job deliverydomain.MediaGenerationJob, next, externalID, responseDigest, errorCode string) error {
	...
	if (next == contentruntime.EffectSucceeded || next == contentruntime.EffectFailed) && effect.State == contentruntime.EffectUnknown {
		if effect, err = s.runtimeService.ReconcileEffect(ctx, job.TenantID, effect.ID, contentruntime.EffectReconciling, externalID, responseDigest, errorCode, effect.Version); err != nil {
			return err
		}
	}
	if next == contentruntime.EffectSucceeded && effect.State == contentruntime.EffectSubmitted {
		if effect, err = s.runtimeService.ReconcileEffect(ctx, job.TenantID, effect.ID, contentruntime.EffectAcknowledged, externalID, responseDigest, errorCode, effect.Version); err != nil {
			return err
		}
	}
...
func (s *DeliveryService) persistMediaOutput(ctx context.Context, adapter mediapipeline.Adapter, outputRef string, profile deliverydomain.ProviderProfile, objectKeyPrefix string) (result persistedMediaOutput, err error) {
	...
	if stream.Body == nil {
		return persistedMediaOutput{}, fault.Invalid("MEDIA_OUTPUT_STREAM_INVALID", "服务商输出流为空")
	}
	...
	if written > maxBytes {
		_ = temp.Close()
		return persistedMediaOutput{}, fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出为空或超过大小限制")
	}
	...
	if readerStore, ok := s.blobs.(blob.ReaderStore); ok {
		...
		if err == nil && int64(len(data)) > maxBytes {
			err = fault.Invalid("MEDIA_OUTPUT_SIZE_INVALID", "服务商输出超过大小限制")
		}
		if err == nil {
...
func validateMediaFileName(value string) error {
	if strings.TrimSpace(value) == "" || strings.ContainsAny(value, "/\\\x00") || value == "." || value == ".." {
		return fault.Invalid("MEDIA_OUTPUT_FILENAME_INVALID", "服务商输出文件名无效")
	}
	return nil
}

func (s *DeliveryService) DecideMediaReview(ctx context.Context, actor Actor, id string, input MediaReviewDecisionInput, requestID string) (WorkTaskView, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "reviewer", "client_approver"); err != nil {
		return WorkTaskView{}, err
	}
	review, err := s.delivery.MediaReview(ctx, actor.TenantID, id)
	if err != nil {
		return WorkTaskView{}, err
	}
	if review.ReviewKind == deliverydomain.MediaReviewTechnical {
		return WorkTaskView{}, fault.Policy("MEDIA_TECHNICAL_REVIEW_IMMUTABLE", "技术审核只能由媒体执行器写入", "请对内容审核或最终审核作出决定")
	}
	if review.ReviewKind == deliverydomain.MediaReviewFinal && actor.Role != "tenant_admin" && actor.Role != "client_approver" {
		return WorkTaskView{}, fault.Policy("MEDIA_FINAL_REVIEW_ROLE_REQUIRED", "最终成片只能由客户决定人或租户管理员批准", "请由最终批准角色处理")
	}
	if review.ReviewKind == deliverydomain.MediaReviewFinal {
		...
		if artifact.Kind != "final_render" || artifact.Purpose != "final_video" || normalizedSHA256(artifact.SHA256) != review.SubjectDigest {
			return WorkTaskView{}, fault.Conflict("FINAL_RENDER_ARTIFACT_REQUIRED", "最终审核必须绑定独立的最终成片成果文件")
		}
	}
	if input.ExpectedVersion != review.RowVersion {
		return WorkTaskView{}, fault.Conflict("MEDIA_REVIEW_STALE", "媒体审核已被其他操作更新")
	}
	decision := strings.ToLower(strings.TrimSpace(input.Decision))
	switch decision {
	case "approved":
		review.Status = deliverydomain.MediaReviewApproved
	case "changes_requested":
		review.Status = deliverydomain.MediaReviewChanges
	case "rejected":
		review.Status = deliverydomain.MediaReviewRejected
	default:
		return WorkTaskView{}, fault.Invalid("MEDIA_REVIEW_DECISION_INVALID", "媒体审核决定无效")
	}
	if decision == "approved" && !input.Selected {
		return WorkTaskView{}, fault.Invalid("MEDIA_REVIEW_SELECTION_REQUIRED", "批准候选成片时必须明确选中对应成果文件")
	}
	now := s.now().UTC()
	...
	if err := s.delivery.SaveMediaReview(ctx, review, input.ExpectedVersion); err != nil {
		return WorkTaskView{}, err
	}
	...
	return s.app.Work.WorkTask(ctx, actor, review.TaskID)
}

func (s *DeliveryService) BuildTaskDeliveryPackage(ctx context.Context, actor Actor, taskID string, input BuildTaskDeliveryPackageInput, requestID string) (deliverydomain.DeliveryPackage, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "editor"); err != nil {
	...
	review, err := s.delivery.MediaReview(ctx, actor.TenantID, input.FinalReviewID)
	if err != nil {
		return deliverydomain.DeliveryPackage{}, err
	}
	if review.TaskID != task.ID || review.ProjectID != task.ProjectID || review.ReviewKind != deliverydomain.MediaReviewFinal || review.Status != deliverydomain.MediaReviewApproved || !review.Selected {
		return deliverydomain.DeliveryPackage{}, fault.Policy("FINAL_MEDIA_REVIEW_REQUIRED", "交付包必须绑定当前任务已批准并选中的最终成片审核", "先完成最终成片批准")
	}
	artifact, err := s.artifacts.Artifact(ctx, actor.TenantID, review.SubjectArtifactID)
	if err != nil {
		return deliverydomain.DeliveryPackage{}, err
	}
	if normalizedSHA256(artifact.SHA256) != review.SubjectDigest || artifact.ProjectID != task.ProjectID || artifact.ByteSize <= 0 {
		return deliverydomain.DeliveryPackage{}, fault.Conflict("FINAL_ARTIFACT_INTEGRITY_FAILED", "最终成果文件与审核摘要不一致或文件无效")
	}
	if artifact.Kind != "final_render" || artifact.Purpose != "final_video" {
		return deliverydomain.DeliveryPackage{}, fault.Policy("FINAL_RENDER_ARTIFACT_REQUIRED", "交付包只能使用独立的最终成片", "先在后期阶段生成并批准最终成片")
	}
...
func (s *DeliveryService) transitionMediaJob(ctx context.Context, job deliverydomain.MediaGenerationJob, state string, mutate func(*deliverydomain.MediaGenerationJob)) (deliverydomain.MediaGenerationJob, error) {
...
func (s *DeliveryService) failMediaJob(ctx context.Context, job deliverydomain.MediaGenerationJob, code, detail string) error {
	_, err := s.transitionMediaJob(ctx, job, deliverydomain.MediaJobFailed, func(value *deliverydomain.MediaGenerationJob) {
	...
	return fault.Policy(code, detail, "检查服务商配置或重试生成")
}
...
func (s *DeliveryService) ensureProviderProfile(ctx context.Context, providerID, version string) (deliverydomain.ProviderProfile, error) {
	profile, err := s.delivery.ProviderProfile(ctx, providerID, version)
	if err == nil || providerID != "fake" || !fault.IsNotFound(err) {
		return profile, err
	}
	profile = fakeProviderProfile()
	if err := s.delivery.CreateProviderProfile(ctx, profile); err != nil {
		if existing, lookupErr := s.delivery.ProviderProfile(ctx, providerID, version); lookupErr == nil {
			return existing, nil
		}
...
func (s *DeliveryService) mediaAdapter(providerID string) (mediapipeline.Adapter, error) {
	...
	return nil, fault.NotFound("服务商适配器")
}

func profileCurrency(profile deliverydomain.ProviderProfile) string {
