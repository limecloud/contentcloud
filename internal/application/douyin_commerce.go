...
func (s *ReviewService) validateDouyinCommercePublication(ctx context.Context, actor Actor, delivery deliverydomain.TaskDelivery, binding deliverydomain.ChannelBinding, input PrepareChannelPublicationInput) (map[string]any, error) {
	if input.ContentProfileID != work.DouyinCommerceProfileID || input.DouyinCommerce == nil {
		return nil, fault.Invalid("DOUYIN_COMMERCE_PUBLICATION_REFS_REQUIRED", "抖音电商发布必须提供 ContentProfileID 和类型化血缘引用")
	}
	...
	if binding.Channel != "douyin" || binding.AccountRef != refs.AccountRef || delivery.ProjectID != refs.ValidationReceipt.ProjectID || refs.ValidationReceipt.AccountRef != binding.AccountRef {
		return nil, fault.Conflict("DOUYIN_COMMERCE_PUBLICATION_SCOPE_INVALID", "抖音电商校验回执、渠道账号和项目不一致")
	}
	if input.ScheduledAt == nil || !input.ScheduledAt.Equal(refs.ValidationReceipt.ScheduledAt) {
		return nil, fault.Conflict("DOUYIN_COMMERCE_SCHEDULE_MISMATCH", "计划发布时间必须与抖音电商校验回执一致")
	}
	if err := s.validateDouyinApprovedInputs(ctx, actor.TenantID, delivery.ProjectID, refs); err != nil {
		return nil, err
	}
	pkg, err := s.artifacts.DeliveryPackage(ctx, actor.TenantID, delivery.DeliveryPackageID)
	if err != nil {
		return nil, err
	}
	if pkg.ProjectID != delivery.ProjectID || pkg.ContentItemID != refs.ContentItemID {
		return nil, fault.Conflict("DOUYIN_COMMERCE_DELIVERY_LINEAGE_INVALID", "DeliveryPackage 的项目或 ContentItem 与校验回执不一致")
	}
	var finalArtifact *deliverydomain.Artifact
	...
	if finalArtifact == nil || normalizedArtifactDigest(finalArtifact.SHA256) != refs.RenderedCreativeDigest {
		return nil, fault.Conflict("DOUYIN_COMMERCE_ARTIFACT_LINEAGE_INVALID", "最终成片 Artifact 不在 DeliveryPackage manifest 中，或摘要不一致")
	}
	if finalArtifact.ProjectID != delivery.ProjectID || finalArtifact.ApprovedSnapshotID == "" {
		return nil, fault.Conflict("DOUYIN_COMMERCE_ARTIFACT_SCOPE_INVALID", "最终成片 Artifact 不属于当前项目或没有批准快照")
	}
...
func (s *ReviewService) validateDouyinApprovedInputs(ctx context.Context, tenantID, projectID string, refs work.DouyinCommercePublicationRefs) error {
	...
	if json.Unmarshal(strategyRaw, &strategy) != nil || strategy.Type != "audience_strategy_version" {
		return fault.Invalid("DOUYIN_COMMERCE_AUDIENCE_INVALID", "人群批准快照不包含有效策略版本")
	}
	...
	if json.Unmarshal(offerRaw, &offer) != nil || offer.Type != "commerce_offer_snapshot" {
		return fault.Invalid("DOUYIN_COMMERCE_OFFER_INVALID", "商品批准快照不包含有效 Offer")
	}
	...
	if rendered.ContentHash != refs.ValidationReceipt.ContentItemDigest || rendered.Item.Channel != "douyin" || rendered.Item.ProjectID != projectID {
		return fault.Conflict("DOUYIN_COMMERCE_CONTENT_LINEAGE_INVALID", "ContentItem 摘要、渠道或项目与校验回执不一致")
	}
	voiceover, onScreen := douyinContentTextForServer(rendered.Item)
	voiceDigest, err := work.DouyinTextDigest(voiceover)
	if err != nil || voiceDigest != refs.ValidationReceipt.VoiceoverTextDigest {
		return fault.Conflict("DOUYIN_COMMERCE_VOICEOVER_DIGEST_INVALID", "口播文本摘要与校验回执不一致")
	}
	onScreenDigest, err := work.DouyinTextDigest(onScreen)
	if err != nil || onScreenDigest != refs.ValidationReceipt.OnScreenTextDigest {
		return fault.Conflict("DOUYIN_COMMERCE_ONSCREEN_DIGEST_INVALID", "字幕文本摘要与校验回执不一致")
	}
	...
	if receiptOffer.SKUID != offer.SKUID || receiptOffer.ProductVersionID != offer.ProductVersionID || receiptOffer.DisplayPrice != strings.TrimSpace(offer.DisplayPrice) || receiptOffer.Currency != strings.ToUpper(strings.TrimSpace(offer.Currency)) || !sameStringSet(receiptOffer.Benefits, offer.Benefits) || !sameStringSet(receiptOffer.Conditions, offer.Conditions) || !stringSubset(refs.ValidationReceipt.ObservedBenefits, offer.Benefits) || !stringSubset(refs.ValidationReceipt.ObservedConditions, offer.Conditions) {
		return fault.Conflict("DOUYIN_COMMERCE_OFFER_FACTS_INVALID", "校验回执中的商品价格、币种、权益或条件与批准 Offer 不一致")
	}
	if refs.ValidationReceipt.ProjectID != projectID || refs.ValidationReceipt.ContentProfileID != work.DouyinCommerceProfileID {
		return fault.Conflict("DOUYIN_COMMERCE_RECEIPT_SCOPE_INVALID", "校验回执的项目、Profile 或时间格式不一致")
	}
	if refs.ValidationReceipt.AudienceStrategyApprovedSnapshotID != strategySnapshot.ID || refs.ValidationReceipt.OfferApprovedSnapshotID != offerSnapshot.ID || refs.ValidationReceipt.ContentApprovedSnapshotID != contentSnapshot.ID {
		return fault.Conflict("DOUYIN_COMMERCE_SNAPSHOT_LINEAGE_INVALID", "校验回执中的批准快照引用与实际对象不一致")
	}
	...
	if json.Unmarshal(storyboardRaw, &storyboard) != nil || storyboard.Type != "storyboard_package" {
		return fault.Invalid("DOUYIN_COMMERCE_STORYBOARD_INVALID", "分镜批准快照不包含有效分镜包")
	}
	...
	if err != nil || lockedDigest != storyboard.LockedDigest || lockedDigest != refs.ValidationReceipt.StoryboardLockedDigest || storyboard.ApprovedSnapshotID != contentSnapshot.ID || storyboard.ContentItemID != rendered.Item.ID {
		return fault.Conflict("DOUYIN_COMMERCE_STORYBOARD_LINEAGE_INVALID", "分镜锁定摘要、内容项或批准快照血缘不一致")
	}
	if refs.ValidationReceipt.StoryboardApprovedSnapshotID != storyboardSnapshot.ID || refs.ValidationReceipt.StoryboardPackageID != storyboard.ID {
		return fault.Conflict("DOUYIN_COMMERCE_STORYBOARD_REF_INVALID", "校验回执中的分镜引用与批准对象不一致")
	}
	if strategy.ProjectID != projectID || offer.ProjectID != projectID || contentSnapshot.ProjectID != projectID || storyboardSnapshot.ProjectID != projectID {
		return fault.Conflict("DOUYIN_COMMERCE_PROJECT_MISMATCH", "抖音电商批准对象不属于当前项目")
	}
	return nil
}

func (s *ReviewService) approvedObject(ctx context.Context, tenantID, projectID, submissionType, snapshotID, objectID string) (json.RawMessage, reviewdomain.ApprovedSnapshot, error) {
	...
	if snapshot.ProjectID != projectID || snapshot.SubmissionType != submissionType || !containsString(snapshot.EligibleIDs, objectID) {
		return nil, snapshot, fault.Conflict("DOUYIN_COMMERCE_APPROVED_REF_INVALID", "批准快照不属于当前项目、类型不匹配或未包含指定对象")
	}
