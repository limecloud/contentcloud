...
type RuntimeHealthReport struct {
	...
	CapacityFairness []contentruntime.RuntimeFairnessReport `json:"capacity_fairness"`
	Thresholds       RuntimeHealthThresholds                `json:"thresholds"`
	GeneratedAt      time.Time                              `json:"generated_at"`
}

func (s *RuntimeService) PlatformRuntimeHealth(ctx context.Context, actor Actor) (RuntimeHealthReport, error) {
	if !actor.PlatformAdmin {
		return RuntimeHealthReport{}, fault.Policy("PLATFORM_ADMIN_REQUIRED", "只有平台管理员可以访问 Runtime 健康状态", "联系系统管理员配置平台权限")
	}
	if s.runtimeService == nil {
		return RuntimeHealthReport{}, fault.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime")
	}
	now := s.now().UTC()
	report := RuntimeHealthReport{
		Status: "healthy", Tenants: []RuntimeTenantHealth{}, CapacityFairness: []contentruntime.RuntimeFairnessReport{}, GeneratedAt: now,
	...
	for _, tenant := range tenants {
		...
		health.ProjectionOutbox, err = s.runtimeService.RuntimeOutboxStats(ctx, tenant.ID, contentruntime.RuntimeOutboxSubscriberProjection)
		if err != nil {
			return RuntimeHealthReport{}, err
		}
		health.BusinessOutbox, err = s.runtimeService.RuntimeOutboxStats(ctx, tenant.ID, contentruntime.RuntimeOutboxSubscriberBusinessResult)
		if err != nil {
			return RuntimeHealthReport{}, err
		}
		health.Reaper, err = s.runtimeHealthHeartbeat(ctx, tenant.ID, contentruntime.RuntimeMaintenanceReaper, now, &health.Alerts)
		if err != nil {
			return RuntimeHealthReport{}, err
		}
		health.Delivery, err = s.runtimeHealthHeartbeat(ctx, tenant.ID, contentruntime.RuntimeMaintenanceDelivery, now, &health.Alerts)
		if err != nil {
			return RuntimeHealthReport{}, err
		}
...
func (s *RuntimeService) runtimeHealthHeartbeat(ctx context.Context, tenantID, kind string, now time.Time, alerts *[]RuntimeHealthAlert) (*contentruntime.RuntimeMaintenanceHeartbeat, error) {
...
func appendRuntimeBacklogAlert(alerts *[]RuntimeHealthAlert, now time.Time, stats contentruntime.RuntimeOutboxStats, code, message string) {
	if stats.Pending == 0 {
		return
	}
	age := time.Duration(0)
