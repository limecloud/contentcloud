...
type FetchSourceReceipt struct {
	...
	Revision      sourcedomain.SourceRevision `json:"revision"`
	Reused        bool                        `json:"reused"`
}

func (s *SourceService) SearchSources(ctx context.Context, actor Actor, input SearchSourcesInput, requestID string) (SearchSourcesResult, error) {
	if strings.TrimSpace(input.ProjectID) == "" {
		return SearchSourcesResult{}, fault.Invalid("SOURCE_SEARCH_PROJECT_REQUIRED", "搜索必须指定项目")
	}
	if _, err := s.workspace.Project(ctx, actor.TenantID, input.ProjectID); err != nil {
		return SearchSourcesResult{}, err
	}
	query := strings.TrimSpace(input.Query)
	if len([]rune(query)) < 2 || len([]rune(query)) > 500 {
		return SearchSourcesResult{}, fault.Invalid("SOURCE_SEARCH_QUERY_INVALID", "搜索词长度必须在 2 到 500 个字符之间")
	}
	limit := input.Limit
	if limit <= 0 {
		limit = 10
	...
	if limit > 20 {
		return SearchSourcesResult{}, fault.Invalid("SOURCE_SEARCH_LIMIT_INVALID", "搜索结果最多 20 条")
	}
	if s.sourceSearch == nil {
		return SearchSourcesResult{}, fault.Policy("SOURCE_SEARCH_UNAVAILABLE", "搜索 Provider 尚未配置", "配置 CONTENTCLOUD_SEARCH_ENDPOINT 或注入搜索适配器")
	}
	results, err := s.sourceSearch.Search(ctx, query, limit)
	if err != nil {
		return SearchSourcesResult{}, fault.Policy("SOURCE_SEARCH_FAILED", "搜索 Provider 暂时不可用", "检查 Provider 配置、网络和配额")
	}
	...
	searchDigest, err := stablehash.Sum(struct {
		ProjectID string `json:"project_id"`
		Query     string `json:"query"`
		Limit     int    `json:"limit"`
...
func (s *SourceService) FetchSource(ctx context.Context, actor Actor, input FetchSourceInput, requestID string) (FetchSourceReceipt, error) {
	...
	if requested == "" {
		return FetchSourceReceipt{}, fault.Invalid("SOURCE_FETCH_URL_REQUIRED", "采集必须指定 URL")
	}
	if s.sourceFetcher == nil {
		return FetchSourceReceipt{}, fault.Policy("SOURCE_FETCH_UNAVAILABLE", "采集器尚未配置", "检查 Source Provider 配置")
	}
	fetched, err := s.sourceFetcher.Fetch(ctx, requested)
	if err != nil {
		return FetchSourceReceipt{}, fault.Policy("SOURCE_FETCH_BLOCKED", "来源采集被安全策略或网络错误阻止", "检查 URL、域名白名单、重定向和响应大小")
	}
	...
	sources, err := s.source.Sources(ctx, actor.TenantID, input.ProjectID)
	if err != nil {
		return FetchSourceReceipt{}, err
	}
	for _, source := range sources {
		revisions, revisionErr := s.source.SourceRevisions(ctx, actor.TenantID, source.ID)
		if revisionErr != nil {
