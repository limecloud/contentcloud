...
type lineageBuilder struct {
	nodes map[string]projection.LineageNode
	edges []projection.LineageEdge
}

func newLineageBuilder() *lineageBuilder {
	return &lineageBuilder{nodes: map[string]projection.LineageNode{}, edges: []projection.LineageEdge{}}
}
...
func (b *lineageBuilder) node(objectType, id, label, status, stage string, createdAt time.Time, metadata map[string]any) {
	...
	b.nodes[lineageKey(objectType, id)] = projection.LineageNode{Key: lineageKey(objectType, id), Type: objectType, ID: id, Label: label, Status: status, Stage: stage, CreatedAt: createdAt, Metadata: metadata}
}
...
func (b *lineageBuilder) edge(fromType, fromID, toType, toID, relation, reason string) {
	...
	b.edges = append(b.edges, projection.LineageEdge{From: lineageKey(fromType, fromID), To: lineageKey(toType, toID), Relation: relation, Reason: reason})
}

func (s *OperationsService) ProjectLineage(ctx context.Context, actor Actor, projectID string, query LineageQuery) (projection.LineageGraph, error) {
	if _, err := s.workspace.Project(ctx, actor.TenantID, projectID); err != nil {
		return projection.LineageGraph{}, err
	}
	...
	if (query.FocusType == "") != (query.FocusID == "") {
		return projection.LineageGraph{}, fault.Invalid("LINEAGE_FOCUS_INVALID", "追踪对象类型（focus_type）和标识（focus_id）必须同时提供")
	}
	if query.Direction == "" {
		query.Direction = "both"
	}
	if query.Direction != "both" && query.Direction != "upstream" && query.Direction != "downstream" {
		return projection.LineageGraph{}, fault.Invalid("LINEAGE_DIRECTION_INVALID", "追踪方向（direction）必须是“上游（upstream）”“下游（downstream）”或“双向（both）”")
	}
...
func (s *OperationsService) ProjectImpact(ctx context.Context, actor Actor, projectID string, query LineageQuery) (projection.ImpactAnalysis, error) {
	query.Direction = "downstream"
	graph, err := s.ProjectLineage(ctx, actor, projectID, query)
	if err != nil {
		return projection.ImpactAnalysis{}, err
	}
	analysis := projection.ImpactAnalysis{ProjectID: projectID, Items: []projection.ImpactItem{}, GeneratedAt: graph.GeneratedAt}
	if graph.FocusKey == "" {
		for _, node := range graph.Nodes {
			...
			analysis.Items = append(analysis.Items, projection.ImpactItem{Node: node, Severity: severity, Reason: "对象当前状态需要人工处理", CurrentStatus: node.Status, RecommendedAction: action})
		}
		return analysis, nil
	}

	nodeByKey := make(map[string]projection.LineageNode, len(graph.Nodes))
	for _, node := range graph.Nodes {
	...
	for _, node := range graph.Nodes {
		...
		analysis.Items = append(analysis.Items, projection.ImpactItem{Node: node, Depth: itemDepth, Severity: severity, Reason: reasons[node.Key], CurrentStatus: node.Status, RecommendedAction: action})
	}
...
func (s *OperationsService) buildProjectLineage(ctx context.Context, tenantID, projectID string) (*lineageBuilder, error) {
	b := newLineageBuilder()
	sources, err := s.source.Sources(ctx, tenantID, projectID)
	if err != nil {
		return nil, err
	}
	revisions := []sourcedomain.SourceRevision{}
	for _, source := range sources {
		...
		items, loadErr := s.source.SourceRevisions(ctx, tenantID, source.ID)
		if loadErr != nil {
			return nil, loadErr
		}
	...
	rights, err := s.source.RightsRecords(ctx, tenantID, "")
	if err != nil {
		return nil, err
	}
	...
	knowledge, err := s.knowledge.KnowledgeObjects(ctx, tenantID, projectID)
	if err != nil {
		return nil, err
	}
	for _, object := range knowledge {
		...
		for _, evidenceID := range object.EvidenceRefs {
			span, spanErr := s.source.EvidenceSpan(ctx, tenantID, evidenceID)
			if spanErr == nil {
	...
	runs, err := s.app.Runtime.runtimeRunsForProject(ctx, tenantID, projectID)
	if err != nil {
		return nil, err
	}
	for _, run := range runs {
		...
		if snapshot, loadErr := s.contexts.Snapshot(ctx, tenantID, run.InputSnapshotID); loadErr == nil {
			for _, source := range snapshot.Sources {
				b.edge("source_revision", source.RevisionID, "runtime_run", run.ID, "frozen_into", "来源版本已固定到 Runtime 执行契约")
			}
		}
	}
	approvedSnapshots, err := s.review.ApprovedSnapshots(ctx, tenantID, projectID, "")
	if err != nil {
		return nil, err
	}
	...
	for _, snapshot := range approvedSnapshots {
		artifacts, loadErr := s.artifacts.ArtifactsByApprovedSnapshot(ctx, tenantID, snapshot.ID)
		if loadErr != nil {
			return nil, loadErr
		}
	...
	deliveryPackages, err := s.artifacts.DeliveryPackages(ctx, tenantID, projectID)
	if err != nil {
		return nil, err
	}
	...
	observations, err := s.performance.PerformanceObservations(ctx, tenantID, projectID)
	if err != nil {
		return nil, err
	}
...
func (b *lineageBuilder) graph(projectID string, query LineageQuery, generatedAt time.Time) (projection.LineageGraph, error) {
	focusKey := ""
	selected := map[string]bool{}
	if query.FocusID != "" {
		focusKey = lineageKey(query.FocusType, query.FocusID)
		if _, ok := b.nodes[focusKey]; !ok {
			return projection.LineageGraph{}, fault.NotFound("追踪对象")
		}
	...
	nodes := make([]projection.LineageNode, 0, len(selected))
	...
	edges := make([]projection.LineageEdge, 0, len(b.edges))
	...
	return projection.LineageGraph{ProjectID: projectID, FocusKey: focusKey, Direction: query.Direction, Nodes: nodes, Edges: edges, StageCount: stageCount, GeneratedAt: generatedAt}, nil
}

func connectedKeys(focus string, edges []projection.LineageEdge, direction string) map[string]bool {
...
func downstreamDepth(focus string, edges []projection.LineageEdge) (map[string]int, map[string]string) {
...
func lineageAdvisory(node projection.LineageNode) (severity, action string, include bool) {
...
func runtimeRunLabel(run work.RuntimeRun) string { return run.TaskType }

func observationLabel(observation performancedomain.PerformanceObservation) string {
	return fmt.Sprintf("%s · %dh · %s", observation.Platform, observation.WindowHours, observation.AccountAlias)
}

func ratingLabel(rating performancedomain.RatingDecision) string {
