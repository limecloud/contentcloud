...
func (s *OperationsService) EnsureMarketingVideoDemoFixture(ctx context.Context, actor Actor, requestID string) (MarketingVideoDemoFixtureResult, error) {
	if actor.Role != "tenant_admin" {
		return MarketingVideoDemoFixtureResult{}, fault.Policy("ROLE_DENIED", "只有租户管理员可以导入开发演示数据", "切换到租户管理员账号")
	}
	projects, err := s.app.Workspace.Projects(ctx, actor)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	var project workspacedomain.Project
	for _, candidate := range projects {
		if candidate.BrandName == "金陵古都" && candidate.ProductName == "金陵古都香" && candidate.ContentType == identitydomain.ContentTypeMarketingVideo {
			project = candidate
			break
		}
	}
	if err := s.identity.SetTenantContentCapability(ctx, identitydomain.TenantContentCapability{TenantID: actor.TenantID, ContentType: identitydomain.ContentTypeMarketingVideo, Enabled: true, UpdatedBy: actor.UserID, UpdatedAt: s.now().UTC()}); err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	if project.ID == "" {
		project, err = s.app.Workspace.CreateProject(ctx, actor, CreateProjectInput{
			BrandName: "金陵古都", ProductName: "金陵古都香", ContentType: identitydomain.ContentTypeMarketingVideo,
	...
	device, _, err := s.app.Review.ensureFixtureWorkspace(ctx, actor, project, fixturev3.WorkspaceSpec{
	...
	project, err = s.app.Workspace.Project(ctx, actor, project.ID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}

	tasks, err := s.app.Work.WorkTasks(ctx, actor, project.ID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	for _, candidate := range tasks {
		...
		view, viewErr := s.app.Work.WorkTask(ctx, actor, candidate.ID)
		if viewErr != nil {
			return MarketingVideoDemoFixtureResult{}, viewErr
		}
		if view.Task.Status != work.TaskStatusDelivered {
			return MarketingVideoDemoFixtureResult{}, fault.Policy("DEV_FIXTURE_MARKETING_VIDEO_INCOMPLETE", "已有营销视频演示任务未完整收敛", "检查任务当前流程阶段后重试开发环境初始化")
		}
	...
	task, err := s.app.Work.CreateWorkTask(ctx, actor, CreateWorkTaskInput{
		ProjectID: project.ID, Title: "别把南京放回抽屉｜26 秒短视频成片", Intent: "marketing_video",
		ContentType: identitydomain.ContentTypeMarketingVideo, InputRefs: []string{sourceRevision.ID, knowledgeSnapshot.ID},
	...
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputSourceRevision, ObjectID: sourceRevision.ID, Role: catalogdomain.StageOutputRolePrimary}}, map[string]any{"source.registered": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	...
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputKnowledgeSnapshot, ObjectID: knowledgeSnapshot.ID, Role: catalogdomain.StageOutputRolePrimary}}, map[string]any{"claim.references": true, "rights.references": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	...
	scriptRevision, err := s.app.Work.CreateTaskRevision(ctx, actor, task.Task.ID, CreateTaskRevisionInput{
		ContentType: identitydomain.ContentTypeMarketingVideo, SchemaVersion: "contentcloud.marketing_video_script/1.0",
	...
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputSubmissionRevision, ObjectID: scriptRevision.ID, ObjectVersion: scriptRevision.RevisionNo, Role: catalogdomain.StageOutputRolePrimary}}, map[string]any{"content.schema": true, "claim.references": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	...
	for _, asset := range storyboardAssets {
		if _, err := s.app.Delivery.UploadStoryboardArtifact(ctx, actor, task.Task.ID, UploadStoryboardArtifactInput{SnapshotID: storyboardSnapshot.ID, AssetID: asset.Asset.ID, FileName: asset.Asset.Path, Body: asset.Body}, fixtureRequestID(requestID, "storyboard-asset")); err != nil {
			return MarketingVideoDemoFixtureResult{}, err
		}
	}
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputStoryboardPackage, ObjectID: storyboardSnapshot.ID, Role: catalogdomain.StageOutputRolePrimary}}, map[string]any{"storyboard.locked": true, "rights.references": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	task, err = s.approveDemoGate(ctx, actor, task, requestID)
	...
	job, err := s.app.Delivery.CreateMediaGenerationJob(ctx, actor, task.Task.ID, CreateMediaGenerationJobInput{
	...
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputGenerationJob, ObjectID: job.ID, ObjectVersion: job.RowVersion, Role: catalogdomain.StageOutputRolePrimary}, {OutputType: catalogdomain.StageOutputArtifact, ObjectID: generatedArtifact.ID, Role: catalogdomain.StageOutputRolePreview}}, map[string]any{"media.technical": true, "cost.confirmed": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	...
	contentReview, err := demoMediaReview(task.MediaReviews, deliverydomain.MediaReviewContent)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	task, err = s.app.Delivery.DecideMediaReview(ctx, actor, contentReview.ID, MediaReviewDecisionInput{ExpectedVersion: contentReview.RowVersion, Decision: "approved", Reason: "画面遵守真实产品占位、无功效与无未授权地标边界", Selected: true, Checks: map[string]any{"media.content": true, "script.alignment": true, "rights.references": true}}, fixtureRequestID(requestID, "take-review"))
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	contentReview, err = demoMediaReview(task.MediaReviews, deliverydomain.MediaReviewContent)
	...
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputMediaReview, ObjectID: contentReview.ID, ObjectVersion: contentReview.RowVersion, Role: catalogdomain.StageOutputRoleSelectedTake}}, map[string]any{"media.technical": true, "media.content": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	task, err = s.approveDemoGate(ctx, actor, task, requestID)
	...
	deliveryPackage, err := s.app.Delivery.BuildTaskDeliveryPackage(ctx, actor, task.Task.ID, BuildTaskDeliveryPackageInput{FinalReviewID: finalReview.ID}, fixtureRequestID(requestID, "delivery-package"))
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	task, err = s.reportDemoStage(ctx, actor, task, []work.TaskStageOutput{{OutputType: catalogdomain.StageOutputDeliveryPackage, ObjectID: deliveryPackage.ID, Role: catalogdomain.StageOutputRoleFinal}}, map[string]any{"delivery.integrity": true}, requestID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	revisions, err := s.app.Work.WorkTaskRevisions(ctx, actor, task.Task.ID)
	if err != nil || len(revisions) == 0 {
		if err == nil {
			err = fault.NotFound("已接受的剧本内容版本")
		}
		return MarketingVideoDemoFixtureResult{}, err
	}
	deliver := true
	if _, err := s.app.Work.CreateTaskDelivery(ctx, actor, task.Task.ID, CreateTaskDeliveryInput{RevisionID: revisions[len(revisions)-1].ID, DeliveryPackageID: deliveryPackage.ID, Destination: "workspace", Deliver: &deliver}, fixtureRequestID(requestID, "delivery")); err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	finalView, err := s.app.Work.WorkTask(ctx, actor, task.Task.ID)
	if err != nil {
		return MarketingVideoDemoFixtureResult{}, err
	}
	project, err = s.app.Workspace.Project(ctx, actor, project.ID)
...
func (s *OperationsService) ensureMarketingVideoDemoDaemon(ctx context.Context, device workspacedomain.Device, project workspacedomain.Project) error {
	...
	workspaceObservation := workspacedomain.DaemonWorkspaceObservation{
	...
	instance := workspacedomain.DaemonInstance{
		...
		Capabilities: map[string]any{
			...
			"workspace_observations": []workspacedomain.DaemonWorkspaceObservation{workspaceObservation},
	...
	return s.workspace.SaveDevice(ctx, device)
}

func (s *OperationsService) createMarketingVideoDemoSource(ctx context.Context, actor Actor, project workspacedomain.Project) (sourcedomain.SourceRevision, []string, error) {
	now := s.now().UTC()
	...
	if err := s.blobs.Put(ctx, objectKey, body); err != nil {
		return sourcedomain.SourceRevision{}, nil, err
	}
	revision := sourcedomain.SourceRevision{ID: revisionID, TenantID: actor.TenantID, ProjectID: project.ID, SourceID: sourceID, FileName: "jinling-gudu-marketing-video-evidence.md", ObjectKey: objectKey, SHA256: mediapipeline.SHA256(body), ByteSize: int64(len(body)), DeclaredMIME: "text/markdown", DetectedMIME: "text/markdown", ProcessingStatus: "ready", ParserVersion: "dev-fixture-marketing-video", UploadedBy: actor.UserID, CreatedAt: now}
	if err := s.source.CreateSource(ctx, sourcedomain.Source{ID: sourceID, TenantID: actor.TenantID, ProjectID: project.ID, Name: "金陵古都香营销视频验收证据包", SourceType: "document", Status: "ready", RevisionCount: 1, LatestRevision: revision.ID, CreatedAt: now}, revision); err != nil {
		return sourcedomain.SourceRevision{}, nil, err
	}
	quotes := []string{
	...
	for index, quote := range quotes {
		id := idgen.New()
		span := sourcedomain.EvidenceSpan{ID: id, TenantID: actor.TenantID, ProjectID: project.ID, RevisionID: revision.ID, LocatorKind: "markdown", Locator: map[string]any{"section": index + 1, "source_ref": "marketing/jinling-gudu/outputs/scripts/jinling-douyin-20260803/11-drawer-reversal.md"}, QuoteText: quote, QuoteHash: mediapipeline.SHA256([]byte(quote)), ReviewStatus: "approved", ReviewedBy: actor.UserID, ReviewedAt: &now, CreatedAt: now}
...
func (s *OperationsService) createMarketingVideoDemoWorkspaceMaterial(ctx context.Context, actor Actor, project workspacedomain.Project, revision sourcedomain.SourceRevision) error {
	now := s.now().UTC()
	folder := workspacedomain.WorkspaceFolder{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, Name: "品牌与产品资料", CreatedBy: actor.UserID, CreatedAt: now, UpdatedAt: now}
	if err := s.workspace.CreateWorkspaceFolder(ctx, folder); err != nil {
		return err
	}
	material := workspacedomain.WorkspaceMaterial{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, FolderID: folder.ID, SourceID: revision.SourceID, SourceRevisionID: revision.ID, MaterialKind: workspacedomain.WorkspaceMaterialDocument, Origin: workspacedomain.WorkspaceMaterialUploaded, Usage: workspacedomain.WorkspaceMaterialProjectMaterial, Title: "金陵古都香项目资料", CreatedBy: actor.UserID, CreatedAt: now, UpdatedAt: now}
	return s.workspace.CreateWorkspaceMaterial(ctx, material)
}

func (s *OperationsService) createMarketingVideoDemoKnowledge(ctx context.Context, actor Actor, project workspacedomain.Project, source sourcedomain.SourceRevision, evidenceIDs []string) (sourcedomain.KnowledgeSnapshot, []string, string, error) {
	now := s.now().UTC()
	asset := sourcedomain.Asset{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, Name: "营销视频分镜开发示意素材", AssetType: "storyboard_fixture", SourceRevisionID: source.ID, UsageMode: "development_fixture", Status: "active", CreatedBy: actor.UserID, CreatedAt: now, UpdatedAt: now}
	if err := s.source.CreateAsset(ctx, asset); err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
	rights := sourcedomain.RightsRecord{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, AssetID: asset.ID, RightsHolder: "Content Work OS 开发演示环境", RightsType: "internal_demo", Territories: []string{"CN"}, Channels: []string{"internal_demo"}, ValidFrom: &now, ProofSourceRevisionID: source.ID, Restrictions: []string{"仅用于本地开发演示", "不得作为金陵古都香正式产品素材投放"}, Status: "valid", ReviewedBy: actor.UserID, ReviewedAt: &now, RowVersion: 1, CreatedAt: now, UpdatedAt: now}
	if err := s.source.CreateRightsRecord(ctx, rights); err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
	...
	objects := make([]sourcedomain.KnowledgeObject, 0, len(specs))
	refs := make([]sourcedomain.KnowledgePackObjectRef, 0, len(specs))
	ids := make([]string, 0, len(specs))
	for index, spec := range specs {
		object := sourcedomain.KnowledgeObject{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, ObjectType: spec.objectType, Layer: spec.layer, Version: 1, Status: spec.status, Title: spec.title, Statement: spec.statement, Payload: spec.payload, Dimensions: []string{fmt.Sprintf("marketing_video_demo_%d", index+1)}, AllowedChannels: []string{"douyin"}, EvidenceRefs: spec.evidence, RightsRefs: spec.rights, CreatedBy: actor.UserID, CreatedAt: now, UpdatedAt: now}
		digest, err := object.ContentDigest()
		if err != nil {
			return sourcedomain.KnowledgeSnapshot{}, nil, "", err
		}
		object.Digest = digest
		if err := s.knowledge.CreateKnowledgeObject(ctx, object); err != nil {
			return sourcedomain.KnowledgeSnapshot{}, nil, "", err
		}
		objects = append(objects, object)
		refs = append(refs, sourcedomain.KnowledgePackObjectRef{ObjectID: object.ID, Version: object.Version})
		ids = append(ids, object.ID)
	}
	pack := sourcedomain.KnowledgePack{ID: idgen.New(), TenantID: actor.TenantID, ProjectID: project.ID, Name: "金陵古都香｜抽屉逆转营销视频知识包", Purpose: "marketing_video", Version: 1, Status: "published", ObjectRefs: refs, QueryPolicy: sourcedomain.DefaultKnowledgeQueryPolicy(), CreatedBy: actor.UserID, PublishedBy: actor.UserID, CreatedAt: now, PublishedAt: &now}
	digest, err := pack.ContentDigest()
	if err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
	pack.Digest = digest
	if err := s.knowledge.CreateKnowledgePack(ctx, pack); err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
	snapshot, err := sourcedomain.BuildKnowledgeSnapshot(pack, objects, now)
	if err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
	if err := s.knowledge.CreateKnowledgeSnapshot(ctx, snapshot); err != nil {
		return sourcedomain.KnowledgeSnapshot{}, nil, "", err
	}
...
type demoStoryboardAsset struct {
	Asset work.StoryboardAsset
	Body  []byte
}

func (s *OperationsService) createMarketingVideoDemoStoryboard(ctx context.Context, actor Actor, task WorkTaskView, contentSnapshot reviewdomain.ApprovedSnapshot, knowledgeIDs []string, rightsID, requestID string) (reviewdomain.ApprovedSnapshot, []demoStoryboardAsset, error) {
	var envelope struct {
		Objects []json.RawMessage `json:"objects"`
	}
	if err := json.Unmarshal(contentSnapshot.CanonicalContent, &envelope); err != nil || len(envelope.Objects) != 1 {
		return reviewdomain.ApprovedSnapshot{}, nil, fault.Invalid("DEV_FIXTURE_CONTENT_SNAPSHOT_INVALID", "剧本批准快照缺少唯一的内容项")
	}
	sourceHash, err := stablehash.Sum(envelope.Objects[0])
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, nil, err
	}
	...
	storyboardShots := make([]work.StoryboardShot, 0, len(shots))
	assets := make([]demoStoryboardAsset, 0, len(shots)+1)
	for index, spec := range shots {
		body, imageErr := marketingVideoDemoFrame(index)
		if imageErr != nil {
			return reviewdomain.ApprovedSnapshot{}, nil, imageErr
		}
		shotID := fmt.Sprintf("shot-%02d", index+1)
		assetID := fmt.Sprintf("asset-%s-first-frame", shotID)
		asset := work.StoryboardAsset{ID: assetID, Role: "first_frame", ShotID: shotID, Path: fmt.Sprintf("50-production/media/%s/first-frame.png", shotID), MediaType: "image/png", SHA256: mediapipeline.SHA256(body), ByteSize: int64(len(body)), RightsRefs: []string{rightsID}}
		assets = append(assets, demoStoryboardAsset{Asset: asset, Body: body})
		storyboardShots = append(storyboardShots, work.StoryboardShot{ShotID: shotID, StartMS: spec.startMS, EndMS: spec.endMS, Role: spec.role, FirstFrameArtifactID: assetID, ImagePromptZH: spec.scene + "；仅作构图示意，不生成产品包装、标志或可读标签。", Subject: "周末书桌与旅行票根", Product: "金陵古都香真实素材占位", Scene: spec.scene, Composition: "9:16 竖屏，桌面主体位于中下三分区", Lighting: "同一扇窗的自然侧光", Camera: spec.camera, Action: spec.action, IncomingState: spec.incoming, OutgoingState: spec.outgoing, MovementAxis: "桌面纵深轴", LightingLock: "全片保持中性自然侧光", ProductLock: "不生成或修改包装、标志、标签", Anchors: []string{"旅行票根", "周末书桌"}, AssetRefs: []string{assetID}, RightsRefs: []string{rightsID}, KnowledgeRefs: append([]string(nil), knowledgeIDs...), NegativeConstraints: []string{"不生成产品包装和品牌标志", "不出现未授权南京地标", "不展示点燃、烟量或功效"}, AcceptanceCriteria: []string{"动作与剧本时间段一致", "产品只以审核素材占位表达", "画面保留 9:16 安全区"}, PlanB: "若产品素材未完成签批，仅保留中性占位和票根动作，不输出对外成片。"})
	}
	reviewBody, err := marketingVideoDemoFrame(len(shots))
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, nil, err
	}
	reviewAsset := work.StoryboardAsset{ID: "asset-review-sheet", Role: "review_sheet", Path: "50-production/media/review-sheet.png", MediaType: "image/png", SHA256: mediapipeline.SHA256(reviewBody), ByteSize: int64(len(reviewBody)), RightsRefs: []string{rightsID}}
	assets = append(assets, demoStoryboardAsset{Asset: reviewAsset, Body: reviewBody})
	storyboardAssets := make([]work.StoryboardAsset, 0, len(assets))
	for _, value := range assets {
		storyboardAssets = append(storyboardAssets, value.Asset)
	}
	storyboard := work.StoryboardPackage{ID: "storyboard:" + task.Task.ID, Type: "storyboard_package", SchemaVersion: work.StoryboardPackageSchema, ProjectID: task.Task.ProjectID, ApprovedSnapshotID: contentSnapshot.ID, ContentItemID: "content-item:" + task.Task.ID, GeneratorCapability: work.CapabilityRef{ID: "contentcloud.storyboard.fixture", Version: marketingVideoDemoFixtureVersion, Digest: "sha256:" + strings.Repeat("b", 64)}, Status: "review_ready", Shots: storyboardShots, Assets: storyboardAssets, ReviewSheetArtifactID: reviewAsset.ID, RightsRefs: []string{rightsID}, SourceDigest: "sha256:" + sourceHash}
	...
	binding, err := s.workspace.WorkspaceBinding(ctx, actor.TenantID, task.Task.ID)
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, nil, err
	}
	...
	revision, err := s.app.Review.CreateSubmission(ctx, workspaceActor, binding, bundle, fixtureRequestID(requestID, "storyboard-submission"))
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, nil, err
	}
	approval, err := s.app.Review.ApproveSubmission(ctx, actor, revision.ID, "分镜结构、素材摘要和权利边界已锁定", fixtureRequestID(requestID, "storyboard-approval"))
	if err != nil || approval.ApprovedSnapshot == nil {
		if err == nil {
			err = fault.NotFound("分镜已批准快照")
		}
		return reviewdomain.ApprovedSnapshot{}, nil, err
	}
...
func (s *OperationsService) startDemoStage(ctx context.Context, actor Actor, task WorkTaskView, requestID string) (WorkTaskView, error) {
	return s.app.Work.TaskAction(ctx, actor, task.Task.ID, TaskActionInput{Action: "start"}, fixtureRequestID(requestID, "start-"+task.Task.CurrentStageID))
}

func (s *OperationsService) reportDemoStage(ctx context.Context, actor Actor, task WorkTaskView, outputs []work.TaskStageOutput, checks map[string]any, requestID string) (WorkTaskView, error) {
	run := demoCurrentRun(task)
	if run.ID == "" {
		return WorkTaskView{}, fault.NotFound("当前流程阶段执行记录")
	}
	return s.app.Work.ReportStage(ctx, actor, task.Task.ID, StageReportInput{StageRunID: run.ID, StageID: run.StageID, Status: work.StageRunStatusCompleted, Outputs: outputs, Checks: checks}, fixtureRequestID(requestID, "report-"+run.StageID))
}

func (s *OperationsService) approveDemoGate(ctx context.Context, actor Actor, task WorkTaskView, requestID string) (WorkTaskView, error) {
	for _, gate := range task.Gates {
		if gate.Status == reviewdomain.GateEvaluationPending {
			return s.app.Work.DecideGate(ctx, actor, task.Task.ID, gate.ID, GateDecisionInput{Decision: "approved", Reason: "营销视频开发演示审核通过"}, fixtureRequestID(requestID, "gate-"+gate.GateID))
		}
	}
	return WorkTaskView{}, fault.NotFound("待处理的检查与审批项")
}

func (s *OperationsService) demoContentSnapshot(ctx context.Context, actor Actor, projectID, contentHash string) (reviewdomain.ApprovedSnapshot, error) {
	snapshots, err := s.app.Review.ApprovedSnapshots(ctx, actor, projectID, "content_batch")
	if err != nil {
		return reviewdomain.ApprovedSnapshot{}, err
	}
	...
	return reviewdomain.ApprovedSnapshot{}, fault.NotFound("剧本已批准快照")
}

func (s *OperationsService) waitForDemoMedia(ctx context.Context, actor Actor, taskID, jobID string) (WorkTaskView, error) {
	for attempt := 0; attempt < 80; attempt++ {
		job, err := s.delivery.MediaGenerationJob(ctx, actor.TenantID, jobID)
		if err != nil {
			return WorkTaskView{}, err
		}
		switch job.State {
		case deliverydomain.MediaJobQueued, deliverydomain.MediaJobRetryWait:
			if err := s.app.Delivery.ProcessMediaGenerationJob(ctx, actor.TenantID, job.ID); err != nil {
				return WorkTaskView{}, err
			}
		case deliverydomain.MediaJobSucceeded:
			return s.app.Work.WorkTask(ctx, actor, taskID)
		case deliverydomain.MediaJobFailed, deliverydomain.MediaJobCancelled, deliverydomain.MediaJobBudgetBlocked, deliverydomain.MediaJobOutputInvalid:
			return WorkTaskView{}, fault.Policy("DEV_FIXTURE_MEDIA_FAILED", "开发演示视频生成未成功", "检查模拟服务商输出和媒体校验")
		}
		select {
		case <-ctx.Done():
	...
	return WorkTaskView{}, fault.Policy("DEV_FIXTURE_MEDIA_TIMEOUT", "开发演示视频生成超时", "检查媒体执行器状态")
}

func demoCurrentRun(task WorkTaskView) work.StageRun {
...
func demoArtifact(artifacts []deliverydomain.Artifact, kind string) (deliverydomain.Artifact, error) {
	for _, artifact := range artifacts {
		if artifact.Kind == kind {
			return artifact, nil
		}
	}
	return deliverydomain.Artifact{}, fault.NotFound("媒体成果文件")
}

func demoMediaReview(reviews []deliverydomain.MediaReview, kind string) (deliverydomain.MediaReview, error) {
	for _, review := range reviews {
		if review.ReviewKind == kind {
			return review, nil
		}
	}
	return deliverydomain.MediaReview{}, fault.NotFound("媒体审核")
}
