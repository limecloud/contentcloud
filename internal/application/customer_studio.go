...
func (s *WorkService) CustomerStudioBootstrap(ctx context.Context, actor Actor, user identitydomain.User) (StudioBootstrap, error) {
	tenant, err := s.app.Identity.Tenant(ctx, actor)
	if err != nil {
		return StudioBootstrap{}, err
	}
	tenants, err := s.app.Identity.Tenants(ctx, actor)
	if err != nil {
		return StudioBootstrap{}, err
	}
...
func (s *WorkService) customerStudioExperiences(ctx context.Context, actor Actor, projects []workspacedomain.Project) ([]StudioExperience, error) {
	capabilities, err := s.identity.TenantContentCapabilities(ctx, actor.TenantID)
	if err != nil {
		return nil, err
	}
	enabledContentTypes := map[string]bool{identitydomain.ContentTypeVideoScript: true}
	...
	for _, project := range projects {
		...
		summary, summaryErr := s.catalog.SOP(ctx, actor.TenantID, sop.SOPID)
		if summaryErr != nil {
...
func customerStudioExperienceIdentity(definition catalogdomain.SOPDefinition, sop catalogdomain.SOPVersion) (string, string) {
...
func (s *WorkService) customerStudioExperienceSOP(ctx context.Context, actor Actor, project workspacedomain.Project, experienceID string) (StudioExperience, catalogdomain.SOPDefinition, catalogdomain.SOPVersion, error) {
	...
	if err != nil {
		return StudioExperience{}, catalogdomain.SOPDefinition{}, catalogdomain.SOPVersion{}, err
	}
	for _, experience := range experiences {
		if experience.ID != experienceID || !containsString(experience.ProjectIDs, project.ID) {
			continue
		}
		definition, sop, loadErr := s.loadTaskSOP(ctx, actor.TenantID, work.WorkTask{TenantID: actor.TenantID, ProjectID: project.ID, SOPID: experience.SOPID, SOPVersion: experience.SOPVersion})
		if loadErr != nil {
			return StudioExperience{}, catalogdomain.SOPDefinition{}, catalogdomain.SOPVersion{}, loadErr
		}
		return experience, definition, sop, nil
	}
	return StudioExperience{}, catalogdomain.SOPDefinition{}, catalogdomain.SOPVersion{}, fault.Policy("STUDIO_EXPERIENCE_UNAVAILABLE", "当前项目尚未启用这条创作流水线", "请联系运营人员检查租户能力、项目和已发布流程版本")
}

func (s *WorkService) CustomerStudioTasks(ctx context.Context, actor Actor) ([]StudioTaskSummary, error) {
	...
	for _, task := range tasks {
		project, projectErr := s.workspace.Project(ctx, actor.TenantID, task.ProjectID)
		if projectErr != nil {
...
func (s *WorkService) CustomerStudioTask(ctx context.Context, actor Actor, taskID string) (StudioTaskView, error) {
...
func (s *WorkService) CreateCustomerStudioTask(ctx context.Context, actor Actor, input StudioCreateTaskInput, requestID string) (StudioTaskView, error) {
	if !canCreateStudioTask(actor.Role) {
		return StudioTaskView{}, fault.Policy("ROLE_DENIED", "当前角色不能创建创作任务", "联系团队管理员调整创作权限")
	}
	if strings.TrimSpace(input.Title) == "" || strings.TrimSpace(input.Goal) == "" {
		return StudioTaskView{}, fault.Invalid("STUDIO_TASK_FIELDS_REQUIRED", "任务名称和创作目标必填")
	}
	...
	if project.ConnectedDevices == 0 {
		return StudioTaskView{}, fault.Conflict("STUDIO_EXECUTION_CLIENT_REQUIRED", "开始新的创作前，需要先连接你的创作电脑")
	}
	...
	materialRefs, err := s.app.Source.resolveCustomerWorkspaceMaterialRefs(ctx, actor, project.ID, input.MaterialRefs)
	if err != nil {
		return StudioTaskView{}, err
	}
	...
	if err := s.app.Source.markCustomerWorkspaceMaterialsUsed(ctx, actor, project.ID, input.MaterialRefs); err != nil {
		return StudioTaskView{}, err
	}
	if brief.TargetTaskID != created.Task.ID || brief.Status != work.InputItemTaskMerged {
...
func (s *WorkService) ensureCustomerStudioRuntime(ctx context.Context, actor Actor, task work.WorkTask, sop catalogdomain.SOPVersion, idempotencyKey, requestID string) error {
	if s.runtimeService == nil {
		err := fault.Policy("RUNTIME_UNAVAILABLE", "创作任务已建立，但运行时尚未配置", "请联系平台运营人员启用 Agentic Job Runtime")
	...
	if err != nil {
		...
		return fault.Policy("RUNTIME_ADMISSION_DIGEST_FAILED", "创作任务的运行时准入快照无法冻结", "请检查任务输入和执行绑定后重试")
	}
	...
	if err != nil {
		...
		return fault.Policy("RUNTIME_EXECUTION_BINDING_FAILED", "创作任务的执行环境绑定无法冻结", "请检查执行环境和插件配置后重试")
	}
	if _, err := s.runtimeService.Start(ctx, contentruntime.StartInput{
		...
		return fault.Policy("RUNTIME_START_FAILED", "创作任务已建立，但执行运行时启动失败", "请在运营后台的 Runtime 运行页刷新或联系平台运营人员处理")
	}
...
func customerStudioRuntimeInputDigest(task work.WorkTask) (string, error) {
...
func (s *WorkService) CustomerStudioTaskAction(ctx context.Context, actor Actor, taskID string, input TaskActionInput, requestID string) (StudioTaskView, error) {
...
func (s *WorkService) AddCustomerStudioInspiration(ctx context.Context, actor Actor, taskID string, input StudioAddInspirationInput, requestID string) (StudioTaskView, error) {
	if !canCreateStudioTask(actor.Role) {
		return StudioTaskView{}, fault.Policy("ROLE_DENIED", "当前角色不能补充创作资料", "联系团队管理员调整创作权限")
	}
...
func (s *WorkService) DecideCustomerStudioTask(ctx context.Context, actor Actor, taskID, decisionID string, input GateDecisionInput, requestID string) (StudioTaskView, error) {
...
func (s *WorkService) AttachCustomerStudioAssets(ctx context.Context, actor Actor, taskID string, input StudioAttachAssetsInput, requestID string) (StudioTaskView, error) {
	if !canCreateStudioTask(actor.Role) {
		return StudioTaskView{}, fault.Policy("ROLE_DENIED", "当前角色不能修改任务资料", "联系团队管理员调整创作权限")
	}
	task, err := s.tasks.WorkTask(ctx, actor.TenantID, taskID)
	if err != nil {
		return StudioTaskView{}, err
	...
	if task.Status == work.TaskStatusDelivered || task.Status == work.TaskStatusCancelled {
		return StudioTaskView{}, fault.Conflict("STUDIO_TASK_INPUT_CLOSED", "已完成或已取消的任务不能继续加入资产")
	}
...
func (s *WorkService) CustomerStudioCreativeResults(ctx context.Context, actor Actor, projectID string) (CreativeResultAssetProjection, error) {
	projects, err := s.app.Workspace.Projects(ctx, actor)
	if err != nil {
	...
	if projectID != "" {
		...
		projects = filtered
		if len(projects) == 0 {
			return CreativeResultAssetProjection{}, fault.NotFound("项目")
		}
	}
...
func (s *WorkService) CustomerStudioCreativeResult(ctx context.Context, actor Actor, taskID, resultID string) (StudioCreativeResultDetail, error) {
	...
	if item == nil {
		return StudioCreativeResultDetail{}, fault.NotFound("创作结果")
	}
...
func (s *WorkService) customerStudioProjectAssets(ctx context.Context, actor Actor, project workspacedomain.Project) ([]StudioAssetItem, error) {
...
func customerStudioTaskAssets(view WorkTaskView) []StudioAssetItem {
	...
	for _, output := range view.StageOutputs {
		if output.OutputType == catalogdomain.StageOutputKnowledgeSnapshot && studioOutputCustomerStep(view, output.StageRunID) == "persona" {
	...
	for _, output := range view.StageOutputs {
		if output.OutputType == catalogdomain.StageOutputArtifact && output.Role == catalogdomain.StageOutputRoleFinal {
...
func studioRevisionAssetStatus(status string) string {
	switch status {
	case reviewdomain.TaskRevisionAccepted:
		return "confirmed"
	case reviewdomain.TaskRevisionRejected:
		return "changes_requested"
	case reviewdomain.TaskRevisionSuperseded:
		return "superseded"
	case reviewdomain.TaskRevisionDraft:
		return "draft"
	default:
...
func studioSnapshotResultType(view WorkTaskView, snapshot reviewdomain.ApprovedSnapshot) string {
	for _, output := range view.StageOutputs {
...
func studioArtifactResultType(artifact deliverydomain.Artifact) string {
	switch {
...
func studioArtifactAssetStatus(view WorkTaskView, artifact deliverydomain.Artifact, final bool) string {
	if final && view.Task.Status == work.TaskStatusDelivered {
		return "delivered"
	}
	...
	for _, review := range view.MediaReviews {
		...
		case deliverydomain.MediaReviewPending:
			pending = true
...
func (s *WorkService) resolveCustomerStudioAssetRefs(ctx context.Context, actor Actor, projectID string, refs []string) ([]string, error) {
	if len(refs) == 0 {
		return []string{}, nil
	}
	...
	for _, ref := range uniqueNonEmpty(refs) {
		...
		if !ok {
			return nil, fault.Policy("STUDIO_ASSET_NOT_REUSABLE", "所选创作结果尚未确认或不属于当前项目", "刷新创作结果并选择已确认的版本")
		}
...
func (s *WorkService) CustomerStudioDeliveries(ctx context.Context, actor Actor) (StudioDeliveries, error) {
	projects, err := s.app.Workspace.Projects(ctx, actor)
	if err != nil {
	...
	for _, project := range projects {
		...
		packages, packageErr := s.app.Review.DeliveryPackages(ctx, actor, project.ID)
		if packageErr != nil {
	...
	publications, err := s.app.Delivery.ChannelPublications(ctx, actor, "")
	if err != nil {
		return StudioDeliveries{}, err
	}
...
func studioProject(project workspacedomain.Project) StudioProject {
...
func (s *WorkService) CreateCustomerStudioConnectSession(ctx context.Context, actor Actor, projectID, requestID string) (StudioConnectSession, error) {
	if !canConnectStudioClient(actor.Role) {
		return StudioConnectSession{}, fault.Policy("ROLE_DENIED", "当前账号不能连接创作电脑", "请联系团队负责人开通创作权限")
	}
	session, err := s.app.Workspace.createConnectSession(ctx, actor, strings.TrimSpace(projectID), requestID)
	return studioConnectSession(session), err
}

func (s *WorkService) CustomerStudioConnectSession(ctx context.Context, actor Actor, sessionID string) (StudioConnectSession, error) {
	session, err := s.app.Workspace.ConnectSession(ctx, actor, strings.TrimSpace(sessionID))
	return studioConnectSession(session), err
}

func (s *WorkService) ApproveCustomerStudioConnectSession(ctx context.Context, actor Actor, sessionID, requestID string) (StudioConnectSession, error) {
	session, err := s.app.Workspace.ConnectSession(ctx, actor, strings.TrimSpace(sessionID))
	if err != nil {
		return StudioConnectSession{}, err
	}
	if session.Progress == nil || session.Progress.AttemptID == "" || session.Progress.UserCode == "" {
		return studioConnectSession(session), fault.Conflict("STUDIO_CLIENT_CONFIRMATION_NOT_READY", "Codex 尚未发起连接确认")
	}
	if !canConnectStudioClient(actor.Role) {
		return studioConnectSession(session), fault.Policy("ROLE_DENIED", "当前账号不能确认创作电脑连接", "请联系团队负责人开通创作权限")
	}
	if _, err := s.app.Workspace.approveBootstrapAuthorization(ctx, actor, session.ID, session.Progress.AttemptID, requestID, true); err != nil {
		return studioConnectSession(session), err
	}
	session, err = s.app.Workspace.ConnectSession(ctx, actor, session.ID)
	return studioConnectSession(session), err
}

func (s *WorkService) DenyCustomerStudioConnectSession(ctx context.Context, actor Actor, sessionID, requestID string) (StudioConnectSession, error) {
	session, err := s.app.Workspace.ConnectSession(ctx, actor, strings.TrimSpace(sessionID))
	if err != nil {
		return StudioConnectSession{}, err
	}
	...
	if !canConnectStudioClient(actor.Role) {
		return studioConnectSession(session), fault.Policy("ROLE_DENIED", "当前账号不能拒绝创作电脑连接", "请联系团队负责人开通创作权限")
	}
	if _, err := s.app.Workspace.denyBootstrapAuthorization(ctx, actor, session.ID, session.Progress.AttemptID, requestID, true); err != nil {
		return studioConnectSession(session), err
	}
	session, err = s.app.Workspace.ConnectSession(ctx, actor, session.ID)
	return studioConnectSession(session), err
...
func (s *WorkService) CancelCustomerStudioConnectSession(ctx context.Context, actor Actor, sessionID, requestID string) (StudioConnectSession, error) {
	if !canConnectStudioClient(actor.Role) {
		return StudioConnectSession{}, fault.Policy("ROLE_DENIED", "当前账号不能取消创作电脑连接", "请联系团队负责人开通创作权限")
	}
	session, err := s.app.Identity.cancelConnectSession(ctx, actor, strings.TrimSpace(sessionID), requestID)
	return studioConnectSession(session), err
}

func studioConnectSession(session workspacedomain.ConnectSession) StudioConnectSession {
...
func studioTaskSummary(task work.WorkTask, project workspacedomain.Project, sop catalogdomain.SOPVersion) StudioTaskSummary {
...
func customerStudioExperienceStepTitles(sop catalogdomain.SOPVersion) []string {
...
func studioCustomerSteps(task work.WorkTask, sop catalogdomain.SOPVersion) []StudioCustomerStep {
	templates := marketingStudioCustomerStepTemplates()
	if !isMarketingStudioSOP(sop) {
	...
	for index := range templates {
		status := "not_started"
		summary := "等待前序结果确认"
		if task.Status == work.TaskStatusDelivered || index < currentIndex {
			status, summary = "completed", "本步骤结果已经确认"
		} else if index == currentIndex {
			switch task.Status {
			case work.TaskStatusNeedsInput:
				status, summary = "needs_input", "还需要补充创作资料"
			case work.TaskStatusWaitingGate:
				status, summary = "needs_decision", "当前结果等待你确认"
			case work.TaskStatusBlocked:
				status, summary = "blocked", "当前步骤需要协助"
			case work.TaskStatusRunning:
				status, summary = "working", "创作流水线正在处理"
			case work.TaskStatusPaused:
...
func studioCustomerStepID(stageID string, sop catalogdomain.SOPVersion) string {
...
func studioExperienceIDForSOP(sop catalogdomain.SOPVersion) string {
	if isMarketingStudioSOP(sop) {
...
func isMarketingStudioSOP(sop catalogdomain.SOPVersion) bool {
...
func studioTaskStatusLabel(status string) string {
	return map[string]string{work.TaskStatusNeedsInput: "需要补充资料", work.TaskStatusReady: "可以开始", work.TaskStatusRunning: "正在创作", work.TaskStatusPaused: "已暂停", work.TaskStatusWaitingGate: "等待你确认", work.TaskStatusBlocked: "需要协助", work.TaskStatusAccepted: "准备交付", work.TaskStatusDelivered: "已完成", work.TaskStatusCancelled: "已取消"}[status]
}

func studioNextAction(task work.WorkTask) string {
	switch task.Status {
	case work.TaskStatusWaitingGate:
		return "查看并确认当前结果"
	case work.TaskStatusNeedsInput:
	...
	case work.TaskStatusBlocked:
		return "查看需要协助的原因"
	case work.TaskStatusDelivered:
		return "查看交付成果"
	case work.TaskStatusReady:
		return "开始创作"
	case work.TaskStatusPaused:
		return "继续创作"
	}
...
func canDecideCustomerStudioGate(role, mode string) bool {
	...
	if mode == catalogdomain.GateModeClientDecision {
...
func taskRevisionTitle(value reviewdomain.TaskRevision) string {
...
func studioDownload(artifact deliverydomain.Artifact) StudioDownload {
