...
func Seedance25ProviderFromEnv(artifacts persistence.ArtifactRepository, review persistence.ReviewRepository, blobs blob.Store) (*mediapipeline.Seedance25Provider, error) {
	...
	if len(allowedHosts) == 0 {
		return nil, fault.Policy("SEEDANCE_ALLOWED_HOSTS_REQUIRED", "Seedance 2.5 生产配置必须声明 API 和输出下载域名白名单", "设置 CONTENTCLOUD_SEEDANCE25_ALLOWED_HOSTS")
	}
	return mediapipeline.NewSeedance25Provider(mediapipeline.Seedance25ProviderConfig{
		...
		Resolver:           NewSeedance25ArtifactResolver(artifacts, review, blobs),
	})
}
