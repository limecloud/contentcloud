...
type ImportPerformanceResult struct {
	...
	Batch        performancedomain.PerformanceImportBatch   `json:"batch"`
	Observations []performancedomain.PerformanceObservation `json:"observations"`
}
...
type CreateRatingDecisionResult struct {
	DryRun   bool                             `json:"dry_run"`
	Decision performancedomain.RatingDecision `json:"decision"`
}

func (s *PerformanceService) ImportPerformanceObservations(ctx context.Context, actor Actor, in ImportPerformanceInput, requestID string) (ImportPerformanceResult, error) {
	result := ImportPerformanceResult{DryRun: in.DryRun, Observations: []performancedomain.PerformanceObservation{}}
	if actor.Type != "user" {
		return result, fault.Policy("USER_ACTOR_REQUIRED", "只有登录用户可以导入业务结果", "使用用户 CLI 凭据或网页会话")
	}
	...
	if len(in.Observations) == 0 || len(in.Observations) > maxPerformanceImportRows {
		return result, fault.Invalid("RESULT_IMPORT_SIZE_INVALID", fmt.Sprintf("每批结果必须包含 1 到 %d 行", maxPerformanceImportRows))
	}
	...
	if format != "manual" && format != "json" && format != "csv" && format != "xlsx" {
		return result, fault.Invalid("RESULT_SOURCE_FORMAT_INVALID", "结果来源格式必须是手动录入（manual）、JSON、CSV 或 XLSX")
	}
	...
	if len(sourceName) > 255 || dangerousSpreadsheetText(sourceName) {
		return result, fault.Invalid("RESULT_SOURCE_NAME_INVALID", "结果来源名称无效")
	}

	now := s.now().UTC()
	errorsByRow := []performancedomain.PerformanceImportRowError{}
	observations := make([]performancedomain.PerformanceObservation, 0, len(in.Observations))
	dedupRows := map[string]int{}
	batchCurrency := ""
	snapshotCache := map[string]reviewdomain.ApprovedSnapshot{}
	...
	for index, raw := range in.Observations {
		...
		if strings.TrimSpace(raw.ApprovedSnapshotID) != "" {
			...
			var loadErr error
			if !cached {
				cachedSnapshot, loadErr = s.review.ApprovedSnapshot(ctx, actor.TenantID, strings.TrimSpace(raw.ApprovedSnapshotID))
				if loadErr == nil {
		...
		dedupKey := performanceDedupKey(in.ProjectID, strings.TrimSpace(raw.ApprovedSnapshotID), raw.Platform, raw.AccountAlias, publishedAt, raw.WindowHours)
		...
		observations = append(observations, performancedomain.PerformanceObservation{
			ID:                 idgen.New(),
	...
	if len(keys) > 0 {
		existing, err := s.performance.ExistingPerformanceDedupKeys(ctx, actor.TenantID, in.ProjectID, keys)
		if err != nil {
			return result, err
		}
		for _, observation := range observations {
			if existingID := existing[observation.DedupKey]; existingID != "" {
				errorsByRow = append(errorsByRow, importRowError(observation.RowNumber, "", "RESULT_DUPLICATE_OBSERVATION", "相同批准快照、平台、账号、发布时间和观察窗口的结果已存在"))
			}
		}
	}
	if len(errorsByRow) > 0 {
		...
		err := fault.Invalid("RESULT_IMPORT_REJECTED", "结果批次校验失败，未写入任何观察")
	...
	if sourceSHA == "" {
		var err error
		sourceSHA, err = stablehash.Sum(in.Observations)
		if err != nil {
			return result, err
		}
	}
	if !isLowerHexSHA256(sourceSHA) {
		return result, fault.Invalid("RESULT_SOURCE_HASH_INVALID", "结果来源的 SHA-256 必须为 64 位小写十六进制")
	}
	batch := performancedomain.PerformanceImportBatch{
		ID:            idgen.New(),
		TenantID:      actor.TenantID,
		ProjectID:     in.ProjectID,
	...
	if err := s.performance.CreatePerformanceImportBatch(ctx, batch, observations); err != nil {
		return result, err
	}
...
func validatePerformanceInput(rowNumber int, projectID string, in CreateObservationInput) []performancedomain.PerformanceImportRowError {
	errors := []performancedomain.PerformanceImportRowError{}
	...
	if strings.TrimSpace(in.ApprovedSnapshotID) == "" {
		add("approved_snapshot_id", "RESULT_VERSION_REQUIRED", "approved_snapshot_id 必填")
	}
...
func (s *PerformanceService) PerformanceObservations(ctx context.Context, actor Actor, projectID string) ([]performancedomain.PerformanceObservation, error) {
	return s.performance.PerformanceObservations(ctx, actor.TenantID, projectID)
}

func (s *PerformanceService) PerformanceImportBatches(ctx context.Context, actor Actor, projectID string) ([]performancedomain.PerformanceImportBatch, error) {
	return s.performance.PerformanceImportBatches(ctx, actor.TenantID, projectID)
}

func (s *PerformanceService) PerformanceImportDetails(ctx context.Context, actor Actor, id string) (performancedomain.PerformanceImportDetails, error) {
	batch, err := s.performance.PerformanceImportBatch(ctx, actor.TenantID, id)
	if err != nil {
		return performancedomain.PerformanceImportDetails{}, err
	}
	all, err := s.performance.PerformanceObservations(ctx, actor.TenantID, batch.ProjectID)
	if err != nil {
		return performancedomain.PerformanceImportDetails{}, err
	}
	items := []performancedomain.PerformanceObservation{}
	for _, observation := range all {
		if observation.ImportBatchID == batch.ID {
			items = append(items, observation)
		}
	}
	return performancedomain.PerformanceImportDetails{Batch: batch, Observations: items}, nil
}

func (s *PerformanceService) CreateRatingDecision(ctx context.Context, actor Actor, in CreateRatingDecisionInput, requestID string) (CreateRatingDecisionResult, error) {
	result := CreateRatingDecisionResult{DryRun: in.DryRun}
	if actor.Type != "user" {
		return result, fault.Policy("USER_ACTOR_REQUIRED", "只有登录用户可以创建评级决策", "使用用户 CLI 凭据或网页会话")
	}
	if err := requireRole(actor, "tenant_admin", "project_manager", "strategist"); err != nil {
		return result, err
	}
	...
	if !performanceSampleStatuses[in.Rating] {
		return result, fault.Invalid("RATING_INVALID", "评级必须是小范围测试候选（seed_candidate）、可修复（repairable）、不采用（discarded）或样本不足（insufficient_sample）")
	}
	if strings.TrimSpace(in.Reason) == "" || strings.TrimSpace(in.NextAction) == "" {
		return result, fault.Invalid("RATING_REASON_REQUIRED", "评级依据和下一步动作必填")
	}
	if len(in.ObservationIDs) == 0 || len(in.ObservationIDs) > 100 {
		return result, fault.Invalid("RATING_OBSERVATIONS_INVALID", "评级必须引用 1 到 100 条结果观察")
	}
	...
	if len(observationIDs) != len(in.ObservationIDs) {
		return result, fault.Invalid("RATING_OBSERVATIONS_DUPLICATE", "评级引用不能包含空值或重复观察")
	}
	for _, id := range observationIDs {
		observation, err := s.performance.PerformanceObservation(ctx, actor.TenantID, id)
		if err != nil || observation.ProjectID != in.ProjectID {
			return result, fault.NotFound("结果观察")
		}
		if observation.ApprovedSnapshotID != in.SubjectID {
			return result, fault.Conflict("RATING_OBSERVATION_SUBJECT_MISMATCH", "评级引用的结果观察不属于当前版本主体")
		}
	}
	decision := performancedomain.RatingDecision{
		ID:             idgen.New(),
		TenantID:       actor.TenantID,
		ProjectID:      in.ProjectID,
	...
	if err := s.performance.CreateRatingDecision(ctx, decision); err != nil {
		return result, err
	}
...
func (s *PerformanceService) RatingDecisions(ctx context.Context, actor Actor, projectID string) ([]performancedomain.RatingDecision, error) {
	return s.performance.RatingDecisions(ctx, actor.TenantID, projectID)
}

func (s *PerformanceService) validateRatingSubject(ctx context.Context, actor Actor, projectID, subjectType, subjectID string) error {
	if strings.TrimSpace(subjectID) == "" {
		return fault.Invalid("RATING_SUBJECT_REQUIRED", "评级对象必填")
	}
...
func performanceDedupKey(projectID, approvedSnapshotID, platform, account string, publishedAt time.Time, windowHours int) string {
	value := strings.Join([]string{projectID, strings.TrimSpace(approvedSnapshotID), strings.ToLower(strings.TrimSpace(platform)), strings.ToLower(strings.TrimSpace(account)), publishedAt.UTC().Format(time.RFC3339Nano), fmt.Sprintf("%d", windowHours)}, "\n")
	return idgen.TokenHash(value)
}

func importRowError(rowNumber int, field, code, message string) performancedomain.PerformanceImportRowError {
	return performancedomain.PerformanceImportRowError{RowNumber: rowNumber, Field: field, Code: code, Message: message}
}
