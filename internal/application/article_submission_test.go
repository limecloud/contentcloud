...
func TestArticleSubmissionRequiresTenantCapabilityAndApprovedEvidence(t *testing.T) {
	service := application.New(application.DependenciesFrom(memory.New()), slog.Default(), application.WithPlatformAdminEmails("article-admin@example.com"))
	session, err := service.Identity.Register(t.Context(), "article-admin@example.com", "long-enough-password", "Article Admin", "Article Tenant")
	must(t, err)
	actor, _, err := service.Identity.SessionActor(t.Context(), session.ID)
	must(t, err)
	project, err := service.Workspace.CreateProject(t.Context(), actor, application.CreateProjectInput{BrandName: "Brand", ProductName: "Product", Channel: localworkspace.WeChatChannel}, "article-project")
	must(t, err)
	connect, err := service.Workspace.CreateConnectSession(t.Context(), actor, project.ID, "article-connect")
	must(t, err)
	connected, err := testsupport.ConnectBootstrap(t.Context(), service, actor, connect, application.ConnectDeviceInput{Hostname: "article-test"})
	must(t, err)
	...
	_, err = service.Identity.UpdatePlatformTenantContentCapability(t.Context(), actor, actor.TenantID, identitydomain.ContentTypeWeChatArticle, false, "disable-wechat")
	must(t, err)
	...
	_, err = service.Identity.UpdatePlatformTenantContentCapability(t.Context(), actor, actor.TenantID, identitydomain.ContentTypeWeChatArticle, true, "re-enable-wechat")
	must(t, err)
...
func publishAndApproveArticleFixture(t *testing.T, service *application.Application, actor, workspaceActor application.Actor, binding workspacedomain.WorkspaceBinding, submissionType string, values []fixtureSubmissionObject, baseSnapshotIDs []string, key string) reviewdomain.ApprovedSnapshot {
	t.Helper()
	objects := make([]reviewdomain.SubmissionObjectRef, 0, len(values))
	for _, value := range values {
		object, err := reviewdomain.NewSubmissionObjectRef(value.ID, value.Type, 1, submissionType+"/"+value.ID+".json", value.Content)
		must(t, err)
		objects = append(objects, object)
	}
	bundle := reviewdomain.SubmissionBundle{BundleVersion: "3.0", SubmissionType: submissionType, ProjectID: binding.ProjectID, WorkspaceID: binding.ID, BaseSnapshotIDs: baseSnapshotIDs, EnvironmentDigest: submissionEnvironmentDigest, Objects: objects, SourceDisclosures: []reviewdomain.SourceDisclosure{}, Artifacts: []reviewdomain.SubmissionArtifact{}, LocalRunSummary: reviewdomain.LocalRunSummary{Checks: []reviewdomain.LocalRunCheck{{Name: submissionType + "-lint", Status: "passed"}}}, IdempotencyKey: key}
	must(t, bundle.SetComputedHash())
	revision, err := service.Review.CreateSubmission(t.Context(), workspaceActor, binding, bundle, key)
	must(t, err)
	approved, err := service.Review.ApproveSubmission(t.Context(), actor, revision.ID, "fixture approved", key+"-approve")
	must(t, err)
...
func createArticleContentSubmission(t *testing.T, service *application.Application, workspaceActor application.Actor, binding workspacedomain.WorkspaceBinding, item localworkspace.ArticleItem, baseSnapshotIDs []string, key string) (reviewdomain.SubmissionRevision, error) {
	t.Helper()
	object, err := reviewdomain.NewSubmissionObjectRef(item.ID, item.Type, 1, "50-production/batches/"+item.ContentBatchID+"/items/"+item.ID+".json", item)
	if err != nil {
	...
	return service.Review.CreateSubmission(t.Context(), workspaceActor, binding, bundle, key)
}
