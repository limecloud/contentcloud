...
func (s *SourceService) ConnectorAdapterIDs() []string { return s.connectorAdapters.IDs() }

func (s *SourceService) CreateConnectorBinding(ctx context.Context, actor Actor, input CreateConnectorBindingInput, requestID string) (connector.Binding, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager"); err != nil {
		return connector.Binding{}, err
	}
	if s.connectorRepository == nil {
		return connector.Binding{}, fault.Policy("CONNECTOR_STORE_UNAVAILABLE", "Connector 持久化未配置", "配置 Connector Repository 后重试")
	}
...
func (s *SourceService) ConnectorBindings(ctx context.Context, actor Actor, projectID string) ([]connector.Binding, error) {
	if s.connectorRepository == nil {
		return nil, fault.Policy("CONNECTOR_STORE_UNAVAILABLE", "Connector 持久化未配置", "配置 Connector Repository 后重试")
	}
...
func (s *SourceService) SyncConnector(ctx context.Context, actor Actor, bindingID string, input SyncConnectorInput, requestID string) (connector.SyncReceipt, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "strategist", "editor"); err != nil {
		return connector.SyncReceipt{}, err
	}
	if s.connectorRepository == nil {
		return connector.SyncReceipt{}, fault.Policy("CONNECTOR_STORE_UNAVAILABLE", "Connector 持久化未配置", "配置 Connector Repository 后重试")
	}
	binding, err := s.connectorRepository.Binding(ctx, actor.TenantID, bindingID)
	if err != nil {
		return connector.SyncReceipt{}, err
	}
	if _, err := s.app.Identity.projectForWrite(ctx, actor, binding.ProjectID); err != nil {
		return connector.SyncReceipt{}, err
	}
	leaseOwner := idgen.New()
...
func (s *SourceService) ConnectorReceipts(ctx context.Context, actor Actor, bindingID string) ([]connector.SyncReceipt, error) {
	if s.connectorRepository == nil {
		return nil, fault.Policy("CONNECTOR_STORE_UNAVAILABLE", "Connector 持久化未配置", "配置 Connector Repository 后重试")
	}
	if bindingID != "" {
...
func (s *SourceService) materializeConnectorRecord(ctx context.Context, actor Actor, binding connector.Binding, record connector.Record, requestID string) error {
	...
	var revision sourcedomain.SourceRevision
	if found && mapping.SourceID != "" {
...
func (s *SourceService) findSourceRevisionByDigest(ctx context.Context, actor Actor, projectID, digest string) (sourcedomain.SourceRevision, error) {
	sources, err := s.source.Sources(ctx, actor.TenantID, projectID)
	if err != nil {
	...
	return sourcedomain.SourceRevision{}, fault.NotFound("相同摘要的来源版本")
}
...
func connectorFileName(record connector.Record) (string, error) {
	...
	if extension == "" {
		return "", fault.Invalid("CONNECTOR_MIME_UNSUPPORTED", "Connector 记录 MIME 不受支持，不能以错误扩展名物化")
	}
