...
func (s *SourceService) CreateKnowledgeExtractionRun(ctx context.Context, actor Actor, in CreateKnowledgeExtractionRunInput, requestID string) (work.RuntimeRun, error) {
	if err := requireRole(actor, "tenant_admin", "project_manager", "strategist", "editor", "reviewer"); err != nil {
		return work.RuntimeRun{}, err
	}
	project, err := s.app.Identity.projectForWrite(ctx, actor, in.ProjectID)
	if err != nil {
		return work.RuntimeRun{}, err
	}
	revisionIDs := uniqueNonEmpty(in.SourceRevisionIDs)
	if len(revisionIDs) == 0 {
		return work.RuntimeRun{}, fault.Invalid("SOURCE_REVISION_REQUIRED", "知识提取至少需要一个来源版本")
	}
	if in.OutputCount == 0 {
		in.OutputCount = 20
	}
	if in.OutputCount < 1 || in.OutputCount > 20 {
		return work.RuntimeRun{}, fault.Invalid("OUTPUT_COUNT_INVALID", "知识候选数量必须在 1 到 20 之间")
	}
	sources := make([]sourcedomain.ContractSource, 0, len(revisionIDs))
	for _, revisionID := range revisionIDs {
		revision, err := s.source.SourceRevision(ctx, actor.TenantID, revisionID)
		if err != nil || revision.ProjectID != project.ID {
			return work.RuntimeRun{}, fault.Policy("SOURCE_REVISION_PROJECT_MISMATCH", "来源版本不属于当前项目", "只选择当前项目内的来源版本")
		}
		if revision.ProcessingStatus != "ready" {
			return work.RuntimeRun{}, fault.Policy("SOURCE_REVISION_NOT_READY", "来源版本尚未完成可信解析", "等待来源状态变为“就绪（ready）”")
		}
		source, err := s.source.Source(ctx, actor.TenantID, revision.SourceID)
		if err != nil {
			return work.RuntimeRun{}, err
		}
		spans, err := s.source.Evidence(ctx, actor.TenantID, revision.ID)
		if err != nil {
			return work.RuntimeRun{}, err
		}
		evidence := []sourcedomain.ContractEvidence{}
		for _, span := range spans {
			if span.ReviewStatus != "accepted" {
				continue
			}
			evidence = append(evidence, sourcedomain.ContractEvidence{ID: span.ID, LocatorKind: span.LocatorKind, Locator: span.Locator, Quote: span.QuoteText, QuoteHash: span.QuoteHash})
		}
		if len(evidence) == 0 {
			return work.RuntimeRun{}, fault.Policy("ACCEPTED_EVIDENCE_REQUIRED", "来源版本没有可用于提取的已接受证据", "先复核并接受至少一个证据片段")
		}
		sources = append(sources, sourcedomain.ContractSource{SourceID: source.ID, RevisionID: revision.ID, Name: source.Name, SourceType: source.SourceType, FileName: revision.FileName, SHA256: revision.SHA256, DetectedMIME: revision.DetectedMIME, Evidence: evidence})
	}
	snapshot, err := sourcedomain.CompileKnowledgeSnapshot(project, sources, s.now())
	if err != nil {
		return work.RuntimeRun{}, err
	}
	...
	if strings.TrimSpace(in.IdempotencyKey) == "" {
		in.IdempotencyKey = idgen.New()
	}
	if s.runtimeService == nil {
		return work.RuntimeRun{}, fault.Policy("RUNTIME_UNAVAILABLE", "知识提取需要已配置的 Runtime", "联系平台运营人员启用 Runtime")
	}
	...
	if existing, lookupErr := s.runtimeService.JobByIdempotencyKey(ctx, actor.TenantID, runtimeKey); lookupErr == nil {
		if existing.ProjectID != project.ID || existing.BusinessType != "knowledge_extract" || existing.InputDigest != "sha256:"+snapshot.ManifestHash || existing.BusinessOutputCount != in.OutputCount {
			return work.RuntimeRun{}, fault.Conflict("JOB_RUN_IDEMPOTENCY_MISMATCH", "知识提取幂等键已用于不同的输入契约")
		}
		return s.app.Runtime.projectRuntimeRun(ctx, existing)
	} else if !fault.IsNotFound(lookupErr) {
		return work.RuntimeRun{}, lookupErr
	}
	if err := s.contexts.CreateSnapshot(ctx, snapshot); err != nil {
		existing, lookupErr := s.contexts.Snapshot(ctx, actor.TenantID, snapshot.ID)
		if lookupErr != nil || !sameContextSnapshotIdentity(existing, snapshot) {
			return work.RuntimeRun{}, err
		}
	}
	environmentID, err := s.app.Catalog.projectRuntimeEnvironmentID(ctx, actor.TenantID, project.ID)
	if err != nil {
		return work.RuntimeRun{}, err
	}
	...
	projected, err := s.app.Runtime.projectRuntimeRun(ctx, start.Job)
	if err != nil {
		return work.RuntimeRun{}, err
	}
...
func sameContextSnapshotIdentity(existing, requested sourcedomain.ContextSnapshot) bool {
...
func knowledgeExtractionSOP() catalogdomain.SOPVersion {
	digest, _ := stablehash.Sum(struct {
		ID      string `json:"id"`
		Version int    `json:"version"`
		Schema  string `json:"schema"`
	}{"contentcloud.knowledge.extract", 1, sourcedomain.KnowledgeCandidatesSchema})
	return catalogdomain.SOPVersion{
		ID: "contentcloud.knowledge.extract/v1", SOPID: "contentcloud.knowledge.extract", Version: 1,
		SchemaVersion: catalogdomain.SOPSchemaVersion, Name: "知识候选提取", Status: "published",
		ContentTypes: []string{"knowledge_extract"}, DefaultExecutionMode: "agent", Digest: "sha256:" + digest,
		Stages: []catalogdomain.StageDefinition{{ID: "knowledge_extract", Name: "提取知识候选", Order: 10, OutputSchema: sourcedomain.KnowledgeCandidatesSchema, RequiredCapabilities: []string{sourcedomain.KnowledgeExtractCapability}, ExecutionModes: []string{"agent"}, RetryMaxAttempts: 3}},
	}
}
...
func (s *SourceService) importKnowledgePackage(ctx context.Context, actor Actor, run work.RuntimeRun, pkg sourcedomain.KnowledgeExtractionPackage, requestID string) (sourcedomain.KnowledgeExtractionResult, string, error) {
	hash, inputs, err := s.validateKnowledgePackage(ctx, actor, run, pkg)
	if err != nil {
		return sourcedomain.KnowledgeExtractionResult{}, "", err
	}
	objects := make([]sourcedomain.KnowledgeObject, 0, len(inputs))
	for _, input := range inputs {
		existing, lookupErr := s.knowledge.KnowledgeObject(ctx, actor.TenantID, input.ID, 0)
		if lookupErr == nil {
			marker, _ := existing.Payload["runtime_result_digest"].(string)
			if marker != "sha256:"+hash || existing.ProjectID != run.ProjectID {
				return sourcedomain.KnowledgeExtractionResult{}, "", fault.Conflict("RUNTIME_BUSINESS_RESULT_CONFLICT", "同一 Runtime 结果引用了不同的知识候选内容")
			}
			objects = append(objects, existing)
			continue
		}
		if !fault.IsNotFound(lookupErr) {
			return sourcedomain.KnowledgeExtractionResult{}, "", lookupErr
		}
		object, createErr := s.CreateKnowledgeObject(ctx, actor, input, requestID)
		if createErr != nil {
			return sourcedomain.KnowledgeExtractionResult{}, "", createErr
		}
		objects = append(objects, object)
	}
	return sourcedomain.KnowledgeExtractionResult{RunID: run.ID, Objects: objects, Warnings: pkg.Warnings}, "sha256:" + hash, nil
}

func (s *SourceService) validateKnowledgePackage(ctx context.Context, actor Actor, run work.RuntimeRun, pkg sourcedomain.KnowledgeExtractionPackage) (string, []CreateKnowledgeObjectInput, error) {
	hash, err := stablehash.Sum(pkg)
	if err != nil {
		return "", nil, err
	}
	if pkg.SchemaVersion != "1.0" || len(pkg.Candidates) == 0 || (run.OutputCount > 0 && len(pkg.Candidates) > run.OutputCount) || len(pkg.Candidates) > 20 {
		return "", nil, fault.Invalid("KNOWLEDGE_PACKAGE_INVALID", "知识候选格式版本或候选数量不符合任务契约")
	}
	project, err := s.app.Identity.projectForWrite(ctx, actor, run.ProjectID)
	if err != nil {
		return "", nil, err
	}
	if project.Status == "archived" {
		return "", nil, fault.Policy("PROJECT_ARCHIVED", "已归档项目不能接收新的智能体结果", "恢复项目或取消任务")
	}
	if strings.TrimSpace(run.InputSnapshotID) == "" {
		return "", nil, fault.Invalid("KNOWLEDGE_INPUT_SNAPSHOT_REQUIRED", "知识提取缺少冻结输入快照")
	}
	snapshot, err := s.contexts.Snapshot(ctx, actor.TenantID, run.InputSnapshotID)
	if err != nil {
		return "", nil, err
	}
	...
	for index, candidate := range pkg.Candidates {
		if !knowledgeCandidateEnumsValid(candidate) || !evidenceWithinKnowledgeContract(candidate.Evidence, snapshot.Sources) {
			return "", nil, fault.E("content", "policy", "KNOWLEDGE_CANDIDATE_GROUNDING_INVALID", "知识候选类型、风险或证据超出任务契约", 7)
		}
...
func knowledgeCandidateObjectInput(run work.RuntimeRun, candidate sourcedomain.KnowledgeCandidate, evidenceIDs []string, index int) CreateKnowledgeObjectInput {
...
func knowledgeCandidateEnumsValid(candidate sourcedomain.KnowledgeCandidate) bool {
...
func (s *SourceService) knowledgeEvidenceIDs(ctx context.Context, tenantID, projectID string, refs []sourcedomain.EvidenceRef) ([]string, error) {
	ids := make([]string, 0, len(refs))
	for _, ref := range refs {
		if strings.TrimSpace(ref.SourceRevisionID) == "" || strings.TrimSpace(ref.LocatorKind) == "" || strings.TrimSpace(ref.Quote) == "" {
			return nil, fault.Invalid("EVIDENCE_REF_INVALID", "证据引用缺少来源版本、定位类型或原文")
		}
		revision, err := s.source.SourceRevision(ctx, tenantID, ref.SourceRevisionID)
		if err != nil {
			return nil, err
		}
		if revision.ProjectID != projectID {
			return nil, fault.Policy("EVIDENCE_PROJECT_MISMATCH", "证据来源不属于当前项目", "选择当前项目内已接受的证据")
		}
		spans, err := s.source.Evidence(ctx, tenantID, revision.ID)
		if err != nil {
			return nil, err
		}
		matched := ""
		...
		if matched == "" {
			return nil, fault.Policy("EVIDENCE_NOT_ACCEPTED", "证据原文或定位与已接受的片段不一致", "重新选择来源中已接受的证据")
		}
...
func evidenceWithinKnowledgeContract(refs []sourcedomain.EvidenceRef, sources []sourcedomain.ContractSource) bool {
...
func evidenceLocatorMatches(encoded string, locator map[string]any) bool {
	...
	wanted, err := stablehash.Sum(decoded)
	if err != nil {
		return false
	}
	actual, err := stablehash.Sum(locator)
...
func decodeStrict(body []byte, target any) error {
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		if err == nil {
			return fault.Invalid("INPUT_INVALID", "JSON 只能包含一个对象")
		}
		return err
	}
	return nil
