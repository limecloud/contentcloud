...
func TestRuntimeMCPStdioProcessCallsAttemptHTTPGateway(t *testing.T) {
	...
	handle, err = service.Runtime().ActivateDispatch(t.Context(), handle, agentadapter.AgentSessionRef{TenantID: started.Job.TenantID, HarnessKind: "fake", SessionID: "mcp-stdio-session"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := json.Unmarshal([]byte(fmt.Sprint(item["text"])), &gateway); err != nil || gateway.ToolCall.State != domain.ToolCallSucceeded {
