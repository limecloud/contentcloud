...
func (r *Root) modelCommand() *cobra.Command {
	...
	generate := &cobra.Command{Use: "generate <task-id>", Args: cobra.ExactArgs(1), Short: "生成 draft TaskRevision，不绕过审核", RunE: func(cmd *cobra.Command, args []string) error {
		...
		input := app.GenerateModelCandidateInput{ProviderID: providerID, Messages: messages, ResponseSchema: map[string]any{"type": "object", "additionalProperties": true}, ContentType: contentType, SchemaVersion: schemaVersion, MaxTokens: maxTokens}
		params := struct {
			TaskID string `json:"task_id"`
			app.GenerateModelCandidateInput
		}{TaskID: args[0], GenerateModelCandidateInput: input}
		var result app.GenerateModelCandidateResult
	...
	receipts := &cobra.Command{Use: "receipts", Short: "列出模型生成摘要和 Token 用量", RunE: func(cmd *cobra.Command, _ []string) error {
		...
		var result []domain.ModelGenerationReceipt
