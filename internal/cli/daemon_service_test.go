...
func TestLaunchdDaemonStartIsIdempotentAndVersionAware(t *testing.T) {
	...
	upgraded := &launchdDaemonService{home: home, executable: "/opt/contentcloud-0.13.0", version: "0.13.0", uid: 501, now: func() time.Time { return now.Add(time.Hour) }, run: runner, environment: launchEnvironment}
	state, err = upgraded.Start()
	if err != nil || state.Version != "0.13.0" || state.Executable != "/opt/contentcloud-0.13.0" || !state.Running {
	...
	if err != nil || !strings.Contains(string(metadata), `"version": "0.13.0"`) {
...
func TestLaunchdDaemonStatusKeepsLastRuntimeStateWhenStopped(t *testing.T) {
	...
	tracker.recordPoll(app.DaemonRuntimePolicy{CurrentVersion: "0.10.0", LatestVersion: "0.13.0", UpdateAvailable: true}, false, nil)
