...
func (r *Root) initCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if target != "none" && !acceptProjectConfig {
				err := domain.Policy("PROJECT_CONFIG_CONFIRMATION_REQUIRED", "初始化将写入 ContentCloud 项目指导与目标 Agent 配置", "检查 --dry-run 输出后传入 --accept-project-config；codex-plugin 模式不会重复写入项目级 Codex Skill 或 MCP 配置")
				err.ExitCode = 2
				return err
			}
	...
	cmd.Flags().StringVar(&target, "target", "all", "project Agent configuration target: codex-plugin, codex, claude, all, or none")
