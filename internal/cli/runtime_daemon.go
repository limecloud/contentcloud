...
func (r *Root) runtimeDaemonRun(cmd *cobra.Command, once, fixture bool, harnessKind, logFile string) error {
	...
	prepareBinding := func(binding localconfig.DaemonBinding) (daemonBindingRuntime, error) {
		...
		if tokenErr != nil {
			return daemonBindingRuntime{}, &domain.Error{Type: "credential", Subtype: "device", Code: "DEVICE_CREDENTIAL_MISSING", Message: tokenErr.Error(), ExitCode: 3}
		}
	...
	if len(runtimes) == 0 {
		return domain.Conflict("DAEMON_BINDINGS_UNAVAILABLE", "没有可用的 Daemon 设备绑定")
	}
	if once {
...
func observeDaemonWorkspaces(workspaces []localconfig.DaemonWorkspace) []domain.DaemonWorkspaceObservation {
	observations := make([]domain.DaemonWorkspaceObservation, 0, len(workspaces))
	for _, workspace := range workspaces {
		...
		if root == "" {
			observations = append(observations, domain.DaemonWorkspaceObservation{ProjectID: projectID, WorkspaceID: workspaceID, Status: "unknown", Reason: "workspace_root_unavailable", ObservedAt: time.Now().UTC()})
			continue
		}
		observation, err := localworkspace.ObserveWorkspace(root, time.Now().UTC())
		if err != nil {
			observation = domain.DaemonWorkspaceObservation{ProjectID: projectID, WorkspaceID: workspaceID, Status: "unknown", Reason: "workspace_observation_failed", ErrorCode: runtimeObservationError(err), ObservedAt: time.Now().UTC()}
		} else if observation.ProjectID != projectID || observation.WorkspaceID != workspaceID {
			observation = domain.DaemonWorkspaceObservation{ProjectID: projectID, WorkspaceID: workspaceID, Status: "blocked", Reason: "workspace_binding_mismatch", ErrorCode: "WORKSPACE_BINDING_MISMATCH", ObservedAt: time.Now().UTC()}
		}
...
func withWorkspaceEnvironmentStatus(capabilities map[string]any, observations []domain.DaemonWorkspaceObservation) map[string]any {
