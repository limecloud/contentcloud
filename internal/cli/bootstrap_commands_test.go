...
func TestBootstrapPlanIsReadOnlyAndUsesOnlyPublicSessionID(t *testing.T) {
	...
	if !envelope.OK || envelope.Data.State != "ready" || !strings.HasPrefix(envelope.Data.PlanID, "bp_") || envelope.Data.CLIPackage != "@limecloud/contentcloud@0.16.0" || len(envelope.Data.Plugin.Actions) != 2 || !envelope.Data.WouldEnableDaemon {
...
func TestBootstrapApplyRejectsPlanAfterCodexStateChanges(t *testing.T) {
	...
	runner := &bootstrapRunner{responses: []bootstrapRunnerResponse{
		{stdout: `{"marketplaces":[{"name":"contentcloud","root":"/tmp/cache","marketplaceSource":{"sourceType":"git","source":"limecloud/contentcloud","ref":"v0.16.0"}}]}`},
		{stdout: `{"installed":[{"pluginId":"contentcloud-video-production@contentcloud","name":"contentcloud-video-production","marketplaceName":"contentcloud","version":"0.16.0","installed":true,"enabled":true}],"available":[]}`},
	}}
	root := &Root{stdout: &bytes.Buffer{}, stderr: &bytes.Buffer{}, codexRunner: runner, bootstrapCheckHook: healthyBootstrapCheck}
	command := root.command()
...
func successfulBootstrapRunner() *bootstrapRunner {
	...
	currentMarketplace := `{"marketplaces":[{"name":"contentcloud","root":"/tmp/cache","marketplaceSource":{"sourceType":"git","source":"limecloud/contentcloud","ref":"v0.16.0"}}]}`
	currentPlugin := `{"installed":[{"pluginId":"contentcloud-video-production@contentcloud","name":"contentcloud-video-production","marketplaceName":"contentcloud","version":"0.16.0","installed":true,"enabled":true}],"available":[]}`
	return &bootstrapRunner{responses: []bootstrapRunnerResponse{
		...
		{stdout: `{"pluginId":"contentcloud-video-production@contentcloud","name":"contentcloud-video-production","marketplaceName":"contentcloud","version":"0.16.0","installedPath":"/tmp/plugin"}`},
