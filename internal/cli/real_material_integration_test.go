...
func TestRealMaterialReachesKnowledgePublishPreflight(t *testing.T) {
	...
	pkg := domain.KnowledgeExtractionPackage{SchemaVersion: "1.0", Candidates: []domain.KnowledgeCandidate{{
		Kind: "fact", Title: "真实材料事实", Statement: span.Quote, Subject: "客户材料", Predicate: "包含内容", Value: domain.TypedValue{Type: "text", Text: span.Quote},
		...
		Evidence: []domain.EvidenceRef{{SourceRevisionID: "source:real-material", LocatorKind: span.LocatorKind, Locator: string(locator), Quote: span.Quote}}, ForbiddenExtensions: []string{}, DependsOnFactIDs: []string{},
	}}, Warnings: []string{}}
