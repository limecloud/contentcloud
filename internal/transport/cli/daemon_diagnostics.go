...
func createDaemonDiagnosticBundle(path string, state userDaemonState, now time.Time) (daemonDiagnosticBundle, error) {
	...
	if path == "" {
		return daemonDiagnosticBundle{}, fault.Invalid("DAEMON_DIAGNOSTIC_OUTPUT_REQUIRED", "生成 Daemon 诊断包需要明确指定输出文件")
	}
	...
	if err == nil && (!info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0) {
		return daemonDiagnosticBundle{}, fault.Policy("DAEMON_DIAGNOSTIC_OUTPUT_UNSAFE", "Daemon 诊断包输出路径必须是普通文件", "选择私有目录中的新 JSON 文件")
	}
