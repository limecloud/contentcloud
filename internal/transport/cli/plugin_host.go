...
func (r *Root) bundledPluginRuntime(hostName, pluginID, version string) (*hostPluginRuntime, error) {
	...
	if err != nil {
		unavailable := fault.Policy("ENVIRONMENT_PLUGIN_ARTIFACT_UNAVAILABLE", "环境准备引用的标准插件包未随当前 CLI 发布", "安装包含该标准包的 ContentCloud CLI 版本后重试")
...
func parsePluginHost(value string) (pluginhost.HostID, error) {
	...
	default:
		return "", fault.Invalid("PLUGIN_HOST_INVALID", "插件宿主必须是 codex 或 claude")
	}
}
