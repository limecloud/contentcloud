...
func mcpToolErrorCodeForTest(t *testing.T, result map[string]any) string {
	...
	domainError, ok := structured["error"].(*fault.Error)
	if !ok {
...
func TestMCPEnvironmentExecutionPlanUsesVerifiedOfflineState(t *testing.T) {
	...
	if !ok || len(context.ContentTypes) != 1 || context.ContentTypes[0] != identitydomain.ContentTypeVideoScript {
		t.Fatalf("workspace context did not expose verified content types: %#v", contextResult)
	}
	arguments := map[string]any{"run_id": "run-1", "intent": "extract verified project knowledge", "required_capabilities": []string{sourcedomain.KnowledgeExtractCapability}, "input_refs": []string{"20-sources/source-registry.json"}}
...
func TestMCPPublishApplyRequiresExactConfirmationBeforeCloudWrite(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		var payload struct {
			...
			Params  reviewdomain.SubmissionBundle `json:"params"`
		}
		...
		revision := reviewdomain.SubmissionRevision{ID: "revision-1", SubmissionID: "submission-1", ProjectID: "project-1", WorkspaceID: "workspace-1", ContentHash: payload.Params.ContentHash, IdempotencyKey: payload.Params.IdempotencyKey}
...
func TestMCPFeedbackPullCreatesImmutableInboxForNewConversation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		pulls++
		comments := []reviewdomain.ReviewComment{{ID: "comment-1", Body: "补充证据", CreatedAt: now}}
		createdAt := now
		if pulls == 2 {
			createdAt = now.Add(time.Minute)
			comments = append(comments, reviewdomain.ReviewComment{ID: "comment-2", Body: "收紧 CTA", CreatedAt: createdAt})
		}
		feedback := []reviewdomain.ReviewFeedbackBundle{{BundleVersion: "1.0", SubmissionID: "submission-1", SubmissionRevisionID: "revision-1", SubjectHash: "sha256:" + strings.Repeat("a", 64), Comments: comments, CreatedAt: createdAt}}
		w.Header().Set("Content-Type", "application/json")
...
func TestMCPApprovedSnapshotPullSupportsOfflineCrossConversationRead(t *testing.T) {
	...
	snapshots := []reviewdomain.ApprovedSnapshot{
...
func approvedSnapshotForMCPTest(t *testing.T, snapshotID, revisionID, objectID string, createdAt time.Time) reviewdomain.ApprovedSnapshot {
	t.Helper()
...
func TestMCPProjectViewTargetSelectionDoesNotInventObjectPrecision(t *testing.T) {
	...
	multipleFeedback := reviewFeedbackProjectView([]reviewdomain.ReviewFeedbackBundle{
	...
	invalidCurrentRevision := submissionStatusProjectView(application.SubmissionDetails{
		Submission: reviewdomain.Submission{CurrentRevisionID: "revision-1"},
		Revisions:  []reviewdomain.SubmissionRevision{{ID: "revision-1", ContentHash: "incomplete"}},
	})
	...
	multipleSnapshots := approvedSnapshotsProjectView([]reviewdomain.ApprovedSnapshot{
