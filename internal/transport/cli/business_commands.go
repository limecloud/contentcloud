...
func (r *Root) authCommand() *cobra.Command {
	...
	login := &cobra.Command{Use: "login", Short: "发起或完成需要浏览器确认的设备登录", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if noWait {
			var result application.StartDeviceLoginResult
		...
		if deviceCode == "" {
			return fault.Invalid("DEVICE_CODE_REQUIRED", "使用 --no-wait 发起登录，或用 --device-code 完成登录")
		}
		var result application.CompleteDeviceLoginResult
		...
		if err := localconfig.SaveUserToken(server, result.AccessToken); err != nil {
			return &fault.Error{Type: "credential", Subtype: "secure_store", Code: "CREDENTIAL_STORE_FAILED", Message: err.Error(), ExitCode: 3}
		}
		result.AccessToken = ""
		cfg.ServerURL = server
		if err := localconfig.Save(cfg); err != nil {
...
func (r *Root) tenantCommand() *cobra.Command {
	...
	switchCmd := &cobra.Command{Use: "switch <tenant-id>", Args: cobra.ExactArgs(1), Short: "切换到另一个已授权租户并更新命令行凭据", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result application.SwitchCLITenantResult
...
func (r *Root) teamCommand() *cobra.Command {
	...
	accept := &cobra.Command{Use: "accept <invite-token>", Args: cobra.ExactArgs(1), Short: "接受与当前登录邮箱绑定的邀请", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result identitydomain.Membership
...
func (r *Root) fullProjectCommand() *cobra.Command {
	...
	resolve := &cobra.Command{Use: "resolve <name-or-slug>", Args: cobra.ExactArgs(1), Short: "将项目名称或短标识解析为稳定 ID", RunE: func(cmd *cobra.Command, args []string) error {
		...
		projects := []workspacedomain.Project{}
		...
		matches := []workspacedomain.Project{}
		...
		if len(matches) != 1 {
			return fault.Invalid("PROJECT_RESOLUTION_AMBIGUOUS", "项目名称未唯一匹配，请使用 project list 后传入 ID")
		}
	...
	create := &cobra.Command{Use: "create", Short: "创建一个聚焦单一产品的项目", RunE: func(cmd *cobra.Command, args []string) error {
		input := application.CreateProjectInput{TemplateID: templateID, BrandName: brand, ProductName: product, Channel: channel, StageObjective: objective, OwnerName: owner, ReviewerName: reviewer, ClientApprover: approver}
		if createDryRun {
	...
	createTemplate := &cobra.Command{Use: "create", Short: "创建不含客户事实和素材的脱敏项目模板", RunE: func(cmd *cobra.Command, args []string) error {
		input := application.CreateProjectTemplateInput{Name: templateName, Channel: templateChannel, StageObjective: templateObjective}
		if templateDryRun {
...
func (r *Root) sourceCommand() *cobra.Command {
	...
	search := &cobra.Command{Use: "search <query>", Args: cobra.ExactArgs(1), Short: "搜索公开来源或验证 URL 入口", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			return fault.Invalid("PROJECT_CONTEXT_REQUIRED", "未解析到唯一项目上下文")
		}
		var result application.SearchSourcesResult
		if err := client.Dispatch(cmd.Context(), "source.search", application.SearchSourcesInput{ProjectID: projectID, Query: args[0], Limit: searchLimit}, &result); err != nil {
			return err
		}
	...
	fetch := &cobra.Command{Use: "fetch <url>", Args: cobra.ExactArgs(1), Short: "在安全策略允许的范围内采集公开来源", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result application.FetchSourceReceipt
		if err := client.Dispatch(cmd.Context(), "source.fetch", application.FetchSourceInput{ProjectID: projectID, URL: args[0], Name: fetchName, SourceType: fetchType}, &result); err != nil {
			return err
		}
	...
	upload := &cobra.Command{Use: "upload <file>", Args: cobra.ExactArgs(1), Short: "上传一个受支持且不可变的来源版本", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(data) > 100*1024*1024 {
			return fault.Invalid("SOURCE_SIZE_INVALID", "文件超过 100MB")
		}
		cfg, err := localconfig.Load()
		if err != nil {
			return err
		...
		var result sourcedomain.SourceRevision
	...
	revise := &cobra.Command{Use: "revise <source-id> <file>", Args: cobra.ExactArgs(2), Short: "为现有逻辑来源上传新的不可变版本", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(data) == 0 || len(data) > 100*1024*1024 {
			return fault.Invalid("SOURCE_SIZE_INVALID", "文件必须在 1B 到 100MB 之间")
		}
		if reviseMIME == "" {
	...
	evidenceReview := &cobra.Command{Use: "evidence-review <evidence-id> <accept|reject>", Args: cobra.ExactArgs(2), Short: "记录对一个已提取证据片段的人工决定", RunE: func(cmd *cobra.Command, args []string) error {
		if args[1] != "accept" && args[1] != "reject" {
			return fault.Invalid("EVIDENCE_DECISION_INVALID", "证据复核只允许 accept 或 reject")
		}
		if evidenceDryRun {
		...
		var result sourcedomain.EvidenceSpan
...
func (r *Root) assetCommand() *cobra.Command {
	...
	create := &cobra.Command{Use: "create", Short: "将一个来源版本登记为受治理素材", RunE: func(command *cobra.Command, args []string) error {
		...
		input := application.CreateAssetInput{ProjectID: projectID, Name: name, AssetType: assetType, SourceRevisionID: sourceRevisionID, UsageMode: usageMode}
		if createDryRun {
	...
	rights := &cobra.Command{Use: "rights <asset-id>", Args: cobra.ExactArgs(1), Short: "列出素材的权利记录", RunE: func(command *cobra.Command, args []string) error {
		...
		var result []sourcedomain.RightsRecord
	...
	rightsCreate := &cobra.Command{Use: "rights-create <asset-id>", Args: cobra.ExactArgs(1), Short: "创建一条待审核的权利记录", RunE: func(command *cobra.Command, args []string) error {
		...
		input := application.CreateRightsRecordInput{AssetID: args[0], RightsHolder: holder, RightsType: rightsType, Territories: territories, Channels: channels, ValidFrom: from, ValidUntil: until, ProofSourceRevisionID: proofRevision, Restrictions: restrictions}
		if rightsDryRun {
	...
	review := &cobra.Command{Use: "rights-review <rights-id> <approve|reject>", Args: cobra.ExactArgs(2), Short: "记录对一条权利记录的审核决定", RunE: func(command *cobra.Command, args []string) error {
		if args[1] != "approve" && args[1] != "reject" {
			return fault.Invalid("RIGHTS_DECISION_INVALID", "权利审核只允许 approve 或 reject")
		}
		if reviewDryRun {
...
func parseOptionalRFC3339(value, flag string) (*time.Time, error) {
	...
	if err != nil {
		return nil, fault.Invalid("TIME_INVALID", flag+" 必须是 RFC3339 时间")
	}
	parsed = parsed.UTC()
	return &parsed, nil
}
...
func (r *Root) knowledgeCommand() *cobra.Command {
	...
	extract := &cobra.Command{Use: "extract", Short: "创建一次基于本地证据的知识提取任务", RunE: func(command *cobra.Command, args []string) error {
		input := application.CreateKnowledgeExtractionRunInput{SourceRevisionIDs: sourceRevisionIDs, IdempotencyKey: idempotencyKey, OutputCount: outputCount}
		if extractDryRun {
			cfg, err := localconfig.Load()
			if err != nil {
				return err
			...
			projectID, err := localconfig.ResolveProject(r.projectID, cfg)
			if err != nil {
				return fault.Invalid("PROJECT_CONTEXT_REQUIRED", "请用 --project 或本地项目上下文指定项目")
			}
	...
	review := &cobra.Command{Use: "review <id> <approve|reject>", Args: cobra.ExactArgs(2), Short: "记录一次知识对象人工审核决定", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var current sourcedomain.KnowledgeObject
...
func (r *Root) runCommand() *cobra.Command {
	...
	events := &cobra.Command{Use: "events <run-id>", Args: cobra.ExactArgs(1), Short: "显示指定游标之后的不可变任务进度事件", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result []work.RuntimeRunEvent
...
func (r *Root) reviewCommand() *cobra.Command {
	...
	create := &cobra.Command{Use: "create <submission-revision-id>", Args: cobra.ExactArgs(1), Short: "创建有效期为 7 天的客户审批链接", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result reviewdomain.ReviewGrant
	...
	status := &cobra.Command{Use: "status <submission-revision-id>", Args: cobra.ExactArgs(1), Short: "显示提交版本（SubmissionRevision）的客户审核状态", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result application.SubmissionReviewStatus
...
func (r *Root) resultCommand() *cobra.Command {
	...
	batchShow := &cobra.Command{Use: "batch-show <batch-id>", Args: cobra.ExactArgs(1), Short: "显示一个导入批次及其观测数据", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var result performancedomain.PerformanceImportDetails
	...
	importCmd := &cobra.Command{Use: "import <json-csv-or-xlsx-file>", Args: cobra.ExactArgs(1), Short: "导入已校验的效果观测数据", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if info.Size() > 20*1024*1024 {
			return fault.Invalid("RESULT_FILE_TOO_LARGE", "结果文件不能超过 20 MB")
		}
		...
		input := application.ImportPerformanceInput{ProjectID: projectID, SourceName: filepath.Base(args[0]), SourceFormat: format, SourceSHA256: hex.EncodeToString(digest[:]), Observations: inputs, DryRun: dryRun}
		var result application.ImportPerformanceResult
	...
	rate := &cobra.Command{Use: "rate <approved_snapshot> <subject-id>", Args: cobra.ExactArgs(2), Short: "创建不可变的人工评级决定", RunE: func(cmd *cobra.Command, args []string) error {
		...
		input := application.CreateRatingDecisionInput{ProjectID: projectID, SubjectType: args[0], SubjectID: args[1], ObservationIDs: observationIDs, Rating: rating, Reason: reason, NextAction: nextAction, DryRun: ratingDryRun}
		var result application.CreateRatingDecisionResult
...
func (r *Root) requestCommand() *cobra.Command {
	...
	get := &cobra.Command{Use: "get <projects|tenants|runs>", Args: cobra.ExactArgs(1), Short: "读取一个白名单集合", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !ok {
			return fault.Invalid("REQUEST_RESOURCE_BLOCKED", "仅允许读取 projects、tenants 或 runs")
		}
		cfg, client, _, err := r.userClient()
		if err != nil {
			return err
...
func (r *Root) userClient() (localconfig.Config, *apiclient.Client, string, error) {
	...
	token, err := localconfig.UserToken(server)
	if err != nil {
		return cfg, nil, "", &fault.Error{Type: "credential", Subtype: "user", Code: "CLI_LOGIN_REQUIRED", Message: "请先运行 contentcloud auth login --no-wait", ExitCode: 3}
	}
...
func (r *Root) resolveProject(cmd *cobra.Command, cfg localconfig.Config, client *apiclient.Client) (string, error) {
	...
	return "", fault.Invalid("PROJECT_CONTEXT_REQUIRED", "请用 --project、CONTENTCLOUD_PROJECT_ID 或 .contentcloud/project.json 指定项目")
}
...
func confirmationRequired(message string) error {
	err := fault.E("confirmation", "high_risk_write", "CONFIRMATION_REQUIRED", message, 10)
