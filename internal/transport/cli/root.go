...
type Root struct {
	...
	bootstrapAuthorizeHook func(context.Context, string, string) (localconfig.Config, application.ConnectDeviceResult, *bootstrapProgressReporter, error)
...
func daemonStartPrerequisites() error {
	...
	if len(cfg.Bindings()) == 0 {
		return fault.Conflict("DEVICE_BINDING_MISSING", "启动自动化后台服务前必须先完成设备注册")
	}
	for _, binding := range cfg.Bindings() {
		if _, err := localconfig.DeviceToken(binding.DeviceID); err != nil {
			return &fault.Error{Type: "credential", Subtype: "device", Code: "DEVICE_CREDENTIAL_MISSING", Message: err.Error(), ExitCode: 3}
		}
	}
	...
	return fault.Policy("AGENT_ADAPTER_UNAVAILABLE", "未检测到可用于 Runtime 的 Codex 或 Claude Code", "安装支持结构化事件的本机智能体客户端后重试")
}
...
func (r *Root) projectCommand() *cobra.Command {
	...
	cmd.AddCommand(&cobra.Command{Use: "show", Short: "显示当前配置的项目身份", RunE: func(cmd *cobra.Command, args []string) error {
		...
		id, err := localconfig.ResolveProject(r.projectID, cfg)
		if err != nil {
			return fault.Invalid("PROJECT_CONTEXT_REQUIRED", "需要显式项目上下文")
		}
...
func (r *Root) writeOK(command string, data any) error {
	if r.json {
		return json.NewEncoder(r.stdout).Encode(success{OK: true, Command: command, RequestID: "local_" + idgen.New(), Data: data, Meta: map[string]any{}})
	}
	switch v := data.(type) {
	case string:
...
func (r *Root) writeError(command string, err error) int {
	de := &fault.Error{}
	if !errors.As(err, &de) {
		de = fault.E("internal", "local", "INTERNAL_ERROR", err.Error(), 1)
	}
	if r.json {
		_ = json.NewEncoder(r.stderr).Encode(failure{OK: false, Command: command, RequestID: "local_" + idgen.New(), Error: de})
	} else {
...
func builtinCapabilities() []catalogdomain.Capability {
...
func skillDestination(target, name string) (string, error) {
	...
	default:
		return "", fault.Invalid("SKILL_TARGET_INVALID", "--target 必须为 codex 或 claude")
	}
}
...
func commandSchemas() map[string]any {
	...
	return map[string]any{
		...
		"local.douyin-commerce.validate": write("none", []string{"--audience-snapshot", "--audience", "--offer-snapshot", "--offer", "--content-snapshot", "--content-item", "--storyboard-snapshot", "--storyboard", "--artifact-id", "--artifact-file", "--landing-page-text", "--benefit", "--condition", "--account-ref", "--product-anchor-ref", "--landing-page-ref", "--scheduled-at", "--out", "--directory"}, "抖音电商最终成片、商品事实和发布血缘校验回执"),
		"local.douyin-commerce.lint":     read([]string{"validation-receipt.json", "--artifact-file", "--landing-page-text", "--directory"}, "复算后的抖音电商发布校验结果"),
		...
		"content_profile.list": userRead(nil, "可安装的内容生产 Profile"), "content_profile.install": write("user", []string{"profile-id"}, "由 Profile 编译并发布的现有 SOP"),
		"channel.binding.create": write("user", []string{"--project", "--channel", "--adapter", "--account", "--authorization-secret-ref", "--region"}, "只保存 SecretRef 的渠道账号绑定"), "channel.binding.list": userRead([]string{"--project"}, "项目渠道绑定"), "channel.adapter.list": userRead(nil, "渠道适配器列表"), "channel.publication.prepare": write("user", []string{"--task-delivery", "--binding", "--idempotency-key", "--schedule", "--content-profile", "--douyin-refs"}, "不可变渠道发布意图和类型化内容血缘"),
		"channel.publication.reconcile": write("user", []string{"--limit"}, "渠道发布对账结果"), "channel.performance.import": write("user", []string{"publication-id", "--approved-snapshot", "--window-hours", "--metric"}, "绑定 published 回执的 PerformanceObservation"),
