...
func TestBootstrapApplyInstallsInitializesDoctorsAndRegisters(t *testing.T) {
	...
	root := &Root{
		...
		bootstrapAuthorizeHook: func(_ context.Context, sessionID, _ string) (localconfig.Config, application.ConnectDeviceResult, *bootstrapProgressReporter, error) {
			if sessionID != testBootstrapSessionID {
				t.Fatalf("unexpected ConnectSession: %s", sessionID)
			}
			return localconfig.Config{ServerURL: server.URL, DaemonBindings: []localconfig.DaemonBinding{{ServerURL: server.URL, DeviceID: "device-1", Workspaces: []localconfig.DaemonWorkspace{{WorkspaceID: "workspace-1", ProjectID: "project-1"}}}}}, application.ConnectDeviceResult{
				Device: workspacedomain.Device{ID: "device-1"}, WorkspaceID: "workspace-1", WorkspaceToken: "wt_test", ProjectID: "project-1", EnvironmentManifest: &manifest,
			}, nil, nil
		},
	}
	command := root.command()
...
func TestBootstrapApplyAuthorizationFailureDoesNotMutatePluginOrWorkspace(t *testing.T) {
	...
	root := &Root{
		...
		bootstrapAuthorizeHook: func(context.Context, string, string) (localconfig.Config, application.ConnectDeviceResult, *bootstrapProgressReporter, error) {
			return localconfig.Config{}, application.ConnectDeviceResult{}, nil, fault.Conflict("BOOTSTRAP_AUTHORIZATION_DENIED", "用户拒绝授权")
		},
	}
...
func healthyBootstrapCheck(context.Context, bootstrapcheck.Options) bootstrapcheck.Report {
	return bootstrapcheck.Report{SchemaVersion: workspacedomain.BootstrapSchemaVersion, OK: true, Platform: "darwin", Arch: "arm64", Checks: []bootstrapcheck.Check{{Stage: "prerequisites", CheckID: "runtime.platform.supported", Status: "passed", Facts: map[string]any{"platform": "darwin", "arch": "arm64", "supported": true}}}}
}
...
func newBootstrapRegisterServer(t *testing.T, manifest environment.Manifest, registry environment.Registry) (*httptest.Server, *bool) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.NewEncoder(writer).Encode(map[string]any{
			...
			"data": workspacedomain.WorkspaceBinding{ID: "workspace-1", ProjectID: "project-1", TemplateID: localworkspace.TemplateID, TemplateVersion: localworkspace.TemplateVersion, Targets: []string{"codex-plugin"}, Status: "active"},
		})
	}))
...
func bootstrapEnvironmentFixtureWithTaskPack(t *testing.T, now time.Time, taskPack bootstrapTaskPackFixture) (environment.Manifest, *environment.Verifier, environment.Registry, *environment.RegistryVerifier) {
	...
	profile := environment.Profile{
		...
		Plugins: []environment.ProfilePlugin{
			{ID: "contentcloud-video-production", Kind: "scene_plugin", Version: pluginidentity.VideoProductionVersion, Required: true, Scope: "environment", Capabilities: []string{sourcedomain.KnowledgeExtractCapability}},
		...
		WorkspaceTemplate: environment.WorkspaceTemplateRef{ID: localworkspace.TemplateID, Version: localworkspace.TemplateVersion, Digest: "sha256:" + strings.Repeat("c", 64)}, Capabilities: []string{sourcedomain.KnowledgeExtractCapability}, Policies: environment.Policies{PublishRequiresConfirmation: true, AutomationEnabled: true},
	}
