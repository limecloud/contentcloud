...
func TestPublishPlanIDIsStableAndBindsExactInputs(t *testing.T) {
	...
	writeJSONFixture(t, disclosuresPath, []reviewdomain.SourceDisclosure{{SourceRef: "source-1", Level: "metadata_only", SHA256: strings.Repeat("a", 64)}})
...
func TestPublishPreflightUsesContentBatchManifestAndAllowsBlockedItems(t *testing.T) {
	...
	batch := localworkspace.ContentBatch{ID: "batch-1", IntentID: "intent:content", ContentKind: identitydomain.ContentTypeVideoScript, ContentSchemaRef: localworkspace.ContentItemSchema, DeliveryProfiles: []string{"json", "markdown", "xlsx"}, Status: "blocked", SchemaVersion: localworkspace.ContentBatchSchema, Publishable: false, BriefRef: "brief:1", KnowledgeSnapshotRefs: []string{"snapshot:knowledge"}, ContentItemRefs: []string{}, BlockedReasons: []string{"批次包含 blocked ContentItem"}, Checks: []localworkspace.ContentBatchCheck{{Name: "content_item_lint", Status: "passed"}}}
	...
	writeJSONFixture(t, filepath.Join(batchRoot, "context.json"), localworkspace.LocalContentContext{SchemaVersion: localworkspace.ContentContextSchema, Batch: batch, ProjectID: "project-1", BriefSnapshotID: "snapshot:brief", ContextSnapshotID: "context:1", DirectionIDs: []string{direction.ID}, RequestedCount: 1, VariantDimension: "hook", ControlledDimensions: []string{}, ContentKind: identitydomain.ContentTypeVideoScript, ContentSchemaRef: localworkspace.ContentItemSchema, DeliveryProfiles: []string{"json", "markdown", "xlsx"}, ContentHash: "sha256:" + strings.Repeat("a", 64), GeneratedAt: time.Now().UTC()})
...
func TestStrategyPublishPreflightIncludesApprovedTaxonomyBaseline(t *testing.T) {
	...
	if _, err := localworkspace.StoreApprovedSnapshots(root, []reviewdomain.ApprovedSnapshot{snapshot}, now); err != nil {
		t.Fatal(err)
	}
