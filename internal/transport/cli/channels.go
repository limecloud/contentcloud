...
func (r *Root) channelCommand() *cobra.Command {
	...
	bind := &cobra.Command{Use: "bind", Short: "创建只保存 SecretRef 的渠道账号绑定", RunE: func(cmd *cobra.Command, _ []string) error {
		...
		var result deliverydomain.ChannelBinding
		input := application.CreateChannelBindingInput{ProjectID: projectID, Channel: channel, AdapterID: adapterID, AccountRef: accountRef, AuthorizationSecretRef: secretRef, Region: region}
	...
	prepare := &cobra.Command{Use: "prepare", Short: "固定交付包、账号和最终预览", RunE: func(cmd *cobra.Command, _ []string) error {
		...
		if schedule != "" {
			...
			if err != nil {
				return fault.Invalid("CHANNEL_SCHEDULE_INVALID", "--schedule 必须是 RFC3339 时间")
			}
			scheduledAt = &parsed
		}
		var result deliverydomain.ChannelPublication
		input := application.PrepareChannelPublicationInput{TaskDeliveryID: taskDeliveryID, ChannelBindingID: bindingID, IdempotencyKey: idempotencyKey, ContentProfileID: contentProfileID, ScheduledAt: scheduledAt, Metadata: map[string]any{}}
		if strings.TrimSpace(douyinRefsFile) != "" {
			body, err := os.ReadFile(douyinRefsFile)
			if err != nil {
				return err
			}
			var refs work.DouyinCommercePublicationRefs
			if err := json.Unmarshal(body, &refs); err != nil {
				return fault.Invalid("DOUYIN_COMMERCE_REFS_JSON_INVALID", "--douyin-refs 必须是有效的 DouyinCommercePublicationRefs JSON")
			}
			input.DouyinCommerce = &refs
			if input.ContentProfileID == "" {
				input.ContentProfileID = work.DouyinCommerceProfileID
			}
		}
...
func (r *Root) channelReconcileCommand() *cobra.Command {
	...
	command := &cobra.Command{Use: "reconcile", Short: "批量检查 submitted 和 unknown 发布的外部状态", RunE: func(cmd *cobra.Command, _ []string) error {
		...
		var result application.ChannelReconcileResult
	...
	command := &cobra.Command{Use: "performance <publication-id>", Args: cobra.ExactArgs(1), Short: "把 published 回执绑定的渠道指标导入 PerformanceObservation", RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, raw := range metricFlags {
			...
			if len(parts) != 2 || strings.TrimSpace(parts[0]) == "" {
				return fault.Invalid("CHANNEL_METRIC_INVALID", "--metric 必须使用 name=value")
			}
			value, err := strconv.ParseFloat(parts[1], 64)
			if err != nil {
				return fault.Invalid("CHANNEL_METRIC_INVALID", "--metric value 必须是数字")
			}
		...
		input := application.ImportChannelPerformanceInput{ApprovedSnapshotID: snapshotID, WindowHours: windowHours, SampleStatus: sampleStatus, Metrics: metrics, Currency: currency, Spend: spend, GMV: gmv, IssueCategory: issueCategory, Notes: notes}
...
func (r *Root) channelPublicationReceiptCommand() *cobra.Command {
	...
	command := &cobra.Command{Use: "receipt <publication-id>", Args: cobra.ExactArgs(1), Short: "记录人工渠道发布证明", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if publishedAt != "" {
			...
			if err != nil {
				return fault.Invalid("CHANNEL_PUBLISHED_AT_INVALID", "--published-at 必须是 RFC3339 时间")
			}
			published = &parsed
		}
		var result deliverydomain.ChannelPublication
