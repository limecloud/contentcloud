...
func newUserDaemonService() (userDaemonService, error) {
	if runtime.GOOS != "darwin" {
		return nil, fault.Policy("DAEMON_PLATFORM_UNSUPPORTED", "当前平台尚不支持 Content Work OS 用户级常驻服务", "在 macOS 上使用 LaunchAgent，其他平台暂时使用 daemon run 前台模式")
	}
...
func (s *launchdDaemonService) start(force bool) (userDaemonState, error) {
	...
	if err == nil && !state.Running {
		return state, fault.Conflict("DAEMON_START_INCOMPLETE", "Content Work OS 后台服务已注册，但尚未进入运行状态")
	}
	return state, err
}
