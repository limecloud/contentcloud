...
func TestRuntimeWorkerUsesHarnessSessionAndRenewsLeaseUntilResult(t *testing.T) {
	...
	handle := contentruntime.DispatchHandle{
		Node:          contentruntime.NodeRun{ID: "node-1"},
		Attempt:       contentruntime.RuntimeAttempt{ID: "attempt-1", TenantID: "tenant-1", JobRunID: "job-1", HarnessKind: "fake", State: contentruntime.RuntimeAttemptPrepared, FenceToken: "fence-1", LeaseExpiresAt: &leaseExpiry},
		ContextView:   contentruntime.ContextView{Digest: "sha256:context"},
	...
	heartbeats := 0
	recordedEvents := 0
	var activated application.RuntimeWorkerActivateInput
	var finalized application.RuntimeWorkerFinalizeInput
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "runtime.worker.prepare_next":
			var input application.RuntimeWorkerPrepareNextInput
		...
		case "runtime.worker.activate":
			...
			handle.Attempt.State = contentruntime.RuntimeAttemptRunning
		...
		case "runtime.worker.event":
			recordedEvents++
			var input application.RuntimeWorkerEventInput
			_ = json.Unmarshal(envelope.Params, &input)
		...
		case "runtime.worker.finalize":
			...
			writeRuntimeWorkerResponse(t, writer, envelope.Command, application.RuntimeWorkerResult{Handle: handle, Job: contentruntime.JobRun{ID: "job-1"}})
		default:
	...
	result, err := root.runRuntimeWorker(t.Context(), apiclient.New(server.URL, "device-token"), runtimeWorkerRunOptions{HarnessKind: "fake", Workspace: t.TempDir(), AutomationBaseDir: filepath.Join(t.TempDir(), "automation")}, false)
	if err != nil {
		t.Fatal(err)
	}
	if result["state"] != contentruntime.RuntimeAttemptSucceeded || heartbeats < 2 || recordedEvents == 0 {
	...
	if finalized.State != contentruntime.RuntimeAttemptSucceeded || len(finalized.OutputRefs) != 1 || finalized.OutputRefs[0] != "artifact:1" || finalized.OutputDigest != "sha256:result" || finalized.UsedCostMinor != 4 || len(finalized.BusinessPayload) != 0 {
...
func TestRuntimeWorkerUsesAttemptWorkspaceAndCleansItAfterFinalization(t *testing.T) {
	...
	handle := contentruntime.DispatchHandle{
		Node:        contentruntime.NodeRun{ID: "node-isolated"},
		Attempt:     contentruntime.RuntimeAttempt{ID: idgen.New(), TenantID: "tenant-1", JobRunID: "job-isolated", HarnessKind: "workspace-capture", State: contentruntime.RuntimeAttemptPrepared, FenceToken: "fence-isolated", LeaseExpiresAt: &leaseExpiry},
		ContextView: contentruntime.ContextView{Digest: "sha256:context"}, Capabilities: agentadapter.HarnessCapabilities{Kind: "workspace-capture", Events: true, StructuredOutput: true, MaxParallelSessions: 1},
	...
	result, err := root.runRuntimeWorker(t.Context(), apiclient.New(server.URL, "device-token"), runtimeWorkerRunOptions{HarnessKind: "workspace-capture", Workspace: interactiveRoot, AutomationBaseDir: filepath.Join(t.TempDir(), "automation")}, false)
	if err != nil || result["state"] != contentruntime.RuntimeAttemptSucceeded {
...
func TestRuntimeWorkerFinalizesRetryableWhenLocalLeaseDrifts(t *testing.T) {
	...
	handle := contentruntime.DispatchHandle{
		Node:        contentruntime.NodeRun{ID: "node-lease-drift"},
		Attempt:     contentruntime.RuntimeAttempt{ID: idgen.New(), TenantID: "tenant-1", JobRunID: "job-lease-drift", HarnessKind: "lease-drift", State: contentruntime.RuntimeAttemptPrepared, FenceToken: "fence-lease-drift", LeaseExpiresAt: &leaseExpiry},
		ContextView: contentruntime.ContextView{Digest: "sha256:context"}, Capabilities: agentadapter.HarnessCapabilities{Kind: "lease-drift", Events: true, StructuredOutput: true, MaxParallelSessions: 1},
	...
	root := &Root{runtimeHarnesses: registry, stderr: &strings.Builder{}}
	_, err := root.runRuntimeWorker(t.Context(), apiclient.New(server.URL, "device-token"), runtimeWorkerRunOptions{HarnessKind: "lease-drift", AutomationBaseDir: filepath.Join(t.TempDir(), "automation")}, false)
	...
	if finalized.State != contentruntime.RuntimeAttemptRetryableFailed || finalized.ErrorCode != "AUTOMATION_WORKSPACE_LEASE_CHANGED" {
...
func TestRuntimeDaemonCapabilitiesReportHealthyAndUnhealthyInventory(t *testing.T) {
	...
	if err := registry.Register("claude", &probeHarness{kind: "claude", err: fault.Policy("CLAUDE_AUTH_REQUIRED", "not logged in", "login")}); err != nil {
		t.Fatal(err)
	}
	root := &Root{runtimeHarnesses: registry}
...
func TestRuntimeWorkerFinalizeInputSeparatesBusinessPayloadFromExecutionEnvelope(t *testing.T) {
	handle := contentruntime.DispatchHandle{Attempt: contentruntime.RuntimeAttempt{ID: "attempt-1", FenceToken: "fence-1"}}
...
func TestRuntimeWorkerOnceTreatsResourceNotFoundAsEmptyQueue(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.NewEncoder(writer).Encode(map[string]any{
			...
			"error":      &fault.Error{Type: "not_found", Subtype: "resource", Code: "RESOURCE_NOT_FOUND", Message: "当前没有可领取任务"},
		})
	}))
	defer server.Close()
...
func TestRuntimeWorkerRetriesTransientHeartbeatWithoutInterruptingHarness(t *testing.T) {
	...
	handle := contentruntime.DispatchHandle{Node: contentruntime.NodeRun{ID: "node-1"}, Attempt: contentruntime.RuntimeAttempt{ID: "attempt-1", TenantID: "tenant-1", JobRunID: "job-1", HarnessKind: "fake", State: contentruntime.RuntimeAttemptPrepared, FenceToken: "fence-1", LeaseExpiresAt: &leaseExpiry}, ContextView: contentruntime.ContextView{Digest: "sha256:context"}, Capabilities: agentadapter.HarnessCapabilities{Kind: "fake", Events: true, StructuredOutput: true}, ExecutionSpec: testRuntimeExecutionSpec("project-1", "job-1")}
	heartbeats := 0
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		var envelope struct {
			Command string `json:"command"`
		...
		case "runtime.worker.heartbeat":
			...
			if heartbeats == 1 {
				writer.Header().Set("Content-Type", "application/json")
				_ = json.NewEncoder(writer).Encode(map[string]any{"ok": false, "command": envelope.Command, "error": &fault.Error{Type: "network", Code: "UPSTREAM_TEMPORARY", Message: "temporary", Retryable: true}})
				return
			}
		...
		case "runtime.worker.event":
			writeRuntimeWorkerResponse(t, writer, envelope.Command, map[string]any{"recorded": true})
		case "runtime.worker.finalize":
			handle.Attempt.State = contentruntime.RuntimeAttemptSucceeded
			writeRuntimeWorkerResponse(t, writer, envelope.Command, application.RuntimeWorkerResult{Handle: handle})
		}
	}))
	defer server.Close()
	root := &Root{runtimeHarnesses: registry, stderr: &strings.Builder{}}
	result, err := root.runRuntimeWorker(t.Context(), apiclient.New(server.URL, "device-token"), runtimeWorkerRunOptions{HarnessKind: "fake", AutomationBaseDir: filepath.Join(t.TempDir(), "automation")}, false)
	if err != nil || result["state"] != contentruntime.RuntimeAttemptSucceeded || heartbeats < 2 {
...
func TestRuntimeWorkerRetriesPendingEventBeforeSuccessfulFinalize(t *testing.T) {
	...
	leaseExpiry := time.Now().UTC().Add(2 * time.Second)
	handle := contentruntime.DispatchHandle{Attempt: contentruntime.RuntimeAttempt{ID: "attempt-1", FenceToken: "fence-1", LeaseExpiresAt: &leaseExpiry}}
...
func TestRuntimeWorkerInterruptsHarnessAfterStructuredProgressTimeout(t *testing.T) {
	...
	leaseExpiry := time.Now().UTC().Add(time.Second)
	handle := contentruntime.DispatchHandle{Node: contentruntime.NodeRun{ID: "node-1"}, Attempt: contentruntime.RuntimeAttempt{ID: "attempt-1", TenantID: "tenant-1", JobRunID: "job-1", HarnessKind: "stalled", State: contentruntime.RuntimeAttemptPrepared, FenceToken: "fence-1", LeaseExpiresAt: &leaseExpiry}, ContextView: contentruntime.ContextView{Digest: "sha256:context"}, Capabilities: agentadapter.HarnessCapabilities{Kind: "stalled", Events: true, StructuredOutput: true}, ExecutionSpec: testRuntimeExecutionSpec("project-1", "job-1")}
	var finalized application.RuntimeWorkerFinalizeInput
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		var envelope struct {
			Command string          `json:"command"`
	...
	defer server.Close()
	root := &Root{runtimeHarnesses: registry, stderr: &strings.Builder{}}
	_, err := root.runRuntimeWorker(t.Context(), apiclient.New(server.URL, "device-token"), runtimeWorkerRunOptions{HarnessKind: "stalled", AutomationBaseDir: filepath.Join(t.TempDir(), "automation"), IdleTimeout: 30 * time.Millisecond}, false)
	var domainErr *fault.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "HARNESS_PROGRESS_TIMEOUT" {
		t.Fatalf("worker progress timeout = %v", err)
	}
	if finalized.State != contentruntime.RuntimeAttemptRetryableFailed || finalized.ErrorCode != "HARNESS_PROGRESS_TIMEOUT" {
...
func testRuntimeExecutionSpec(projectID, jobID string) contentruntime.RemoteExecutionSpec {
	return contentruntime.RemoteExecutionSpec{
		...
		TaskContract: sourcedomain.TaskContract{
			ContractVersion: "1.0", ContractID: "contract-" + jobID, RunID: jobID, TaskType: "test",
			Project: workspacedomain.Project{ID: projectID}, OutputSchema: sourcedomain.KnowledgeCandidatesSchema,
			Capability: catalogdomain.Capability{ID: sourcedomain.KnowledgeExtractCapability, Digest: "sha256:" + strings.Repeat("a", 64)},
		},
	}
}

func runtimeWorkerHandleServer(t *testing.T, handle *contentruntime.DispatchHandle, finalized *application.RuntimeWorkerFinalizeInput) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "runtime.worker.event":
			writeRuntimeWorkerResponse(t, writer, envelope.Command, map[string]any{"recorded": true})
		case "runtime.worker.finalize":
			var input application.RuntimeWorkerFinalizeInput
			_ = json.Unmarshal(envelope.Params, &input)
			if finalized != nil {
				*finalized = input
			}
			handle.Attempt.State = input.State
			writeRuntimeWorkerResponse(t, writer, envelope.Command, application.RuntimeWorkerResult{Handle: *handle, Job: contentruntime.JobRun{ID: handle.Attempt.JobRunID}})
		default:
...
func TestRuntimeWakeClientSendsRedactedWorkspaceInventoryInInitialFrame(t *testing.T) {
	...
	state.setWorkspaceObservations([]workspacedomain.DaemonWorkspaceObservation{{
...
func TestRuntimeWakeClientResendsWorkspaceInventoryWhenGenerationChanges(t *testing.T) {
	...
	state := newRuntimeWakeClientState("test", map[string]any{"runtime_status": "healthy"})
	state.setWorkspaceObservations([]workspacedomain.DaemonWorkspaceObservation{{WorkspaceID: "workspace-1", ProjectID: "project-1", Status: "ready", Reason: "local_components_observed", Generation: "sha256:first", ObservedAt: time.Now().UTC()}})
	accepted := make(chan struct{}, 1)
	ctx, cancel := context.WithCancel(t.Context())
	done := make(chan error, 1)
	go func() {
	...
	state.setWorkspaceObservations([]workspacedomain.DaemonWorkspaceObservation{{WorkspaceID: "workspace-1", ProjectID: "project-1", Status: "repair_required", Reason: "skill_drift", Generation: "sha256:second", ObservedAt: time.Now().UTC()}})
	select {
	case frame := <-reported:
