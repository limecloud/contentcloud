...
func (r *Root) publishCommand() *cobra.Command {
	...
	for _, submissionType := range reviewdomain.SubmissionTypes() {
...
func buildPublishCheckpoint(options publishBuildOptions) (reviewdomain.SubmissionBundle, publishPreflight, error) {
	resolvedFiles, err := resolvePublishFiles(options.Root, options.SubmissionType, options.Files)
	if err != nil {
		return reviewdomain.SubmissionBundle{}, publishPreflight{}, err
	}
	...
	if environmentDigest == "" {
		return reviewdomain.SubmissionBundle{}, publishPreflight{}, fault.Invalid("ENVIRONMENT_DIGEST_REQUIRED", "工作区绑定缺少 environment_digest，请重新初始化创作环境")
	}
	workspaceStateHash, err := stablehash.Sum(status.Template)
	if err != nil {
		return reviewdomain.SubmissionBundle{}, publishPreflight{}, err
	}
	...
	bundle := reviewdomain.SubmissionBundle{
		...
		LocalRunSummary: reviewdomain.LocalRunSummary{Stage: "publish_preflight", Checks: publishChecks(options.SubmissionType), InputHash: inputHash, OutputHash: inputHash, Versions: map[string]string{"cli": Version, "template": status.Template.TemplateVersion, "environment": environmentDigest}},
		Objects:         objects, SourceDisclosures: disclosures, EnvironmentDigest: environmentDigest, Artifacts: []reviewdomain.SubmissionArtifact{}, Message: strings.TrimSpace(options.Message), IdempotencyKey: options.IdempotencyKey,
	}
	...
	preflight := publishPreflight{
		...
		SubmissionType: options.SubmissionType, SchemaVersion: reviewdomain.SubmissionSchemaVersion(options.SubmissionType), Files: relativePaths(options.Root, resolvedFiles), ObjectCount: len(objects), BlockedCount: blocked,
	...
	planHash, err := stablehash.Sum(preflight)
	if err != nil {
		return reviewdomain.SubmissionBundle{}, publishPreflight{}, err
	}
...
func (r *Root) applyPublishCheckpoint(ctx context.Context, root string, bundle reviewdomain.SubmissionBundle, preflight publishPreflight, approvedPlanID string, confirmed bool) (reviewdomain.SubmissionRevision, error) {
	approvedPlanID = strings.TrimSpace(approvedPlanID)
	if approvedPlanID == "" {
		return reviewdomain.SubmissionRevision{}, fault.Invalid("PUBLISH_PLAN_ID_REQUIRED", "--plan-id 必填；请先运行发布预检并确认返回的 plan_id")
	}
	if approvedPlanID != preflight.PlanID {
		err := fault.Conflict("PUBLISH_PLAN_STALE", "本地发布内容、披露或环境与已确认的 plan_id 不一致")
	...
	if !confirmed {
		return reviewdomain.SubmissionRevision{}, confirmationRequired("发布会将预检中列出的结构化对象和来源披露发送到云端审核；原始文件不会上传")
	}
...
func (r *Root) pullCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "pull <feedback|decisions|approved>", Args: cobra.ExactArgs(1), Short: "拉取不可变审核数据包，不修改业务文件", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if kind != "feedback" && kind != "decisions" && kind != "approved" {
			return fault.Invalid("PULL_KIND_INVALID", "拉取类型只支持 feedback、decisions 或 approved")
		}
		...
		switch kind {
		case "feedback":
			var bundles []reviewdomain.ReviewFeedbackBundle
		...
		case "decisions":
			var delta reviewdomain.DecisionDelta
			...
			if !dryRun && len(delta.Decisions) > 0 {
				hash, _ := stablehash.Sum(delta.Decisions)
...
func (r *Root) submissionCommand() *cobra.Command {
	...
	list := &cobra.Command{Use: "list", Short: "列出当前工作区创建的提交记录", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var values []reviewdomain.Submission
	...
	show := &cobra.Command{Use: "show <submission-id>", Args: cobra.ExactArgs(1), Short: "显示一条提交记录及其不可变版本", RunE: func(cmd *cobra.Command, args []string) error {
		...
		var value application.SubmissionDetails
	...
	approve := &cobra.Command{Use: "approve <revision-id>", Args: cobra.ExactArgs(1), Short: "记录当前提交版本的内部批准结果", RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(reason) == "" {
			return fault.Invalid("APPROVAL_REASON_REQUIRED", "--reason 必填")
		}
		if dryRun {
		...
		var result application.SubmissionApprovalResult
	...
	requestChanges := &cobra.Command{Use: "request-changes <revision-id>", Args: cobra.ExactArgs(1), Short: "退回当前版本并提供可执行的修改意见", RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(changeReason) == "" {
			return fault.Invalid("CHANGE_REASON_REQUIRED", "--reason 必填")
		}
		if changeDryRun {
		...
		var submission reviewdomain.Submission
...
func (r *Root) workspaceClient(root string) (string, *apiclient.Client, localworkspace.Binding, error) {
	...
	if r.projectID != "" && r.projectID != binding.ProjectID {
		return "", nil, binding, fault.Conflict("PROJECT_CONTEXT_MISMATCH", "--project 与本地工作区绑定不一致")
	}
	token, err := localconfig.WorkspaceToken(binding.WorkspaceID)
	if err != nil {
		return "", nil, binding, &fault.Error{Type: "credential", Subtype: "workspace", Code: "WORKSPACE_CREDENTIAL_MISSING", Message: err.Error(), Hint: "重新运行项目 init，或修复系统安全凭据存储", ExitCode: 3}
	}
...
func resolvePublishFiles(root, submissionType string, explicit []string) ([]string, error) {
	...
	if submissionType == "storyboard" {
		...
		sort.Strings(values)
		if len(values) == 0 {
			return nil, fault.Invalid("PUBLISH_FILE_REQUIRED", "没有找到可发布的分镜包清单；请使用 --file 明确指定 manifest.json")
		}
		if len(values) > 1 {
			return nil, fault.Invalid("PUBLISH_FILE_AMBIGUOUS", "发现多个分镜包清单；请使用 --file 明确指定本次审核的 manifest.json")
		}
		return values, nil
	}
	...
	if directory == "" {
		return nil, fault.Invalid("SUBMISSION_TYPE_INVALID", "不支持该提交类型（submission_type）")
	}
	...
	sort.Strings(values)
	if len(values) == 0 {
		return nil, fault.Invalid("PUBLISH_FILE_REQUIRED", "没有找到可发布 JSON；使用 --file 明确指定检查点文件")
	}
	return values, nil
}
...
func discoverContentBatchPublishFiles(root string) ([]string, error) {
	...
	if len(manifests) == 0 {
		return nil, fault.Invalid("PUBLISH_FILE_REQUIRED", "没有找到可发布的内容批次清单；请使用 --file 明确指定 manifest.yaml")
	}
	if len(manifests) > 1 {
		return nil, fault.Invalid("PUBLISH_FILE_AMBIGUOUS", "发现多个内容批次清单；请使用 --file 明确指定本次审核的 manifest.yaml")
	}
...
func contentBatchPublishFiles(root, manifest string) ([]string, error) {
	...
	if len(batch.ContentItemRefs) == 0 {
		return nil, fault.Invalid("CONTENT_BATCH_OBJECTS_REQUIRED", "内容批次清单尚未列出可审核的内容项")
	}
...
func validatePublishDomainFiles(root, submissionType string, files []string) error {
	...
	case "strategy":
		for _, file := range files {
			...
			if err := json.Unmarshal(body, &identity); err != nil {
				return fault.Invalid("STRATEGY_JSON_INVALID", "策略发布文件不是有效的 JSON："+file)
			}
			...
			default:
				return fault.Invalid("STRATEGY_OBJECT_TYPE_INVALID", "策略提交只接受受众分类快照或受众策略版本："+file)
			}
			if err != nil {
				return err
			}
			if !report.Valid {
				lintErr := fault.Invalid("STRATEGY_LINT_FAILED", "策略发布前校验失败："+report.File)
	...
	case "offer":
		for _, file := range files {
			...
			if !report.Valid {
				lintErr := fault.Invalid("COMMERCE_OFFER_LINT_FAILED", "商品方案发布前校验失败："+report.File)
				lintErr.Details = report
				return lintErr
			}
		}
	case "brief":
		for _, file := range files {
			...
			if err := json.Unmarshal(body, &identity); err != nil {
				return fault.Invalid("BRIEF_JSON_INVALID", "创作简报发布文件不是有效的 JSON："+file)
			}
			if identity.SchemaVersion == localworkspace.ArticleBriefSchema {
				...
				if !report.Valid {
					lintErr := fault.Invalid("ARTICLE_BRIEF_LINT_FAILED", "文章简报发布前校验失败："+file)
					lintErr.Details = report
					return lintErr
				}
				continue
			...
			if !report.Valid {
				lintErr := fault.Invalid("BRIEF_LINT_FAILED", "创作简报发布前校验失败："+file)
				lintErr.Details = report
				return lintErr
			}
		}
	case "content_batch":
		for _, file := range files {
			...
			if err := json.Unmarshal(body, &identity); err != nil {
				return fault.Invalid("CONTENT_ITEM_JSON_INVALID", "内容对象不是有效 JSON："+file)
			}
			...
			case localworkspace.ContentItemSchema:
				...
				if !report.Valid {
					lintErr := fault.Invalid("CONTENT_ITEM_LINT_FAILED", "内容项发布前校验失败："+report.File)
					lintErr.Details = report
					return lintErr
				}
			case localworkspace.ArticleSchema:
				...
				if !report.Valid {
					lintErr := fault.Invalid("ARTICLE_ITEM_LINT_FAILED", "文章内容项发布前校验失败："+report.File)
					lintErr.Details = report
					return lintErr
				}
			default:
				return fault.Invalid("CONTENT_SCHEMA_UNSUPPORTED", "内容批次包含不受支持的内容结构版本："+identity.SchemaVersion)
			}
		}
	case "storyboard":
		for _, file := range files {
			...
			if !report.Valid {
				lintErr := fault.Invalid("STORYBOARD_LINT_FAILED", "分镜包发布前校验失败："+report.File)
				lintErr.Details = report
				return lintErr
			}
		}
...
func requiredSubmissionBaseSnapshotIDs(root, submissionType string, objects []reviewdomain.SubmissionObjectRef) ([]string, error) {
	values := []string{}
	for _, object := range objects {
		...
		case "strategy":
			...
			if err := json.Unmarshal(object.Content, &strategy); err != nil {
				return nil, fault.Invalid("AUDIENCE_STRATEGY_JSON_INVALID", "受众策略版本不是有效的 JSON")
			}
			snapshot, err := localworkspace.ApprovedSnapshotForObject(root, "strategy", strategy.TaxonomySnapshotID)
			if err != nil {
				if fault.IsNotFound(err) {
					return nil, fault.Policy("AUDIENCE_TAXONOMY_BASE_SNAPSHOT_REQUIRED", "受众策略版本必须引用本机已拉取的受众分类批准快照", "先执行 contentcloud pull approved --type strategy")
				}
				return nil, err
			}
			values = append(values, snapshot.ID)
		case "storyboard":
			var storyboard work.StoryboardPackage
			if err := json.Unmarshal(object.Content, &storyboard); err != nil {
				return nil, fault.Invalid("STORYBOARD_JSON_INVALID", "分镜包不是有效的 JSON")
			}
			if strings.TrimSpace(storyboard.ApprovedSnapshotID) == "" {
				return nil, fault.Invalid("STORYBOARD_BASE_SNAPSHOT_REQUIRED", "分镜包缺少 approved_snapshot_id")
			}
		...
		case "content_batch":
			...
			if err := json.Unmarshal(object.Content, &identity); err != nil {
				return nil, fault.Invalid("CONTENT_ITEM_JSON_INVALID", "内容批次对象不是有效的 JSON")
			}
...
func publishChecks(submissionType string) []reviewdomain.LocalRunCheck {
	checks := []reviewdomain.LocalRunCheck{{Name: submissionType + "-json", Status: "passed"}, {Name: submissionType + "-preflight", Status: "passed"}}
	if submissionType == "content_batch" {
...
func readPublishObjects(root, submissionType string, files []string) ([]reviewdomain.SubmissionObjectRef, int64, int, string, error) {
	objects := []reviewdomain.SubmissionObjectRef{}
	var total int64
	blocked := 0
	for _, path := range files {
		...
		if len(body) > 20<<20 || total+int64(len(body)) > 50<<20 {
			return nil, 0, 0, "", fault.Invalid("PUBLISH_SIZE_LIMIT", "单个文件不能超过 20 MB，单次发布不能超过 50 MB")
		}
		total += int64(len(body))
		trimmed := strings.TrimSpace(string(body))
		if !json.Valid(body) {
			return nil, 0, 0, "", fault.Invalid("PUBLISH_JSON_INVALID", "发布文件不是有效的 JSON："+relative)
		}
		...
		for _, content := range contents {
			...
			if err != nil {
				return nil, 0, 0, "", fault.Invalid("PUBLISH_OBJECT_INVALID", fmt.Sprintf("对象 %d: %s", len(objects)+1, err.Error()))
			}
			if objectBlocked {
				blocked++
			}
			...
			if id == "" {
				return nil, 0, 0, "", fault.Invalid("PUBLISH_OBJECT_ID_REQUIRED", "每个发布对象都必须有稳定的 ID")
			}
			...
			object, err := reviewdomain.NewSubmissionObjectRef(id, objectType, version, filepath.ToSlash(relative), content)
			if err != nil {
				return nil, 0, 0, "", err
			}
	...
	inputHash, err := stablehash.Sum(objects)
	if err != nil {
		return nil, 0, 0, "", err
	}
...
func readDisclosures(root, path string) ([]reviewdomain.SourceDisclosure, int64, error) {
	if strings.TrimSpace(path) == "" {
		return []reviewdomain.SourceDisclosure{}, 0, nil
	}
	...
	var values []reviewdomain.SourceDisclosure
	if err := json.Unmarshal(body, &values); err != nil {
		return nil, 0, fault.Invalid("DISCLOSURE_JSON_INVALID", "来源披露文件必须是 JSON 数组")
	}
...
func resolvePublishReadPath(root, path, code, message, hint string) (string, string, error) {
	...
	if err != nil {
		...
		if errors.As(err, &domainError) && domainError.Code == "LOCAL_FILE_OUTSIDE_WORKSPACE" {
			return "", "", fault.Policy(code, message, hint)
		}
		return "", "", err
	}
