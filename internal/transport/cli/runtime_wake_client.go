...
func newRuntimeWakeClientState(version string, capabilities map[string]any) *runtimeWakeClientState {
	return &runtimeWakeClientState{
		...
		startedAt: time.Now().UTC(), capabilities: cloneRuntimeCapabilities(capabilities), workspaceObservations: []workspacedomain.DaemonWorkspaceObservation{},
...
func (s *runtimeWakeClientState) nextFrameLocked() runtimeWakeFrame {
	...
	return runtimeWakeFrame{
		...
		WorkspaceObservations: append([]workspacedomain.DaemonWorkspaceObservation(nil), s.workspaceObservations...),
	}
}

func (s *runtimeWakeClientState) setWorkspaceObservations(observations []workspacedomain.DaemonWorkspaceObservation) {
	if s == nil {
		return
	}
	s.mu.Lock()
	next := append([]workspacedomain.DaemonWorkspaceObservation(nil), observations...)
