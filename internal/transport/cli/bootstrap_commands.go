...
func (r *Root) bootstrapApplyCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			if plan.State != "ready" {
				err := fault.Conflict("BOOTSTRAP_PLAN_BLOCKED", "当前插件宿主或目标目录不能执行 bootstrap apply")
			...
			if strings.TrimSpace(planID) == "" {
				return fault.Invalid("BOOTSTRAP_PLAN_ID_REQUIRED", "--plan-id 必填；请使用刚刚确认的 bootstrap plan_id")
			}
			if plan.PlanID != strings.TrimSpace(planID) {
				err := fault.Conflict("BOOTSTRAP_PLAN_STALE", "当前 bootstrap 状态与用户确认的 plan_id 不一致")
			...
			if !accept {
				err := fault.Policy("BOOTSTRAP_CONFIRMATION_REQUIRED", "bootstrap 将安装固定版本的 Agent Plugin、绑定设备、写入目标工作区并启动本机自动化后台服务", "先检查 bootstrap plan，再传入 --accept")
				err.ExitCode = 2
				return err
			}
			...
			var connected application.ConnectDeviceResult
			...
			if connected.EnvironmentManifest == nil {
				return withBootstrapDetails(fault.Conflict("ENVIRONMENT_MANIFEST_REQUIRED", "服务端未返回已签名的创作环境清单（Creative Environment Manifest）"), map[string]any{"recovery": "配置服务端环境控制平面后，运行 bootstrap resume"})
			}
...
func (r *Root) bootstrapResumeCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			if !accept {
				err := fault.Policy("BOOTSTRAP_RESUME_CONFIRMATION_REQUIRED", "resume 将修复固定版本的 Agent Plugin 并重新注册现有工作区", "检查当前工作区后传入 --accept")
				err.ExitCode = 2
				return err
			}
			verifier, err := r.environmentManifestVerifier()
			...
			case "empty", "missing":
				if !hasPrimaryWorkspace || primaryBinding.DeviceID == "" || primaryWorkspace.WorkspaceID == "" || primaryWorkspace.ProjectID == "" {
					return fault.Conflict("BOOTSTRAP_RESUME_BINDING_MISSING", "本机没有可恢复的 Content Work OS 设备、工作区和项目绑定")
				}
			...
			case "workspace":
				...
				if !hasTargetValue(status.Template.Targets, target) {
					return fault.Conflict("BOOTSTRAP_TARGET_MISMATCH", "当前工作区与指定插件宿主不一致")
				}
			default:
				return fault.Conflict("BOOTSTRAP_RESUME_DIRECTORY_CONFLICT", "恢复目标包含未知文件，拒绝覆盖")
			}
			...
			if needsInitialize {
				status, err = initializePluginWorkspace(workspacePlan.Root, string(hostID), r.resolveServer(cfg), application.ConnectDeviceResult{
					Device: workspacedomain.Device{ID: primaryBinding.DeviceID}, WorkspaceID: primaryWorkspace.WorkspaceID, ProjectID: primaryWorkspace.ProjectID,
				}, r.currentTime())
				if err != nil {
					return err
				}
...
func initializePluginWorkspace(root, host, server string, connected application.ConnectDeviceResult, now time.Time) (localworkspace.Status, error) {
	environmentDigest := ""
	if connected.EnvironmentManifest != nil {
...
func storeBootstrapEnvironment(root string, manifest environment.Manifest, registry environment.Registry, pkg plugin.Package, verifier *environment.Verifier, registryVerifier *environment.RegistryVerifier, now time.Time) (localworkspace.EnvironmentState, error) {
	...
	for _, plugin := range manifest.Distribution.Plugins {
		...
		if plugin.ID != pkg.Manifest.Name || plugin.Kind != "scene_plugin" || plugin.Version != pkg.Manifest.Version || plugin.Digest != pkg.Digest {
			return localworkspace.EnvironmentState{}, fault.Conflict("BOOTSTRAP_ENVIRONMENT_PLUGIN_MISMATCH", "创作环境清单中的必装插件与本次已验证的标准包不一致")
		}
	...
	if len(installed) != 1 {
		return localworkspace.EnvironmentState{}, fault.Conflict("BOOTSTRAP_ENVIRONMENT_PLUGIN_UNSUPPORTED", "首版初始化只接受一个环境级必装场景插件")
	}
...
func registerBootstrapWorkspace(ctx context.Context, server, token string, status localworkspace.Status) (workspacedomain.WorkspaceBinding, error) {
	var registered workspacedomain.WorkspaceBinding
...
func requireHealthyWorkspace(report localworkspace.DoctorReport) error {
	...
	err := fault.Conflict("WORKSPACE_DOCTOR_FAILED", "工作区必需检查未全部通过，已阻止云端注册")
	err.Details = report
...
func validateBootstrapServer(value string) error {
	...
	if err != nil || (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" || parsed.User != nil || (parsed.Path != "" && parsed.Path != "/") || parsed.RawQuery != "" || parsed.Fragment != "" {
		return fault.Invalid("SERVER_URL_INVALID", "bootstrap 服务端地址必须是没有内嵌凭据的 HTTP(S) 源地址")
	}
	return nil
}

func withBootstrapDetails(err error, details map[string]any) error {
	if domainError, ok := err.(*fault.Error); ok {
		copy := *domainError
	...
	wrapped := fault.E("runtime", "bootstrap", "BOOTSTRAP_FAILED", err.Error(), 5)
