...
import (
	...
	automationworkspace "github.com/limecloud/contentcloud/internal/local/automation"
...
type runtimeWorkerRunOptions struct {
	...
	// AutomationBaseDir is injectable for tests; production uses the user cache default.
	AutomationBaseDir string
	ResultFile        string
...
func (r *Root) runtimeWorkerCommand() *cobra.Command {
	...
	run := &cobra.Command{Use: "run", Short: "领取、激活、续租并收敛一个 Runtime 节点", RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !ok {
			return fault.Conflict("DEVICE_BINDING_MISSING", "Runtime worker 启动前必须先完成设备注册")
		}
...
func (r *Root) runRuntimeWorker(ctx context.Context, client *apiclient.Client, options runtimeWorkerRunOptions, once bool) (result map[string]any, runErr error) {
	...
	prepare := application.RuntimeWorkerPrepareNextInput{RuntimeWorkerPrepareInput: application.RuntimeWorkerPrepareInput{DaemonInstanceID: options.DaemonInstanceID, HarnessKind: harnessKind, Capabilities: capabilities}}
	if err := client.Dispatch(ctx, "runtime.worker.prepare_next", prepare, &handle); err != nil {
		if once && fault.IsNotFound(err) {
	...
	automation, err := beginRuntimeAttemptWorkspace(handle, interactiveWorkspace, options.AutomationBaseDir)
	if err != nil {
		return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, options.DaemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
	}
	defer func() {
	...
	if stream == nil {
		err = fault.Conflict("HARNESS_STREAM_MISSING", "Runtime Harness 未返回结构化事件流")
		return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, options.DaemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
	}
	defer stream.Close()
	if err := r.refreshRuntimeHandle(ctx, client, "runtime.worker.activate", application.RuntimeWorkerActivateInput{DaemonInstanceID: options.DaemonInstanceID, AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken, Session: session}, &handle); err != nil {
...
func (r *Root) runFixtureRuntimeWorker(ctx context.Context, client *apiclient.Client, handle contentruntime.DispatchHandle, harnessKind string, options runtimeWorkerRunOptions) (map[string]any, error) {
	...
	if err != nil {
		return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, options.DaemonInstanceID, contentruntime.RuntimeAttemptFailed, err)
	}
...
func (r *Root) driveRuntimeWorker(ctx context.Context, client *apiclient.Client, harness agentadapter.AgentHarnessAdapter, session agentadapter.AgentSessionRef, stream agentadapter.EventStream, handle contentruntime.DispatchHandle, workspace *automationworkspace.Workspace, harnessKind, daemonInstanceID string, idleTimeout time.Duration, observe func(runtimeWorkerObservation)) (map[string]any, error) {
	...
	for {
		...
		case <-ctx.Done():
			r.interruptRuntimeHarness(harness, session)
			return nil, r.finalizeRuntimeWorkerError(context.WithoutCancel(ctx), client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, ctx.Err())
		case <-idle.C:
			err := &fault.Error{Type: "runtime", Subtype: harnessKind, Code: "HARNESS_PROGRESS_TIMEOUT", Message: "Runtime Harness 长时间没有结构化进展", Retryable: true, ExitCode: 5}
			r.interruptRuntimeHarness(harness, session)
			return nil, r.finalizeRuntimeWorkerError(context.WithoutCancel(ctx), client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
		case <-heartbeat.C:
			var flushErr error
			...
			if flushErr != nil && !isRetryableDispatchError(flushErr) {
				r.interruptRuntimeHarness(harness, session)
				return nil, r.finalizeRuntimeWorkerError(context.WithoutCancel(ctx), client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, flushErr)
			}
			if err := r.refreshRuntimeHandleWithRetry(ctx, client, "runtime.worker.heartbeat", application.RuntimeWorkerHeartbeatInput{DaemonInstanceID: daemonInstanceID, AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken}, &handle, handle.Attempt.LeaseExpiresAt); err != nil {
				r.interruptRuntimeHarness(harness, session)
				return nil, r.finalizeRuntimeWorkerError(context.WithoutCancel(ctx), client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
			}
		...
		case event, ok := <-stream.Events():
			if !ok {
				err := fault.Conflict("HARNESS_STREAM_CLOSED", "Runtime Harness 在提交结构化结果前关闭了事件流")
				return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
			}
			if event.Session != session {
				err := fault.Conflict("HARNESS_SESSION_MISMATCH", "Runtime Harness 事件会话与当前 Attempt 不一致")
				return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, daemonInstanceID, contentruntime.RuntimeAttemptFailed, err)
			}
			...
			case "result.completed":
				if len(pendingEvents) > 0 {
					...
					if flushErr != nil {
						return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, flushErr)
					}
				}
			...
			case "session.failed", "session.interrupted":
				...
				err := &fault.Error{Type: "runtime", Subtype: harnessKind, Code: code, Message: "Runtime Harness 会话未完成", Retryable: true, ExitCode: 5}
				return nil, r.finalizeRuntimeWorkerError(ctx, client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
			default:
				pendingEvents = append(pendingEvents, event)
				if len(pendingEvents) > runtimeEventBufferMax {
					err := fault.Conflict("RUNTIME_EVENT_BUFFER_FULL", "Runtime 事件缓冲已满，无法在当前网络状态下继续可靠执行")
					r.interruptRuntimeHarness(harness, session)
					return nil, r.finalizeRuntimeWorkerError(context.WithoutCancel(ctx), client, handle, daemonInstanceID, contentruntime.RuntimeAttemptRetryableFailed, err)
				}
				var flushErr error
...
func beginRuntimeAttemptWorkspace(handle contentruntime.DispatchHandle, interactiveRoot, automationBaseDir string) (*automationworkspace.Workspace, error) {
	if handle.Attempt.LeaseExpiresAt == nil {
		return nil, fault.Invalid("RUNTIME_ATTEMPT_LEASE_INVALID", "RuntimeAttempt 缺少自动化工作区租约截止时间")
	}
	forbidden := []string{}
	...
	return automationworkspace.Begin(automationworkspace.Options{
		BaseDir: automationBaseDir, AttemptID: handle.Attempt.ID, RunID: handle.ExecutionSpec.TaskContract.RunID, ProjectID: handle.ExecutionSpec.ProjectID,
...
func resolveRuntimeGatewayURL(serverBaseURL, gatewayURL string) (string, error) {
	...
	if err != nil || base.Scheme == "" || base.Host == "" {
		return "", fault.Invalid("RUNTIME_GATEWAY_BASE_URL_INVALID", "Runtime Gateway 缺少有效的服务端来源")
	}
	target, err := url.Parse(gatewayURL)
	if err != nil {
		return "", fault.Invalid("RUNTIME_GATEWAY_URL_INVALID", "Runtime Gateway 地址无效")
	}
	if target.IsAbs() {
		if !strings.EqualFold(target.Scheme, base.Scheme) || !strings.EqualFold(target.Host, base.Host) {
			return "", fault.Policy("RUNTIME_GATEWAY_ORIGIN_MISMATCH", "Runtime Gateway 地址不是当前 ContentCloud 服务端来源", "拒绝向跨源地址发送 Attempt 凭据")
		}
		if target.RawQuery != "" || target.Fragment != "" || target.User != nil {
			return "", fault.Invalid("RUNTIME_GATEWAY_URL_INVALID", "Runtime Gateway 地址不能包含查询、片段或用户信息")
		}
		return target.String(), nil
	}
	if !strings.HasPrefix(target.Path, "/") {
		return "", fault.Invalid("RUNTIME_GATEWAY_URL_INVALID", "Runtime Gateway 相对地址必须以 / 开头")
	}
	if target.RawQuery != "" || target.Fragment != "" {
		return "", fault.Invalid("RUNTIME_GATEWAY_URL_INVALID", "Runtime Gateway 地址不能包含查询或片段")
	}
	base.RawQuery = ""
	base.Fragment = ""
	base.User = nil
...
func (r *Root) flushRuntimeEvents(ctx context.Context, client *apiclient.Client, handle contentruntime.DispatchHandle, pending []agentadapter.AgentEvent, daemonInstanceIDs ...string) ([]agentadapter.AgentEvent, error) {
	...
	for len(pending) > 0 {
		err := client.Dispatch(ctx, "runtime.worker.event", application.RuntimeWorkerEventInput{DaemonInstanceID: daemonInstanceID, AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken, Event: pending[0]}, nil)
		if err != nil {
...
func (r *Root) retryRuntimeEvents(ctx context.Context, client *apiclient.Client, handle contentruntime.DispatchHandle, pending []agentadapter.AgentEvent, daemonInstanceIDs ...string) ([]agentadapter.AgentEvent, error) {
	daemonInstanceID := firstDaemonInstanceID(daemonInstanceIDs)
	for len(pending) > 0 {
		err := r.retryRuntimeDispatch(ctx, client, "runtime.worker.event", application.RuntimeWorkerEventInput{DaemonInstanceID: daemonInstanceID, AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken, Event: pending[0]}, nil, handle.Attempt.LeaseExpiresAt)
		if err != nil {
			return pending, err
		}
		pending = pending[1:]
...
func (r *Root) finalizeRuntimeWorkerSuccess(ctx context.Context, client *apiclient.Client, handle contentruntime.DispatchHandle, harnessKind, daemonInstanceID string, payload json.RawMessage) (map[string]any, error) {
	...
	var finalized application.RuntimeWorkerResult
...
func (r *Root) finalizeRuntimeWorkerError(ctx context.Context, client *apiclient.Client, handle contentruntime.DispatchHandle, daemonInstanceID, state string, runErr error) error {
	...
	var finalized application.RuntimeWorkerResult
	if err := r.retryRuntimeDispatch(ctx, client, "runtime.worker.finalize", application.RuntimeWorkerFinalizeInput{DaemonInstanceID: daemonInstanceID, AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken, State: state, ErrorCode: code, SafeSummary: map[string]any{"worker_error": code}}, &finalized, handle.Attempt.LeaseExpiresAt); err != nil {
...
func runtimeWorkerPayload(fixture bool, resultFile string) (json.RawMessage, error) {
	...
	return nil, fault.Invalid("RUNTIME_WORKER_RESULT_REQUIRED", "Runtime worker 缺少 Harness 结果或结构化结果文件")
}

func runtimeWorkerFinalizeInput(handle contentruntime.DispatchHandle, harnessKind string, payload json.RawMessage) (application.RuntimeWorkerFinalizeInput, error) {
	var object map[string]json.RawMessage
	if len(payload) == 0 || json.Unmarshal(payload, &object) != nil {
		return application.RuntimeWorkerFinalizeInput{}, fault.Invalid("HARNESS_RESULT_INVALID", "Runtime Harness 返回的结构化结果无效")
	}
	input := application.RuntimeWorkerFinalizeInput{AttemptID: handle.Attempt.ID, FenceToken: handle.Attempt.FenceToken, State: contentruntime.RuntimeAttemptSucceeded, SafeSummary: map[string]any{"worker": harnessKind}}
	...
	if err := json.Unmarshal(payload, &envelope); err != nil {
		return application.RuntimeWorkerFinalizeInput{}, fault.Invalid("HARNESS_RESULT_INVALID", "Runtime Harness 结果信封无效")
	}
...
func (r *Root) resolveRuntimeWorkerHarness(ctx context.Context, requested string, fixture bool) (string, agentadapter.AgentHarnessAdapter, agentadapter.HarnessCapabilities, error) {
	...
	return "", nil, agentadapter.HarnessCapabilities{}, fault.Policy("AGENT_ADAPTER_UNAVAILABLE", "未检测到支持 Runtime 协议的 Codex 或 Claude Code", "安装并登录支持结构化事件的本机智能体客户端")
}
