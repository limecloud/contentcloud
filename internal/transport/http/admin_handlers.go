...
func (s *Server) platformOverview(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Identity.PlatformOverview(r.Context(), actor)
	s.dispatchResult(w, r, "platform.overview", value, err)
}

func (s *Server) platformRuntimeHealth(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Runtime.PlatformRuntimeHealth(r.Context(), actor)
...
func (s *Server) updatePlatformTenant(w http.ResponseWriter, r *http.Request) {
	...
	value, err := s.service.Identity.UpdatePlatformTenantStatus(r.Context(), actor, chi.URLParam(r, "tenantID"), input.Status, middleware.GetReqID(r.Context()))
...
func (s *Server) updatePlatformTenantContentCapability(w http.ResponseWriter, r *http.Request) {
	...
	value, err := s.service.Identity.UpdatePlatformTenantContentCapability(r.Context(), actor, chi.URLParam(r, "tenantID"), chi.URLParam(r, "contentType"), input.Enabled, middleware.GetReqID(r.Context()))
