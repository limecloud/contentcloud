...
func (s *Server) runtimeWorkerControl(w http.ResponseWriter, r *http.Request) {
	...
	actor, _, err := s.service.Workspace.DeviceActor(r.Context(), token)
	if err != nil {
	...
	go func() {
		for {
			...
			if json.Unmarshal(body, &next) != nil || next.Type != "control.sync_state" {
				readerDone <- fault.Invalid("DAEMON_CONTROL_FRAME_INVALID", "Runtime 控制通道只接受 current-state 状态报告")
				return
			}
			select {
...
func readRuntimeControlSync(ctx context.Context, connection *websocket.Conn) (runtimeControlFrame, error) {
	...
	if messageType != websocket.MessageText || json.Unmarshal(body, &frame) != nil || frame.Type != "control.sync_state" {
		return runtimeControlFrame{}, fault.Invalid("DAEMON_CONTROL_SYNC_REQUIRED", "Runtime 控制通道第一帧必须是 current-state 状态报告")
	}
	return frame, nil
}

func (s *Server) reportRuntimeControlState(ctx context.Context, actor application.Actor, frame runtimeControlFrame, state string) (workspacedomain.DaemonInstance, error) {
	return s.service.Workspace.ReportDaemonInstance(ctx, actor, application.DaemonInstanceReportInput{
