...
func (s *Server) modelProviders(w http.ResponseWriter, r *http.Request) {
	s.ok(w, r, "model.provider.list", s.service.Delivery.ModelProviderIDs())
}

func (s *Server) generateModelCandidate(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.GenerateModelCandidateInput
	if !s.decodeLimit(w, r, &input, 2<<20) {
		return
	}
	value, err := s.service.Delivery.GenerateModelCandidate(r.Context(), actor, chi.URLParam(r, "taskID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) modelGenerationReceipts(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Delivery.ModelGenerationReceipts(r.Context(), actor, chi.URLParam(r, "taskID"))
