...
func (s *Server) customerStudioBootstrap(w http.ResponseWriter, r *http.Request) {
	actor, user := auth(r)
	value, err := s.service.Work.CustomerStudioBootstrap(r.Context(), actor, user)
...
func (s *Server) createCustomerStudioConnectSession(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CreateCustomerStudioConnectSession(r.Context(), actor, chi.URLParam(r, "projectID"), middleware.GetReqID(r.Context()))
...
func (s *Server) customerStudioConnectSession(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CustomerStudioConnectSession(r.Context(), actor, chi.URLParam(r, "sessionID"))
...
func (s *Server) approveCustomerStudioConnectSession(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.ApproveCustomerStudioConnectSession(r.Context(), actor, chi.URLParam(r, "sessionID"), middleware.GetReqID(r.Context()))
...
func (s *Server) denyCustomerStudioConnectSession(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.DenyCustomerStudioConnectSession(r.Context(), actor, chi.URLParam(r, "sessionID"), middleware.GetReqID(r.Context()))
...
func (s *Server) cancelCustomerStudioConnectSession(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CancelCustomerStudioConnectSession(r.Context(), actor, chi.URLParam(r, "sessionID"), middleware.GetReqID(r.Context()))
...
func (s *Server) customerStudioTasks(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CustomerStudioTasks(r.Context(), actor)
	s.dispatchResult(w, r, "studio.task.list", value, err)
}

func (s *Server) customerStudioTask(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CustomerStudioTask(r.Context(), actor, chi.URLParam(r, "taskID"))
...
func (s *Server) createCustomerStudioTask(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.StudioCreateTaskInput
	...
	value, err := s.service.Work.CreateCustomerStudioTask(r.Context(), actor, input, middleware.GetReqID(r.Context()))
...
func (s *Server) customerStudioTaskAction(w http.ResponseWriter, r *http.Request) {
	...
	value, err := s.service.Work.CustomerStudioTaskAction(r.Context(), actor, chi.URLParam(r, "taskID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) addCustomerStudioInspiration(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.StudioAddInspirationInput
	if !s.decode(w, r, &input) {
		return
	}
	if strings.TrimSpace(input.IdempotencyKey) == "" {
	...
	value, err := s.service.Work.AddCustomerStudioInspiration(r.Context(), actor, chi.URLParam(r, "taskID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) decideCustomerStudioTask(w http.ResponseWriter, r *http.Request) {
	...
	value, err := s.service.Work.DecideCustomerStudioTask(r.Context(), actor, chi.URLParam(r, "taskID"), chi.URLParam(r, "decisionID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) attachCustomerStudioAssets(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.StudioAttachAssetsInput
	if !s.decode(w, r, &input) {
		return
	}
	value, err := s.service.Work.AttachCustomerStudioAssets(r.Context(), actor, chi.URLParam(r, "taskID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) attachCustomerStudioMaterials(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.StudioAttachMaterialsInput
	if !s.decode(w, r, &input) {
		return
	}
	value, err := s.service.Source.AttachCustomerWorkspaceMaterials(r.Context(), actor, chi.URLParam(r, "taskID"), input, middleware.GetReqID(r.Context()))
...
func (s *Server) createCustomerStudioFolder(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	var input application.CreateWorkspaceFolderInput
	if !s.decode(w, r, &input) {
		return
	}
	value, err := s.service.Source.CreateCustomerWorkspaceFolder(r.Context(), actor, input, middleware.GetReqID(r.Context()))
...
func (s *Server) uploadCustomerStudioMaterial(w http.ResponseWriter, r *http.Request) {
	...
	value, err := s.service.Source.UploadCustomerWorkspaceMaterial(r.Context(), actor, strings.TrimSpace(r.FormValue("project_id")), strings.TrimSpace(r.FormValue("folder_ref")), strings.TrimSpace(r.FormValue("title")), fileName, r.FormValue("file_type"), data, middleware.GetReqID(r.Context()))
...
func (s *Server) downloadCustomerStudioMaterial(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	material, data, err := s.service.Source.CustomerWorkspaceMaterialBytes(r.Context(), actor, chi.URLParam(r, "id"))
	if err != nil {
...
func (s *Server) customerStudioAssets(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Source.CustomerStudioAssets(r.Context(), actor, strings.TrimSpace(r.URL.Query().Get("project_id")))
...
func (s *Server) customerStudioCreativeResult(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CustomerStudioCreativeResult(r.Context(), actor, strings.TrimSpace(r.URL.Query().Get("task_id")), chi.URLParam(r, "resultID"))
...
func (s *Server) customerStudioDeliveries(w http.ResponseWriter, r *http.Request) {
	actor, _ := auth(r)
	value, err := s.service.Work.CustomerStudioDeliveries(r.Context(), actor)
