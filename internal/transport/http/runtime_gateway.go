...
func (s *Server) runtimeGatewayCall(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.HasPrefix(token, "rtg_") {
		s.fail(w, r, "runtime.gateway.call", fault.E("authentication", "runtime_gateway", "RUNTIME_GATEWAY_TOKEN_INVALID", "Runtime Gateway 凭据无效", 3))
		return
	}
	var input application.RuntimeGatewayCallInput
	if !s.decodeLimit(w, r, &input, 128<<10) {
		return
	}
	value, err := s.service.Runtime.CallRuntimeMCPWithGatewayToken(r.Context(), token, input)
	if err != nil {
