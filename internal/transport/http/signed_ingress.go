...
func verifySignedIngress(w http.ResponseWriter, r *http.Request, secret []byte, codePrefix, subject string) ([]byte, string, *fault.Error) {
	...
	if err != nil || timestamp == "" || provided == "" || absDuration(time.Since(time.Unix(seconds, 0))) > providerIngressClockSkew {
		return nil, "", fault.Invalid(codePrefix+"_REPLAY", subject+" ingress 时间戳无效或已超出重放保护窗口")
	}
	...
	if err != nil || int64(len(body)) > providerIngressBodyLimit {
		return nil, "", fault.Invalid(codePrefix+"_BODY_TOO_LARGE", subject+" ingress 请求体超过大小限制")
	}
	...
	if !hmac.Equal([]byte(expected), []byte(provided)) {
		return nil, "", fault.Invalid(codePrefix+"_SIGNATURE_INVALID", subject+" ingress 签名校验失败")
	}
