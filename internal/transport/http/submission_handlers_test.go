...
func TestSubmissionBFFReviewDoesNotEditRevisionContent(t *testing.T) {
	...
	session, err := service.Identity.Register(t.Context(), "submission-bff@example.com", "long-enough-password", "Reviewer", "Agency")
	if err != nil {
		t.Fatal(err)
	}
	actor, _, _ := service.Identity.SessionActor(t.Context(), session.ID)
	project, _ := service.Workspace.CreateProject(t.Context(), actor, application.CreateProjectInput{BrandName: "Brand", ProductName: "Product"}, "")
	connect, _ := service.Workspace.CreateConnectSession(t.Context(), actor, project.ID, "")
	connected, err := testsupport.ConnectBootstrap(t.Context(), service, actor, connect, application.ConnectDeviceInput{Hostname: "local"})
	if err != nil {
		t.Fatal(err)
	}
	workspaceActor, binding, _ := service.Workspace.WorkspaceActor(t.Context(), connected.WorkspaceToken)
	...
	listed := callBFF[[]reviewdomain.Submission](t, client, http.MethodGet, server.URL+"/api/bff/projects/"+project.ID+"/submissions", nil)
	if len(listed) != 1 || listed[0].CurrentRevisionID != revision.ID {
		t.Fatalf("unexpected submission list: %#v", listed)
	}
	details := callBFF[application.SubmissionDetails](t, client, http.MethodGet, server.URL+"/api/bff/submissions/"+listed[0].ID, nil)
	...
	focused := callBFF[application.SubmissionRevisionView](t, client, http.MethodGet, server.URL+"/api/bff/projects/"+project.ID+"/submission-revisions/"+revision.ID, nil)
	...
	otherProject, err := service.Workspace.CreateProject(t.Context(), actor, application.CreateProjectInput{BrandName: "Other", ProductName: "Product"}, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	foreignSession, err := service.Identity.Register(t.Context(), "foreign-submission@example.com", "long-enough-password", "Foreign", "Foreign Tenant")
	if err != nil {
		t.Fatal(err)
	}
	...
	returned := callBFF[reviewdomain.Submission](t, client, http.MethodPost, server.URL+"/api/bff/submission-revisions/"+revision.ID+"/request-changes", map[string]string{"reason": "补充范围", "json_pointer": "/0/scope"})
	if returned.Status != "changes_requested" {
		t.Fatalf("unexpected returned state: %#v", returned)
	}
	persisted, err := service.Review.SubmissionDetails(t.Context(), actor, returned.ID)
...
func TestV3ContentBatchBFFCompletesClientApprovalChain(t *testing.T) {
	ctx := t.Context()
	service := application.New(application.DependenciesFrom(memory.New()), slog.Default())
	session, err := service.Identity.Register(ctx, "v2-bff@example.com", "long-enough-password", "Owner", "Agency")
	if err != nil {
		t.Fatal(err)
	}
	...
	project, err := service.Workspace.CreateProject(ctx, actor, application.CreateProjectInput{BrandName: "Brand", ProductName: "Product", Channel: "douyin"}, "project")
	if err != nil {
		t.Fatal(err)
	}
	connect, err := service.Workspace.CreateConnectSession(ctx, actor, project.ID, "connect")
	if err != nil {
		t.Fatal(err)
	}
	connected, err := testsupport.ConnectBootstrap(ctx, service, actor, connect, application.ConnectDeviceInput{Hostname: "local"})
	if err != nil {
		t.Fatal(err)
	}
	...
	contentBatch := map[string]any{
		...
		"content_kind":            identitydomain.ContentTypeVideoScript,
	...
	bundle := reviewdomain.SubmissionBundle{BundleVersion: "3.0", SubmissionType: "content_batch", ProjectID: project.ID, WorkspaceID: binding.ID, BaseSnapshotIDs: []string{}, EnvironmentDigest: httpSubmissionEnvironmentDigest, Objects: []reviewdomain.SubmissionObjectRef{mustHTTPSubmissionObject(t, "content-batch:bff:v1", "content_batch", "50-production/batches/bff/manifest.yaml", contentBatch)}, SourceDisclosures: []reviewdomain.SourceDisclosure{}, Artifacts: []reviewdomain.SubmissionArtifact{}, LocalRunSummary: reviewdomain.LocalRunSummary{Checks: []reviewdomain.LocalRunCheck{{Name: "content-batch-lint", Status: "passed"}}}, IdempotencyKey: "bff-content-batch-v3"}
	if err := bundle.SetComputedHash(); err != nil {
		t.Fatal(err)
	}
	revision, err := service.Review.CreateSubmission(ctx, workspaceActor, binding, bundle, "publish")
	if err != nil {
		t.Fatal(err)
	}
	...
	internal := callBFF[application.SubmissionApprovalResult](t, client, http.MethodPost, server.URL+"/api/bff/submission-revisions/"+revision.ID+"/approve", map[string]string{"reason": "internal approved"})
	...
	grant := callBFF[reviewdomain.ReviewGrant](t, client, http.MethodPost, server.URL+"/api/bff/submission-revisions/"+revision.ID+"/review-grants", map[string]string{"reviewer_email": "client@example.com"})
	...
	grants := callBFF[[]reviewdomain.ReviewGrant](t, client, http.MethodGet, server.URL+"/api/bff/submission-revisions/"+revision.ID+"/review-grants", nil)
	...
	unverified := callBFF[application.ReviewProjection](t, client, http.MethodGet, server.URL+"/api/review/"+grant.PlaintextToken+"/projection", nil)
	if unverified.Verified || unverified.Submission != nil {
		t.Fatalf("unverified projection leaked revision content: %#v", unverified)
	}
	verified := callBFF[application.ReviewProjection](t, client, http.MethodPost, server.URL+"/api/review/"+grant.PlaintextToken+"/verify", map[string]string{"otp": grant.PlaintextOTP})
	...
	decision := callBFF[application.ReviewDecisionResult](t, client, http.MethodPost, server.URL+"/api/review/"+grant.PlaintextToken+"/decision", map[string]string{"decision": "approve", "reason": "client approved"})
	...
	listed := callBFF[[]reviewdomain.ApprovedSnapshot](t, client, http.MethodGet, server.URL+"/api/bff/projects/"+project.ID+"/approved-snapshots?type=content_batch", nil)
	if len(listed) != 1 || listed[0].ID != snapshot.ID {
		t.Fatalf("snapshot list mismatch: %#v", listed)
	}
	projection := callBFF[projection.ProjectProjection](t, client, http.MethodGet, server.URL+"/api/bff/projects/"+project.ID+"/projection", nil)
...
func mustHTTPSubmissionObject(t *testing.T, id, objectType, path string, content any) reviewdomain.SubmissionObjectRef {
	t.Helper()
