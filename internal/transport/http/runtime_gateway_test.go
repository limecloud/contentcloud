...
func TestRuntimeGatewayHTTPIsAttemptScopedAndRejectsForgedIdentity(t *testing.T) {
	...
	handle, err = service.Runtime.Runtime().ActivateDispatch(t.Context(), handle, agentadapter.AgentSessionRef{TenantID: started.Job.TenantID, HarnessKind: "fake", SessionID: "gateway-session"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.Runtime.Runtime().FinalizeDispatch(t.Context(), handle, contentruntime.DispatchOutcome{State: contentruntime.RuntimeAttemptSucceeded}); err != nil {
		t.Fatal(err)
	}
	request, _ = http.NewRequestWithContext(t.Context(), http.MethodPost, endpoint, strings.NewReader(validBody))
	request.Header.Set("Authorization", "Bearer "+handle.GatewayToken)
