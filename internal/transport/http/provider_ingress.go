...
func (s *Server) providerCallback(w http.ResponseWriter, r *http.Request) {
	...
	if !decodeProviderIngressBody(body, &input) {
		s.fail(w, r, "provider.callback", fault.Invalid("PROVIDER_CALLBACK_INVALID", "Provider 回调 JSON 无效"))
		return
	}
	if s.service.Runtime.Runtime() == nil {
		s.fail(w, r, "provider.callback", fault.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime"))
		return
	}
	message, effect, err := s.service.Runtime.Runtime().ReceiveProviderCallback(r.Context(), contentruntime.ProviderCallbackInput{
...
func (s *Server) providerBill(w http.ResponseWriter, r *http.Request) {
	...
	if !decodeProviderIngressBody(body, &input) {
		s.fail(w, r, "provider.bill", fault.Invalid("PROVIDER_BILL_INVALID", "Provider 账单 JSON 无效"))
		return
	}
	if s.service.Runtime.Runtime() == nil {
		s.fail(w, r, "provider.bill", fault.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime"))
		return
	}
	...
	if strings.TrimSpace(input.ObservedAt) != "" {
		...
		if err != nil {
			s.fail(w, r, "provider.bill", fault.Invalid("PROVIDER_BILL_INVALID", "Provider 账单 observed_at 必须是 RFC3339 时间"))
			return
		}
		observedAt = parsed.UTC()
	}
	bill, err := s.service.Runtime.Runtime().RecordProviderBill(r.Context(), contentruntime.ProviderBillInput{
		TenantID: tenantID, JobRunID: strings.TrimSpace(input.JobRunID), EffectID: strings.TrimSpace(input.EffectID), ProviderID: providerID,
...
func (s *Server) authenticateProviderIngress(w http.ResponseWriter, r *http.Request) (string, string, []byte, bool) {
	...
	if providerID == "" || tenantID == "" {
		s.fail(w, r, "provider.ingress.auth", fault.Invalid("PROVIDER_INGRESS_SCOPE_INVALID", "Provider ingress 缺少服务商或租户作用域"))
		return "", "", nil, false
	}
	binding, err := s.service.Delivery.ProviderBinding(r.Context(), tenantID, providerID)
	if err != nil {
		s.fail(w, r, "provider.ingress.auth", fault.Policy("PROVIDER_INGRESS_UNBOUND", "Provider ingress 没有有效的租户绑定", "先配置当前租户的 Provider Binding"))
		return "", "", nil, false
	}
	if binding.State != "active" {
		s.fail(w, r, "provider.ingress.auth", fault.Policy("PROVIDER_INGRESS_DISABLED", "Provider ingress 绑定未启用", "启用当前租户的 Provider Binding"))
		return "", "", nil, false
	}
	secret := s.providerCallbackSecrets[tenantID+":"+providerID]
	if len(secret) == 0 {
		s.fail(w, r, "provider.ingress.auth", fault.Policy("PROVIDER_INGRESS_SECRET_UNAVAILABLE", "Provider ingress 未配置回调密钥", "在服务端密钥配置中登记当前租户和 Provider 的 HMAC 密钥"))
		return "", "", nil, false
	}
