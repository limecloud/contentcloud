...
func TestProviderCallbackIngressAuthenticatesAndDeduplicates(t *testing.T) {
	...
	now := time.Now().UTC()
	profile := deliverydomain.ProviderProfile{ProviderID: "provider-a", Version: "1.0.0", Digest: "sha256:" + strings.Repeat("a", 64), AdapterVersion: "provider-a/1", Model: "model", Region: "global", Modes: []string{"image_to_video"}, InputMediaTypes: []string{"application/json"}, OutputMediaType: "video/mp4", DataRetention: "ephemeral", Status: "published", VerifiedAt: now.Add(-time.Minute), ExpiresAt: now.Add(time.Hour)}
	if err := store.CreateProviderProfile(t.Context(), profile); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveProviderBinding(t.Context(), deliverydomain.ProviderBinding{TenantID: "tenant-1", ProviderID: "provider-a", ProfileVersion: profile.Version, State: "active", EgressPolicy: "public", MaxConcurrency: 1, UpdatedBy: "test", UpdatedAt: now}); err != nil {
		t.Fatal(err)
	}
	started, err := service.Runtime.Runtime().Start(t.Context(), contentruntime.StartInput{TenantID: "tenant-1", ProjectID: "project-1", WorkTaskID: "provider-task", BusinessType: "media.generate", SOP: ingressSOP(), BindingDigest: "sha256:" + strings.Repeat("b", 64), InputDigest: "sha256:" + strings.Repeat("c", 64), RuntimePolicyID: "runtime.test/1", ContractMajor: 1, CreatedBy: "test", IdempotencyKey: "provider-ingress-job"})
	if err != nil {
		t.Fatal(err)
	}
	effect, err := service.Runtime.Runtime().RegisterEffect(t.Context(), contentruntime.ExternalEffect{TenantID: "tenant-1", JobRunID: started.Job.ID, NodeRunID: started.Nodes[0].ID, Kind: "media.generate", IdempotencyKey: "provider-ingress-effect", RequestDigest: "sha256:" + strings.Repeat("d", 64), Currency: "CNY", SafeSummary: map[string]any{}})
	if err != nil {
		t.Fatal(err)
	}
	effect, err = service.Runtime.Runtime().ReconcileEffect(t.Context(), "tenant-1", effect.ID, contentruntime.EffectSubmitted, "external-1", "", "", effect.Version)
	if err != nil {
		t.Fatal(err)
	}
	...
	stored, err := service.Runtime.Runtime().Effect(t.Context(), "tenant-1", effect.ID)
	if err != nil || stored.State != contentruntime.EffectSucceeded {
...
func TestProviderCallbackIngressRejectsBadSignatureAndOversizeBody(t *testing.T) {
	...
	_ = store.SaveProviderBinding(t.Context(), deliverydomain.ProviderBinding{TenantID: "tenant-1", ProviderID: "provider-a", ProfileVersion: profile.Version, State: "active", EgressPolicy: "public", MaxConcurrency: 1, UpdatedBy: "test", UpdatedAt: now})
...
func TestProviderBillIngressAuthenticatesMatchesAndDeduplicates(t *testing.T) {
	...
	started, err := service.Runtime.Runtime().Start(t.Context(), contentruntime.StartInput{TenantID: "tenant-1", ProjectID: "project-1", WorkTaskID: "provider-bill-job", BusinessType: "media.generate", SOP: ingressSOP(), BindingDigest: "sha256:" + strings.Repeat("b", 64), InputDigest: "sha256:" + strings.Repeat("c", 64), RuntimePolicyID: "runtime.test/1", ContractMajor: 1, CreatedBy: "test", IdempotencyKey: "provider-bill-job"})
	if err != nil {
		t.Fatal(err)
	}
	effect, err := service.Runtime.Runtime().RegisterEffect(t.Context(), contentruntime.ExternalEffect{TenantID: "tenant-1", JobRunID: started.Job.ID, NodeRunID: started.Nodes[0].ID, Kind: "media.generate", IdempotencyKey: "provider-bill-effect", RequestDigest: "sha256:" + strings.Repeat("d", 64), CostMinor: 120, Currency: "CNY", SafeSummary: map[string]any{}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.Runtime.Runtime().ReconcileEffect(t.Context(), "tenant-1", effect.ID, contentruntime.EffectSubmitted, "external-1", "", "", effect.Version); err != nil {
		t.Fatal(err)
	}
	secret := []byte("ingress-secret")
	server := httptest.NewServer(httpapi.New(service, nil, false, "", httpapi.WithProviderCallbackSecret("tenant-1", "provider-a", secret)).Handler())
	...
	if err != nil || len(bills) != 1 || bills[0].Status != contentruntime.ProviderBillMatched || bills[0].EffectID != effect.ID {
...
func ingressSOP() catalogdomain.SOPVersion {
	return catalogdomain.SOPVersion{ID: "sop-v1", TenantID: "tenant-1", SOPID: "sop-1", Version: 1, SchemaVersion: catalogdomain.SOPSchemaVersion, Name: "Provider ingress", Status: "published", ContentTypes: []string{identitydomain.ContentTypeMarketingVideo}, DefaultExecutionMode: "local", Stages: []catalogdomain.StageDefinition{{ID: "sources", Name: "资料", Order: 10, OutputSchema: "contentcloud.sources/1.0", ExecutionModes: []string{"local"}}}}
}
