...
func (c *Client) Dispatch(ctx context.Context, command string, params any, out any) error {
	...
	if err != nil {
		return &fault.Error{Type: "network", Subtype: "request", Code: "NETWORK_ERROR", Message: err.Error(), Retryable: true, Hint: "检查 server URL 和网络连接", ExitCode: 5}
	}
	defer resp.Body.Close()
	if resp.StatusCode == http.StatusNoContent {
		return &fault.Error{Type: "not_found", Subtype: "empty_queue", Code: "NO_TASK", Message: "当前没有可领取任务", ExitCode: 0}
	}
	...
	if resp.StatusCode == http.StatusTooManyRequests || resp.StatusCode >= http.StatusInternalServerError {
		return &fault.Error{Type: "network", Subtype: "upstream", Code: "UPSTREAM_UNAVAILABLE", Message: fmt.Sprintf("服务端暂时不可用（HTTP %d）", resp.StatusCode), Retryable: true, Hint: "稍后自动重试", ExitCode: 5}
	}
	var env Envelope
