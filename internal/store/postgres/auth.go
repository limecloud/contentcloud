...
func (s *Store) CreateUserDeviceFlow(ctx context.Context, v domain.UserDeviceFlow) error {
...
func scanUserDeviceFlow(row pgx.Row) (domain.UserDeviceFlow, error) {
	var v domain.UserDeviceFlow
	err := row.Scan(&v.ID, &v.DeviceCodeHash, &v.UserCode, &v.UserID, &v.TenantID, &v.State, &v.ExpiresAt, &v.ApprovedAt, &v.ConsumedAt)
	return v, err
}

func (s *Store) UserDeviceFlowByCodeHash(ctx context.Context, hash string) (domain.UserDeviceFlow, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return v, domain.NotFound("登录授权")
	}
	return v, dbError(err)
}

func (s *Store) UserDeviceFlowByUserCode(ctx context.Context, code string) (domain.UserDeviceFlow, error) {
...
func (s *Store) SaveUserDeviceFlow(ctx context.Context, v domain.UserDeviceFlow) error {
...
func (s *Store) CreateCLIToken(ctx context.Context, v domain.CLIToken) error {
...
func scanCLIToken(row pgx.Row) (domain.CLIToken, error) {
	var v domain.CLIToken
	err := row.Scan(&v.ID, &v.UserID, &v.TenantID, &v.TokenHash, &v.ExpiresAt, &v.RevokedAt)
	return v, err
}

func (s *Store) CLITokenByHash(ctx context.Context, hash string) (domain.CLIToken, error) {
