...
func scanBootstrapAttempt(row pgx.Row) (domain.BootstrapAttempt, error) {
	var value domain.BootstrapAttempt
...
func (s *Store) CreateBootstrapAttemptForSession(ctx context.Context, sessionID string, attempt domain.BootstrapAttempt, now time.Time) (domain.BootstrapAttempt, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if active {
			return domain.Conflict("BOOTSTRAP_AUTHORIZATION_ALREADY_STARTED", "这个初始化会话已有一台电脑等待确认")
		}
...
func (s *Store) BootstrapAttempt(ctx context.Context, tenantID, id string) (domain.BootstrapAttempt, error) {
	var result domain.BootstrapAttempt
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanBootstrapAttempt(tx.QueryRow(ctx, bootstrapAttemptSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.NotFound("初始化尝试")
		}
		return err
	})
...
func (s *Store) BootstrapAttemptByTokenHash(ctx context.Context, tokenHash string) (domain.BootstrapAttempt, error) {
...
func (s *Store) ApproveBootstrapAttempt(ctx context.Context, tenantID, sessionID, attemptID, userID string, now time.Time) (domain.BootstrapAttempt, error) {
	return s.decideBootstrapAttempt(ctx, tenantID, sessionID, attemptID, userID, "approved", now)
}

func (s *Store) DenyBootstrapAttempt(ctx context.Context, tenantID, sessionID, attemptID, userID string, now time.Time) (domain.BootstrapAttempt, error) {
	return s.decideBootstrapAttempt(ctx, tenantID, sessionID, attemptID, userID, "denied", now)
}

func (s *Store) decideBootstrapAttempt(ctx context.Context, tenantID, sessionID, attemptID, userID, state string, now time.Time) (domain.BootstrapAttempt, error) {
	var result domain.BootstrapAttempt
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return domain.NotFound("初始化授权")
			}
			return err
		}
		if attempt.State != "pending" || now.After(attempt.ExpiresAt) {
			return domain.Conflict("BOOTSTRAP_AUTHORIZATION_STATE_INVALID", "初始化授权已过期或已经处理")
		}
...
func (s *Store) AppendBootstrapProgress(ctx context.Context, tokenHash string, event domain.BootstrapProgressEvent, now time.Time) (domain.BootstrapProgressEvent, error) {
	...
	err = s.withTenant(ctx, attempt.TenantID, func(tx pgx.Tx) error {
		...
		if now.After(locked.ExpiresAt.Add(30 * time.Minute)) {
			return domain.Conflict("BOOTSTRAP_ATTEMPT_TOKEN_INVALID", "初始化尝试凭据无效或已过期")
		}
		event.AttemptID = locked.ID
		var existing domain.BootstrapProgressEvent
		var facts []byte
...
func (s *Store) BootstrapProgressForSession(ctx context.Context, tenantID, sessionID string) (*domain.BootstrapProgress, error) {
...
func (s *Store) ConsumeBootstrapAttempt(ctx context.Context, tokenHash string, device domain.Device, workspace domain.WorkspaceBinding, now time.Time) (domain.ConnectSession, domain.BootstrapAttempt, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if err != nil || value.State != "approved" || value.ConsumedAt != nil || now.After(value.ExpiresAt) {
			return domain.Conflict("BOOTSTRAP_AUTHORIZATION_INVALID", "初始化授权无效、已使用或已过期")
		}
...
func (s *Store) CompleteBootstrapAttempt(ctx context.Context, tokenHash, state string, now time.Time) (domain.BootstrapAttempt, error) {
	...
	err = s.withTenant(ctx, attempt.TenantID, func(tx pgx.Tx) error {
		...
		if locked.State != "consumed" || now.After(locked.ExpiresAt.Add(30*time.Minute)) {
			return domain.Conflict("BOOTSTRAP_ATTEMPT_STATE_INVALID", "只有已完成浏览器授权并创建设备的初始化尝试可以进入终态")
		}
...
func (s *Store) CreateBootstrapDiagnostic(ctx context.Context, diagnostic domain.BootstrapDiagnostic) (domain.BootstrapDiagnostic, error) {
	var stored domain.BootstrapDiagnostic
	var summary []byte
	...
	stored.Summary, err = decodeJSON[domain.BootstrapDiagnosticSummary](summary)
	return stored, err
}
