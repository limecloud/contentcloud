...
func (s *Store) CreateKnowledgeObject(ctx context.Context, value domain.KnowledgeObject) error {
...
func insertKnowledgeObject(ctx context.Context, tx pgx.Tx, value domain.KnowledgeObject) error {
...
func (s *Store) CreateKnowledgeObjectDecision(ctx context.Context, object domain.KnowledgeObject, decision domain.KnowledgeDecision) error {
	...
	return s.withTenant(ctx, object.TenantID, func(tx pgx.Tx) error {
		...
		if err := tx.QueryRow(ctx, `SELECT digest FROM knowledge_objects WHERE tenant_id=$1 AND id=$2 AND version=$3`, object.TenantID, object.ID, decision.PreviousVersion).Scan(&previousDigest); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return domain.NotFound("知识决策原对象")
			}
			return err
		}
...
func scanKnowledgeObject(row pgx.Row) (domain.KnowledgeObject, error) {
	var value domain.KnowledgeObject
...
func (s *Store) KnowledgeObjects(ctx context.Context, tenantID, projectID string) ([]domain.KnowledgeObject, error) {
...
func (s *Store) KnowledgeObject(ctx context.Context, tenantID, objectID string, version int) (domain.KnowledgeObject, error) {
...
func (s *Store) CreateKnowledgePack(ctx context.Context, value domain.KnowledgePack) error {
	if err := value.Validate(); err != nil {
		return err
	}
	if value.Digest == "" {
...
func scanKnowledgePack(row pgx.Row) (domain.KnowledgePack, error) {
	var value domain.KnowledgePack
	...
	if err == nil {
		value.ObjectRefs, err = decodeJSON[[]domain.KnowledgePackObjectRef](refs)
	}
	if err == nil {
		value.QueryPolicy, err = decodeJSON[domain.KnowledgeQueryPolicy](policy)
	}
	return value, err
}

func (s *Store) KnowledgePacks(ctx context.Context, tenantID, projectID string) ([]domain.KnowledgePack, error) {
...
func (s *Store) KnowledgePack(ctx context.Context, tenantID, id string) (domain.KnowledgePack, error) {
	var result domain.KnowledgePack
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveKnowledgePack(ctx context.Context, value domain.KnowledgePack) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
		...
		if result.RowsAffected() == 0 {
			return domain.Conflict("KNOWLEDGE_PACK_IMMUTABLE", "知识包不存在或已不可修改")
		}
		return nil
	})
}

func (s *Store) CreateKnowledgeSnapshot(ctx context.Context, value domain.KnowledgeSnapshot) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func scanKnowledgeSnapshot(row pgx.Row) (domain.KnowledgeSnapshot, error) {
	var value domain.KnowledgeSnapshot
	var objects []byte
	...
	if err == nil {
		value.Objects, err = decodeJSON[[]domain.KnowledgeObject](objects)
	}
	return value, err
}

func (s *Store) KnowledgeSnapshots(ctx context.Context, tenantID, projectID, packID string) ([]domain.KnowledgeSnapshot, error) {
...
func (s *Store) KnowledgeSnapshot(ctx context.Context, tenantID, id string) (domain.KnowledgeSnapshot, error) {
	var result domain.KnowledgeSnapshot
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanKnowledgeSnapshot(tx.QueryRow(ctx, knowledgeSnapshotSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.NotFound("知识快照")
		}
		result = value
		return err
	})
...
func (s *Store) KnowledgeDecisions(ctx context.Context, tenantID, objectID string) ([]domain.KnowledgeDecision, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var value domain.KnowledgeDecision
