...
func scanChannelBinding(row pgx.Row) (domain.ChannelBinding, error) {
	var value domain.ChannelBinding
...
func (s *Store) CreateChannelBinding(ctx context.Context, value domain.ChannelBinding) error {
...
func (s *Store) ChannelBindings(ctx context.Context, tenantID, projectID string) ([]domain.ChannelBinding, error) {
...
func (s *Store) ChannelBinding(ctx context.Context, tenantID, id string) (domain.ChannelBinding, error) {
	var value domain.ChannelBinding
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		var err error
		value, err = scanChannelBinding(tx.QueryRow(ctx, channelBindingSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		if errors.Is(err, pgx.ErrNoRows) || domain.IsNotFound(err) {
...
func (s *Store) SaveChannelBinding(ctx context.Context, value domain.ChannelBinding) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func scanChannelPublication(row pgx.Row) (domain.ChannelPublication, error) {
	var value domain.ChannelPublication
...
func (s *Store) CreateChannelPublication(ctx context.Context, value domain.ChannelPublication) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) ChannelPublicationByIdempotencyKey(ctx context.Context, tenantID, key string) (domain.ChannelPublication, error) {
	var value domain.ChannelPublication
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		var err error
...
func (s *Store) ChannelPublications(ctx context.Context, tenantID, taskID string) ([]domain.ChannelPublication, error) {
...
func (s *Store) ChannelPublication(ctx context.Context, tenantID, id string) (domain.ChannelPublication, error) {
	var value domain.ChannelPublication
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		var err error
...
func (s *Store) SaveChannelPublication(ctx context.Context, value domain.ChannelPublication) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) ApplyChannelCallback(ctx context.Context, value domain.ChannelPublication, receipt domain.ChannelCallbackReceipt) (bool, error) {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return false, err
	}
