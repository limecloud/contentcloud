...
func (s *Store) CreateModelGenerationReceipt(_ context.Context, value domain.ModelGenerationReceipt) error {
	...
	if _, exists := s.modelGenerationReceipts[value.ID]; exists {
		return domain.Conflict("MODEL_GENERATION_RECEIPT_EXISTS", "模型生成回执已存在")
	}
	s.modelGenerationReceipts[value.ID] = value
	return nil
}

func (s *Store) CreateModelGeneratedRevision(_ context.Context, revision domain.TaskRevision, receipt domain.ModelGenerationReceipt) error {
	...
	if receipt.TaskRevisionID != revision.ID || receipt.TaskID != revision.TaskID || receipt.TenantID != revision.TenantID {
		return domain.Invalid("MODEL_GENERATION_SCOPE_MISMATCH", "模型回执与候选修订作用域不一致")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) ModelGenerationReceipts(_ context.Context, tenantID, taskID string) ([]domain.ModelGenerationReceipt, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	values := []domain.ModelGenerationReceipt{}
