...
func codexHandoffContent(spec codexplugin.Spec, request HandoffRequest) (string, []string, error) {
	switch request.Kind {
	case "project":
		prompt := fmt.Sprintf("[@ContentCloud Video Production](plugin://%s) 在当前已选择的本机 Workspace 中继续 ContentCloud 项目 %s。先调用 workspace_context，并验证返回的 project_id 必须等于 %s；如果未选择 Workspace 或 project_id 不匹配，立即停止，不要扫描其他目录。不要从旧对话历史重建状态，也不要自动执行 pull、claim、publish 或任何本地写入；先报告当前状态和下一步。", spec.PluginID, request.ProjectID, request.ProjectID)
	...
	default:
		return "", nil, domain.Invalid("AGENT_HANDOFF_KIND_INVALID", "不支持的 Agent 恢复入口类型")
	}
}
