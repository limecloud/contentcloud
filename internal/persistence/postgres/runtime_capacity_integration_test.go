...
func TestRuntimePostgresClaimsOneHundredNodesWithTwentyConcurrentWorkers(t *testing.T) {
	...
	suffix := idgen.New()
	session, err := service.Identity.Register(ctx, fmt.Sprintf("capacity-%s@example.com", suffix), "long-enough-password", "Capacity", "Capacity "+suffix)
	if err != nil {
		t.Fatal(err)
	}
	...
	project, err := service.Workspace.CreateProject(ctx, actor, application.CreateProjectInput{BrandName: "Capacity", ProductName: "Runtime Capacity"}, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	sop := catalogdomain.SOPVersion{
		...
		SchemaVersion: catalogdomain.SOPSchemaVersion, Name: "PostgreSQL Capacity", Status: "published", DefaultExecutionMode: "agent",
	}
	...
	started, err := service.Runtime.Runtime().Start(ctx, contentStartInput(actor.TenantID, project.ID, "postgres-capacity-"+suffix, sop))
	if err != nil {
		t.Fatal(err)
	}
	...
	for wave := 0; wave < 5; wave++ {
		...
		for workerIndex := 0; workerIndex < 20; workerIndex++ {
			...
			go func(owner string) {
				defer wait.Done()
				handle, dispatchErr := service.Runtime.Runtime().PrepareRemoteDispatch(ctx, contentDispatchInput(actor.TenantID, started.Job.ID, owner), capabilities)
				if dispatchErr != nil {
	...
	attempts, err := service.Runtime.Runtime().Attempts(ctx, actor.TenantID, started.Job.ID)
	if err != nil || len(attempts) != 100 {
		t.Fatalf("PostgreSQL created unexpected RuntimeAttempt count: attempts=%d err=%v", len(attempts), err)
	}
	if _, err := service.Runtime.Runtime().PrepareRemoteDispatch(ctx, contentDispatchInput(actor.TenantID, started.Job.ID, "postgres-worker-overflow"), capabilities); !fault.IsNotFound(err) {
		t.Fatalf("PostgreSQL scheduler returned work after all 100 nodes were leased: %v", err)
	}
}

func contentStartInput(tenantID, projectID, idempotencyKey string, sop catalogdomain.SOPVersion) contentruntime.StartInput {
