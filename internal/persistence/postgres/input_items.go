...
func scanInputItem(row pgx.Row) (work.InputItem, error) {
	var value work.InputItem
...
func (s *Store) CreateInputItem(ctx context.Context, value work.InputItem) error {
	value.NormalizeCollections()
...
func (s *Store) InputItems(ctx context.Context, tenantID, projectID, status, assigneeUserID string) ([]work.InputItem, error) {
...
func (s *Store) InputItem(ctx context.Context, tenantID, id string) (work.InputItem, error) {
	var result work.InputItem
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) InputItemByIdempotencyKey(ctx context.Context, tenantID, key string) (work.InputItem, error) {
	var result work.InputItem
	if key == "" {
		return result, fault.NotFound("输入收集记录")
	}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveInputItem(ctx context.Context, value work.InputItem, expectedVersion int) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
