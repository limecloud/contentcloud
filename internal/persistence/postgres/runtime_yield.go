...
func scanRuntimeYield(row pgx.Row) (contentruntime.RuntimeYield, error) {
	var value contentruntime.RuntimeYield
	var refs []byte
...
func (s *Store) RuntimeYield(ctx context.Context, tenantID, id string) (contentruntime.RuntimeYield, error) {
	var result contentruntime.RuntimeYield
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanRuntimeYield(tx.QueryRow(ctx, runtimeYieldSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("RuntimeYield")
		}
		return err
	})
	return result, err
...
func (s *Store) RuntimeYields(ctx context.Context, tenantID, jobID string) ([]contentruntime.RuntimeYield, error) {
...
func (s *Store) YieldDispatch(ctx context.Context, yielded contentruntime.RuntimeYield, node contentruntime.NodeRun, expectedNodeVersion int, attempt contentruntime.RuntimeAttempt, expectedAttemptVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int, fenceToken string, event contentruntime.JobEvent) (contentruntime.RuntimeYield, contentruntime.NodeRun, contentruntime.RuntimeAttempt, contentruntime.AgentInstance, error) {
	...
	err := s.withTenantCommand(ctx, yielded.TenantID, "runtime.yield_dispatch", func(tx pgx.Tx) error {
		...
		if currentNode.State != contentruntime.NodeRunning || currentAttempt.State != contentruntime.RuntimeAttemptRunning || currentAgent.State != contentruntime.AgentActive || currentNode.LeaseOwner != currentAttempt.LeaseOwner || currentAttempt.LeaseOwner != event.ActorID || fenceToken == "" || currentNode.FenceToken != fenceToken || currentAttempt.FenceToken != fenceToken {
			return fault.Conflict("DISPATCH_LEASE_STALE", "让出请求不属于当前调度租约")
		}
		if yielded.TenantID != node.TenantID || yielded.JobRunID != node.JobRunID || yielded.NodeRunID != node.ID || yielded.AttemptID != attempt.ID || yielded.AgentInstanceID != agent.ID {
			return fault.Invalid("RUNTIME_YIELD_SCOPE_INVALID", "RuntimeYield 不属于当前调度范围")
		}
		if node.Version != expectedNodeVersion+1 || attempt.Version != expectedAttemptVersion+1 || agent.Version != expectedAgentVersion+1 {
			return fault.Invalid("RUNTIME_YIELD_VERSION_INVALID", "让出后的调度对象版本无效")
		}
	...
	err := s.withTenantCommand(ctx, yielded.TenantID, "runtime.resolve_yield", func(tx pgx.Tx) error {
		...
		if currentYield.State == contentruntime.RuntimeYieldResolved {
			if currentYield.ResumeKey != yielded.ResumeKey {
				return fault.Conflict("RUNTIME_YIELD_ALREADY_RESOLVED", "RuntimeYield 已由其他恢复请求处理")
			}
		...
		if currentYield.Version != expectedYieldVersion || currentNode.Version != expectedNodeVersion || currentAgent.Version != expectedAgentVersion || yielded.Version != expectedYieldVersion+1 || node.Version != expectedNodeVersion+1 || agent.Version != expectedAgentVersion+1 {
			return fault.Conflict("RUNTIME_YIELD_VERSION_CONFLICT", "RuntimeYield 已被更新，请重新读取")
		}
		if err := currentNode.Transition(node.State); err != nil {
			return err
		}
