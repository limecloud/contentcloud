...
func (s *Store) CreateApproval(ctx context.Context, value reviewdomain.ApprovalDecision) error {
...
func (s *Store) Approvals(ctx context.Context, tenantID, subjectID string) ([]reviewdomain.ApprovalDecision, error) {
	result := []reviewdomain.ApprovalDecision{}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var value reviewdomain.ApprovalDecision
...
func (s *Store) AppendAudit(ctx context.Context, value auditdomain.AuditEvent) error {
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) AuditEvents(ctx context.Context, tenantID, projectID string, limit int) ([]auditdomain.AuditEvent, error) {
	result := []auditdomain.AuditEvent{}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var value auditdomain.AuditEvent
			var summary []byte
