...
func scanDaemonInstance(row pgx.Row) (workspacedomain.DaemonInstance, error) {
	var value workspacedomain.DaemonInstance
...
func (s *Store) SaveDaemonInstance(ctx context.Context, value workspacedomain.DaemonInstance) error {
...
func (s *Store) DaemonInstance(ctx context.Context, tenantID, id string) (workspacedomain.DaemonInstance, error) {
	var result workspacedomain.DaemonInstance
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanDaemonInstance(tx.QueryRow(ctx, daemonInstanceSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("DaemonInstance")
		}
		result = value
		return err
	})
...
func (s *Store) DaemonInstances(ctx context.Context, tenantID, deviceID string) ([]workspacedomain.DaemonInstance, error) {
