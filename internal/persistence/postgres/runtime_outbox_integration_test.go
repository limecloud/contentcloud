...
func TestRuntimeOutboxPostgresClaimAndCommandRollback(t *testing.T) {
	...
	tenantID, projectID, taskID, environmentID, sopID := idgen.New(), idgen.New(), idgen.New(), idgen.New(), idgen.New()
	sopVersionID := idgen.New()
	planID, jobID, nodeID := idgen.New(), idgen.New(), idgen.New()
	defer func() {
	...
	setupStatements := []struct {
		...
		{`INSERT INTO sop_versions(tenant_id,id,sop_id,version,schema_version,name,content_types,stages,gates,default_execution_mode,digest,status,created_by,published_by,created_at,published_at) VALUES($1,$2,$3,1,$4,'Runtime test','["marketing_video"]','[{"id":"source","name":"Source","order":10,"output_schema":"contentcloud.source/1.0","execution_modes":["local"]}]','[]','local',$5,'published','test','test',$6,$6)`, []any{tenantID, sopVersionID, sopID, catalogdomain.SOPSchemaVersion, "sha256:" + repeatHex(64, 'a'), now}},
	...
	sop := catalogdomain.SOPVersion{ID: sopVersionID, TenantID: tenantID, SOPID: sopID, Version: 1, SchemaVersion: catalogdomain.SOPSchemaVersion, Name: "Runtime test", Status: "published", Digest: "sha256:" + repeatHex(64, 'a'), ContentTypes: []string{"marketing_video"}, Stages: []catalogdomain.StageDefinition{{ID: "source", Name: "Source", Order: 10, OutputSchema: "contentcloud.source/1.0", ExecutionModes: []string{"local"}}}}
	plan, err := contentruntime.NewCompiler(contentruntime.DefaultRuntimeLimits()).CompileSOP(sop, tenantID, "test", now)
	if err != nil {
		t.Fatal(err)
	}
	plan.ID = planID
	...
	job := contentruntime.JobRun{
		...
		State: contentruntime.JobRunCreated, Version: 1, CreatedBy: "test", CreatedAt: now, UpdatedAt: now,
	}
	binding := contentruntime.ExecutionBindingSnapshot{
		TenantID: tenantID, Digest: job.BindingDigest, SchemaVersion: contentruntime.ExecutionBindingSnapshotSchema,
	...
	node := contentruntime.NodeRun{ID: nodeID, TenantID: tenantID, JobRunID: jobID, NodeKey: "stage:source", State: contentruntime.NodePending, OutputRefs: []string{}, Version: 1, CreatedAt: now, UpdatedAt: now}
	initialEvent := contentruntime.JobEvent{ID: idgen.New(), TenantID: tenantID, JobRunID: jobID, Sequence: 1, Type: "job.created", ActorType: "test", Payload: map[string]any{}, OccurredAt: now}
	if err := store.CreateJobBundle(ctx, job, []contentruntime.NodeRun{node}, initialEvent); err != nil {
		t.Fatal(err)
	}
	...
	claimed, err := commands.ClaimRuntimeOutbox(ctx, tenantID, contentruntime.RuntimeOutboxSubscriberProjection, "projector-a", now, time.Minute, 1)
	if err != nil || len(claimed) != 1 {
		t.Fatalf("expected one claimed message, got %#v, err=%v", claimed, err)
	}
	var second []contentruntime.RuntimeOutboxMessage
	var secondErr error
	var wait sync.WaitGroup
	wait.Add(1)
	go func() {
		defer wait.Done()
		second, secondErr = commands.ClaimRuntimeOutbox(ctx, tenantID, contentruntime.RuntimeOutboxSubscriberProjection, "projector-b", now, time.Minute, 1)
	}()
	wait.Wait()
	...
	if err := commands.AckRuntimeOutbox(ctx, tenantID, claimed[0].ID, contentruntime.RuntimeOutboxSubscriberProjection, "projector-a", now.Add(10*time.Second)); err != nil {
		t.Fatal(err)
	}
	terminalEvent, err := commands.AppendRuntimeEvent(ctx, contentruntime.JobEvent{ID: idgen.New(), TenantID: tenantID, JobRunID: jobID, Type: "attempt.succeeded", ActorType: "worker", Payload: map[string]any{"attempt_id": "attempt-outbox-subscriber-test"}, OccurredAt: now.Add(20 * time.Second)})
	if err != nil {
		t.Fatal(err)
	}
	businessClaim, err := commands.ClaimRuntimeOutbox(ctx, tenantID, contentruntime.RuntimeOutboxSubscriberBusinessResult, "business-a", now.Add(20*time.Second), time.Minute, 1)
	...
	projectionClaim, err := commands.ClaimRuntimeOutbox(ctx, tenantID, contentruntime.RuntimeOutboxSubscriberProjection, "projector-c", now.Add(20*time.Second), time.Minute, 1)
	...
	if err := commands.AckRuntimeOutbox(ctx, tenantID, projectionClaim[0].ID, contentruntime.RuntimeOutboxSubscriberProjection, "projector-c", now.Add(30*time.Second)); err != nil {
		t.Fatal(err)
	}
	if err := commands.AckRuntimeOutbox(ctx, tenantID, businessClaim[0].ID, contentruntime.RuntimeOutboxSubscriberBusinessResult, "business-a", now.Add(30*time.Second)); err != nil {
	...
	next := beforeNode
	if err := next.Transition(contentruntime.NodeReady); err != nil {
		t.Fatal(err)
	}
	next.State = contentruntime.NodeReady
	next.Version++
	_, err = commands.ApplyNodeTransition(ctx, next, beforeNode.Version, contentruntime.JobEvent{ID: beforeEvents[0].ID, TenantID: tenantID, JobRunID: jobID, Type: "node.ready", ActorType: "test", Payload: map[string]any{}, OccurredAt: now.Add(time.Minute)})
	if err == nil {
	...
	patched, err := contentruntime.ApplyGraphPatch(plan, plan.GraphVersion, contentruntime.GraphPatch{
		...
		AddNodes:             []contentruntime.JobPlanNode{{Key: "fanout:item-a", Kind: "fanout_item", Name: "Fanout item A", DependsOn: []string{"stage:source"}, OutputSchema: "contentcloud.test/1.0", RetryMaxAttempts: 1}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	fanoutSetID, fanoutMemberID, fanoutNodeID := idgen.New(), idgen.New(), idgen.New()
	fanoutSet := contentruntime.FanoutSet{ID: fanoutSetID, TenantID: tenantID, JobRunID: jobID, MapNodeKey: "stage:source", JoinNodeKey: "stage:source", Generation: 1, IdempotencyKey: "fanout-postgres-1", MembershipDigest: "sha256:" + repeatHex(64, 'e'), RequestDigest: "sha256:" + repeatHex(64, 'f'), MemberCount: 1, JoinPolicy: contentruntime.JoinPolicy{Strategy: contentruntime.JoinAll, ZeroMemberPolicy: contentruntime.ZeroMemberFail}, Status: contentruntime.FanoutClosed, Version: 1, ClosedAt: &closedAt, CreatedAt: closedAt, UpdatedAt: closedAt}
	fanoutNode := contentruntime.NodeRun{ID: fanoutNodeID, TenantID: tenantID, JobRunID: jobID, NodeKey: "fanout:item-a", State: contentruntime.NodePending, OutputRefs: []string{}, Version: 1, CreatedAt: closedAt, UpdatedAt: closedAt}
	fanoutMember := contentruntime.FanoutMember{ID: fanoutMemberID, TenantID: tenantID, FanoutSetID: fanoutSetID, MemberKey: "fanout:item-a", ItemKey: "item-a", ItemDigest: "sha256:item-a", Generation: 1, NodeRunID: fanoutNodeID, State: contentruntime.FanoutMemberPending, OutputRefs: []string{}, Version: 1, CreatedAt: closedAt, UpdatedAt: closedAt}
	...
	viewID, agentID, attemptID := idgen.New(), idgen.New(), idgen.New()
	reservationID, collectionID, recordID, toolCallID := idgen.New(), idgen.New(), idgen.New(), idgen.New()
	fixtureStatements := []struct {
		query string
		args  []any
	}{
		{`INSERT INTO runtime_context_views(tenant_id,id,job_run_id,node_run_id,attempt_id,schema_version,input_refs,state_refs,event_refs,allowed_tools,max_tokens,budget_minor,digest,created_at,expires_at) VALUES($1,$2,$3,$4,$5,$6,'[]','[]','[]','[]',1024,0,$7,$8,$9)`, []any{tenantID, viewID, jobID, nodeID, attemptID, contentruntime.ContextViewSchema, "sha256:" + repeatHex(64, 'b'), now, now.Add(time.Hour)}},
