...
func (s *Store) CreatePerformanceImportBatch(ctx context.Context, batch performancedomain.PerformanceImportBatch, observations []performancedomain.PerformanceObservation) error {
...
func (s *Store) PerformanceImportBatches(ctx context.Context, tenantID, projectID string) ([]performancedomain.PerformanceImportBatch, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var v performancedomain.PerformanceImportBatch
...
func (s *Store) PerformanceImportBatch(ctx context.Context, tenantID, id string) (performancedomain.PerformanceImportBatch, error) {
	var result performancedomain.PerformanceImportBatch
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) PerformanceObservation(ctx context.Context, tenantID, id string) (performancedomain.PerformanceObservation, error) {
	var result performancedomain.PerformanceObservation
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		v, err := scanPerformanceObservation(tx.QueryRow(ctx, performanceObservationSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = v
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("结果观察")
		}
		return err
	})
	return result, err
...
func (s *Store) PerformanceObservations(ctx context.Context, tenantID, projectID string) ([]performancedomain.PerformanceObservation, error) {
...
func scanPerformanceObservation(row pgx.Row) (performancedomain.PerformanceObservation, error) {
	var v performancedomain.PerformanceObservation
	var metrics []byte
...
func (s *Store) CreateRatingDecision(ctx context.Context, v performancedomain.RatingDecision) error {
...
func (s *Store) RatingDecisions(ctx context.Context, tenantID, projectID string) ([]performancedomain.RatingDecision, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var v performancedomain.RatingDecision
