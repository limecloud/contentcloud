...
func scanEnvironment(row pgx.Row) (catalogdomain.Environment, error) {
	var value catalogdomain.Environment
	...
	if err == nil {
		value.Capabilities, err = decodeJSON[[]catalogdomain.EnvironmentCapability](capabilities)
	}
	value.NormalizeCollections()
	return value, dbError(err)
}

func (s *Store) CreateEnvironment(ctx context.Context, value catalogdomain.Environment) error {
...
func (s *Store) Environments(ctx context.Context, tenantID string) ([]catalogdomain.Environment, error) {
...
func (s *Store) Environment(ctx context.Context, tenantID, id string) (catalogdomain.Environment, error) {
	var result catalogdomain.Environment
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanEnvironment(tx.QueryRow(ctx, environmentSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("执行环境")
		}
		return err
	})
	return result, err
...
func (s *Store) SaveEnvironment(ctx context.Context, value catalogdomain.Environment) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func scanSOPDefinition(row pgx.Row) (catalogdomain.SOPDefinition, error) {
	var value catalogdomain.SOPDefinition
...
func scanSOPVersion(row pgx.Row) (catalogdomain.SOPVersion, error) {
	...
	if err == nil {
		value.Stages, err = decodeJSON[[]catalogdomain.StageDefinition](stages)
	}
	if err == nil {
		value.Gates, err = decodeJSON[[]catalogdomain.GateDefinition](gates)
	}
	value.NormalizeCollections()
	return value, dbError(err)
}

func (s *Store) CreateSOP(ctx context.Context, definition catalogdomain.SOPDefinition, version catalogdomain.SOPVersion) error {
...
func (s *Store) SaveSOPDefinition(ctx context.Context, value catalogdomain.SOPDefinition) error {
	if value.ID == "" || value.TenantID == "" || value.Name == "" {
		return fault.Invalid("SOP_INVALID", "流程规范定义缺少 ID、租户或名称")
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) CreateSOPVersion(ctx context.Context, version catalogdomain.SOPVersion) error {
	if err := version.Validate(); err != nil {
		return err
	}
...
func (s *Store) SOPs(ctx context.Context, tenantID string) ([]catalogdomain.SOPSummary, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		definitions := []catalogdomain.SOPDefinition{}
		for rows.Next() {
		...
		bySOP := map[string][]catalogdomain.SOPVersion{}
		for versions.Next() {
		...
		for _, definition := range definitions {
			result = append(result, catalogdomain.SOPSummary{Definition: definition, Versions: bySOP[definition.ID]})
		}
		return nil
	})
	return result, err
...
func (s *Store) SOP(ctx context.Context, tenantID, id string) (catalogdomain.SOPSummary, error) {
	result := catalogdomain.SOPSummary{Versions: []catalogdomain.SOPVersion{}}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveSOPVersion(ctx context.Context, value catalogdomain.SOPVersion) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
		...
		if err == nil && command.RowsAffected() == 0 {
			return fault.Conflict("SOP_VERSION_IMMUTABLE", "流程规范草稿不存在或版本已发布")
		}
		return dbError(err)
	})
}

func (s *Store) PublishSOPVersion(ctx context.Context, tenantID, sopID string, version int, publishedBy string, publishedAt time.Time) (catalogdomain.SOPVersion, error) {
...
func (s *Store) RetireSOPVersion(ctx context.Context, tenantID, sopID string, version int, retiredAt time.Time) error {
	return s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if err == nil && command.RowsAffected() == 0 {
			return fault.Conflict("SOP_VERSION_STATE_INVALID", "流程规范版本状态已变化")
		}
		if err != nil {
			return dbError(err)
		}
...
func (s *Store) SaveProjectSOPBinding(ctx context.Context, value catalogdomain.ProjectSOPBinding) error {
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) ProjectSOPBinding(ctx context.Context, tenantID, projectID string) (catalogdomain.ProjectSOPBinding, error) {
	var result catalogdomain.ProjectSOPBinding
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ProjectSOPBindings(ctx context.Context, tenantID string) ([]catalogdomain.ProjectSOPBinding, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		defer rows.Close()
		for rows.Next() {
			var value catalogdomain.ProjectSOPBinding
...
func scanWorkTask(row pgx.Row) (work.WorkTask, error) {
	var value work.WorkTask
...
func (s *Store) CreateWorkTask(ctx context.Context, value work.WorkTask) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) WorkTaskByIdempotencyKey(ctx context.Context, tenantID, key string) (work.WorkTask, error) {
	var result work.WorkTask
	if key == "" {
		return result, fault.NotFound("任务")
	}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) WorkTasks(ctx context.Context, tenantID, projectID string) ([]work.WorkTask, error) {
...
func scanConversationImport(row pgx.Row) (work.ConversationImport, error) {
	var value work.ConversationImport
	...
	if err == nil && bundleText != "" {
		bundle, decodeErr := decodeJSON[work.ConversationBundle]([]byte(bundleText))
		err = decodeErr
		if err == nil {
			value.Bundle = &bundle
		}
...
func (s *Store) CreateConversationImport(ctx context.Context, value work.ConversationImport) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) ConversationImport(ctx context.Context, tenantID, id string) (work.ConversationImport, error) {
	var result work.ConversationImport
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ConversationImportByIdempotencyKey(ctx context.Context, tenantID, key string) (work.ConversationImport, error) {
	var result work.ConversationImport
	if key == "" {
		return result, fault.NotFound("对话导入请求")
	}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ConversationImportsForTask(ctx context.Context, tenantID, taskID string) ([]work.ConversationImport, error) {
...
func (s *Store) SaveConversationImport(ctx context.Context, value work.ConversationImport) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func nullableJSONBundle(value *work.ConversationBundle) any {
	if value == nil {
		return nil
	}
...
func (s *Store) WorkTask(ctx context.Context, tenantID, id string) (work.WorkTask, error) {
	var result work.WorkTask
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveWorkTask(ctx context.Context, value work.WorkTask) error {
	if err := value.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func scanStageRun(row pgx.Row) (work.StageRun, error) {
	var value work.StageRun
...
func (s *Store) StageRuns(ctx context.Context, tenantID, taskID string) ([]work.StageRun, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		byRun := map[string][]work.TaskStageOutput{}
		...
		for index := range result {
			...
			if result[index].Outputs == nil {
				result[index].Outputs = []work.TaskStageOutput{}
			}
		}
		return nil
	})
...
func (s *Store) CreateStageRun(ctx context.Context, value work.StageRun) error {
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveStageRun(ctx context.Context, value work.StageRun) error {
	return s.withTenant(ctx, value.TenantID, func(tx pgx.Tx) error {
