...
func (s *Store) NextReadyNode(ctx context.Context, tenantID, jobID string, allowedProjectIDs []string) (contentruntime.NodeRun, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("可调度的执行节点")
		}
		return err
	})
	return result, err
...
func (s *Store) AgentInstanceForNode(ctx context.Context, tenantID, nodeID string) (contentruntime.AgentInstance, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("节点 AgentInstance")
		}
		return err
	})
	return result, err
...
func scanRuntimeAttempt(row pgx.Row) (contentruntime.RuntimeAttempt, error) {
	var value contentruntime.RuntimeAttempt
...
func (s *Store) RuntimeAttempt(ctx context.Context, tenantID, id string) (contentruntime.RuntimeAttempt, error) {
	var result contentruntime.RuntimeAttempt
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) RuntimeAttemptByGatewayTokenHash(ctx context.Context, tokenHash string) (contentruntime.RuntimeAttempt, error) {
	...
	if err := s.pool.QueryRow(ctx, `SELECT tenant_id,attempt_id FROM contentcloud_lookup_runtime_gateway_token($1)`, tokenHash).Scan(&tenantID, &attemptID); err != nil {
		...
		return contentruntime.RuntimeAttempt{}, err
	}
	return s.RuntimeAttempt(ctx, tenantID, attemptID)
}

func (s *Store) RuntimeAttempts(ctx context.Context, tenantID, jobID string) ([]contentruntime.RuntimeAttempt, error) {
	...
	err := s.withTenantCommand(ctx, node.TenantID, "runtime.prepare_dispatch", func(tx pgx.Tx) error {
		...
		if createAgent {
			...
			if currentAgent.Version != expectedAgentVersion || agent.Version != expectedAgentVersion+1 || currentAgent.JobRunID != agent.JobRunID || currentAgent.NodeRunID != agent.NodeRunID || currentAgent.ParentAgentInstanceID != "" || currentAgent.HarnessKind != agent.HarnessKind || currentAgent.ExecutionProfileID != agent.ExecutionProfileID {
				return fault.Conflict("AGENT_INSTANCE_DISPATCH_CONFLICT", "节点 AgentInstance 已被更新或执行配置发生变化")
			}
			...
			if result.RowsAffected() != 1 {
				return fault.Conflict("AGENT_INSTANCE_DISPATCH_CONFLICT", "节点 AgentInstance 已被更新")
			}
		}
...
func (s *Store) ActivateDispatch(ctx context.Context, node contentruntime.NodeRun, expectedNodeVersion int, attempt contentruntime.RuntimeAttempt, expectedAttemptVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int, event contentruntime.JobEvent) (contentruntime.NodeRun, contentruntime.RuntimeAttempt, contentruntime.AgentInstance, error) {
	...
	err := s.withTenantCommand(ctx, node.TenantID, "runtime.activate_dispatch", func(tx pgx.Tx) error {
		...
		if currentNode.Version != expectedNodeVersion || currentAttempt.Version != expectedAttemptVersion || currentAgent.Version != expectedAgentVersion {
			return fault.Conflict("DISPATCH_VERSION_CONFLICT", "调度状态已经被其他执行者更新")
		}
		if currentNode.State != contentruntime.NodeLeased || currentAttempt.State != contentruntime.RuntimeAttemptPrepared || currentAgent.State != contentruntime.AgentRunnable || currentNode.LeaseOwner != currentAttempt.LeaseOwner || currentAttempt.LeaseOwner != event.ActorID || currentNode.FenceToken == "" || currentNode.FenceToken != currentAttempt.FenceToken || currentNode.FenceToken != node.FenceToken || currentAttempt.LeaseExpiresAt == nil || !currentAttempt.LeaseExpiresAt.After(attempt.UpdatedAt) {
			return fault.Conflict("DISPATCH_LEASE_STALE", "调度租约无效、已过期或不属于当前执行者")
		}
		if currentAttempt.NodeRunID != currentNode.ID || currentAttempt.AgentInstanceID != currentAgent.ID || currentAttempt.ContextViewID != currentAgent.ContextViewID {
			return fault.Conflict("DISPATCH_SCOPE_INVALID", "调度对象不属于同一 Node、Attempt 或 Agent")
		}
		...
		if node.Version != expectedNodeVersion+1 || attempt.Version != expectedAttemptVersion+1 || agent.Version != expectedAgentVersion+1 || attempt.SessionRef == "" || agent.SessionRef != attempt.SessionRef {
			return fault.Invalid("DISPATCH_ACTIVATE_INVALID", "激活后的调度状态不一致")
		}
...
func (s *Store) HeartbeatDispatch(ctx context.Context, tenantID, attemptID, owner, fenceToken string, expectedNodeVersion, expectedAttemptVersion int, now time.Time, leaseFor time.Duration) (contentruntime.NodeRun, contentruntime.RuntimeAttempt, error) {
	...
	var attempt contentruntime.RuntimeAttempt
	err := s.withTenantCommand(ctx, tenantID, "runtime.heartbeat_dispatch", func(tx pgx.Tx) error {
		...
		if currentNode.Version != expectedNodeVersion || currentAttempt.Version != expectedAttemptVersion {
			return fault.Conflict("DISPATCH_VERSION_CONFLICT", "调度状态已经被更新，请重新读取")
		}
		if currentNode.State != contentruntime.NodeRunning || currentAttempt.State != contentruntime.RuntimeAttemptRunning || currentNode.LeaseOwner != owner || currentAttempt.LeaseOwner != owner || currentNode.FenceToken != fenceToken || currentAttempt.FenceToken != fenceToken || currentNode.LeaseExpiresAt == nil || currentAttempt.LeaseExpiresAt == nil || !currentNode.LeaseExpiresAt.After(now) || !currentAttempt.LeaseExpiresAt.After(now) {
			return fault.Conflict("DISPATCH_LEASE_STALE", "调度租约无效、已过期或不属于当前执行者")
		}
		...
		if nodeResult.RowsAffected() != 1 || attemptResult.RowsAffected() != 1 {
			return fault.Conflict("DISPATCH_LEASE_STALE", "调度租约已经失效，请重新领取")
		}
...
func (s *Store) FinalizeDispatch(ctx context.Context, node contentruntime.NodeRun, expectedNodeVersion int, attempt contentruntime.RuntimeAttempt, expectedAttemptVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int, fenceToken string, event contentruntime.JobEvent) (contentruntime.NodeRun, contentruntime.RuntimeAttempt, contentruntime.AgentInstance, error) {
	...
	err := s.withTenantCommand(ctx, node.TenantID, "runtime.finalize_dispatch", func(tx pgx.Tx) error {
		...
		if currentAttempt.Terminal() {
			...
			return fault.Conflict("RUNTIME_ATTEMPT_RESULT_CONFLICT", "RuntimeAttempt 已收到不同的终态结果")
		}
		if currentNode.Version != expectedNodeVersion || currentAttempt.Version != expectedAttemptVersion || currentAgent.Version != expectedAgentVersion {
			return fault.Conflict("DISPATCH_VERSION_CONFLICT", "调度状态已经被其他执行者更新")
		}
		...
		if currentNode.LeaseOwner == "" || currentNode.LeaseOwner != currentAttempt.LeaseOwner || currentAttempt.LeaseOwner != event.ActorID || strings.TrimSpace(fenceToken) == "" || currentNode.FenceToken != fenceToken || currentAttempt.FenceToken != fenceToken || currentNode.LeaseExpiresAt == nil || currentAttempt.LeaseExpiresAt == nil || !currentNode.LeaseExpiresAt.After(event.OccurredAt) || !currentAttempt.LeaseExpiresAt.After(event.OccurredAt) {
			return fault.Conflict("DISPATCH_LEASE_STALE", "终态结果不属于当前调度租约")
		}
		if err := currentNode.Transition(node.State); err != nil {
			return err
		}
		...
		if node.Version != expectedNodeVersion+1 || attempt.Version != expectedAttemptVersion+1 || agent.Version != expectedAgentVersion+1 {
			return fault.Invalid("DISPATCH_FINALIZE_INVALID", "终态调度对象的版本不一致")
		}
		if err := updateDispatchStateTx(ctx, tx, node, expectedNodeVersion, attempt, expectedAttemptVersion, agent, expectedAgentVersion); err != nil {
			return err
		}
...
func lockDispatchState(ctx context.Context, tx pgx.Tx, tenantID, nodeID, attemptID, agentID string) (contentruntime.NodeRun, contentruntime.RuntimeAttempt, contentruntime.AgentInstance, error) {
...
func updateDispatchStateTx(ctx context.Context, tx pgx.Tx, node contentruntime.NodeRun, expectedNodeVersion int, attempt contentruntime.RuntimeAttempt, expectedAttemptVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int) error {
...
func appendRuntimeEventTx(ctx context.Context, tx pgx.Tx, event contentruntime.JobEvent) (contentruntime.JobEvent, error) {
	if event.ID == "" || event.TenantID == "" || event.JobRunID == "" || strings.TrimSpace(event.Type) == "" || strings.TrimSpace(event.ActorType) == "" || event.OccurredAt.IsZero() {
		return event, fault.Invalid("JOB_EVENT_INVALID", "JobEvent 缺少执行实例、类型或执行者")
	}
