...
func scanRuntimeFanoutSet(row pgx.Row) (contentruntime.FanoutSet, error) {
	var value contentruntime.FanoutSet
...
func scanRuntimeFanoutMember(row pgx.Row) (contentruntime.FanoutMember, error) {
	var value contentruntime.FanoutMember
	var outputRefs []byte
...
func (s *Store) FanoutSet(ctx context.Context, tenantID, id string) (contentruntime.FanoutSet, error) {
	var result contentruntime.FanoutSet
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("FanoutSet")
		}
		return err
	})
	return result, err
...
func (s *Store) FanoutSetByIdempotencyKey(ctx context.Context, tenantID, jobID, idempotencyKey string) (contentruntime.FanoutSet, error) {
	var result contentruntime.FanoutSet
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) FanoutSets(ctx context.Context, tenantID, jobID string) ([]contentruntime.FanoutSet, error) {
...
func (s *Store) FanoutMembers(ctx context.Context, tenantID, setID string) ([]contentruntime.FanoutMember, error) {
...
func (s *Store) CreateFanoutSetCommand(ctx context.Context, nextJob contentruntime.JobRun, expectedJobVersion int, plan contentruntime.JobPlanRevision, set contentruntime.FanoutSet, members []contentruntime.FanoutMember, nodes []contentruntime.NodeRun, event contentruntime.JobEvent) (contentruntime.JobRun, error) {
	...
	err := s.withTenantCommand(ctx, set.TenantID, "runtime.create_fanout", func(tx pgx.Tx) error {
		...
		if currentJob.Version != expectedJobVersion || nextJob.Version != expectedJobVersion+1 || currentJob.PlanRevisionID != plan.BaseRevisionID || nextJob.PlanRevisionID != plan.ID || nextJob.PlanDigest != plan.Digest {
			return fault.Conflict("GRAPH_VERSION_CONFLICT", "执行图版本已经变化，请重新读取")
		}
		...
		for _, node := range nodes {
			if node.TenantID != set.TenantID || node.JobRunID != set.JobRunID {
				return fault.Invalid("NODE_RUN_SCOPE_INVALID", "FanoutSet 子节点不属于当前执行实例")
			}
...
func (s *Store) ApplyFanoutJoinCommand(ctx context.Context, set contentruntime.FanoutSet, expectedVersion int, members []contentruntime.FanoutMember, cancelMemberKeys []string, event contentruntime.JobEvent) (contentruntime.FanoutSet, error) {
	if err := set.Validate(); err != nil {
		return set, err
	}
	var result contentruntime.FanoutSet
	err := s.withTenantCommand(ctx, set.TenantID, "runtime.apply_fanout_join", func(tx pgx.Tx) error {
		...
		if current.Version != expectedVersion || set.Version != expectedVersion+1 {
			return fault.Conflict("FANOUT_SET_VERSION_CONFLICT", "FanoutSet 已被更新，请重新读取")
		}
		if len(members) != current.MemberCount {
			return fault.Conflict("FANOUT_MEMBER_COUNT_MISMATCH", "FanoutSet 成员数量与封存快照不一致")
		}
		...
		for _, member := range members {
			...
			if errors.Is(err, pgx.ErrNoRows) {
				return fault.NotFound("FanoutSet 成员")
			}
			if err != nil {
				return err
			}
			if member.TenantID != current.TenantID || member.FanoutSetID != current.ID || (member.Version != stored.Version && member.Version != stored.Version+1) {
				return fault.Conflict("FANOUT_MEMBER_VERSION_CONFLICT", "FanoutSet 成员已被更新，请重新读取")
			}
		...
		for _, memberKey := range cancelMemberKeys {
			...
			if member.State != contentruntime.FanoutMemberPending {
				continue
			}
			...
			if errors.Is(err, pgx.ErrNoRows) {
				return fault.NotFound("FanoutSet 子节点")
			}
			if err != nil {
				return err
			}
			if node.JobRunID != current.JobRunID || (node.State != contentruntime.NodePending && node.State != contentruntime.NodeReady) {
				return fault.Conflict("FANOUT_CANCEL_CONFLICT", "只能取消尚未领取的 Fanout 子节点")
			}
