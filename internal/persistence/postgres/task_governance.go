...
func scanGateEvaluation(row pgx.Row) (reviewdomain.GateEvaluation, error) {
	var value reviewdomain.GateEvaluation
...
func (s *Store) CreateGateEvaluation(ctx context.Context, value reviewdomain.GateEvaluation) error {
	value.NormalizeCollections()
...
func (s *Store) GateEvaluations(ctx context.Context, tenantID, taskID string) ([]reviewdomain.GateEvaluation, error) {
...
func (s *Store) GateEvaluation(ctx context.Context, tenantID, id string) (reviewdomain.GateEvaluation, error) {
	var result reviewdomain.GateEvaluation
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveGateEvaluation(ctx context.Context, value reviewdomain.GateEvaluation) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func scanTaskRevision(row pgx.Row) (reviewdomain.TaskRevision, error) {
	var value reviewdomain.TaskRevision
...
func (s *Store) CreateTaskRevision(ctx context.Context, value reviewdomain.TaskRevision) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) TaskRevisions(ctx context.Context, tenantID, taskID string) ([]reviewdomain.TaskRevision, error) {
...
func (s *Store) TaskRevision(ctx context.Context, tenantID, id string) (reviewdomain.TaskRevision, error) {
	var result reviewdomain.TaskRevision
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanTaskRevision(tx.QueryRow(ctx, taskRevisionSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("任务修订版本")
		}
		return err
	})
	return result, err
...
func scanTaskDelivery(row pgx.Row) (deliverydomain.TaskDelivery, error) {
	var value deliverydomain.TaskDelivery
	var manifest []byte
...
func (s *Store) CreateTaskDelivery(ctx context.Context, value deliverydomain.TaskDelivery) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) TaskDeliveries(ctx context.Context, tenantID, taskID string) ([]deliverydomain.TaskDelivery, error) {
...
func (s *Store) TaskDelivery(ctx context.Context, tenantID, id string) (deliverydomain.TaskDelivery, error) {
	var result deliverydomain.TaskDelivery
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) SaveTaskDelivery(ctx context.Context, value deliverydomain.TaskDelivery) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
