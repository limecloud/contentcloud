...
func (s *Store) CreateExecutionBindingSnapshot(ctx context.Context, value contentruntime.ExecutionBindingSnapshot) error {
...
func (s *Store) ExecutionBindingSnapshot(ctx context.Context, tenantID, digest string) (contentruntime.ExecutionBindingSnapshot, error) {
	var value contentruntime.ExecutionBindingSnapshot
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		...
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("ExecutionBindingSnapshot")
		}
		if err != nil {
			return err
		}
