...
func scanRuntimeProjectionRebuild(row pgx.Row) (contentruntime.RuntimeProjectionRebuildRun, error) {
	var value contentruntime.RuntimeProjectionRebuildRun
...
func (s *Store) CreateRuntimeProjectionRebuild(ctx context.Context, run contentruntime.RuntimeProjectionRebuildRun) error {
...
func (s *Store) UpdateRuntimeProjectionRebuild(ctx context.Context, run contentruntime.RuntimeProjectionRebuildRun, expectedVersion int) error {
	if err := run.Validate(); err != nil {
		return err
	}
	return s.withTenant(ctx, run.TenantID, func(tx pgx.Tx) error {
...
func (s *Store) RuntimeProjectionRebuilds(ctx context.Context, tenantID, jobID string) ([]contentruntime.RuntimeProjectionRebuildRun, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return result, fault.NotFound("投影重建运行事实")
	}
	return result, err
}
