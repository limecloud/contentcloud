...
func scanStateCollection(row pgx.Row) (contentruntime.StateCollection, error) {
	var value contentruntime.StateCollection
...
func scanStateRecord(row pgx.Row) (contentruntime.StateRecord, error) {
	var value contentruntime.StateRecord
	var body []byte
...
func scanToolCall(row pgx.Row) (contentruntime.ToolCall, error) {
	var value contentruntime.ToolCall
...
func (s *Store) CreateStateCollection(ctx context.Context, collection contentruntime.StateCollection) error {
...
func (s *Store) StateCollection(ctx context.Context, tenantID, id string) (contentruntime.StateCollection, error) {
	var result contentruntime.StateCollection
...
func (s *Store) StateCollections(ctx context.Context, tenantID, jobID string) ([]contentruntime.StateCollection, error) {
	result := []contentruntime.StateCollection{}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) StateRecord(ctx context.Context, tenantID, id string) (contentruntime.StateRecord, error) {
	var result contentruntime.StateRecord
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		value, err := scanStateRecord(tx.QueryRow(ctx, runtimeStateRecordSelect+` WHERE tenant_id=$1 AND id=$2`, tenantID, id))
		result = value
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("状态记录")
		}
		return err
	})
	return result, err
...
func (s *Store) StateRecords(ctx context.Context, tenantID, collectionID string) ([]contentruntime.StateRecord, error) {
	result := []contentruntime.StateRecord{}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ApplyStateRecordCAS(ctx context.Context, record contentruntime.StateRecord, expectedVersion int) (contentruntime.StateRecord, error) {
...
func applyStateRecordTx(ctx context.Context, tx pgx.Tx, record contentruntime.StateRecord, expectedVersion int) (contentruntime.StateRecord, error) {
	var result contentruntime.StateRecord
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return result, fault.NotFound("状态集合")
	}
	if err != nil {
		return result, err
	}
	if record.TenantID != collection.TenantID {
		return result, fault.Invalid("STATE_RECORD_SCOPE_INVALID", "状态记录与集合不属于同一租户")
	}
	if record.SchemaRevision != collection.SchemaRevision {
		return result, fault.Conflict("STATE_SCHEMA_REVISION_CONFLICT", "状态记录 SchemaRevision 与集合已发布版本不一致")
	}
	...
	if (expectedVersion == 0 && exists) || (exists && current.Version != expectedVersion) || (!exists && expectedVersion != 0) {
		...
		return result, fault.Conflict("STATE_RECORD_VERSION_CONFLICT", "状态记录版本已变化")
	}
	if collection.Consistency == "append_only" && exists {
		return current, fault.Policy("STATE_APPEND_ONLY_UPDATE_FORBIDDEN", "append_only 集合不允许覆盖既有记录", "使用新的记录键追加一条记录")
	}
	if !exists {
		var count int
		...
		if count >= collection.MaxRecords {
			return result, fault.Policy("STATE_COLLECTION_RECORD_LIMIT", "状态集合已达到最大记录数", "清理过期记录或提高已发布集合上限")
		}
	}
	if record.Value != nil && len(jsonValue(record.Value)) > collection.MaxRecordBytes {
		return result, fault.Policy("STATE_RECORD_TOO_LARGE", "状态记录超过集合大小限制", "减少记录内容或改用受控 Artifact 引用")
	}
	if !exists {
		record.Version = 1
...
func (s *Store) ApplyStateRecordCommand(ctx context.Context, record contentruntime.StateRecord, expectedVersion int, event contentruntime.JobEvent) (contentruntime.StateRecord, error) {
	if err := record.Validate(); err != nil {
		return record, err
	}
	var result contentruntime.StateRecord
...
func (s *Store) ApplyFencedStateRecordCommand(ctx context.Context, record contentruntime.StateRecord, expectedVersion int, attemptID, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.StateRecord, error) {
	if err := record.Validate(); err != nil {
		return record, err
	}
	var result contentruntime.StateRecord
...
func (s *Store) RegisterToolCallCommand(ctx context.Context, call contentruntime.ToolCall, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
...
func (s *Store) RegisterFencedToolCallCommand(ctx context.Context, call contentruntime.ToolCall, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	if err := call.Validate(); err != nil {
		return call, err
	}
...
func (s *Store) ApplyToolCallTransitionCommand(ctx context.Context, next contentruntime.ToolCall, expectedVersion int, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	...
	err := s.withTenantCommand(ctx, next.TenantID, "runtime.apply_tool_call_transition", func(tx pgx.Tx) error {
		...
		if errors.Is(err, pgx.ErrNoRows) {
			return fault.NotFound("ToolCall")
		}
		if err != nil {
			return err
		}
		if current.Version != expectedVersion || next.Version != expectedVersion+1 {
			return fault.Conflict("TOOL_CALL_VERSION_CONFLICT", "ToolCall 已被更新")
		}
		if current.State == contentruntime.ToolCallSucceeded || current.State == contentruntime.ToolCallFailed || current.State == contentruntime.ToolCallUnknown {
			return fault.Conflict("TOOL_CALL_TERMINAL", "终态 ToolCall 不能原地修改")
		}
...
func (s *Store) ApplyFencedToolCallTransitionCommand(ctx context.Context, next contentruntime.ToolCall, expectedVersion int, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	if err := next.Validate(); err != nil {
		return next, err
	}
...
func validateRuntimeEventTx(ctx context.Context, tx pgx.Tx, event contentruntime.JobEvent) error {
	...
	if event.Sequence != count+1 {
		return fault.Conflict("JOB_EVENT_SEQUENCE_CONFLICT", "JobEvent 序号必须连续")
	}
	return nil
}

func (s *Store) CreateToolCall(ctx context.Context, call contentruntime.ToolCall) error {
	if err := call.Validate(); err != nil {
		return err
	}
...
func validateToolCallTx(ctx context.Context, tx pgx.Tx, call contentruntime.ToolCall) error {
	...
	if attemptState != contentruntime.RuntimeAttemptPrepared && attemptState != contentruntime.RuntimeAttemptRunning {
		return fault.Conflict("TOOL_CALL_ATTEMPT_NOT_ACTIVE", "只有准备中或运行中的 Attempt 可以创建或推进 ToolCall")
	}
	if agentState != contentruntime.AgentRunnable && agentState != contentruntime.AgentActive {
...
func (s *Store) ToolCall(ctx context.Context, tenantID, id string) (contentruntime.ToolCall, error) {
	var result contentruntime.ToolCall
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ToolCallByIdempotencyKey(ctx context.Context, tenantID, attemptID, toolName, idempotencyKey string) (contentruntime.ToolCall, error) {
	var result contentruntime.ToolCall
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ToolCalls(ctx context.Context, tenantID, attemptID string) ([]contentruntime.ToolCall, error) {
	result := []contentruntime.ToolCall{}
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
...
func (s *Store) ApplyToolCallTransition(ctx context.Context, next contentruntime.ToolCall, expectedVersion int) (contentruntime.ToolCall, error) {
	if err := next.Validate(); err != nil {
		return next, err
	}
	var result contentruntime.ToolCall
