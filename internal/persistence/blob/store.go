...
const (
	maxMemoryObjectBytes = 150 * 1024 * 1024
	maxLocalObjectBytes  = 512 * 1024 * 1024
)
...
func (s *LocalStore) PutReader(_ context.Context, key string, reader io.Reader, size int64) error {
	...
	written, copyErr := io.Copy(tmp, io.LimitReader(reader, maxLocalObjectBytes+1))
	if copyErr != nil {
		_ = tmp.Close()
		return copyErr
	}
	if written > maxLocalObjectBytes || (size >= 0 && written != size) {
		_ = tmp.Close()
