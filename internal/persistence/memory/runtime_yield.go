...
func (s *Store) RuntimeYield(_ context.Context, tenantID, id string) (contentruntime.RuntimeYield, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.runtimeYields[runtimePlanKey(tenantID, id)]
	if !ok {
		return contentruntime.RuntimeYield{}, fault.NotFound("RuntimeYield")
	}
	value.WaitRefs = append([]string(nil), value.WaitRefs...)
	return value, nil
}

func (s *Store) RuntimeYields(_ context.Context, tenantID, jobID string) ([]contentruntime.RuntimeYield, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []contentruntime.RuntimeYield{}
...
func (s *Store) YieldDispatch(_ context.Context, yielded contentruntime.RuntimeYield, node contentruntime.NodeRun, expectedNodeVersion int, attempt contentruntime.RuntimeAttempt, expectedAttemptVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int, fenceToken string, event contentruntime.JobEvent) (contentruntime.RuntimeYield, contentruntime.NodeRun, contentruntime.RuntimeAttempt, contentruntime.AgentInstance, error) {
	...
	if currentNode.Version != expectedNodeVersion || currentAttempt.Version != expectedAttemptVersion || currentAgent.Version != expectedAgentVersion {
		return yielded, node, attempt, agent, fault.Conflict("DISPATCH_VERSION_CONFLICT", "调度状态已经被其他执行者更新")
	}
	if currentNode.State != contentruntime.NodeRunning || currentAttempt.State != contentruntime.RuntimeAttemptRunning || currentAgent.State != contentruntime.AgentActive || currentNode.LeaseOwner != currentAttempt.LeaseOwner || currentAttempt.LeaseOwner != event.ActorID || fenceToken == "" || currentNode.FenceToken != fenceToken || currentAttempt.FenceToken != fenceToken {
		return yielded, node, attempt, agent, fault.Conflict("DISPATCH_LEASE_STALE", "让出请求不属于当前调度租约")
	}
	if yielded.TenantID != node.TenantID || yielded.JobRunID != node.JobRunID || yielded.NodeRunID != node.ID || yielded.AttemptID != attempt.ID || yielded.AgentInstanceID != agent.ID {
		return yielded, node, attempt, agent, fault.Invalid("RUNTIME_YIELD_SCOPE_INVALID", "RuntimeYield 不属于当前调度范围")
	}
	if node.Version != expectedNodeVersion+1 || attempt.Version != expectedAttemptVersion+1 || agent.Version != expectedAgentVersion+1 {
		return yielded, node, attempt, agent, fault.Invalid("RUNTIME_YIELD_VERSION_INVALID", "让出后的调度对象版本无效")
	}
	...
	for key, reservation := range s.runtimeReservations {
		if reservation.TenantID == attempt.TenantID && reservation.AttemptID == attempt.ID && reservation.State == contentruntime.ReservationHeld && reservation.FenceToken == fenceToken {
			reservation.State = contentruntime.ReservationReleased
...
func (s *Store) ResolveRuntimeYield(_ context.Context, yielded contentruntime.RuntimeYield, expectedYieldVersion int, node contentruntime.NodeRun, expectedNodeVersion int, agent contentruntime.AgentInstance, expectedAgentVersion int, event contentruntime.JobEvent) (contentruntime.RuntimeYield, contentruntime.NodeRun, contentruntime.AgentInstance, error) {
	if err := yielded.Validate(); err != nil {
	...
	if !ok {
		return yielded, node, agent, fault.NotFound("RuntimeYield")
	}
	if currentYield.State == contentruntime.RuntimeYieldResolved {
		...
		return yielded, node, agent, fault.Conflict("RUNTIME_YIELD_ALREADY_RESOLVED", "RuntimeYield 已由其他恢复请求处理")
	}
	...
	if !nodeOK || !agentOK {
		return yielded, node, agent, fault.NotFound("RuntimeYield 调度对象")
	}
	if currentYield.Version != expectedYieldVersion || currentNode.Version != expectedNodeVersion || currentAgent.Version != expectedAgentVersion || yielded.Version != expectedYieldVersion+1 || node.Version != expectedNodeVersion+1 || agent.Version != expectedAgentVersion+1 {
		return yielded, node, agent, fault.Conflict("RUNTIME_YIELD_VERSION_CONFLICT", "RuntimeYield 已被更新，请重新读取")
	}
	if err := currentNode.Transition(node.State); err != nil {
		return yielded, node, agent, err
	}
