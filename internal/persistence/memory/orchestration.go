...
func cloneEnvironment(value catalogdomain.Environment) catalogdomain.Environment {
	value.NormalizeCollections()
	value.Capabilities = append([]catalogdomain.EnvironmentCapability{}, value.Capabilities...)
	return value
}

func cloneSOPVersion(value catalogdomain.SOPVersion) catalogdomain.SOPVersion {
	value.NormalizeCollections()
	value.ContentTypes = append([]string{}, value.ContentTypes...)
	value.Stages = append([]catalogdomain.StageDefinition{}, value.Stages...)
	value.Gates = append([]catalogdomain.GateDefinition{}, value.Gates...)
	for i := range value.Stages {
		...
		value.Stages[i].AcceptedInputTypes = append([]catalogdomain.StageObjectRequirement{}, value.Stages[i].AcceptedInputTypes...)
		value.Stages[i].RequiredOutputTypes = append([]catalogdomain.StageObjectRequirement{}, value.Stages[i].RequiredOutputTypes...)
...
func cloneWorkTask(value work.WorkTask) work.WorkTask {
...
func cloneConversationImport(value work.ConversationImport) work.ConversationImport {
	if value.Bundle == nil {
		return value
	}
	bundle := *value.Bundle
	bundle.NormalizeCollections()
	bundle.Content = append([]work.ConversationContent{}, bundle.Content...)
...
func cloneGateEvaluation(value reviewdomain.GateEvaluation) reviewdomain.GateEvaluation {
	value.NormalizeCollections()
...
func cloneTaskRevision(value reviewdomain.TaskRevision) reviewdomain.TaskRevision {
	value.NormalizeCollections()
...
func cloneTaskDelivery(value deliverydomain.TaskDelivery) deliverydomain.TaskDelivery {
	value.NormalizeCollections()
	value.Manifest = append([]string{}, value.Manifest...)
	return value
}

func (s *Store) CreateEnvironment(_ context.Context, value catalogdomain.Environment) error {
	...
	if _, exists := s.environments[value.ID]; exists {
		return fault.Conflict("ENVIRONMENT_EXISTS", "环境已存在")
	}
	for _, existing := range s.environments {
		if existing.TenantID == value.TenantID && existing.Slug == value.Slug {
			return fault.Conflict("ENVIRONMENT_SLUG_EXISTS", "环境标识已存在")
		}
	}
...
func (s *Store) Environments(_ context.Context, tenantID string) ([]catalogdomain.Environment, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []catalogdomain.Environment{}
...
func (s *Store) Environment(_ context.Context, tenantID, id string) (catalogdomain.Environment, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.environments[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("环境")
	}
	return cloneEnvironment(value), nil
}

func (s *Store) SaveEnvironment(_ context.Context, value catalogdomain.Environment) error {
	if err := value.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
...
func (s *Store) CreateSOP(_ context.Context, definition catalogdomain.SOPDefinition, version catalogdomain.SOPVersion) error {
	...
	for _, existing := range s.sopVersions {
		if existing.TenantID == version.TenantID && existing.SOPID == version.SOPID && existing.Version == version.Version {
			return fault.Conflict("SOP_VERSION_EXISTS", "SOP 版本已存在")
		}
	}
...
func (s *Store) SaveSOPDefinition(_ context.Context, value catalogdomain.SOPDefinition) error {
	if value.ID == "" || value.TenantID == "" || value.Name == "" {
		return fault.Invalid("SOP_INVALID", "SOP 定义缺少 ID、租户或名称")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if value.BuiltIn && value.TemplateKey != "" {
		for id, existing := range s.sopDefinitions {
			if id != definitionKey && existing.TenantID == value.TenantID && existing.BuiltIn && existing.TemplateKey == value.TemplateKey {
				return fault.Conflict("SOP_TEMPLATE_EXISTS", "同一内置 SOP 模板已存在")
			}
		}
	}
	value.NormalizeCollections()
...
func (s *Store) CreateSOPVersion(_ context.Context, version catalogdomain.SOPVersion) error {
	if err := version.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
...
func (s *Store) SOPs(_ context.Context, tenantID string) ([]catalogdomain.SOPSummary, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []catalogdomain.SOPSummary{}
	for _, definition := range s.sopDefinitions {
		if definition.TenantID != tenantID {
			continue
		}
		summary := catalogdomain.SOPSummary{Definition: definition, Versions: []catalogdomain.SOPVersion{}}
...
func (s *Store) SOP(_ context.Context, tenantID, id string) (catalogdomain.SOPSummary, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	definition, ok := s.sopDefinitions[sopDefinitionKey(tenantID, id)]
	if !ok || definition.TenantID != tenantID {
		return catalogdomain.SOPSummary{}, fault.NotFound("流程规范")
	}
	result := catalogdomain.SOPSummary{Definition: definition, Versions: []catalogdomain.SOPVersion{}}
	for _, version := range s.sopVersions {
...
func (s *Store) SaveSOPVersion(_ context.Context, value catalogdomain.SOPVersion) error {
	if err := value.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
	...
	if !ok {
		return fault.NotFound("流程规范版本")
	}
	if current.Status != "draft" {
		return fault.Conflict("SOP_VERSION_IMMUTABLE", "已发布或已退休的 SOP 版本不可修改")
	}
	s.sopVersions[key] = cloneSOPVersion(value)
	return nil
}

func (s *Store) PublishSOPVersion(_ context.Context, tenantID, sopID string, version int, publishedBy string, publishedAt time.Time) (catalogdomain.SOPVersion, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	key := sopVersionKey(tenantID, sopID, version)
	value, ok := s.sopVersions[key]
	if !ok {
		return value, fault.NotFound("流程规范版本")
	}
	if err := value.Validate(); err != nil {
		return value, err
	}
...
func (s *Store) RetireSOPVersion(_ context.Context, tenantID, sopID string, version int, retiredAt time.Time) error {
	...
	if value.Status != "published" {
		return fault.Conflict("SOP_VERSION_STATE_INVALID", "只有已发布 SOP 版本可以退休")
	}
	...
	if definition.CurrentVersion == version {
		return fault.Policy("SOP_CURRENT_VERSION_CANNOT_RETIRE", "当前生效版本不能直接退休", "先发布另一个版本或执行回滚")
	}
...
func (s *Store) SaveProjectSOPBinding(_ context.Context, value catalogdomain.ProjectSOPBinding) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) ProjectSOPBinding(_ context.Context, tenantID, projectID string) (catalogdomain.ProjectSOPBinding, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.projectSOPBindings[projectID]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("项目 SOP")
	}
	return value, nil
}

func (s *Store) ProjectSOPBindings(_ context.Context, tenantID string) ([]catalogdomain.ProjectSOPBinding, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []catalogdomain.ProjectSOPBinding{}
...
func (s *Store) CreateWorkTask(_ context.Context, value work.WorkTask) error {
	if err := value.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
...
func (s *Store) WorkTaskByIdempotencyKey(_ context.Context, tenantID, key string) (work.WorkTask, error) {
	if key == "" {
		return work.WorkTask{}, fault.NotFound("任务")
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) WorkTasks(_ context.Context, tenantID, projectID string) ([]work.WorkTask, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []work.WorkTask{}
	for _, value := range s.workTasks {
...
func (s *Store) CreateConversationImport(_ context.Context, value work.ConversationImport) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
	...
	if _, exists := s.conversationImports[value.ID]; exists {
		return fault.Conflict("CONVERSATION_IMPORT_EXISTS", "对话导入请求已存在")
	}
	if value.IdempotencyKey != "" {
		for _, existing := range s.conversationImports {
			if existing.TenantID == value.TenantID && existing.IdempotencyKey == value.IdempotencyKey {
				return fault.Conflict("IDEMPOTENCY_REPLAY", "相同幂等键已经创建过对话导入请求")
			}
		}
	}
...
func (s *Store) ConversationImport(_ context.Context, tenantID, id string) (work.ConversationImport, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.conversationImports[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("对话导入请求")
	}
	return cloneConversationImport(value), nil
}

func (s *Store) ConversationImportByIdempotencyKey(_ context.Context, tenantID, key string) (work.ConversationImport, error) {
	if key == "" {
		return work.ConversationImport{}, fault.NotFound("对话导入请求")
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) ConversationImportsForTask(_ context.Context, tenantID, taskID string) ([]work.ConversationImport, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []work.ConversationImport{}
	for _, value := range s.conversationImports {
...
func (s *Store) SaveConversationImport(_ context.Context, value work.ConversationImport) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
	...
	if !ok || current.TenantID != value.TenantID {
		return fault.NotFound("对话导入请求")
	}
	if current.Status == work.ConversationImportUploaded && value.Status != current.Status {
		return fault.Conflict("CONVERSATION_IMPORT_IMMUTABLE", "已上传 Bundle 的对话导入请求不可再次改变状态")
	}
	s.conversationImports[value.ID] = cloneConversationImport(value)
	return nil
}

func (s *Store) WorkTask(_ context.Context, tenantID, id string) (work.WorkTask, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.workTasks[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("任务")
	}
	return cloneWorkTask(value), nil
}

func (s *Store) SaveWorkTask(_ context.Context, value work.WorkTask) error {
	if err := value.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
...
func (s *Store) StageRuns(_ context.Context, tenantID, taskID string) ([]work.StageRun, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []work.StageRun{}
	for _, value := range s.stageRuns {
		if value.TenantID == tenantID && value.TaskID == taskID {
			value.Outputs = []work.TaskStageOutput{}
...
func (s *Store) CreateStageRun(_ context.Context, value work.StageRun) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, exists := s.stageRuns[value.ID]; exists {
		return fault.Conflict("STAGE_RUN_EXISTS", "阶段执行记录已存在")
	}
	s.stageRuns[value.ID] = value
	return nil
}

func (s *Store) SaveStageRun(_ context.Context, value work.StageRun) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, exists := s.stageRuns[value.ID]; !exists {
		return fault.NotFound("阶段执行记录")
	}
	s.stageRuns[value.ID] = value
	return nil
}

func (s *Store) CreateGateEvaluation(_ context.Context, value reviewdomain.GateEvaluation) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
	...
	if _, exists := s.gateEvaluations[value.ID]; exists {
		return fault.Conflict("GATE_EVALUATION_EXISTS", "审核门评估已存在")
	}
	s.gateEvaluations[value.ID] = cloneGateEvaluation(value)
	return nil
}

func (s *Store) GateEvaluations(_ context.Context, tenantID, taskID string) ([]reviewdomain.GateEvaluation, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []reviewdomain.GateEvaluation{}
...
func (s *Store) GateEvaluation(_ context.Context, tenantID, id string) (reviewdomain.GateEvaluation, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.gateEvaluations[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("审核门评估")
	}
	return cloneGateEvaluation(value), nil
}

func (s *Store) SaveGateEvaluation(_ context.Context, value reviewdomain.GateEvaluation) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
	...
	if !ok || current.TenantID != value.TenantID {
		return fault.NotFound("审核门评估")
	}
	s.gateEvaluations[value.ID] = cloneGateEvaluation(value)
	return nil
}

func (s *Store) CreateTaskRevision(_ context.Context, value reviewdomain.TaskRevision) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
...
func (s *Store) TaskRevisions(_ context.Context, tenantID, taskID string) ([]reviewdomain.TaskRevision, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []reviewdomain.TaskRevision{}
...
func (s *Store) TaskRevision(_ context.Context, tenantID, id string) (reviewdomain.TaskRevision, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.taskRevisions[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("任务版本")
	}
	return cloneTaskRevision(value), nil
}

func (s *Store) CreateTaskDelivery(_ context.Context, value deliverydomain.TaskDelivery) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
	...
	if _, exists := s.taskDeliveries[value.ID]; exists {
		return fault.Conflict("TASK_DELIVERY_EXISTS", "任务交付已存在")
	}
	s.taskDeliveries[value.ID] = cloneTaskDelivery(value)
	return nil
}

func (s *Store) TaskDeliveries(_ context.Context, tenantID, taskID string) ([]deliverydomain.TaskDelivery, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := []deliverydomain.TaskDelivery{}
...
func (s *Store) TaskDelivery(_ context.Context, tenantID, id string) (deliverydomain.TaskDelivery, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.taskDeliveries[id]
	if !ok || value.TenantID != tenantID {
		return value, fault.NotFound("任务交付")
	}
	return cloneTaskDelivery(value), nil
}

func (s *Store) SaveTaskDelivery(_ context.Context, value deliverydomain.TaskDelivery) error {
	value.NormalizeCollections()
	if err := value.Validate(); err != nil {
		return err
	}
