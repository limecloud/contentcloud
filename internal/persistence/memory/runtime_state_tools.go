...
func (s *Store) ApplyStateRecordCommand(_ context.Context, record contentruntime.StateRecord, expectedVersion int, event contentruntime.JobEvent) (contentruntime.StateRecord, error) {
...
func (s *Store) ApplyFencedStateRecordCommand(_ context.Context, record contentruntime.StateRecord, expectedVersion int, attemptID, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.StateRecord, error) {
	if err := record.Validate(); err != nil {
		return record, err
	}
	s.mu.Lock()
...
func (s *Store) RegisterToolCallCommand(_ context.Context, call contentruntime.ToolCall, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	...
	if _, ok := s.runtimeToolCalls[key]; ok {
		return call, fault.Conflict("TOOL_CALL_EXISTS", "ToolCall 已存在")
	}
	if toolCallIdempotencyExistsLocked(s, call) {
		return call, fault.Conflict("TOOL_CALL_IDEMPOTENCY_EXISTS", "ToolCall 幂等键已存在")
	}
...
func (s *Store) RegisterFencedToolCallCommand(_ context.Context, call contentruntime.ToolCall, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	if err := call.Validate(); err != nil {
		return call, err
	}
	s.mu.Lock()
...
func (s *Store) ApplyToolCallTransitionCommand(_ context.Context, next contentruntime.ToolCall, expectedVersion int, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	...
	if !ok {
		return next, fault.NotFound("ToolCall")
	}
	...
	if current.State == contentruntime.ToolCallSucceeded || current.State == contentruntime.ToolCallFailed || current.State == contentruntime.ToolCallUnknown {
		return current, fault.Conflict("TOOL_CALL_TERMINAL", "终态 ToolCall 不能原地修改")
	}
...
func (s *Store) ApplyFencedToolCallTransitionCommand(_ context.Context, next contentruntime.ToolCall, expectedVersion int, fenceToken string, now time.Time, event contentruntime.JobEvent) (contentruntime.ToolCall, error) {
	if err := next.Validate(); err != nil {
		return next, err
	}
	s.mu.Lock()
...
func (s *Store) CreateStateCollection(_ context.Context, collection contentruntime.StateCollection) error {
	...
	if _, ok := s.runtimeStateCollections[stateCollectionKey(collection.TenantID, collection.ID)]; ok {
		return fault.Conflict("STATE_COLLECTION_EXISTS", "状态集合已存在")
	}
	for _, existing := range s.runtimeStateCollections {
		if existing.TenantID == collection.TenantID && existing.JobRunID == collection.JobRunID && existing.CollectionKey == collection.CollectionKey {
			return fault.Conflict("STATE_COLLECTION_KEY_EXISTS", "状态集合名称已存在")
		}
	}
...
func (s *Store) StateCollection(_ context.Context, tenantID, id string) (contentruntime.StateCollection, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.runtimeStateCollections[stateCollectionKey(tenantID, id)]
	if !ok {
		return contentruntime.StateCollection{}, fault.NotFound("状态集合")
	}
	return value, nil
}

func (s *Store) StateCollections(_ context.Context, tenantID, jobID string) ([]contentruntime.StateCollection, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) StateRecord(_ context.Context, tenantID, id string) (contentruntime.StateRecord, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	return contentruntime.StateRecord{}, fault.NotFound("状态记录")
}

func (s *Store) StateRecords(_ context.Context, tenantID, collectionID string) ([]contentruntime.StateRecord, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) ApplyStateRecordCAS(_ context.Context, record contentruntime.StateRecord, expectedVersion int) (contentruntime.StateRecord, error) {
	if err := record.Validate(); err != nil {
		return record, err
	}
	s.mu.Lock()
...
func applyStateRecordLocked(s *Store, record contentruntime.StateRecord, expectedVersion int) (contentruntime.StateRecord, error) {
	collection, ok := s.runtimeStateCollections[stateCollectionKey(record.TenantID, record.CollectionID)]
	if !ok {
		return record, fault.NotFound("状态集合")
	}
	...
	if expectedVersion == 0 && exists {
		return current, fault.Conflict("STATE_RECORD_VERSION_CONFLICT", "状态记录已经存在")
	}
	if exists && current.Version != expectedVersion {
		return current, fault.Conflict("STATE_RECORD_VERSION_CONFLICT", "状态记录版本已变化")
	}
	if !exists && expectedVersion != 0 {
		return record, fault.Conflict("STATE_RECORD_VERSION_CONFLICT", "状态记录不存在")
	}
	...
	if !exists {
		...
		if count >= collection.MaxRecords {
			return record, fault.Policy("STATE_COLLECTION_RECORD_LIMIT", "状态集合已达到最大记录数", "清理过期记录或提高已发布集合上限")
		}
	}
	if record.Value != nil {
		body, _ := json.Marshal(record.Value)
		if len(body) > collection.MaxRecordBytes {
			return record, fault.Policy("STATE_RECORD_TOO_LARGE", "状态记录超过集合大小限制", "减少记录内容或改用受控 Artifact 引用")
		}
	}
	if !exists {
		record.Version = 1
...
func (s *Store) CreateToolCall(_ context.Context, call contentruntime.ToolCall) error {
	if err := call.Validate(); err != nil {
		return err
	}
	s.mu.Lock()
	...
	key := toolCallKey(call.TenantID, call.ID)
	if _, ok := s.runtimeToolCalls[key]; ok {
		return fault.Conflict("TOOL_CALL_EXISTS", "ToolCall 已存在")
	}
	if toolCallIdempotencyExistsLocked(s, call) {
		return fault.Conflict("TOOL_CALL_IDEMPOTENCY_EXISTS", "ToolCall 幂等键已存在")
	}
	s.runtimeToolCalls[key] = call
	return nil
}

func (s *Store) ToolCall(_ context.Context, tenantID, id string) (contentruntime.ToolCall, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	value, ok := s.runtimeToolCalls[toolCallKey(tenantID, id)]
	if !ok {
		return contentruntime.ToolCall{}, fault.NotFound("ToolCall")
	}
	return value, nil
}

func (s *Store) ToolCallByIdempotencyKey(_ context.Context, tenantID, attemptID, toolName, idempotencyKey string) (contentruntime.ToolCall, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) ToolCalls(_ context.Context, tenantID, attemptID string) ([]contentruntime.ToolCall, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) ApplyToolCallTransition(_ context.Context, next contentruntime.ToolCall, expectedVersion int) (contentruntime.ToolCall, error) {
	if err := next.Validate(); err != nil {
		return next, err
	}
	s.mu.Lock()
...
func validateToolCallLocked(s *Store, call contentruntime.ToolCall) error {
	...
	if attempt.State != contentruntime.RuntimeAttemptPrepared && attempt.State != contentruntime.RuntimeAttemptRunning {
		return fault.Conflict("TOOL_CALL_ATTEMPT_NOT_ACTIVE", "只有准备中或运行中的 Attempt 可以创建或推进 ToolCall")
	}
	if agent.State != contentruntime.AgentRunnable && agent.State != contentruntime.AgentActive {
...
func toolCallIdempotencyExistsLocked(s *Store, call contentruntime.ToolCall) bool {
...
func toolCallIdempotencyKey(call contentruntime.ToolCall) string {
