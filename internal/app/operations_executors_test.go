...
func TestOperationsExecutorsProjectDeviceFactsAndHealth(t *testing.T) {
	...
	instance := domain.DaemonInstance{
		ID: domain.NewID(), TenantID: actor.TenantID, DeviceID: connected.Device.ID,
		...
		Capabilities: map[string]any{
			...
			"workspace_observations": []domain.DaemonWorkspaceObservation{{
	...
	must(t, store.SaveDaemonInstance(t.Context(), instance))
	executor, err = service.OperationsExecutor(t.Context(), actor, connected.Device.ID)
	must(t, err)
	...
	_, err = service.RevokeDevice(t.Context(), actor, connected.Device.ID, "executor-revoke")
	must(t, err)
	executor, err = service.OperationsExecutor(t.Context(), actor, connected.Device.ID)
	must(t, err)
...
func TestOperationsExecutorWithoutDaemonInstanceIsUnknown(t *testing.T) {
	store := memory.New()
	service := app.New(store, slog.Default())
	session, err := service.Register(t.Context(), "unknown-executor@example.com", "long-enough-password", "运营人员", "执行端租户")
	must(t, err)
	actor, _, err := service.SessionActor(t.Context(), session.ID)
	must(t, err)
	device := domain.Device{ID: domain.NewID(), TenantID: actor.TenantID, OwnerUserID: actor.UserID, DisplayName: "未上报实例", LastSeenAt: time.Now().UTC()}
	must(t, store.SaveDevice(t.Context(), device))

	executor, err := service.OperationsExecutor(t.Context(), actor, device.ID)
	must(t, err)
...
func TestOperationsExecutorsChooseLatestDaemonProcessAcrossInstanceEpochs(t *testing.T) {
	store := memory.New()
	service := app.New(store, slog.Default())
	session, err := service.Register(t.Context(), "latest-executor@example.com", "long-enough-password", "运营人员", "执行端租户")
	must(t, err)
	actor, _, err := service.SessionActor(t.Context(), session.ID)
	must(t, err)
	device := domain.Device{ID: domain.NewID(), TenantID: actor.TenantID, OwnerUserID: actor.UserID, DisplayName: "多进程执行端", LastSeenAt: time.Now().UTC()}
	must(t, store.SaveDevice(t.Context(), device))
	base := time.Now().UTC()
	must(t, store.SaveDaemonInstance(t.Context(), domain.DaemonInstance{
		ID: domain.NewID(), TenantID: actor.TenantID, DeviceID: device.ID, ConnectionEpoch: 9, ReportSequence: 20,
...
func TestOperationsExecutorsPreferLiveProcessOverLaterStoppedReport(t *testing.T) {
	store := memory.New()
	service := app.New(store, slog.Default())
	session, err := service.Register(t.Context(), "live-executor@example.com", "long-enough-password", "运营人员", "执行端租户")
	must(t, err)
	actor, _, err := service.SessionActor(t.Context(), session.ID)
	must(t, err)
	device := domain.Device{ID: domain.NewID(), TenantID: actor.TenantID, OwnerUserID: actor.UserID, DisplayName: "重连执行端", LastSeenAt: time.Now().UTC()}
	must(t, store.SaveDevice(t.Context(), device))
	base := time.Now().UTC()
	stoppedAt := base
	must(t, store.SaveDaemonInstance(t.Context(), domain.DaemonInstance{
		ID: domain.NewID(), TenantID: actor.TenantID, DeviceID: device.ID, ConnectionEpoch: 4, ReportSequence: 10,
