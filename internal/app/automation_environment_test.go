...
func TestAutomationPollRequiresVerifiedEnvironmentPackAndCapabilityBeforeLease(t *testing.T) {
	...
	lease, err := service.PollWithRuntime(ctx, deviceActor, device, []domain.Capability{capability}, []app.AutomationEnvironmentClaim{{Manifest: manifest, Lock: lock}}, "0.18.0")
	must(t, err)
	...
	must(t, err)
	if len(devices) != 1 || devices[0].Version != "0.18.0" {
