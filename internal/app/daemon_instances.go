...
func (s *Service) ReportDaemonInstance(ctx context.Context, actor Actor, input DaemonInstanceReportInput) (domain.DaemonInstance, error) {
	if actor.Type != "device" || strings.TrimSpace(actor.DeviceID) == "" {
		return domain.DaemonInstance{}, domain.Policy("DEVICE_AUTH_REQUIRED", "DaemonInstance 状态报告只接受设备凭据", "使用已注册设备的凭据")
	}
	...
	if _, err := uuid.Parse(input.ID); err != nil || input.ConnectionEpoch < 1 || input.ReportSequence < 1 || input.PID < 0 || input.Version == "" || input.StartedAt.IsZero() {
		return domain.DaemonInstance{}, domain.Invalid("DAEMON_INSTANCE_REPORT_INVALID", "DaemonInstance 状态报告缺少有效的实例、代际、序列或进程信息")
	}
	if input.State != "connected" && input.State != "degraded" && input.State != "stopped" {
		return domain.DaemonInstance{}, domain.Invalid("DAEMON_INSTANCE_STATE_INVALID", "DaemonInstance 状态无效")
	}
	if len(input.ActiveAttempts) > 32 {
		return domain.DaemonInstance{}, domain.Policy("DAEMON_INSTANCE_REPORT_TOO_LARGE", "DaemonInstance 活跃 Attempt 数量超过上限", "检查本地 worker 状态并重新同步")
	}
	if len(input.WorkspaceObservations) > 64 {
		return domain.DaemonInstance{}, domain.Policy("DAEMON_INSTANCE_REPORT_TOO_LARGE", "DaemonInstance 工作区观察数量超过上限", "减少本地工作区绑定后重新同步")
	}
	...
	if input.WorkspaceObservations != nil {
		observations := make([]domain.DaemonWorkspaceObservation, 0, len(input.WorkspaceObservations))
		...
		for _, observation := range input.WorkspaceObservations {
			...
			if observation.WorkspaceID == "" || observation.ProjectID == "" || observation.ObservedAt.IsZero() {
				return domain.DaemonInstance{}, domain.Invalid("DAEMON_WORKSPACE_OBSERVATION_INVALID", "DaemonInstance 工作区观察缺少工作区、项目或观察时间")
			}
			if _, exists := seenWorkspaces[observation.WorkspaceID]; exists {
				return domain.DaemonInstance{}, domain.Conflict("DAEMON_WORKSPACE_OBSERVATION_DUPLICATED", "DaemonInstance 工作区观察包含重复工作区")
			}
			...
			if observation.Status != "ready" && observation.Status != "repair_required" && observation.Status != "blocked" && observation.Status != "unknown" {
				return domain.DaemonInstance{}, domain.Invalid("DAEMON_WORKSPACE_OBSERVATION_INVALID", "DaemonInstance 工作区环境状态无效")
			}
	...
	if s.deviceControl == nil {
		return domain.DaemonInstance{}, domain.Policy("DAEMON_INSTANCE_STORE_UNAVAILABLE", "DaemonInstance 持久层未配置", "检查服务端设备控制存储配置")
	}
	if err := s.deviceControl.SaveDaemonInstance(ctx, instance); err != nil {
		return domain.DaemonInstance{}, err
	}
	device, err := s.store.Device(ctx, actor.TenantID, actor.DeviceID)
	if err != nil {
		return domain.DaemonInstance{}, err
	}
...
func (s *Service) DaemonInstances(ctx context.Context, actor Actor, deviceID string) ([]domain.DaemonInstance, error) {
	if strings.TrimSpace(actor.TenantID) == "" {
		return nil, domain.Policy("TENANT_REQUIRED", "查询 DaemonInstance 缺少租户范围", "重新登录后重试")
	}
	if s.deviceControl == nil {
		return nil, domain.Policy("DAEMON_INSTANCE_STORE_UNAVAILABLE", "DaemonInstance 持久层未配置", "检查服务端设备控制存储配置")
	}
