...
func TestConnectorSyncMaterializesSourceRevisionAndTombstone(t *testing.T) {
	...
	service := app.New(store, nil, app.WithConnectorRegistry(registry))
	session, err := service.Register(ctx, "connector-owner@example.com", "long-enough-password", "Connector", "Connector Tenant")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "cms", AuthorizationRef: "plaintext-token", Region: "cn"}, "bad-secret"); err == nil {
		t.Fatal("明文 token 不得进入 Connector 绑定")
	}
	binding, err := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "cms", AuthorizationRef: "secret://connectors/cms", Region: "cn"}, "bind")
	if err != nil {
		t.Fatal(err)
	}
	first, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{Limit: 100}, "sync-1")
	...
	second, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{Limit: 100}, "sync-2")
	...
	bindings, _ := service.ConnectorBindings(ctx, actor, project.ID)
	if len(bindings) != 1 || bindings[0].Cursor != "cursor-2" {
		t.Fatalf("opaque cursor was not committed: %#v", bindings)
	}
	receipts, _ := service.ConnectorReceipts(ctx, actor, binding.ID)
...
func TestConnectorSyncRejectsUnsupportedMIMEBeforeMaterialization(t *testing.T) {
	...
	service := app.New(store, nil, app.WithConnectorRegistry(registry))
	session, err := service.Register(ctx, "connector-mime@example.com", "long-enough-password", "Connector", "Connector Team")
	if err != nil {
		t.Fatal(err)
	}
	actor, _, err := service.SessionActor(ctx, session.ID)
	...
	binding, err := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "fixture", AuthorizationRef: "secret://fixture"}, "")
	if err != nil {
		t.Fatal(err)
	}
	_, err = service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "")
...
func TestConnectorSyncLeaseRejectsConcurrentPull(t *testing.T) {
	...
	service := app.New(store, nil, app.WithConnectorRegistry(registry))
	session, err := service.Register(ctx, "connector-lease@example.com", "long-enough-password", "Connector", "Lease Team")
	if err != nil {
		t.Fatal(err)
	}
	...
	binding, err := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "blocking", AuthorizationRef: "secret://blocking"}, "")
	if err != nil {
		t.Fatal(err)
	}
	firstDone := make(chan error, 1)
	go func() {
		_, syncErr := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "first")
		firstDone <- syncErr
	}()
	<-adapter.started
	_, err = service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "second")
	var domainErr *domain.Error
...
func TestConnectorSyncReplaysMaterializedPageAfterCommitInterruption(t *testing.T) {
	...
	service := app.New(store, nil, app.WithConnectorRegistry(registry), app.WithConnectorRepository(repository))
	session, err := service.Register(ctx, "connector-replay@example.com", "long-enough-password", "Connector", "Replay Team")
	if err != nil {
		t.Fatal(err)
	}
	actor, _, _ := service.SessionActor(ctx, session.ID)
	project, _ := service.CreateProject(ctx, actor, app.CreateProjectInput{BrandName: "Brand", ProductName: "Product"}, "")
	binding, _ := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "fixture", AuthorizationRef: "secret://fixture"}, "")
	if _, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "interrupted"); err == nil {
		t.Fatal("fixture commit interruption was not returned")
	}
	receipt, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "replay")
...
func TestConnectorTombstoneKeepsSharedSourceUntilLastActiveRecord(t *testing.T) {
	...
	service := app.New(store, nil, app.WithConnectorRegistry(registry))
	session, err := service.Register(ctx, "connector-shared@example.com", "long-enough-password", "Connector", "Shared Team")
	if err != nil {
		t.Fatal(err)
	}
	actor, _, _ := service.SessionActor(ctx, session.ID)
	project, _ := service.CreateProject(ctx, actor, app.CreateProjectInput{BrandName: "Brand", ProductName: "Product"}, "")
	binding, _ := service.CreateConnectorBinding(ctx, actor, app.CreateConnectorBindingInput{ProjectID: project.ID, ConnectorID: "shared", AuthorizationRef: "secret://shared"}, "")
	if _, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "create"); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "delete-a"); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.SyncConnector(ctx, actor, binding.ID, app.SyncConnectorInput{}, "delete-b"); err != nil {
		t.Fatal(err)
	}
	b, _ = store.Record(ctx, actor.TenantID, binding.ID, "record-b")
