...
func (s *Service) OperationsSkills(ctx context.Context, actor Actor) (OperationsSkillDirectory, error) {
	if !actor.PlatformAdmin {
		return OperationsSkillDirectory{}, domain.Policy("PLATFORM_ADMIN_REQUIRED", "只有平台管理员可以查看技能包目录", "使用已授权的平台管理员账号")
	}
	now := s.now().UTC()
...
func (s *Service) OperationsSkill(ctx context.Context, actor Actor, id, version string) (OperationsSkill, error) {
	...
	return OperationsSkill{}, domain.NotFound("技能包版本")
}
