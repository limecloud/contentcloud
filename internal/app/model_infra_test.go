...
func TestGenerateModelCandidateCreatesDraftAndReceiptAtomically(t *testing.T) {
	...
	service := app.New(store, nil, app.WithModelProviderRegistry(registry))
	task := domain.WorkTask{ID: "task-1", TenantID: "tenant-1", ProjectID: "project-1", SOPID: "sop-1", SOPVersion: 1, SOPDigest: "sha256:" + strings.Repeat("a", 64), Title: "公众号文章", ContentType: "wechat_article", InputRefs: []string{"source:1"}, RequestedOutput: map[string]any{}, Priority: "normal", RiskProfile: "low", Status: domain.TaskStatusRunning, CreatedBy: "user-1", CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
	if err := store.CreateWorkTask(ctx, task); err != nil {
		t.Fatal(err)
	}
	actor := app.Actor{UserID: "user-1", TenantID: "tenant-1", Role: "editor", Type: "user"}
	result, err := service.GenerateModelCandidate(ctx, actor, task.ID, app.GenerateModelCandidateInput{ProviderID: "pi-backed-saas", Messages: []modelprovider.Message{{Role: "user", Content: "生成公众号候选"}}, ResponseSchema: map[string]any{"type": "object"}, ContentType: "wechat_article", SchemaVersion: "contentcloud.article/1.0", RequestID: "request-1"}, "audit-1")
	if err != nil {
		t.Fatal(err)
	}
	if result.Revision.Status != domain.TaskRevisionDraft || result.Revision.SubmittedAt != nil {
