...
type RuntimeWorkerPrepareInput struct {
	...
	ResourceRequests     []domain.ResourceRequest `json:"-"`
}
...
func (s *Service) validateRuntimeWorkspaceObservation(ctx context.Context, tenantID, instanceID string, spec contentruntime.RemoteExecutionSpec) (domain.DaemonWorkspaceObservation, error) {
	if s.deviceControl == nil {
		return domain.DaemonWorkspaceObservation{}, domain.Policy("RUNTIME_WORKSPACE_OBSERVATION_REQUIRED", "远程 Attempt 缺少本地工作区观察", "让 Daemon 重新建立 current-state 后重试")
	}
	...
	if json.Unmarshal(body, &observations) != nil {
		return domain.DaemonWorkspaceObservation{}, domain.Policy("RUNTIME_WORKSPACE_OBSERVATION_INVALID", "Daemon 上报的工作区观察无法解析", "升级 Daemon 后重新同步 current-state")
	}
	sort.Slice(observations, func(i, j int) bool { return observations[i].ProjectID < observations[j].ProjectID })
	matches := []domain.DaemonWorkspaceObservation{}
	...
	if len(matches) == 0 {
		return domain.DaemonWorkspaceObservation{}, domain.Policy("RUNTIME_WORKSPACE_NOT_AUTHORIZED", "当前 Daemon 没有该项目的本地工作区观察", "为项目绑定工作区并等待 Daemon 同步")
	}
	if len(matches) > 1 {
		return domain.DaemonWorkspaceObservation{}, domain.Conflict("RUNTIME_WORKSPACE_AMBIGUOUS", "当前项目在 Daemon 上绑定了多个本地工作区，无法安全选择执行目录")
	}
	matched := matches[0]
	if matched.Status != "ready" {
		failure := domain.Policy("RUNTIME_ENVIRONMENT_NOT_READY", "本地工作区环境尚未达到可执行状态", "修复 Environment、Plugin、Skill 或 MCP 漂移后重试")
		...
		return domain.DaemonWorkspaceObservation{}, failure
	}
	...
	for _, check := range checks {
		if check.expected != "" && check.expected != check.actual {
			failure := domain.Conflict("RUNTIME_ENVIRONMENT_DRIFT", "本地 Environment、Plugin、Skill、MCP 或 Workspace 声明与冻结执行绑定不一致")
...
func (s *Service) enrichRemoteExecutionSpec(ctx context.Context, tenantID string, spec contentruntime.RemoteExecutionSpec) (contentruntime.RemoteExecutionSpec, error) {
	if len(spec.RequiredCapabilities) != 1 {
		return spec, domain.Policy("RUNTIME_CAPABILITY_REQUIRED", "远程智能体节点必须冻结且只声明一个可执行能力", "为节点配置一个已发布的 ContentCloud 能力后重试")
	}
	capability, ok := capabilitycatalog.Exact(spec.RequiredCapabilities[0], pluginidentity.VideoProductionVersion)
	if !ok {
		return spec, domain.Policy("RUNTIME_CAPABILITY_UNSUPPORTED", "远程智能体节点引用的能力没有当前服务端实现", "发布能力实现和对应 Skill 后再调度")
	}
	if capability.OutputSchema != spec.OutputSchemaRef {
		return spec, domain.Conflict("RUNTIME_CAPABILITY_SCHEMA_MISMATCH", "节点输出 Schema 与冻结能力定义不一致")
	}
	project, err := s.store.Project(ctx, tenantID, spec.ProjectID)
	if err != nil {
		return spec, err
	}
	if strings.TrimSpace(spec.InputSnapshotID) == "" {
		return spec, domain.Policy("RUNTIME_INPUT_SNAPSHOT_REQUIRED", "远程智能体节点缺少冻结输入快照", "重新创建包含输入快照的任务")
	}
	snapshot, err := s.store.Snapshot(ctx, tenantID, spec.InputSnapshotID)
	if err != nil {
		return spec, err
	}
	if snapshot.ProjectID != project.ID || snapshot.TenantID != tenantID {
		return spec, domain.Conflict("RUNTIME_INPUT_SNAPSHOT_SCOPE_MISMATCH", "冻结输入快照不属于当前任务项目")
	}
	...
	if job.InputDigest != "sha256:"+snapshot.ManifestHash {
		return spec, domain.Conflict("RUNTIME_INPUT_SNAPSHOT_DIGEST_MISMATCH", "JobRun 输入摘要与冻结输入快照不一致")
	}
	...
	contract := domain.TaskContract{
		...
		Project: project, Sources: append([]domain.ContractSource(nil), snapshot.Sources...), InputSnapshotID: snapshot.ID,
	...
	if contractVersion != "1.0" || contract.TaskType == "" || contract.ManifestHash == "" {
		return spec, domain.Conflict("RUNTIME_TASK_CONTRACT_INVALID", "Runtime 任务无法组装为当前支持的 TaskContract")
	}
	spec.OutputSchema = append(json.RawMessage(nil), outputSchema...)
	spec.OutputSchemaDigest = "sha256:" + domain.TokenHash(string(outputSchema))
	spec.TaskContract = contract
	spec.SkillID = skillID
	spec.Skill = string(skillBody)
	spec.SkillContentDigest = "sha256:" + domain.TokenHash(string(skillBody))
	return spec, nil
}

func remoteOutputSchema(schemaRef string) ([]byte, error) {
	switch strings.TrimSpace(schemaRef) {
	case domain.KnowledgeCandidatesSchema:
		return append([]byte(nil), contracts.KnowledgeCandidatesSchema...), nil
	default:
		return nil, domain.Policy("RUNTIME_OUTPUT_SCHEMA_UNAVAILABLE", "远程智能体节点的输出 Schema 没有可执行定义", "将已发布 Schema 注册到远程执行材料解析器")
	}
}
...
func remoteCapabilitySkill(capabilityID string) (string, []byte, error) {
	...
	case domain.KnowledgeExtractCapability:
		skillID = builtinskills.KnowledgeExtraction
	default:
		return "", nil, domain.Policy("RUNTIME_SKILL_UNAVAILABLE", "远程智能体节点的能力没有对应 Skill", "发布能力到 Skill 的受控映射后再调度")
	}
...
func (s *Service) requireRuntimeAdmissionCapacity(ctx context.Context, tenantID string) error {
	for _, subscriber := range []string{domain.RuntimeOutboxSubscriberProjection, domain.RuntimeOutboxSubscriberBusinessResult} {
...
func (s *Service) CallRuntimeMCPWithGatewayToken(ctx context.Context, token string, input RuntimeGatewayCallInput) (contentruntime.GatewayResponse, error) {
	if s.runtimeService == nil {
		return contentruntime.GatewayResponse{}, domain.Policy("RUNTIME_UNAVAILABLE", "当前运行时尚未配置持久化存储", "联系平台运营人员启用 Runtime")
	}
...
func (s *Service) ActivateRuntimeWorker(ctx context.Context, actor Actor, input RuntimeWorkerActivateInput) (contentruntime.DispatchHandle, error) {
	...
	if strings.TrimSpace(input.Session.TenantID) != "" && strings.TrimSpace(input.Session.TenantID) != handle.Attempt.TenantID {
		return contentruntime.DispatchHandle{}, domain.Policy("RUNTIME_SESSION_TENANT_MISMATCH", "Harness 会话租户与 RuntimeAttempt 不一致", "使用当前 Attempt 返回的会话范围")
	}
...
func (s *Service) workerHandle(ctx context.Context, actor Actor, daemonInstanceID, attemptID, fenceToken string) (contentruntime.DispatchHandle, error) {
...
func (s *Service) workerFinalizeHandle(ctx context.Context, actor Actor, daemonInstanceID, attemptID, fenceToken string) (contentruntime.DispatchHandle, error) {
	handle, err := s.loadRuntimeWorkerHandle(ctx, actor, attemptID)
	if err != nil {
		return contentruntime.DispatchHandle{}, err
	}
...
func (s *Service) runtimeWorkerOwner(ctx context.Context, actor Actor, daemonInstanceID string, requireOnline bool) (string, error) {
	if strings.TrimSpace(actor.DeviceID) != "" {
		...
		if instanceID == "" {
			...
			if _, lookupErr := s.store.Device(ctx, actor.TenantID, actor.DeviceID); domain.IsNotFound(lookupErr) {
				return "device:" + strings.TrimSpace(actor.DeviceID), nil
			}
			return "", domain.Policy("DAEMON_INSTANCE_REQUIRED", "设备 Runtime 请求缺少 DaemonInstance", "先建立 Runtime 控制通道并完成当前态同步")
		}
		if s.deviceControl == nil {
		...
		if err != nil {
			return "", domain.Policy("DAEMON_INSTANCE_INVALID", "DaemonInstance 不存在或不属于当前设备", "重新建立 Runtime 控制通道")
		}
		if instance.DeviceID != strings.TrimSpace(actor.DeviceID) {
			return "", domain.Policy("DAEMON_INSTANCE_DEVICE_MISMATCH", "DaemonInstance 不属于当前设备", "使用当前设备控制通道返回的实例标识")
		}
		if requireOnline && (instance.State != "connected" || s.now().UTC().Sub(instance.LastSeenAt) > daemonInstanceFreshFor) {
			return "", domain.Policy("DAEMON_INSTANCE_OFFLINE", "DaemonInstance 当前不在线", "等待控制通道重连后重试")
		}
