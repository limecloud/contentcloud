...
func (v Binding) Validate() error {
	if strings.TrimSpace(v.ID) == "" || strings.TrimSpace(v.TenantID) == "" || strings.TrimSpace(v.ProjectID) == "" || strings.TrimSpace(v.ConnectorID) == "" {
		return domain.Invalid("CONNECTOR_BINDING_INVALID", "Connector 绑定缺少租户、项目或 Connector")
	}
	if !validSecretRef(v.AuthorizationRef) {
		return domain.Invalid("CONNECTOR_AUTHORIZATION_REF_INVALID", "Connector 授权必须保存 SecretRef，不能保存明文 Token")
	}
	if v.Status != BindingActive && v.Status != BindingDisabled {
		return domain.Invalid("CONNECTOR_BINDING_STATUS_INVALID", "Connector 绑定状态无效")
	}
	return nil
}
...
func (e *Engine) Sync(ctx context.Context, request PullRequest) (SyncReceipt, error) {
	if e == nil || e.adapter == nil {
		return SyncReceipt{}, domain.Policy("CONNECTOR_ADAPTER_UNAVAILABLE", "Connector Adapter 未配置", "绑定可用 Connector 后重试")
	}
	if err := request.Binding.Validate(); err != nil {
		return SyncReceipt{}, err
	}
	if request.Binding.Status != BindingActive {
		return SyncReceipt{}, domain.Policy("CONNECTOR_BINDING_DISABLED", "Connector 绑定已停用", "启用绑定后重试")
	}
	if request.Limit <= 0 {
		request.Limit = 100
	}
	if request.Limit > 1000 {
		return SyncReceipt{}, domain.Invalid("CONNECTOR_LIMIT_INVALID", "单次 Connector 同步最多 1000 条")
	}
	...
	if len(result.Records) > request.Limit || (result.HasMore && strings.TrimSpace(result.NextCursor) == "") {
		return SyncReceipt{}, domain.Invalid("CONNECTOR_RESULT_INVALID", "Connector 返回数量超限或缺少下一游标")
	}
	...
	for index := range result.Records {
		...
		if record.ExternalID == "" || record.Version == "" || record.UpdatedAt.IsZero() {
			return SyncReceipt{}, domain.Invalid("CONNECTOR_RECORD_INVALID", "Connector 记录缺少外部 ID、版本或更新时间")
		}
		if prior, exists := seen[record.ExternalID]; exists {
			if prior == record.Version {
				return SyncReceipt{}, domain.Conflict("CONNECTOR_RECORD_DUPLICATE", "Connector 同一页返回了重复记录")
			}
			return SyncReceipt{}, domain.Conflict("CONNECTOR_VERSION_AMBIGUOUS", "Connector 同一页返回同一对象的多个版本")
		}
		...
		if record.Deleted {
			if len(record.Body) != 0 || record.DeletedAt == nil {
				return SyncReceipt{}, domain.Invalid("CONNECTOR_TOMBSTONE_INVALID", "删除记录必须包含 deleted_at 且不能携带正文")
			}
			record.Digest = ""
			tombstones++
			continue
		...
		if len(record.Body) == 0 || strings.TrimSpace(record.MIME) == "" {
			return SyncReceipt{}, domain.Invalid("CONNECTOR_RECORD_BODY_REQUIRED", "非删除记录必须包含正文和 MIME")
		}
