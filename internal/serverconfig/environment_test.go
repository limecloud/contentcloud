...
func TestLoadEnvironmentBuildsVerifiedControlPlaneAndAutomationPolicy(t *testing.T) {
	...
	expected, _ := capabilitycatalog.Exact(domain.KnowledgeExtractCapability, "0.6.0")
...
func environmentConfigFixture(t *testing.T) serverconfig.EnvironmentConfig {
	...
	registry := environment.Registry{SchemaURL: "test", SchemaVersion: "1.0", Entries: []environment.RegistryEntry{
		configRegistryEntry("contentcloud-video-production", "scene_plugin", "0.6.0", "v0.6.0", "a"),
	...
	return serverconfig.EnvironmentConfig{
		...
		SigningKeyPath: signingKeyPath, SigningKeyID: "environment-config-test", CapabilityReleaseVersion: "0.6.0", ManifestTTL: 24 * time.Hour, RepositoryRoot: repositoryRoot,
	}
}
