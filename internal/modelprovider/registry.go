...
func (r *Registry) Resolve(id string) (Provider, error) {
	if r == nil {
		return nil, domain.Policy("MODEL_PROVIDER_UNAVAILABLE", "模型 Provider 注册表未配置", "配置模型 Provider 后重试")
	}
	r.mu.RLock()
	defer r.mu.RUnlock()
	provider, ok := r.providers[strings.ToLower(strings.TrimSpace(id))]
	if !ok {
		return nil, domain.Policy("MODEL_PROVIDER_UNAVAILABLE", "模型 Provider 不可用", "配置 vLLM、SGLang 或注入兼容 Provider")
	}
	return provider, nil
}
