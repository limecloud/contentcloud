...
func TestStoryboardApprovalBoundaryAndSeedanceExport(t *testing.T) {
	...
	created, err := CreateStoryboardPackage(CreateStoryboardPackageOptions{
		...
		Capability: domain.CapabilityRef{ID: "image.test", Version: "1.0.0", Digest: "sha256:" + strings.Repeat("b", 64)},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	exported, err := ExportSeedancePackage(ExportSeedancePackageOptions{
		...
		ProviderProfileVersion: "seedance-profile:manual-2026-07-29", AdapterCapability: domain.CapabilityRef{ID: "contentcloud.seedance-export", Version: "1.0.0", Digest: "sha256:" + strings.Repeat("c", 64)},
	...
	dynamicOffer.Shots = append([]domain.StoryboardShot(nil), prepared.Shots...)
