...
func TestReviewFeedbackInboxKeepsImmutableRevisionsOfOneSubmissionRevision(t *testing.T) {
	...
	feedback.Comments = append(feedback.Comments, domain.ReviewComment{ID: "comment-2", Body: "请收紧 CTA", CreatedAt: now.Add(2 * time.Minute)})
...
func reviewFeedbackFixture(now time.Time) domain.ReviewFeedbackBundle {
	return domain.ReviewFeedbackBundle{
		...
		Comments: []domain.ReviewComment{{ID: "comment-1", Body: "请补充证据", CreatedAt: now}},
	}
}
