...
type ExportSeedancePackageOptions struct {
	...
	AdapterCapability      domain.CapabilityRef
	Mode                   string
	AspectRatio            string
	Sound                  string
...
type ExportSeedancePackageResult struct {
	...
	Package     domain.SeedancePromptPackage `json:"package"`
}
...
func ExportSeedancePackage(options ExportSeedancePackageOptions) (ExportSeedancePackageResult, error) {
	...
	value := domain.SeedancePromptPackage{
		ID: options.PackageID, Type: "seedance_prompt_package", SchemaVersion: domain.SeedancePromptPackageSchema,
		...
		Mode: defaultStringV5(options.Mode, "all_reference"), Settings: domain.SeedanceSettings{AspectRatio: defaultStringV5(options.AspectRatio, "9:16"), DurationSeconds: durationSeconds, Sound: defaultStringV5(options.Sound, "environment_only")},
	...
	if _, err := os.Stat(finalDirectory); err == nil {
		return ExportSeedancePackageResult{}, domain.Conflict("SEEDANCE_PACKAGE_EXISTS", "Seedance 交付目录已存在，拒绝覆盖不可变交付包")
...
func LintSeedancePackage(root, file string) (V5LintReport, domain.SeedancePromptPackage, error) {
	resolved, path, err := resolveV5JSON(root, file)
	if err != nil {
		return V5LintReport{}, domain.SeedancePromptPackage{}, err
	}
	var value domain.SeedancePromptPackage
	if err := readStrictJSON(path, &value); err != nil {
		return V5LintReport{}, value, domain.Invalid("SEEDANCE_PACKAGE_JSON_INVALID", err.Error())
	}
...
func seedanceUploads(storyboard domain.StoryboardPackage) ([]domain.SeedanceUpload, map[string]domain.StoryboardAsset, map[string]int, error) {
	assets := map[string]domain.StoryboardAsset{}
	for _, asset := range storyboard.Assets {
		assets[asset.ID] = asset
	}
	ordered := []domain.StoryboardAsset{}
	...
	shots := append([]domain.StoryboardShot(nil), storyboard.Shots...)
	...
	for _, asset := range ordered {
		...
		if kind == "" {
			return nil, nil, nil, domain.Invalid("SEEDANCE_MEDIA_TYPE_UNSUPPORTED", "Seedance 输入素材类型不受支持："+asset.MediaType)
		}
		counters[kind]++
		extension := strings.ToLower(filepath.Ext(asset.Path))
		file := fmt.Sprintf("media/%s-%02d%s", prefix, counters[kind], extension)
		uploads = append(uploads, domain.SeedanceUpload{Reference: fmt.Sprintf("@%s%d", label, counters[kind]), ArtifactID: asset.ID, File: file, Purpose: seedanceAssetPurpose(asset), SHA256: asset.SHA256})
	}
	return uploads, assets, counters, nil
}

func seedanceSegments(storyboard domain.StoryboardPackage, uploads []domain.SeedanceUpload, assets map[string]domain.StoryboardAsset, minSeconds, maxSeconds int, aspectRatio, sound string) (int, []domain.SeedanceSegment, []string, error) {
	...
	segments := make([]domain.SeedanceSegment, 0, len(shots))
	...
	for index, shot := range shots {
		if dynamicOfferTextPattern.MatchString(strings.Join([]string{shot.ImagePromptZH, shot.Subject, shot.Product, shot.Scene, shot.Action}, " ")) {
			return 0, nil, nil, domain.Policy("SEEDANCE_DYNAMIC_OFFER_TEXT_BLOCKED", "镜头 "+shot.ShotID+" 包含价格、优惠、库存或赠品等动态权益文本", "从生成画面移除动态权益，并在有效 OfferSnapshot 校验后的后期阶段合成")
		}
		shotSeconds := int(math.Ceil(float64(shot.EndMS-shot.StartMS) / 1000))
		if shotSeconds > maxSeconds {
			return 0, nil, nil, domain.Policy("SEEDANCE_SEGMENT_TOO_LONG", "镜头 "+shot.ShotID+" 超过当前 provider profile 的单段时长", "先在规范剧本中按叙事动作拆镜并重新审核分镜")
		}
		...
		if len(refs) == 0 {
			return 0, nil, nil, domain.Invalid("SEEDANCE_SHOT_REFERENCE_REQUIRED", "镜头 "+shot.ShotID+" 没有可导出的首尾帧或公共参考")
		}
		...
		segments = append(segments, domain.SeedanceSegment{ID: fmt.Sprintf("segment-%02d", index+1), Order: index + 1, StartMS: shot.StartMS, EndMS: shot.EndMS, PromptZH: prompt, IncomingState: shot.IncomingState, OutgoingState: shot.OutgoingState, AcceptanceCriteria: append([]string(nil), shot.AcceptanceCriteria...)})
...
func compileSeedancePrompt(shot domain.StoryboardShot, refs []string, firstRef, endRef, aspectRatio, sound string, minSeconds int) string {
...
func writeSeedancePackageFiles(root, directory string, storyboard domain.StoryboardPackage, value domain.SeedancePromptPackage, assets map[string]domain.StoryboardAsset, prompts []string) error {
...
func seedanceReadme(storyboard domain.StoryboardPackage, value domain.SeedancePromptPackage) string {
	var builder strings.Builder
...
func seedanceAssetPurpose(asset domain.StoryboardAsset) string {
	if asset.ShotID == "" {
		return asset.Role
	}
