...
func storeApprovedObject(t *testing.T, root, snapshotID, submissionType, objectID string, object any, createdAt time.Time) {
	...
	canonical, err := json.Marshal(map[string]any{"schema_version": domain.SubmissionSchemaVersion(submissionType), "submission_type": submissionType, "objects": json.RawMessage(objects)})
	if err != nil {
		t.Fatal(err)
	}
