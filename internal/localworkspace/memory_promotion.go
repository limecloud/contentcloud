...
func PromoteMemory(options MemoryPromoteOptions) (MemoryPromotionReport, error) {
	...
	if memoryID == "" || memoryID != localSafeName(memoryID) || memoryID == "." || memoryID == ".." || strings.ContainsAny(memoryID, `/\\`) {
		return MemoryPromotionReport{}, domain.Invalid("MEMORY_PROMOTION_ID_INVALID", "必须指定有效的记忆候选 ID")
	}
	...
	if !found {
		return MemoryPromotionReport{}, domain.NotFound("本地记忆候选")
	}
	if record.Status != "active" {
		return MemoryPromotionReport{}, domain.Policy("MEMORY_PROMOTION_NOT_ELIGIBLE", "陈旧、冲突或非 active 的记忆候选不能晋升", "先修复来源或解决记忆冲突后重试")
	}
	kind := strings.ToLower(strings.TrimSpace(options.KnowledgeKind))
	if !validKnowledgeKind(kind) {
		return MemoryPromotionReport{}, domain.Invalid("MEMORY_PROMOTION_KIND_INVALID", "knowledge_kind 必须为 fact、claim、visual_rule 或 methodology")
	}
	...
	if subject == "" || predicate == "" {
		return MemoryPromotionReport{}, domain.Invalid("MEMORY_PROMOTION_SUBJECT_REQUIRED", "显式晋升必须提供 subject 和 predicate")
	}
	...
	if risk != "low" && risk != "medium" && risk != "high" {
		return MemoryPromotionReport{}, domain.Invalid("MEMORY_PROMOTION_RISK_INVALID", "risk_level 必须为 low、medium 或 high")
	}
	...
	candidate := domain.KnowledgeCandidate{
		...
		Subject:             subject,
		Predicate:           predicate,
		Value:               domain.TypedValue{Type: "text", Text: record.Summary},
		Scope:               domain.KnowledgeScope{Regions: []string{}, Channels: []string{}, Audiences: []string{}, ProductVariants: []string{}},
		RiskLevel:           risk,
	...
	pkg := domain.KnowledgeExtractionPackage{SchemaVersion: "1.0", Candidates: []domain.KnowledgeCandidate{candidate}, Warnings: []string{}}
...
func memoryPromotionEvidence(root string, ids []string) ([]domain.EvidenceRef, error) {
	cleanIDs := uniqueStrings(ids)
	if len(cleanIDs) == 0 {
		return nil, domain.Invalid("MEMORY_PROMOTION_EVIDENCE_REQUIRED", "晋升为正式知识候选必须指定已接受的 evidence_id")
	}
	...
	result := make([]domain.EvidenceRef, 0, len(cleanIDs))
	for _, id := range cleanIDs {
		span, ok := byID[id]
		if !ok {
			return nil, domain.Invalid("MEMORY_PROMOTION_EVIDENCE_NOT_FOUND", "evidence_id 不存在："+id)
		}
		if span.ReviewStatus != "accepted" {
			return nil, domain.Policy("MEMORY_PROMOTION_EVIDENCE_REVIEW_REQUIRED", "记忆候选引用的 evidence 尚未通过人工复核："+id, "先执行来源复核，再晋升记忆")
		}
		locator, err := json.Marshal(span.Locator)
		if err != nil {
			return nil, err
		}
		result = append(result, domain.EvidenceRef{SourceRevisionID: span.SourceID, LocatorKind: span.LocatorKind, Locator: string(locator), Quote: span.Quote})
	}
	return result, nil
}

func writeMemoryPromotionPackage(path string, pkg domain.KnowledgeExtractionPackage) error {
	body, err := json.Marshal(pkg)
	if err != nil {
		return err
	}
	if existing, readErr := os.ReadFile(path); readErr == nil {
		var current domain.KnowledgeExtractionPackage
		if err := strictUnmarshal(existing, &current); err != nil {
			return domain.Conflict("MEMORY_PROMOTION_PACKAGE_CONFLICT", "已有记忆晋升包格式无效，拒绝覆盖："+filepath.Base(path))
		}
		currentBody, _ := json.Marshal(current)
		if string(currentBody) == string(body) {
			return nil
		}
		return domain.Conflict("MEMORY_PROMOTION_PACKAGE_CONFLICT", "已有记忆晋升包内容不同，拒绝覆盖："+filepath.Base(path))
