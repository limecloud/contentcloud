...
func RememberMemory(options MemoryRememberOptions) (MemoryRememberReport, error) {
	...
	if len(kinds) != 1 {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_KIND_REQUIRED", "记忆候选必须指定 kind")
	}
	...
	if summary == "" {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_SUMMARY_REQUIRED", "记忆候选必须包含非空 summary")
	}
	if len([]rune(summary)) > memoryRecordSummaryRunes {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_SUMMARY_TOO_LARGE", fmt.Sprintf("记忆候选 summary 不能超过 %d 个字符", memoryRecordSummaryRunes))
	}
	...
	if len([]rune(formedBy)) > 160 {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_FORMED_BY_INVALID", "formed_by 不能超过 160 个字符")
	}
	claimKey := normalizeMemoryClaimKey(options.ClaimKey)
	if len([]rune(claimKey)) > 256 {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_CLAIM_KEY_INVALID", "claim_key 不能超过 256 个字符")
	}
	...
	if memoryID != localSafeName(memoryID) || memoryID == "." || memoryID == ".." || strings.ContainsAny(memoryID, `/\\`) {
		return MemoryRememberReport{}, domain.Invalid("MEMORY_ID_INVALID", "记忆候选 ID 只能包含字母、数字、点、短横线和下划线")
	}
	record := MemoryRecord{
	...
	if existing, readErr := readMemoryRecord(path); readErr == nil {
		...
		return MemoryRememberReport{}, domain.Conflict("MEMORY_RECORD_IMMUTABLE_CONFLICT", "相同记忆 ID 已存在不同内容："+memoryID)
...
func resolveMemoryRecordSource(root, value string) (string, string, uint32, error) {
	...
	if clean == "." || clean == "" || filepath.IsAbs(value) || strings.HasPrefix(clean, "../") || !isMemorySourceRefAllowed(clean) {
		return "", "", 0, domain.Invalid("MEMORY_SOURCE_REF_INVALID", "source_ref 必须是允许记忆来源目录内的相对路径")
	}
	...
	if info.Size() <= 0 || info.Size() > memoryMaxFileBytes || !memoryTextExtensions[strings.ToLower(filepath.Ext(path))] {
		return "", "", 0, domain.Policy("MEMORY_SOURCE_NOT_INDEXED", "source_ref 不是可安全读取的记忆文本来源", "选择允许目录中的 UTF-8 文本文件")
	}
	...
	if len(body) > memoryMaxFileBytes || !utf8.Valid(body) || bytes.IndexByte(body, 0) >= 0 {
		return "", "", 0, domain.Policy("MEMORY_SOURCE_NOT_INDEXED", "source_ref 不是可安全读取的 UTF-8 文本来源", "选择允许目录中的 UTF-8 文本文件")
	}
