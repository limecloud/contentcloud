...
func (page knowledgePage) item() (LocalKnowledgeItem, error) {
	...
	evidence := make([]domain.EvidenceRef, 0, len(page.Evidence))
	for _, ref := range page.Evidence {
		evidence = append(evidence, domain.EvidenceRef{SourceRevisionID: ref.SourceRevisionID, LocatorKind: ref.LocatorKind, Locator: ref.Locator, Quote: ref.Quote})
	}
	return LocalKnowledgeItem{
		...
		Value:  domain.TypedValue{Type: page.Value.Type, Text: page.Value.Text, Number: page.Value.Number, Boolean: page.Value.Boolean, Unit: page.Value.Unit},
		Scope:  domain.KnowledgeScope{Regions: nonNilStrings(page.Scope.Regions), Channels: nonNilStrings(page.Scope.Channels), Audiences: nonNilStrings(page.Scope.Audiences), ProductVariants: nonNilStrings(page.Scope.ProductVariants)},
