...
func ExtractMemory(ctx context.Context, options MemoryExtractOptions) (MemoryExtractionReport, error) {
	if options.Adapter == nil {
		return MemoryExtractionReport{}, domain.Invalid("MEMORY_EXTRACT_ADAPTER_REQUIRED", "记忆抽取必须提供显式远程适配器")
	}
	...
	if len(options.SourceRefs) > 0 && len(sources) != len(uniqueStrings(options.SourceRefs)) {
		return MemoryExtractionReport{}, domain.Invalid("MEMORY_EXTRACT_SOURCE_INVALID", "source_refs 必须全部指向当前允许读取的来源文件")
	}
	var inputBytes int64
	for _, source := range sources {
		inputBytes += int64(len(source.Contents))
		if inputBytes > memoryMaxExtractInputSize {
			return MemoryExtractionReport{}, domain.Policy("MEMORY_EXTRACT_INPUT_TOO_LARGE", "远程记忆抽取的来源正文总量不能超过 8 MiB", "缩小 source_refs 范围后重试")
		}
	}
	...
	for _, source := range sources {
		...
		if len(candidates) > 100 {
			return MemoryExtractionReport{}, domain.Policy("MEMORY_EXTRACT_TOO_MANY_CANDIDATES", "单个来源的记忆抽取候选不能超过 100 条", "缩小来源范围或让抽取器先做摘要")
		}
