...
func TestPlanIsReadOnlyAndPinsMarketplaceAndPlugin(t *testing.T) {
	...
	adapter := mustAdapter(t, DefaultSpec("0.5.0"), runner)
	...
	wantMarketplace := []string{"codex", "plugin", "marketplace", "add", "limecloud/contentcloud", "--ref", "v0.5.0", "--json"}
...
func TestDetectClassifiesCurrentOutdatedAndBroken(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "current",
			marketplace: `{"marketplaces":[{"name":"contentcloud","root":"/tmp/cache","marketplaceSource":{"sourceType":"git","source":"https://github.com/limecloud/contentcloud.git","ref":"v0.5.0"}}]}`,
			plugin:      `{"installed":[{"pluginId":"contentcloud-video-production@contentcloud","name":"contentcloud-video-production","marketplaceName":"contentcloud","version":"0.5.0","installed":true,"enabled":true}],"available":[]}`,
			want:        "current",
		},
		{
			name:        "outdated ref",
		...
		},
		{
			...
			marketplace: `{"marketplaces":[{"name":"contentcloud","root":"/tmp/cache","marketplaceSource":{"sourceType":"git","source":"limecloud/contentcloud","ref":"v0.5.0"}}]}`,
			plugin:      `{"installed":[{"pluginId":"contentcloud-video-production@contentcloud","name":"contentcloud-video-production","marketplaceName":"contentcloud","version":"0.5.0","installed":true,"enabled":false}],"available":[]}`,
			want:        "broken",
		},
	}
...
func TestApplyRollsBackActualPluginIdentityReturnedByCodex(t *testing.T) {
	...
	runner := &fakeRunner{responses: []fakeResponse{
		...
		{stdout: missingMarketplace}, {stdout: missingPlugin},
		{stdout: `{"marketplaceName":"contentcloud","installedRoot":"/tmp/cache","alreadyAdded":false}`},
		{stdout: `{"pluginId":"unexpected-plugin@contentcloud","name":"unexpected-plugin","marketplaceName":"contentcloud","version":"0.5.0","installedPath":"/tmp/unexpected"}`},
		{stdout: `{"removed":true}`},
		{stdout: `{"removed":true}`},
	}}
	adapter := mustAdapter(t, DefaultSpec("0.5.0"), runner)
