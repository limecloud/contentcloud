-- Re-queue persisted asynchronous Provider jobs after a worker restart. The
-- latest ProviderAttempt owns the poll deadline; no new submit is implied.
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE j.state IN ('queued','retry_wait')
     OR (j.state = 'awaiting_external_result' AND (latest.next_poll_at IS NULL OR latest.next_poll_at <= now()))
