...
| `PROJECT_VIEW_LINK_UNTRUSTED` | A URL/host/path/token is supplied outside `contentcloud_open_project_view`, or the builder rejects the binding. | Reject the target and require a valid WorkspaceBinding plus allowlisted view/focus. | Opening the supplied URL directly or weakening origin validation. |
