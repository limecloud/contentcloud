...
- 业务事实所有权或聚合边界变化。
- WorkTask、StageRun、RuntimeRun 读模型与 JobRun/NodeRun/RuntimeAttempt 的处置，以及任何恢复 dead 执行模型或旧 DTO 的提案。
- 模块化单体与独立服务边界变化。
...
| ADR-0004 | 运行对象收敛 | 记录 Runtime current、公开读模型和 V7 执行模型 dead 的最终关系 | StageRun 业务投影退场 |
