...
状态：`目标架构；ADR-0011/0013 已接受，Customer Studio 动态结果投影已实现，持久化投影与治理链路待完成`。

更新时间：2026-08-07。

## 1. 核心决定

统一创作资产目录使用跨域只读投影，不创建吞并所有内容的超级 `Asset` 聚合。客户侧目录只投影流水线生成结果；输入和治理对象仍由任务输入投影与运营投影管理。

当前 `CustomerStudioAssets` 在查询时从 `WorkTask` 相关事实确定性组装同一行契约，用于验证产品边界。目标 `Catalog Projector` 落地后应替换这一动态组装路径，而不是与其长期双写或形成第二套客户目录。
...
| 客户边界 | 底层事实对象 | 客户投影 | 说明 |
| --- | --- | --- | --- |
| 任务输入 / 项目参考 | `SourceRevision`、`Asset`、`KnowledgeObject`、`RightsRecord` | 当前任务输入和项目参考 | 不进入客户创作资产库 |
...
结果状态与结果类型分开。`persona`、`script`、`storyboard`、`image`、`video` 表达结果类型；`draft`、`pending_confirmation`、`changes_requested`、`confirmed`、`delivered`、`superseded`、`blocked` 表达状态。
...
| `Asset` | 已登记、受用途和权利约束的参考素材 | Source & Knowledge | 进入任务输入或项目参考，不作为客户结果类型 |
...
- 来源、知识、权利、搜索结果、人工灵感和任务输入不收录到客户创作资产库。
...
- 列表：结果类型、关键词、项目、状态、时间和稳定游标。
- 详情：客户可见摘要、预览、来源任务、版本、可用范围摘要、使用历史和允许动作。
...
| 路径 | 必测情况 |
| --- | --- |
| 收录 | 结果快照和 Artifact 自动出现；来源、项目参考和临时候选不进入客户资产库 |
