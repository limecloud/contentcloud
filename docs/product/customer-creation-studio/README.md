...
客户创作台还通过独立“资产库”复用历史生成结果。来源、灵感、知识和参考素材属于当前任务输入或项目参考，不与人物原型、剧本、分镜、图片和视频结果混在客户资产页；详细规范见[创作资产库](../creative-asset-library/README.md)。
...
- 客户场景：`IP 人设营销视频`。
- 可复用阶段：`灵感采集`。
- 横向复用闭环：`生成并确认结果 -> 资产库 -> 新任务 -> 新结果继续沉淀`。
...
| 2026-08-07 | 客户资产以生成结果为中心 | 输入和治理对象留在任务/运营面，客户资产页只展示人物、剧本、分镜、图片和视频结果 |
