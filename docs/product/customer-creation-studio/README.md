...
```text
资料与已有资产 -> Content Work OS 创作任务 -> 可确认结果、交付和后续专业工具
```
...
客户创作台还通过独立“资产库”复用历史来源、批准内容和成果。资产库不是另一套状态或文件仓库，而是对现有业务事实的统一客户投影；详细规范见[创作资产库](../creative-asset-library/README.md)。
...
- [`docs/roadmap/v5`](../../roadmap/v5/README.md) 继续定义抖音电商视频、分镜、Seedance 交付和结果归因纵向切片。
...
8. **完成一次，复用下一次。** 保存、批准和交付结果进入统一资产目录；新任务引用固定事实版本，不复制正文。
