...
上位规范：[ContentCloud 平台基线](../../foundation/README.md)。来源、知识、Runtime、执行者和发布边界必须遵循该基线。跨任务保存和复用遵循[客户资产](../creative-asset-library/README.md)。
...
“加入本次创作”固定为当前任务输入；“保留为项目参考”使来源在项目范围内可发现。二者都不自动批准来源中的事实、营销主张或权利，也不会自动写入“我的资产”或“创作结果”。客户需要长期保存文件时，必须另行执行明确的“导入到我的资产”动作。
...
- 未保存、未选择的搜索结果只保留在任务候选范围，不进入“我的资产”或“创作结果”。
- 客户保留的是 SourceRevision 等项目参考引用，不复制网页正文或候选洞察正文；客户明确导入的文件进入“我的资产”，人物、剧本、分镜、图片和视频等生成结果进入“创作结果”。
...
- 同一来源重复保存不会生成重复项目参考，未保存候选不会污染项目参考或客户资产入口。
...
6. 客户保存或选择的灵感会保留为项目参考，并能在另一任务中以固定来源版本和摘要复用；它不会自动出现在“我的资产”，也不会与生成结果混在同一投影。
