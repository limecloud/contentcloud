...
“加入本次创作”固定为当前任务输入；“保存到资产库”使来源在跨任务目录中可发现。二者都不自动批准来源中的事实、营销主张或权利。
...
collected source
    ...
    -> candidate insight proposed
         +-> customer saved for reuse -> CreativeAssetCatalogItem
         |
...
- 未保存、未选择的搜索结果只保留在任务候选范围，不进入跨任务资产库。
- 保存或选择进入资产库的是 SourceRevision 等事实引用，不复制网页正文或候选洞察正文。
...
- 只有客户保存或选择的 SourceRevision 进入资产目录，并能从目录固定引用到另一任务。
...
- 同一来源重复保存不会生成重复目录项，未保存候选不会污染资产库。
...
6. 客户保存或选择的灵感会进入资产库，并能在另一任务中以固定来源版本和摘要复用。
