...
async function fixtureTarget(target) {
  ...
  await writeFile(join(makeDir, 'dmg', 'arm64', 'Content Work OS-0.28.0-arm64.dmg'), 'dmg');
  await writeFile(join(makeDir, 'zip', 'darwin', 'arm64', 'Content Work OS-darwin-arm64-0.28.0.zip'), 'zip');
...
test('stages macOS Forge assets with checksums and verified signing metadata', async () => {
  ...
  const checksums = await readFile(join(fixture.outDir, 'darwin-arm64-checksums.sha256'), 'utf8');
...
test('aggregates target metadata into a release index', async () => {
  ...
  assert.match(await readFile(join(aggregateDir, 'checksums.txt'), 'utf8'), /desktop-beta-latest\.json/);
});
