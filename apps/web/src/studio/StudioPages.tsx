...
const assetStatusLabels:Record<AssetStatus,string>={all:'全部状态',draft:'草稿',pending_confirmation:'待确认',changes_requested:'需修改',confirmed:'已确认',delivered:'已交付',superseded:'已被替代',blocked:'暂时无法使用'};
...
export function StudioHomePage(){
  ...
  return <div className="studio-view studio-home studio-home-layout">
    <aside className="studio-home-context" aria-label="创作任务概览">
      <header className="studio-context-heading"><span>CONTENT WORK OS</span><h1>我的创作</h1><p>继续上次的工作，或开始制作新的内容。</p></header>
      <div className="studio-context-actions">
        {bootstrap.session.can_create&&firstExperience?<Link className="studio-context-action is-primary" to={startPath}><Sparkles size={18}/><span>{hasConnectedProject?'开始新创作':'连接工作电脑'}</span><ArrowRight size={16}/></Link>:<Link className="studio-context-action is-primary" to="/studio/tasks"><ListTodo size={18}/><span>查看我的创作</span><ArrowRight size={16}/></Link>}
        <Link className="studio-context-action" to="/studio/assets"><Archive size={17}/><span>从已有内容开始</span><ArrowRight size={15}/></Link>
      </div>
      <section className="studio-context-section"><SectionHeading icon={<ClipboardCheck size={17}/>} title="需要我处理" count={attention.length}/>{loading?<StudioLoading label="正在整理要处理的内容…"/>:attention.length===0?<CompactEmpty icon={<CheckCircle2 size={21}/>} title="目前没有需要你处理的事" detail="有内容需要补充或确认时，会显示在这里。"/>:<div className="studio-task-stack">{attention.slice(0,3).map(task=><TaskRow key={task.id} task={task}/>)}</div>}</section>
...
export function StudioConnectPage(){
  ...
  return <div className="studio-view studio-connect-view">
    <PageHeading eyebrow="开始创作" title={session?'连接工作电脑':connectedCount>0?'管理已连接电脑':'连接工作电脑'} detail="连接完成后，就可以直接使用团队已经准备好的创作步骤。" actions={connectedCount>0?<span className="studio-connected-count"><CheckCircle2 size={16}/>{connectedCount} 台已连接</span>:undefined}/>
    ...
    {loading?<StudioLoading label="正在读取连接状态…"/>:session?<section className={`studio-connection-state is-${session.status}`}>
      <header><span>{session.status==='connected'?<CheckCircle2 size={24}/>:session.status==='confirmation_required'?<ShieldCheck size={24}/>:session.status==='failed'||session.status==='expired'||session.status==='canceled'?<AlertCircle size={24}/>:<LoaderCircle className={session.status==='connecting'?'is-spinning':''} size={24}/>}</span><div><small>{project?.brand_name} · {client?.display_name||'连接工具'}</small><h2>{connectionTitle(session.status)}</h2><p>{session.message}</p></div></header>
      {session.status==='waiting_for_computer'&&<div className="studio-connect-instruction"><ol><li><span>1</span><div><strong>打开电脑上的连接工具</strong><p>新建一个会话，并选择这个项目所在的文件夹。</p></div></li><li><span>2</span><div><strong>发送连接文字</strong><p>连接工具会检查电脑准备情况，然后请你回到此页确认。</p></div></li></ol><pre><code>{prompt}</code></pre><div><Button onClick={()=>void copyPrompt()}>{copied?<Check size={16}/>:<Clipboard size={16}/>} {copied?'已复制':'复制连接文字'}</Button><Button variant="secondary" disabled={busy==='cancel'} onClick={()=>void cancel()}>取消</Button></div></div>}
      {session.status==='confirmation_required'&&<div className="studio-connect-confirm"><span>核对连接代码</span><code>{session.verification_code}</code><p>只有电脑上的连接工具显示相同代码时，才确认连接。</p><div><Button variant="danger" disabled={Boolean(busy)} onClick={()=>void decide('deny')}>不是这台电脑</Button><Button disabled={Boolean(busy)} onClick={()=>void decide('approve')}><ShieldCheck size={16}/>{busy==='approve'?'确认中…':'确认连接'}</Button></div></div>}
      {session.status==='connecting'&&<div className="studio-connect-wait"><LoaderCircle className="is-spinning" size={19}/><span>连接完成后会自动更新。{session.support_code&&<> 问题编号 <code>{session.support_code}</code></>}</span></div>}
      {session.status==='connected'&&<div className="studio-connect-success"><div><CheckCircle2 size={18}/><span><strong>可以开始创作</strong><small>接下来每一步会自动安排合适的工具或人工确认，你只需要按提示继续。</small></span></div><Button onClick={()=>navigate('/studio',{replace:true})}>回到我的创作<ArrowRight size={15}/></Button></div>}
...
function StudioUnavailableWorkbench({tasks,operationsPath}:{tasks:StudioTaskSummary[];operationsPath?:string}){
  ...
  return <section className="studio-workbench is-unavailable"><header className="studio-workbench-header"><div><span><CircleHelp size={15}/>暂时不能开始新创作</span><h2>先继续已有工作，准备好后就能开始新的内容</h2><p>你仍然可以处理任务、整理资料、使用已有结果和下载已经确认的文件。</p></div>{operationsPath&&<Link className="studio-secondary-link" to={operationsPath}>前往后台设置<ArrowRight size={14}/></Link>}</header><div className="studio-recovery-actions">{activeTask&&<Link to={`/studio/tasks/${encodeURIComponent(activeTask.id)}`}><Play size={17}/><span><strong>继续上次工作</strong><small>{activeTask.title} · {activeTask.next_action}</small></span><ArrowRight size={14}/></Link>}<Link to="/studio/tasks"><ListTodo size={17}/><span><strong>查看我的创作</strong><small>继续已有任务或处理需要确认的事</small></span><ArrowRight size={14}/></Link><Link to="/studio/assets"><Archive size={17}/><span><strong>打开资料</strong><small>整理资料并查找可以重复使用的内容</small></span><ArrowRight size={14}/></Link><Link to="/studio/deliveries"><PackageCheck size={17}/><span><strong>查看交付</strong><small>下载文件并核对发布记录</small></span><ArrowRight size={14}/></Link></div></section>;
}
...
export function StudioNewTaskPage(){
  ...
  return <div className="studio-view studio-new-task">
    ...
    <PageHeading eyebrow="开始新创作" title={`制作一条${experience.name}`} detail="先说清楚这次想达到的结果，接下来的步骤会自动安排。"/>
  ...
  return <div className="studio-view studio-assets">
    ...
    <div className="studio-asset-toolbar">
      <div className="studio-asset-views" role="tablist" aria-label="资料视图">
        <button type="button" role="tab" aria-selected={view==='mine'} className={view==='mine'?'is-active':''} onClick={()=>chooseView('mine')}>我的资料<span>{(surface?.workspace.counts.all||0)+folders.length}</span></button>
    ...
    {loading?<StudioLoading label="正在整理资料…"/>:view==='mine'?<>
      ...
      <div className="studio-asset-workspace">
        <section className="studio-asset-library">
          <div className="studio-asset-location"><div><LayoutGrid size={16}/><strong>{currentFolder?.name||'全部资料'}</strong><span>{visibleFolders.length+visibleMaterials.length} 项</span></div>{currentFolder&&<button type="button" onClick={()=>setFolderRef(currentFolder.parent_ref||'')}>返回上一级</button>}</div>
          <div className="studio-asset-grid">
            ...
            {materialCategory==='folder'?<button className="studio-asset-import-card" type="button" disabled={!projectID} onClick={()=>setShowFolderForm(true)}><FolderPlus size={28}/><strong>新建文件夹</strong><small>{projectID?'整理当前项目资料':'先选择一个项目'}</small></button>:<label className={`studio-asset-import-card ${!projectID||busy?'is-disabled':''}`}><Upload size={28}/><strong>{busy?'正在导入…':'导入资料'}</strong><small>{projectID?'文档、图片、视频、音频或表格':'先选择一个项目'}</small><input type="file" accept=".pdf,.docx,.xlsx,.pptx,.png,.jpg,.jpeg,.mp4,.mov,.webm,.mp3,.wav,.m4a,.csv,.md,.txt" onChange={upload} disabled={busy||!projectID}/></label>}
          </div>
        </section>
      </div>
    </>:view==='results'?<>
      ...
      <section className="studio-recent-group"><SectionHeading icon={<Clock3 size={17}/>} title="最近使用的项目资料" count={recentMaterials.length}/>{recentMaterials.length===0?<CompactEmpty icon={<Archive size={20}/>} title="还没有使用记录" detail="把资料加入创作后会显示在这里。"/>:<div className="studio-asset-grid">{recentMaterials.map(item=><WorkspaceMaterialCard key={item.material_ref} item={item} taskID={requestedTaskID}/>)}</div>}</section>
...
function CollectionMethod({icon,title,detail,available,href}:{icon:ReactNode;title:string;detail:string;available:boolean;href?:string}){const enabled=available&&Boolean(href);const body=<><span>{icon}</span><div><strong>{title}</strong><small>{detail}</small></div><b>{available?'可用':'暂未开通'}</b>{enabled&&<ChevronRight size={15}/>}</>;return enabled?<Link className="studio-collection-method" to={href||''}>{body}</Link>:<div className={`studio-collection-method ${available?'':'is-disabled'}`} aria-disabled={!available}>{body}</div>}
function MaterialPicker({items,selectedRefs,onChange}:{items:WorkspaceMaterialItem[];selectedRefs:string[];onChange:(refs:string[])=>void}){return <fieldset className="studio-asset-picker"><legend>从我的资料带入 <em>可选</em></legend><p>选择已上传或导入的文档、图片、视频、音频或表格，创建任务时会保留当前文件版本。</p>{items.length===0?<small>当前项目还没有资料。</small>:<div>{items.slice(0,8).map(item=><label key={item.material_ref}><input type="checkbox" checked={selectedRefs.includes(item.material_ref)} onChange={event=>onChange(event.target.checked?[...selectedRefs,item.material_ref]:selectedRefs.filter(ref=>ref!==item.material_ref))}/><span><strong>{item.title}</strong><small>{workspaceMaterialKindLabel(item.material_kind)} · {item.file_name} · {formatBytes(item.byte_size)}</small></span></label>)}</div>}</fieldset>}
...
function connectionTitle(status:StudioConnectSession['status']){return {waiting_for_computer:'在电脑上继续',connecting:'正在完成连接',confirmation_required:'确认这台电脑',connected:'工作电脑已连接',failed:'连接未完成',expired:'连接已过期',canceled:'连接已取消'}[status]}
