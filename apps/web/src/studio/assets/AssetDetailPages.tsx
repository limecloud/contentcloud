...
export function StudioMaterialDetailPage(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      if(!found)throw new Error('没有找到这项资料，可能已被移除或不属于当前团队');
      setItem(found);setTasks(nextTasks);
    }catch(value){setError(value instanceof Error?value.message:'资料加载失败')}
    finally{setLoading(false)}
  },[materialID]);
  useEffect(()=>{void load()},[load]);
  if(loading)return <AssetPageLoading/>;
  if(error||!item)return <AssetPageError message={error||'没有找到这项资料'} onRetry={load}/>;
  return <AssetDetailFrame backTo="/studio/assets" eyebrow={`我的资料 · ${materialTypeLabels[item.material_kind]}`} title={item.title} state={materialStateLabel(item.processing_state)} icon={<MaterialIcon kind={item.material_kind}/> }>
...
function AssetPageLoading(){return <div className="studio-asset-page-state"><LoaderCircle className="is-spinning" size={20}/>正在打开资料…</div>}
function AssetPageError({message,onRetry}:{message:string;onRetry:()=>Promise<void>}){return <div className="studio-view studio-asset-detail-page"><Link className="studio-back" to="/studio/assets"><ArrowLeft size={15}/>返回资料</Link><div className="studio-asset-page-state is-error"><AlertCircle size={22}/><strong>资料无法打开</strong><p>{message}</p><Button variant="secondary" onClick={()=>void onRetry()}>重新加载</Button></div></div>}
