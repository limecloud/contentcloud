...
export function TeamView({session, onChanged}: {session: TeamSession; onChanged: () => Promise<void>}) {
  ...
  return <div className="page team-page">
    <div className="page-heading"><div><span className="eyebrow">团队访问设置</span><h1>团队与权限</h1><p>{session.tenant.name} 使用预设角色，移除成员后其登录会立即失效。</p></div></div>
    ...
    <section className="team-accept section">
      ...
      <div><strong>加入团队</strong><span>邀请码必须与当前登录邮箱一致，加入后可从左侧切换团队。</span></div>
    ...
    {!canViewMembers ? <section className="section"><Empty title="当前账号不能查看成员列表" detail="你仍可以接受邀请；成员和权限设置仅对团队管理员、项目负责人和审核人员开放。"/></section> : loading ? <section className="team-loading section"><Loading/></section> : <>
      ...
      {isAdmin && <div className="team-admin-grid">
        <section className="section invite-create">
          ...
          <div className="team-form">
            ...
            <Field label="团队角色"><select value={inviteForm.role} onChange={event => setInviteForm({...inviteForm, role: event.target.value})}>{roles.map(role => <option key={role} value={role}>{roleLabel(role)}</option>)}</select></Field>
            <Button disabled={!inviteForm.email.includes('@') || busy === 'invite'} onClick={invite}>{busy === 'invite' ? '创建中…' : '创建邀请'}</Button>
          </div>
          {createdToken && <div className="invite-token"><span>一次性邀请码</span><div><code>{createdToken}</code><IconButton label="复制邀请码" onClick={copyToken}>{copied ? <Check size={17}/> : <Clipboard size={17}/>}</IconButton></div></div>}
        </section>
