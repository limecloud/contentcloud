...
describe('ContentCloud Agent bootstrap',()=>{
  ...
  it('provides fixed preflight, plan, resume, and diagnostic commands',()=>{
    ...
    expect(commands.preflight).toBe("npx --yes @limecloud/contentcloud@0.28.0 bootstrap preflight . --server-url 'https://content.example.com' --json");
  ...
  it('distinguishes active connection from completed registration',()=>{
    ...
    expect(connectStateCopy({...waitingSession,state:'verifying'}).title).toBe('正在连接工作电脑');
    expect(connectStateCopy({...waitingSession,state:'connected'}).title).toBe('工作电脑已连接');
    expect(connectStateCopy(waitingSession,true).title).toBe('仍在等待连接工具');
  });
