...
export function TenantTable({tenants,currentTenantID,busy,onAction,onContentAction,compact=false}:{tenants:PlatformTenant[];currentTenantID:string;busy:string;onAction?:(tenant:PlatformTenant)=>void;onContentAction?:(tenant:PlatformTenant)=>void;compact?:boolean}) {
  return <div className="admin-table-scroll"><div className={`admin-tenant-table ${compact?'is-compact':''}`}>
    <header><span>客户</span><span>状态</span><span>内容能力</span><span>成员</span><span>项目</span><span>在线设备</span><span>活跃任务</span><span>最近活动</span>{!compact&&<span>操作</span>}</header>
    {tenants.length===0?<Empty title="没有匹配的客户"/>:tenants.map(tenant=><TenantRow key={tenant.id} tenant={tenant} currentTenantID={currentTenantID} busy={busy} onAction={onAction} onContentAction={onContentAction} compact={compact}/>) }
  </div></div>;
}
...
function TenantRow({tenant,currentTenantID,busy,onAction,onContentAction,compact}:{tenant:PlatformTenant;currentTenantID:string;busy:string;onAction?:(tenant:PlatformTenant)=>void;onContentAction?:(tenant:PlatformTenant)=>void;compact:boolean}) {
  ...
  return <article>
    ...
    {!compact&&<div className="admin-row-action">{tenant.id===currentTenantID?<span>当前客户</span>:tenant.status==='active'?<IconButton label={`停用 ${tenant.name}`} disabled={busy===tenant.id} onClick={()=>onAction?.(tenant)}><PauseCircle size={17}/></IconButton>:<IconButton label={`恢复 ${tenant.name}`} disabled={busy===tenant.id} onClick={()=>onAction?.(tenant)}><PlayCircle size={17}/></IconButton>}</div>}
  </article>;
}
