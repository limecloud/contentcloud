import { Archive, ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Check, CircleAlert, CircleCheck, Copy, Eye, GitBranch, GitCompare, Plus, RotateCcw, Search, ShieldAlert, Trash2, Users, Workflow } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { Link } from 'react-router-dom';
import { api, patch, post } from '../api';
import type { AdminWorkOSView, Environment, EnvironmentCapability, GateDefinition, SOPCapabilityCoverage, SOPDiffChange, SOPEnvironmentBindingPreview, SOPLintReport, SOPSummary, SOPVersion, SOPVersionDiff, SOPVersionImpact, SOPVersionPreview, StageDefinition } from '../types';
import { Button, Empty, Field, Modal } from '../components/ui';
import { normalizeEnvironment, normalizeSOPSummary, normalizeSOPVersion, normalizeSOPVersionPreview } from './operationsData';
...
export function AdminEnvironmentPanel({environments, sops, onSaved, initialEnvironmentID, initialProductID, availableCapabilities=[], variant='environment', directoryOnly=false, scoped=false, filterProductID, environmentPath, onSelectionChange, onCreated}: AdminEnvironmentPanelProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setNotice(enrollment ? '客户使用设置已保存。新设置只影响后续创建的任务，历史任务和结果保持不变。' : '客户设置已保存，后续新任务会使用这份设置。');
      await onSaved();
    } catch (value) { setNotice(value instanceof Error ? value.message : enrollment ? '客户使用设置保存失败' : '客户设置保存失败'); }
  ...
  const create = async () => {
    ...
    try {
      ...
      setNotice(enrollment ? `客户「${created.name}」已开通。` : `客户设置「${created.name}」已创建。`);
      await onSaved();
      onCreated?.(created.id);
    } catch (value) { setNotice(value instanceof Error ? value.message : enrollment ? '客户开通创建失败' : '客户设置创建失败'); }
    finally { setSaving(false); }
  };
  ...
  const emptyAction = <Button onClick={openCreate}><Plus size={15}/>{enrollment ? '新建客户设置' : '新建客户设置'}</Button>;
  const directory = !scoped && <section className="workos-section admin-config-section">
    <div className="admin-config-toolbar"><label className="search-field"><Search size={16}/><input placeholder={enrollment ? '搜索客户或创作流程' : '搜索客户设置'} aria-label={enrollment ? '搜索客户设置' : '搜索客户设置'} value={query} onChange={event => setQuery(event.target.value)}/></label><span className="toolbar-count">{filterProductID ? `${filtered.length}/${environments.length}` : environments.length} 个{enrollment ? '客户设置' : '客户设置'}</span><Button onClick={openCreate}><Plus size={15}/>新建</Button></div>
    <div className="admin-config-table">{filtered.length === 0 ? <Empty title={environments.length ? `没有匹配的${enrollment ? '客户设置' : '客户设置'}` : `还没有${enrollment ? '客户设置' : '客户设置'}`} detail={environments.length ? '换一个搜索词或清除流程筛选。' : enrollment ? '选择已发布的创作流程，为第一个客户建立设置。' : '先建立客户设置，再为它选择创作流程和可用功能。'} action={emptyAction}/> : filtered.map(environment => {
      ...
      const row = <><span className="admin-row-icon">{enrollment ? <Users size={17}/> : <Workflow size={17}/>}</span><span><strong>{environment.name}</strong><small>{enrollment ? `${product?.label || '尚未选择创作流程'} · ${environment.capabilities.filter(capability => capability.enabled).length} 项可用功能` : `${environment.slug} · ${environment.manifest_digest ? `设置记录：${environment.manifest_digest.slice(0, 18)}…` : '还没有设置记录'}`}</small></span><span className={`config-state ${environment.status === 'active' ? 'is-success' : 'is-warning'}`}>{environment.status === 'active' ? '已启用' : '已暂停'}</span><span className="admin-row-chevron">›</span></>;
  ...
  return <>
    ...
    {!directoryOnly && form && <section className="workos-section admin-editor-section operations-enrollment-editor">
      <header className="workos-section-header"><div><span>{enrollment ? '客户使用设置' : '客户设置'}</span><h2>{form.name}</h2></div><span className={`config-state ${form.status === 'active' ? 'is-success' : 'is-warning'}`}>{form.status === 'active' ? '可以开始新创作' : '暂时不能开始新创作'}</span></header>
      {enrollment && <div className="operations-enrollment-summary"><span><small>当前使用版本</small><strong>{selectedProduct?.label || '尚未选择'}</strong></span><span><small>功能准备情况</small><strong>{required.length === 0 ? '产品未说明' : `${required.length - missing.length}/${required.length}`}</strong></span><span><small>当前影响</small><strong>{form.status === 'active' ? '允许新任务' : '阻止新任务'}</strong></span></div>}
      <div className="form-grid"><Field label={enrollment ? '客户名称' : '名称'}><input value={form.name} onChange={event => setForm({...form, name: event.target.value})}/></Field><Field label={enrollment ? '客户标识' : '标识'}><input value={form.slug} onChange={event => setForm({...form, slug: event.target.value})}/></Field><Field label="状态"><select value={form.status} onChange={event => setForm({...form, status: event.target.value})}><option value="active">已启用，可以开始新创作</option><option value="paused">已暂停，暂时不能开始新创作</option></select></Field><Field label={enrollment ? '已开通创作流程' : '默认创作流程'}><select value={`${form.default_sop_id || ''}:${form.default_sop_version || 0}`} onChange={event => {const [sopID, version] = event.target.value.split(':'); setForm({...form, default_sop_id: sopID, default_sop_version: Number(version)});}}><option value=":0" disabled>请选择已发布版本</option>{publishedSOPs.map(version => <option key={`${version.sop_id}:${version.version}`} value={`${version.sop_id}:${version.version}`}>{version.label}</option>)}</select></Field></div>
      <div className="admin-capability-list"><strong>{enrollment ? '客户可用功能' : '电脑可用功能'}</strong>{editorCapabilities.length === 0 ? <Empty title="没有已登记功能" detail="功能清单还没有返回可配置项。"/> : editorCapabilities.map(capability => <label className="toggle-line" key={capability.id}><span><strong>{capability.id}</strong><small>版本 {capability.version}{required.includes(capability.id) ? ' · 当前流程需要' : ''}</small></span><input type="checkbox" checked={enabled.has(capability.id)} onChange={event => updateCapability(capability.id, capability.version, event.target.checked)}/></label>)}</div>
      {enrollment && <div className={`operations-enrollment-impact ${missing.length ? 'is-warning' : 'is-info'}`}>{missing.length ? <CircleAlert size={17}/> : <CircleCheck size={17}/>}<div><strong>{missing.length ? `还缺少 ${missing.length} 项必需功能` : '当前功能已准备齐全'}</strong><span>{missing.length ? `${missing.join('、')} 尚未开启，后续任务可能无法完成对应步骤。` : '保存后只影响新任务；历史任务、审批和创作结果不会被改写。'}</span></div></div>}
      <div className="new-task-actions"><Button onClick={save} disabled={saving || !form.default_sop_id || (form.default_sop_version || 0) < 1}><Check size={15}/>{saving ? '保存中…' : '保存客户设置'}</Button></div>{notice && <div className="workos-notice is-info">{notice}</div>}
    </section>}
    ...
    </div></Modal> : <Modal title="新增客户设置" onClose={closeCreate}><div className="admin-modal-form"><Field label="名称"><input autoFocus value={createForm.name} onChange={event => setCreateForm({...createForm, name: event.target.value})} placeholder="例如：内容审核客户"/></Field><Field label="标识"><input value={createForm.slug} onChange={event => setCreateForm({...createForm, slug: event.target.value})} placeholder="例如：review"/></Field><Field label="状态"><select value={createForm.status} onChange={event => setCreateForm({...createForm, status: event.target.value})}><option value="active">已启用，保存后立即可用</option><option value="paused">已暂停，保存后暂不启用</option></select></Field><Field label="默认创作流程"><select value={`${createForm.default_sop_id}:${createForm.default_sop_version}`} onChange={event => selectCreateProduct(event.target.value)}><option value=":0" disabled>请选择默认创作流程</option>{publishedSOPs.map(version => <option key={`${version.sop_id}:${version.version}`} value={`${version.sop_id}:${version.version}`}>{version.label}</option>)}</select></Field><footer className="modal-actions"><Button variant="secondary" onClick={closeCreate}>取消</Button><Button onClick={create} disabled={saving || !createForm.name.trim() || !createForm.slug.trim() || !createForm.default_sop_id || createForm.default_sop_version < 1}><Plus size={15}/>{saving ? '保存中…' : '保存客户设置'}</Button></footer></div></Modal>)}
  </>;
}
...
export function AdminSOPPanel({sops, environments=[], onChanged, onPublished, initialSOPID, initialVersion=0, scoped=false, onSelectionChange}: AdminSOPPanelProps) {
  ...
  const [preview, setPreview] = useState<SOPVersionPreview>();
  ...
  useEffect(() => { setDraft(version); setLint(undefined); setDiff(undefined); setImpact(undefined); setPreview(undefined); setPreviewEnvironmentID(''); setCompareVersion(summary?.versions.find(candidate => candidate.version !== version?.version)?.version || 0); }, [selectedSOPID, selectedVersion, sops]);
  const createSOP = async () => {
    ...
    } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程创建失败'); }
  ...
  const saveDraft = async () => { if (!editable) return; setBusy(true); setNotice(''); try { const saved = normalizeSOPVersion(await patch<SOPVersion>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}`, {name: draft.name, description: draft.description, content_types: draft.content_types, stages: draft.stages, gates: draft.gates, default_execution_mode: draft.default_execution_mode})); setDraft(saved); setNotice('创作流程草稿已保存。'); await onChanged(); } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程草稿保存失败'); } finally { setBusy(false); } };
  const runLint = async () => { setBusy(true); setNotice(''); try { setLint(await api<SOPLintReport>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}/lint`)); } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程检查失败'); } finally { setBusy(false); } };
  const publish = async () => { setBusy(true); setNotice(''); try { const report = await api<SOPLintReport>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}/lint`); setLint(report); if (!report.valid) { setNotice('发布前检查未通过，请先修正配置。'); return; } const published = normalizeSOPVersion(await post<SOPVersion>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}/publish`)); setDraft(published); setNotice(`创作流程 v${published.version} 已发布，设置记录已固定。`); await onChanged(); onPublished?.(published.sop_id, published.version); } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程发布失败'); } finally { setBusy(false); } };
  ...
  const loadPreview = async () => { setBusy(true); setNotice(''); try { const query = previewEnvironmentID ? `?environment_id=${encodeURIComponent(previewEnvironmentID)}` : ''; setPreview(normalizeSOPVersionPreview(await api<SOPVersionPreview>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}/preview${query}`))); } catch (value) { setNotice(value instanceof Error ? value.message : '发布预览读取失败'); } finally { setBusy(false); } };
  const rollback = async () => { const target = summary.versions.find(candidate => candidate.version === compareVersion); if (!target || compareVersion >= summary.definition.current_version || !window.confirm(`确认恢复到 v${compareVersion}？历史执行记录不会改变，绑定当前版本的未来客户设置和项目会切换到新版本。`)) return; setBusy(true); setNotice(''); try { const result = await post<{version:SOPVersion; rebound_environments:number; rebound_projects:number}>(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/rollback`, {target_version: compareVersion}); setSelectedVersion(result.version.version); setNotice(`已创建恢复版本 v${result.version.version}，切换 ${result.rebound_environments} 个客户设置、${result.rebound_projects} 个项目。`); await onChanged(); } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程恢复失败'); } finally { setBusy(false); } };
  const retire = async () => { if (draft.status !== 'published' || draft.version === summary.definition.current_version || !window.confirm(`确认停用创作流程 v${draft.version}？已绑定的历史任务仍可只读完成。`)) return; setBusy(true); setNotice(''); try { await post(`/api/bff/admin/sops/${encodeURIComponent(draft.sop_id)}/versions/${draft.version}/retire`, {}); setNotice(`创作流程 v${draft.version} 已停用，新任务不能再绑定它。`); await onChanged(); } catch (value) { setNotice(value instanceof Error ? value.message : '创作流程停用失败'); } finally { setBusy(false); } };
  ...
  return <>
    ...
    <section className="workos-section sop-governance-panel"><header className="workos-section-header"><div><span>版本管理</span><h2>比较版本、查看影响和恢复</h2><small className="config-source-note">历史任务会继续使用原版本；恢复旧版只会影响客户设置和项目接下来的工作。</small></div><ShieldAlert size={18}/></header><div className="form-grid"><Field label="比较 / 恢复目标"><select value={compareVersion} onChange={event => {setCompareVersion(Number(event.target.value)); setDiff(undefined);}}><option value={0}>选择历史版本</option>{summary.versions.filter(candidate => candidate.version !== draft.version).map(candidate => <option key={candidate.version} value={candidate.version}>v{candidate.version} · {statusLabel(candidate.status)}</option>)}</select></Field><div className="new-task-actions sop-governance-actions"><Button variant="secondary" onClick={compare} disabled={busy || !compareVersion}><GitCompare size={15}/>比较版本</Button><Button variant="secondary" onClick={loadImpact} disabled={busy}><ShieldAlert size={15}/>查看影响</Button>{draft.status === 'published' && compareVersion > 0 && compareVersion < summary.definition.current_version && <Button onClick={rollback} disabled={busy}><RotateCcw size={15}/>恢复到 v{compareVersion}</Button>}</div></div>{diff && <SOPDiffPanel diff={diff}/>} {impact && <SOPImpactPanel impact={impact}/>}<div className="sop-preview-workbench"><div className="sop-preview-toolbar"><div><strong>发布预览与连接设置</strong><small>只读核对版本、功能准备情况和目标客户设置；不会选择具体视频服务，也不会创建固定任务版本。</small></div><div className="sop-preview-actions"><label><span>目标客户设置</span><select value={previewEnvironmentID} onChange={event => {setPreviewEnvironmentID(event.target.value); setPreview(undefined);}}><option value="">全部客户设置</option>{environments.map(environment => <option key={environment.id} value={environment.id}>{environment.name} · {environment.status === 'active' ? '已启用' : '已暂停'}</option>)}</select></label><Button variant="secondary" onClick={loadPreview} disabled={busy}><Eye size={15}/>{busy ? '读取中…' : '生成发布预览'}</Button></div></div>{preview && <SOPPreviewPanel preview={preview}/>}</div></section>
...
function SOPImpactPanel({impact}: {impact: SOPVersionImpact}) { return <div className="sop-impact-panel"><div className="sop-governance-heading"><ShieldAlert size={15}/><strong>v{impact.version} 影响范围</strong><span>{impact.counts.tasks || 0} 个任务 · {impact.counts.projects || 0} 个项目 · {impact.counts.environments || 0} 个客户设置</span></div><div className="sop-impact-grid"><div><strong>客户设置</strong>{impact.environments.length === 0 ? <small>没有默认绑定</small> : impact.environments.slice(0, 4).map(item => <small key={item.environment_id}>{item.name} · {statusLabel(item.status)}</small>)}</div><div><strong>项目</strong>{impact.projects.length === 0 ? <small>没有固定绑定</small> : impact.projects.slice(0, 4).map(item => <small key={item.project_id}>{item.name} · v{item.bound_sop_version}</small>)}</div><div><strong>任务</strong>{impact.tasks.length === 0 ? <small>没有历史任务</small> : impact.tasks.slice(0, 4).map(item => <small key={item.task_id}>{item.title} · {statusLabel(item.status)}</small>)}</div></div></div>; }
function SOPPreviewPanel({preview}: {preview:SOPVersionPreview}) {
  return <div className={`sop-preview-panel ${preview.publishable ? 'is-ready' : 'is-blocked'}`}>
    <div className="sop-preview-conclusion"><span>{preview.publishable ? <CircleCheck size={16}/> : <CircleAlert size={16}/>}</span><div><strong>{preview.publishable ? '当前版本可以进入发布确认' : '当前版本不能进入发布确认'}</strong><small>{preview.sop.digest ? `版本摘要：${preview.sop.digest}` : '版本摘要还在生成中'}</small></div><b>{preview.publishable ? '可发布' : '需处理'}</b></div>
    {(preview.blockers.length > 0 || preview.warnings.length > 0) && <div className="sop-preview-messages">{preview.blockers.map(message => <p className="is-blocker" key={`blocker:${message}`}><CircleAlert size={14}/><span>{message}</span></p>)}{preview.warnings.map(message => <p className="is-warning" key={`warning:${message}`}><ShieldAlert size={14}/><span>{message}</span></p>)}</div>}
    <div className="sop-preview-grid"><section><header><strong>功能准备情况</strong><small>{preview.required_capabilities.length ? `${preview.required_capabilities.length} 项流程要求` : '产品未说明所需功能'}</small></header>{preview.capabilities.length === 0 ? <p className="sop-preview-empty">没有需要绑定的功能。</p> : preview.capabilities.map(item => <CapabilityCoverageRow key={item.id} item={item}/>)}</section><section><header><strong>客户设置</strong><small>{preview.environments.length ? `${preview.environments.length} 个预览对象` : '没有匹配客户设置'}</small></header>{preview.environments.length === 0 ? <p className="sop-preview-empty">当前没有客户设置记录；不会填充演示连接。</p> : preview.environments.map(item => <EnvironmentPreviewRow key={item.environment_id} item={item}/>)}</section></div>
  </div>;
}
function CapabilityCoverageRow({item}: {item:SOPCapabilityCoverage}) { return <div className="sop-preview-row"><span className="sop-preview-state"><CircleCheck size={14}/></span><span><strong>{item.id}</strong><small>使用于 {item.required_by_stages.join('、') || '未返回阶段'} · 连接的电脑 {item.registered_executor_count} 台</small></span><code>{item.registered_versions.length ? item.registered_versions.join('、') : '未登记'}</code></div>; }
function EnvironmentPreviewRow({item}: {item:SOPEnvironmentBindingPreview}) { return <div className={`sop-preview-row ${item.ready ? 'is-ready' : 'is-missing'}`}><span className="sop-preview-state">{item.ready ? <CircleCheck size={14}/> : <CircleAlert size={14}/>}</span><span><strong>{item.name}</strong><small>{item.status === 'active' ? '已启用' : '已暂停'} · 可用电脑 {item.candidate_executor_count} 台</small></span><code>{item.ready ? '可连接' : item.missing_capabilities.length ? `缺少 ${item.missing_capabilities.length} 项` : '未登记'}</code></div>; }
...
function StageEditor({stage, index, editable, canMoveUp, canMoveDown, onChange, onMove, onRemove, gates}: {stage: StageDefinition; index: number; editable: boolean; canMoveUp: boolean; canMoveDown: boolean; onChange: (changes: Partial<StageDefinition>) => void; onMove: (direction: -1 | 1) => void; onRemove: () => void; gates: GateDefinition[]}) {
  return <article className="admin-stage-editor-row">
    ...
    <div className="form-grid">
      <Field label="步骤编号"><input value={stage.stage_id} readOnly={!editable} onChange={event => onChange({stage_id: event.target.value})}/></Field>
      <Field label="名称"><input value={stage.name} readOnly={!editable} onChange={event => onChange({name: event.target.value})}/></Field>
      <Field label="输出格式"><input value={stage.output_schema} readOnly={!editable} onChange={event => onChange({output_schema: event.target.value})}/></Field>
      ...
      <Field label="使用的资料"><input value={joinList(stage.input_refs)} readOnly={!editable} onChange={event => onChange({input_refs: splitList(event.target.value)})}/></Field>
      <Field label="所需功能"><input value={joinList(stage.required_capabilities)} readOnly={!editable} onChange={event => onChange({required_capabilities: splitList(event.target.value)})}/></Field>
      <Field label="处理方式"><input value={editable ? joinList(stage.execution_modes) : stage.execution_modes.map(executionModeLabel).join('、')} readOnly={!editable} onChange={event => onChange({execution_modes: splitList(event.target.value)})}/></Field>
...
function GateEditor({gate, editable, onChange, onRemove}: {gate: GateDefinition; editable: boolean; onChange: (changes: Partial<GateDefinition>) => void; onRemove: () => void}) {
  return <article className="admin-gate-editor-row">
    ...
    <div className="form-grid">
      <Field label="检查编号"><input value={gate.gate_id} readOnly={!editable} onChange={event => onChange({gate_id: event.target.value})}/></Field>
