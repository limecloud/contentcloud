...
export interface SOPTaskImpact { task_id:string; project_id:string; title:string; status:string; sop_version:number; runtime_run_bound:boolean }
...
export interface SOPCapabilityCoverage { id:string; required_by_stages:string[]; registered_versions:string[]; registered_executor_count:number }
export interface SOPEnvironmentBindingPreview { environment_id:string; name:string; status:string; configured_capabilities:EnvironmentCapability[]; required_capabilities:string[]; available_capabilities:string[]; missing_capabilities:string[]; candidate_executor_count:number; ready:boolean; reasons:string[] }
export interface SOPVersionPreview { sop:SOPVersion; lint:SOPLintReport; impact:SOPVersionImpact; required_capabilities:string[]; capabilities:SOPCapabilityCoverage[]; environments:SOPEnvironmentBindingPreview[]; selected_environment_id?:string; publishable:boolean; blockers:string[]; warnings:string[] }
...
export type ExecutorPresenceStatus='online'|'offline'|'unknown';
export type ExecutorEnvironmentStatus='ready'|'repair_required'|'blocked'|'unknown';
export type ExecutorRuntimeStatus='healthy'|'degraded'|'throttled'|'unavailable'|'unknown';
export interface OperationsExecutorRuntime { kind:string; version?:string; status:'healthy'|'unhealthy'|string; error_code?:string; selected:boolean; capabilities:{events?:boolean;resume?:boolean;fork?:boolean;mcp_stdio?:boolean;mcp_http?:boolean;structured_output?:boolean;sandbox_profile?:string;max_parallel_sessions?:number;transcript_export?:boolean} }
export interface OperationsExecutorWorkspace { workspace_id:string; project_id:string; status:ExecutorEnvironmentStatus; reason:string; error_code?:string; generation?:string; environment_digest?:string; plugin_declaration_digest?:string; skill_declaration_digest?:string; mcp_declaration_digest?:string; workspace_declaration_digest?:string; plugin_receipt_digest?:string; skill_observation_digest?:string; mcp_observation_digest?:string; workspace_observation_digest?:string; observed_at:string }
export interface OperationsExecutor { id:string; tenant_id:string; display_name:string; executor_type:string; status:'online'|'offline'|'revoked'|'unknown'|string; status_reason:string; presence_status:ExecutorPresenceStatus; presence_reason?:string; environment_status:ExecutorEnvironmentStatus; environment_reason?:string; runtime_status:ExecutorRuntimeStatus; runtime_reason?:string; daemon_instance_id?:string; connection_epoch?:number; active_attempt_ids:string[]; runtimes:OperationsExecutorRuntime[]; workspaces?:OperationsExecutorWorkspace[]; hostname:string; platform:string; arch:string; version:string; capabilities:AdminCapability[]; projects:OperationsExecutorProject[]; last_seen_at:string; revoked_at?:string }
...
export interface RuntimeCostView { status:'not_recorded'|'recorded'|'mixed_currency'|string; amount_minor:number; currency?:string; effect_count:number }
export interface RuntimeJobSummary { id:string; work_task_id:string; business_type?:string; task_title:string; customer_name:string; project_id:string; project_name:string; product_name:string; product_version:number; current_step_id?:string; current_step_name?:string; completed_steps:number; total_steps:number; task_status:string; task_next_action?:string; state:string; status_since:string; blocking_reason?:string; recommended_action?:string; cost:RuntimeCostView; plan_digest:string; binding_digest:string; input_digest:string; runtime_policy_id:string; contract_major:number; contract_minor:number; root_job_run_id:string; source_job_run_id?:string; checkpoint_id?:string; priority:number; error_code?:string; allowed_actions:string[]; node_count:number; node_states:Record<string,number>; effect_count:number; checkpoint_count:number; created_at:string; updated_at:string }
export interface RuntimeJobList { items:RuntimeJobSummary[]; next_after?:number; generated_at:string }
export interface RuntimeReplayResult { tenant_id:string; job_run_id:string; event_count:number; last_sequence:number; external_calls:number; projection_rebuilt:boolean; integrity_status:string }
...
export interface RuntimeEffectView { id:string; node_run_id:string; kind:string; state:string; external_id?:string; request_digest:string; response_digest?:string; cost_minor:number; currency:string; safe_summary:Record<string,unknown>; error_code?:string; version:number; allowed_actions:string[]; created_at:string; updated_at:string }
export interface RuntimeAttemptView { id:string; node_run_id:string; attempt_no:number; harness_kind:string; state:string; executor_ref?:string; session_bound:boolean; lease_expires_at?:string; result_digest?:string; safe_summary:Record<string,unknown>; error_code?:string; created_at:string; started_at?:string; finished_at?:string; updated_at:string }
export interface RuntimeGateView { id:string; name:string; mode:string; state:string; decision?:string; reason?:string; updated_at:string }
export interface RuntimeStateCollectionView { id:string; collection_key:string; scope:string; schema_id:string; schema_revision:number; consistency:string; writer_node_key?:string; record_count:number; revision:number; watermark:number; updated_at:string }
export interface RuntimeCheckpointView { id:string; node_key:string; plan_digest:string; state_ref_count:number; output_ref_count:number; completed_nodes:string[]; digest:string; allowed_actions:string[]; blocked_reason?:string; created_at:string }
...
export interface RuntimeJobDetail { summary:RuntimeJobSummary; plan:RuntimePlanView; nodes:RuntimeNodeView[]; attempts:RuntimeAttemptView[]; events:RuntimeEventView[]; effects:RuntimeEffectView[]; checkpoints:RuntimeCheckpointView[]; agents:RuntimeAgentView[]; gates:RuntimeGateView[]; state_collections:RuntimeStateCollectionView[]; generated_at:string }
...
export interface RuntimeRun { id:string; tenant_id:string; project_id:string; work_task_id?:string; sop_id?:string; sop_version?:number; sop_digest?:string; stage_id?:string; execution_mode?:string; executor_kind?:string; output_refs?:string[]; task_revision_id?:string; gate_evaluation_id?:string; input_snapshot_id:string; task_type:string; capability_id:string; capability_version:string; input_schema:string; output_schema:string; output_count:number; delivery_profiles:string[]; state:string; priority:number; attempt_count:number; progress_label?:string; error_code?:string; created_at:string; updated_at:string }
...
export interface ProviderBinding { tenant_id:string; provider_id:string; profile_version:string; state:string; credential_configured?:boolean; egress_policy:string; monthly_budget_minor:number; max_job_cost_minor:number; max_concurrency:number; max_retries:number; updated_by:string; updated_at:string }
export interface WorkTaskView { task:WorkTask; project:Project; environment:Environment; sop:SOPVersion; source_revisions:SourceRevision[]; knowledge_snapshots:KnowledgeSnapshot[]; approved_snapshots:ApprovedSnapshot[]; stage_runs:StageRun[]; runs:RuntimeRun[]; gates:GateEvaluation[]; revisions:TaskRevision[]; deliveries:TaskDelivery[]; stage_outputs:TaskStageOutput[]; media_jobs:MediaGenerationJob[]; provider_attempts:ProviderAttempt[]; media_reviews:MediaReview[]; delivery_packages:DeliveryPackage[]; artifacts:Artifact[]; allowed_actions:string[]; generated_at:string }
