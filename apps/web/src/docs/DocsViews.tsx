...
function DocsNavigation({catalog,onNavigate}:{catalog:DocsCatalog;onNavigate:()=>void}){
  return <nav aria-label="文档目录">
    ...
    <section><span>连接工具</span>{catalog.clients.map(client=><NavLink key={client.id} to={docsWebPath(client.page_slug)!} onClick={onNavigate}>{client.display_name}<StatusDot status={client.status}/></NavLink>)}</section>
    <section><span>内容形式</span>{catalog.content_types.map(content=><NavLink key={content.id} to={docsWebPath(content.page_slug)!} onClick={onNavigate}>{content.title}<StatusDot status={content.status}/></NavLink>)}</section>
    <section><span>使用指南</span>{catalog.guides.map(guide=><NavLink key={guide.id} to={docsWebPath(guide.page_slug)!} onClick={onNavigate}>{guide.title}</NavLink>)}</section>
  </nav>;
}
...
export function DocsHome(){
  ...
  return <div className="docs-home">
    ...
    <section className="docs-index-section"><header><div><span className="section-kicker">连接工具</span><h2>按连接工具查看</h2></div><p>这里的状态表示每种连接工具目前能做到什么。</p></header><div className="docs-client-index">{catalog.clients.map(client=><Link key={client.id} to={docsWebPath(client.page_slug)!}><span className="docs-index-icon"><MonitorUp size={18}/></span><span><strong>{client.display_name}</strong><small>{client.summary}</small></span><b className={`docs-status is-${client.status}`}>{docsStatusLabel(client.status)}</b><ChevronRight size={17}/></Link>)}</div></section>
    <section className="docs-index-section"><header><div><span className="section-kicker">内容形式</span><h2>按内容形式查看</h2></div><p>只有已经准备好的内容形式，才会提供可以照着做的指南。</p></header><div className="docs-content-index">{catalog.content_types.map(content=><Link key={content.id} to={docsWebPath(content.page_slug)!}><span className="docs-index-icon">{content.id==='marketing-video'?<Video size={19}/>:<FileText size={19}/>}</span><span><strong>{content.title}</strong><small>{content.summary}</small></span><b className={`docs-status is-${content.status}`}>{docsStatusLabel(content.status)}</b><ChevronRight size={17}/></Link>)}</div></section>
...
export function DocsPageView({slug}:{slug:string}){
  ...
  return <div className="docs-page-layout"><article className="docs-article"><header className="docs-article-header"><Link to="/docs"><CircleHelp size={14}/>使用文档</Link><span>/</span><span>{kindLabel(page.kind)}</span><b className={`docs-status is-${page.status}`}>{docsStatusLabel(page.status)}</b></header>{page.status!=='available'&&<aside className={`docs-availability is-${page.status}`}>{page.status==='limited'?<CircleHelp size={19}/>:<Clock3 size={19}/>}<div><strong>{docsStatusLabel(page.status)}</strong><span>{page.status==='limited'?'部分功能可以使用，请以页面列出的范围为准。':'该功能已经预留入口，但目前还不能实际使用。'}</span></div></aside>}<div className="docs-markdown"><ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml components={components}>{page.markdown}</ReactMarkdown></div></article><aside className="docs-page-meta"><span>页面状态</span><strong>{docsStatusLabel(page.status)}</strong><p>{page.description}</p></aside></div>;
}

function useDocsCatalog(){const value=useContext(DocsContext);if(!value)throw new Error('文档目录尚未加载');return value}
function kindLabel(kind:string){return kind==='client'?'连接工具':kind==='content_type'?'内容形式':kind==='guide'?'场景指南':kind==='troubleshooting'?'故障排查':kind==='concept'?'概念':'指南'}
